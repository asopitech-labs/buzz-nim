...
if [[ "$instance_id" != "com.asopitech.nimino.dev" && "$instance_id" != com.asopitech.nimino.dev.* ]]; then
    echo "reset-desktop-standalone-state: refusing non-dev bundle identifier: $instance_id" >&2
    exit 1
fi
if [[ "$keyring_service" != "nimino-desktop-dev" && "$keyring_service" != nimino-desktop-dev.* ]]; then
...
case "${NIMINO_TEST_PLATFORM:-$(uname -s)}" in
    Darwin)
