...
env_args=(
    ...
    NIMINO_PRIVATE_KEY="$_key"
    NIMINO_ACP_AGENT_COMMAND=goose
    NIMINO_ACP_AGENT_ARGS=acp
    NIMINO_ACP_AGENTS="$_agents"
    GOOSE_MODE=auto
)
[[ -n "$_prompt" ]] && env_args+=(NIMINO_ACP_SYSTEM_PROMPT="$_prompt")
if [[ "$_heartbeat" != "0" ]]; then
    env_args+=(NIMINO_ACP_HEARTBEAT_INTERVAL="$_heartbeat")
fi
