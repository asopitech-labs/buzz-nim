...
tags: [nostr, nip-rs, read-state, formal-model, nimino]
status: active
created: 2026-07-16
---
...
(below): every context ID Nimino actually generates (channel UUID,
...
**Cost:** zero bytes for every context ID Nimino generates today (channel
...
">256-byte key drop hazard" section: Nimino's own key shapes cannot
