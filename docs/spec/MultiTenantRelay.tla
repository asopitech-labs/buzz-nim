...
(***************************************************************************)
(* Formal model of Nimino's proposed multi-tenant relay/database isolation.    *)
(*                                                                         *)
...
(*                                                                         *)
(* Source grounding from today's Nimino:                                      *)
...
(*   - crates/nimino-db/src/event.rs: EventQuery has channel_id/channel_ids    *)
(*     but no community_id; inserts use ON CONFLICT DO NOTHING.              *)
(*   - crates/nimino-db/src/channel.rs: get_accessible_channel_ids currently   *)
(*     unions all open channels in the DB; that unscoped variant is the      *)
(*     explicit I1 mutation.                                                 *)
(*   - crates/nimino-relay/src/state.rs: process-global AppState/caches today. *)
(*                                                                         *)
