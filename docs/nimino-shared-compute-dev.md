# Nimino shared compute: local GUI verification

This runbook verifies the actual desktop path used by the built-in **Fizz** agent:

`Nimino Desktop → nimino-acp → nimino-agent → MeshLLM SDK → local/remote compute`

It does not use a substitute agent harness.

## Before starting

Run from the `asopitech-labs/nimino` repository root on the mesh-enabled branch.
...
2. Use any local name, such as **Local Nimino**.
...
Stop only stale Nimino/MeshLLM processes shown by that command. Do not leave a
...
take several minutes. Wait for the Nimino window to open and for the terminal to
...
Nimino may download the model on first use. The model picker ranks models for the
...
resolves to the bundled `nimino-agent`. No API key is required.
...
**nimino-agent** as the runtime, **Nimino shared compute** as the LLM provider,
...
- no current Nimino membership snapshot is available (admission fails closed).

## Security boundary

Nimino publishes member-signed discovery notes through an ordinary relay-supported
...
tokens in the note. Current Nimino membership controls which node identities are
...
MeshLLM—not the Nimino relay—carries inference over direct QUIC or its encrypted
...
is an upstream protocol change and is not claimed by the Nimino-side checks above.
