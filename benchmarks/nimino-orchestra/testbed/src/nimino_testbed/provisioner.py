"""Channel-per-trial Nimino provisioning against a local benchmark stack."""
...
def provisioner_from_dict(config: dict[str, object]) -> NiminoTrialProvisioner:
    """Harbor CLI factory for a JSON-decoded testbed configuration."""
    return NiminoTrialProvisioner(TestbedConfig(**config))


class NiminoTrialProvisioner:
    """Implements the TrialHandle v1.1 contract against a live Nimino relay.

    Guarantees (contract PLANS/HARBOR_NIMINO_TRIALHANDLE_CONTRACT.md):
    ...
    def _provision(
        ...
        # mirroring production Nimino, where a human owns the channel their
        # agents work in.
    ...
    def _seed_directory(
        self, task_name: str | None, observer: NiminoCli
    ...
    def _stable_credential(
        ...
        digest = hashlib.sha256(
            b"nimino-benchmark-directory-v1\0"
    ...
    def _cli_for(self, credential: AgentCredential) -> NiminoCli:
        return NiminoCli(
