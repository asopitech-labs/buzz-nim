"""Thin subprocess wrapper over the ``nimino`` CLI — the production client path."""
...
class NiminoCliError(RuntimeError):
    """A nimino CLI invocation failed."""


class NiminoCli:
    """Run nimino CLI commands as one relay identity (key + NIP-OA auth tag)."""
    ...
    def run(self, *args: str) -> Any:
        """Run a nimino subcommand and return its parsed JSON stdout."""
        ...
        try:
            ...
            raise NiminoCliError(f"nimino {args[0]}: {error}") from error
        if completed.returncode != 0:
            raise NiminoCliError(
                f"nimino {' '.join(args)} exited {completed.returncode}: "
        ...
        try:
            ...
            raise NiminoCliError(
                f"nimino {args[0]} returned non-JSON output: {completed.stdout[:200]!r}"
            ) from error
    ...
    def create_private_channel(self, name: str, description: str) -> str:
        ...
        if not channel_id:
            raise NiminoCliError(f"channel create returned no channel_id: {response}")
        return channel_id
