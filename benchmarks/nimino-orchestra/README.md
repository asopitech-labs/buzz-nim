# Harbor Nimino Orchestra

A stock-Harbor custom agent that runs a manifest-defined team through the real
Nimino stack. Harbor sees one `NiminoOrchestraAgent`; behind that adapter, one
...
`nimino-acp` → `nimino-agent` → `nimino-dev-mcp` process tree the desktop app
launches: the production MCP toolset (shell, file tools, todo) with the
`nimino` CLI on the shell's PATH. No Harbor fork or patch is required.
...
```bash
uv run --project benchmarks/nimino-orchestra/testbed harbor run --yes -p <TASK_OR_DIRECTORY> --agent nimino_orchestra:NiminoOrchestraAgent --agent-kwarg manifest=<CONDITION.yaml> --agent-kwarg provisioner_factory=nimino_testbed:provisioner_from_dict --agent-kwarg provisioner_config=<PROVISIONER.json> --agent-kwarg endpoint_config=<ENDPOINTS.json> --agent-kwarg artifact_root=benchmarks/nimino-orchestra --agent-kwarg nimino_acp_binary=<LINUX_BIN>/nimino-acp --agent-kwarg nimino_agent_binary=<LINUX_BIN>/nimino-agent --agent-kwarg nimino_dev_mcp_binary=<LINUX_BIN>/nimino-dev-mcp --agent-kwarg nimino_cli_binary=target/debug/nimino --agent-kwarg run_id="bench-$(date -u +%Y%m%dT%H%M%SZ)" --agent-timeout-multiplier 15 --n-concurrent 1
```

`nimino_acp_binary`/`nimino_agent_binary`/`nimino_dev_mcp_binary` must be **Linux**
...
any Linux base image works). `nimino_cli_binary` is the **host** CLI the harness
...
Each trial gets fresh keys and a private Nimino channel. The provisioner archives
rather than deletes that channel, leaving the relay/Postgres event timeline
and the per-agent acp/agent logs (downloaded into the trial's `nimino/`
artifacts) available for analysis.

### Nimino-native tasks

The local [`benchmarks/nimino-dataset`](../nimino-dataset) suite — a sibling
directory of this harness, not a subdirectory of it — scores Nimino product
...
```bash
just benchmark \
  --path benchmarks/nimino-dataset/reply-to-thread \
  --attempts 1 \
  --manifest benchmarks/nimino-orchestra/manifests/nimino-native-solo-luna.yaml \
  --endpoint-config benchmarks/nimino-orchestra/testbed/endpoints/openai-live.json \
  --n-concurrent 1
```

The default condition is `nimino-native-solo-luna.yaml` — one solo agent on
...
`anthropic-live.json`. Swap in `nimino-native-solo-sonnet.yaml` (no
...
Replace the path with `benchmarks/nimino-dataset/create-channel-invite-users`
...
`/logs/artifacts/nimino-evidence.json`. The task verifier reads that post-agent
...
cause is written to the trial's `nimino/nimino-evidence-error.txt`.
...
the tasks whose graded Nimino behavior is deliberately absent from
...
stack (`nimino-benchmark` compose project — relay :3600, Postgres :5633, secrets
...
relay membership list and opens the Nimino desktop app logged in as them, so
...
uv run --project benchmarks/nimino-orchestra/testbed \
    benchmarks/nimino-orchestra/scripts/run_leaderboard.py \
    --dataset terminal-bench/terminal-bench-2-1 \
    --attempts 5 \
    --manifest benchmarks/nimino-orchestra/manifests/<TEAM>.yaml \
    --endpoint-config benchmarks/nimino-orchestra/testbed/endpoints/<ENDPOINTS>.json \
...
```bash
cd benchmarks/nimino-orchestra
