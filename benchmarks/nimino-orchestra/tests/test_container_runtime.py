...
from nimino_orchestra.container_runtime import (
    REMOTE_BIN,
    REMOTE_EVIDENCE,
    REMOTE_LOGS,
    THINKING_EFFORT,
...
from nimino_orchestra.provisioning import (
    AgentCredential,
    FixtureActor,
    TrialHandle,
)
from nimino_orchestra.task_fixtures import fixture_for
...
async def test_collects_task_declared_channel_membership(tmp_path, monkeypatch):
    ...
    async def nimino_json(credential_arg, trial_arg, *args):
    ...
    monkeypatch.setattr(rt, "_nimino_json", nimino_json)
...
async def test_install_stack_uploads_the_pinned_stack(tmp_path):
    binaries = {}
    for name in ("nimino-acp", "nimino-agent", "nimino-dev-mcp"):
        path = tmp_path / name
        path.write_text("#!binary")
        binaries[name] = str(path)
    rt = runtime(
        tmp_path,
        nimino_acp_binary=binaries["nimino-acp"],
        nimino_agent_binary=binaries["nimino-agent"],
        nimino_dev_mcp_binary=binaries["nimino-dev-mcp"],
    )
    environment = Environment()
    await rt._install_stack(environment)
    assert {target for _, target in environment.uploads} == {
        f"{REMOTE_BIN}/nimino-acp",
        f"{REMOTE_BIN}/nimino-agent",
        f"{REMOTE_BIN}/nimino-dev-mcp",
    }
...
async def test_install_stack_requires_binaries_on_disk(tmp_path):
    rt = runtime(tmp_path, nimino_acp_binary=str(tmp_path / "missing"))
...
async def test_forwarder_bridges_the_canonical_relay_address(tmp_path):
    from nimino_orchestra.container_runtime import FORWARDER
...
async def test_launch_wires_the_desktop_environment(tmp_path, configured, expected):
    ...
    environment = Environment(
        responses={"nimino-acp": ExecResult(stdout="4242\n", stderr="", return_code=0)}
    )
    ...
    assert f"{REMOTE_BIN}/nimino-acp" in command
    # The real product wiring: acp spawns nimino-agent, which gets nimino-dev-mcp.
    assert env["NIMINO_ACP_AGENT_COMMAND"] == f"{REMOTE_BIN}/nimino-agent"
    assert env["NIMINO_ACP_MCP_COMMAND"] == f"{REMOTE_BIN}/nimino-dev-mcp"
...
async def test_wait_for_agents_ready_requires_every_channel_subscription(tmp_path):
    ...
    from nimino_orchestra.container_runtime import _Agent
...
async def test_send_mentions_by_pubkey_so_task_text_stays_inert(tmp_path, monkeypatch):
    ...
    async def nimino_json(credential, trial, *args):
        calls.append(args)
        return {}
...
async def test_wait_for_done_requires_orchestrator_authorship(tmp_path, monkeypatch):
    ...
    async def nimino_json(credential, *args, **kwargs):
...
async def test_solo_turn_end_completes_without_done_message(tmp_path, monkeypatch):
    ...
    async def nimino_json(*args, **kwargs):
        return []
...
def test_turn_status_parses_completed_batch_and_successful_steer_receipts():
    ...
    output = "\n".join(
        [
            ...
            f"turn delivered Nimino events for channel test: {batch_event_id}",
    ...
    assert NiminoContainerRuntime._parse_turn_status(output) == (
        1,
        1,
...
async def test_collect_evidence_uploads_verifier_artifact(tmp_path, monkeypatch):
    ...
    evidence = json.loads((trial_dir / "nimino-evidence.json").read_text())
...
async def test_failed_evidence_snapshot_records_the_reason(tmp_path, monkeypatch):
    ...
    assert "relay unreachable" in (trial_dir / "nimino-evidence-error.txt").read_text()
    assert not (trial_dir / "nimino-evidence.json").exists()


def test_runtime_logging_keeps_readiness_and_turn_completion_signals(tmp_path):
    rt = runtime(tmp_path)
    assert rt._rust_log(None) == "nimino_acp=info,pool::prompt=info"
    assert rt._rust_log("custom=debug") == (
        "custom=debug,nimino_acp=info,pool::prompt=info"
    )
...
async def test_stop_agents_sweeps_the_uploaded_stack(tmp_path):
    ...
    await NiminoContainerRuntime._stop_agents(environment, agents)
