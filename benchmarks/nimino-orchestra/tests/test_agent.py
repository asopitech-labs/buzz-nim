...
async def test_missing_integrations_fail_explicitly(tmp_path, manifest_data):
    agent = NiminoOrchestraAgent(logs_dir=tmp_path, manifest=manifest_data)
...
async def test_cli_runtime_construction_from_json(tmp_path, manifest_data):
    ...
    agent = NiminoOrchestraAgent(
        ...
        nimino_acp_binary="/pinned/nimino-acp",
        nimino_agent_binary="/pinned/nimino-agent",
        nimino_dev_mcp_binary="/pinned/nimino-dev-mcp",
        nimino_cli_binary="/pinned/nimino",
    )
    assert agent.runtime.artifact_root == tmp_path
    assert agent.runtime.endpoints["frontier/rev"].provider == "anthropic"
    assert agent.runtime.nimino_acp_binary == "/pinned/nimino-acp"
    assert agent.runtime.nimino_agent_binary == "/pinned/nimino-agent"
    assert agent.runtime.nimino_dev_mcp_binary == "/pinned/nimino-dev-mcp"
    assert agent.runtime.nimino_cli_binary == "/pinned/nimino"
