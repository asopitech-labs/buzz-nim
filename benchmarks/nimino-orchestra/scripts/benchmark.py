#!/usr/bin/env python3
"""One-command benchmark: bring up the Nimino stack in Docker and run it.
...
- A dedicated ``nimino-benchmark`` compose project reusing the production
...
- ``--gui`` adds that user to the relay membership list and opens the Nimino
...
Run inside the testbed environment (the just recipe does this):
    ...
    uv run --project benchmarks/nimino-orchestra/testbed \
        benchmarks/nimino-orchestra/scripts/benchmark.py [--gui] [...]
"""
...
COMPOSE_PROJECT = "nimino-benchmark"
COMPOSE_FILES = (
...
GUI_BUNDLE_IDENTIFIER = "xyz.block.nimino.app.benchmark"
...
AGENT_BINARIES = ("nimino-acp", "nimino-agent", "nimino-dev-mcp")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--gui",
        action="store_true",
        help="Open the Nimino desktop app as the benchmark user to watch the run live",
    )
    parser.add_argument(
        "--fresh",
        action="store_true",
...
def load_state() -> dict[str, str]:
    ...
    from nimino_testbed.keys import generate_keypair, keypair_from_secret
...
def print_user_identity(state: dict[str, str]) -> None:
    ...
    nothing beyond it)."""
    from nimino_testbed.keys import encode_nsec
...
def write_env_file(state: dict[str, str]) -> Path:
    ...
    lines = {
        "NIMINO_IMAGE": os.environ.get("NIMINO_IMAGE", "ghcr.io/asopitech-labs/nimino:main"),
        "NIMINO_DOMAIN": "localhost",
        "RELAY_URL": f"ws://localhost:{RELAY_HTTP_PORT}",
        "NIMINO_MEDIA_BASE_URL": f"http://localhost:{RELAY_HTTP_PORT}/media",
        "NIMINO_MEDIA_SERVER_DOMAIN": "localhost",
        "NIMINO_CORS_ORIGINS": f"http://localhost:{RELAY_HTTP_PORT}",
        "NIMINO_REQUIRE_AUTH_TOKEN": "true",
        "NIMINO_REQUIRE_RELAY_MEMBERSHIP": "true",
        "NIMINO_ALLOW_NIP_OA_AUTH": "true",
        "NIMINO_AUTO_MIGRATE": "true",
        "NIMINO_GIT_CONFORMANCE_PROBE": "true",
        "RUST_LOG": "nimino_relay=info,nimino_db=info,nimino_auth=info",
        "RELAY_OWNER_PUBKEY": state["owner_pubkey"],
        "NIMINO_RELAY_PRIVATE_KEY": state["relay_private_key"],
        "NIMINO_GIT_HOOK_HMAC_SECRET": state["git_hook_hmac_secret"],
        "POSTGRES_DB": "nimino",
        "POSTGRES_USER": "nimino",
        "POSTGRES_PASSWORD": state["postgres_password"],
        "REDIS_PASSWORD": state["redis_password"],
        "NIMINO_S3_ACCESS_KEY": state["s3_access_key"],
        "NIMINO_S3_SECRET_KEY": state["s3_secret_key"],
        "NIMINO_S3_BUCKET": "nimino-media",
        "NIMINO_HTTP_PORT": str(RELAY_HTTP_PORT),
        "NIMINO_PG_HOST_PORT": str(PG_HOST_PORT),
        "NIMINO_METRICS_HOST_PORT": str(METRICS_HOST_PORT),
    }
...
def postgres_dsn(state: dict[str, str]) -> str:
    return (
        f"postgresql://nimino:{state['postgres_password']}@127.0.0.1:{PG_HOST_PORT}/nimino"
    )
...
def ensure_binaries() -> dict[str, Path]:
    """Find the host nimino CLI, building it once if missing."""
    try:
        return run_leaderboard.find_binaries(None)
    except SystemExit:
        print("host nimino CLI missing — building (cargo build, first run only)...")
    cargo = REPO_ROOT / "bin" / "cargo"
    subprocess.run(
        [str(cargo), "build", "-p", "nimino-cli"],
        cwd=REPO_ROOT,
        check=True,
    )
    return run_leaderboard.find_binaries(None)
...
def ensure_agent_binaries() -> Path:
    ...
    nimino-acp → nimino-agent → nimino-dev-mcp stack, so the binaries must be
...
def launch_gui(state: dict[str, str]) -> subprocess.Popen:
    """Open the Nimino desktop app logged in as the benchmark user.

    The relay runs closed (membership required), so the user pubkey is first
    added to the relay membership list via nimino-admin inside the container —
    ...
    for name in (
        ...
        if not stub.exists():
            stub.touch()
    real_cli = sidecar_dir / f"nimino-{triple}"
    real_cli.write_bytes(binaries["nimino"].read_bytes())
    real_cli.chmod(0o755)

    print(
        f"Opening Nimino GUI as the benchmark user ({state['user_pubkey'][:16]}…).\n"
    ...
    # workspace's relay URL overrides NIMINO_RELAY_URL by design. Reusing the
    ...
    tauri_config = json.dumps(
        {"identifier": GUI_BUNDLE_IDENTIFIER, "productName": "Nimino Benchmark"}
    )
    return subprocess.Popen(
        ...
        env={
            **os.environ,
            "NIMINO_RELAY_URL": f"ws://localhost:{RELAY_HTTP_PORT}",
            "NIMINO_PRIVATE_KEY": state["user_secret_key"],
        },
    )
...
def leaderboard_argv(
    ...
    argv += [
        ...
        # NIMINO_BENCHMARK_DOCKER_HOST if your engine exposes the host
        # differently.
        "--relay-gateway",
        (
            f"{os.environ.get('NIMINO_BENCHMARK_DOCKER_HOST', 'host.docker.internal')}"
            f":{RELAY_HTTP_PORT}"
        ),
        "--n-concurrent",
