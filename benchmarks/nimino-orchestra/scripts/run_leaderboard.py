...
importable:
    ...
    uv run --project benchmarks/nimino-orchestra/testbed \
        ...
        --agent-bin-dir <DIR with Linux nimino-acp/nimino-agent/nimino-dev-mcp>
"""
...
AGENT_IMPORT = "nimino_orchestra:NiminoOrchestraAgent"
PROVISIONER_FACTORY = "nimino_testbed:provisioner_from_dict"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="JSON config for the Nimino relay/Postgres provisioner",
    )
    parser.add_argument(
        "--nimino-bin-dir",
        type=Path,
        default=None,
        help="Directory with the host nimino CLI (default: repo target/release, then target/debug)",
    )
    parser.add_argument(
        "--agent-bin-dir",
        type=Path,
        required=True,
        help="Directory with Linux builds of nimino-acp/nimino-agent/nimino-dev-mcp "
...
def find_binaries(bin_dir: Path | None) -> dict[str, Path]:
    ...
    raise SystemExit(
        f"nimino binaries not found (need {', '.join(BINARIES)}; searched {searched}). "
        "Build them with `cargo build` or pass --nimino-bin-dir."
    )
...
def build_command(
    ...
    kwargs = {
        ...
        "nimino_acp_binary": agent_binaries["nimino-acp"],
        "nimino_agent_binary": agent_binaries["nimino-agent"],
        "nimino_dev_mcp_binary": agent_binaries["nimino-dev-mcp"],
        "nimino_cli_binary": binaries["nimino"],
...
def write_metadata_template(args: argparse.Namespace, job_dir: Path) -> Path:
    ...
    metadata = {
        "agent_url": "https://github.com/asopitech-labs/nimino",
        "agent_display_name": f"Nimino Orchestra ({manifest.get('condition', 'team')})",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.dry_run:
        ...
        bin_dir = args.nimino_bin_dir or PACKAGE_ROOT.parents[1] / "target" / "release"
        ...
        return 0
    binaries = find_binaries(args.nimino_bin_dir)
