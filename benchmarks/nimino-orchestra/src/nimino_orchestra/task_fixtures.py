"""Public setup declarations for Nimino-native benchmark tasks."""
...
_CREATE_CHANNEL_FIXTURE = NiminoTaskFixture(
    directory=tuple(
        [
...
_USER_MENTION_FIXTURE = NiminoTaskFixture(
    user_display_name=USER_MENTION_DISPLAY_NAME,
    requires_evidence=True,
)

_NARRATIVE_AGENT_NAMES_FIXTURE = NiminoTaskFixture(
    directory=(
...
_INTERLEAVED_AGENT_REPORTS_FIXTURE = NiminoTaskFixture(
    directory=(
...
_CROSS_THREAD_REQUESTS_FIXTURE = NiminoTaskFixture(
    scripted_messages=(
        ScriptedMessage(
            label="beta-request",
            actor="user",
...
_AMBIGUOUS_USER_MENTION_FIXTURE = NiminoTaskFixture(
    directory=(
        DirectoryEntry(
            "Taylor Morgan Lee",
            "user",
...
_FIXTURES = {
    ...
    REPLY_TO_THREAD_TASK: NiminoTaskFixture(requires_evidence=True),
    READ_NAMED_PATH_TASK: NiminoTaskFixture(requires_evidence=True),
    MULTILINE_MESSAGE_TASK: NiminoTaskFixture(
...
def fixture_for(task_name: str | None) -> NiminoTaskFixture:
    """Return the declared setup for a task, or an empty setup."""
    return _FIXTURES.get(task_name or "", NiminoTaskFixture())
