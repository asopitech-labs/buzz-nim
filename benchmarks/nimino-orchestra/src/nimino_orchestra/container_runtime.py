"""Run the production Nimino agent stack inside the Harbor task container.

Each provisioned identity is a full ``nimino-acp`` → ``nimino-agent`` →
``nimino-dev-mcp`` process tree launched *inside* the task container — the same
binaries and the same MCP toolset (shell, file tools, the ``nimino`` CLI on
PATH) that the desktop app gives a Nimino agent. The harness stays outside:
...
from .evidence import build_nimino_evidence
...
# Container-side layout for the uploaded Nimino stack.
REMOTE_ROOT = "/opt/nimino"
REMOTE_BIN = f"{REMOTE_ROOT}/bin"
REMOTE_PROMPTS = f"{REMOTE_ROOT}/prompts"
REMOTE_LOGS = f"{REMOTE_ROOT}/logs"
REMOTE_EVIDENCE = "/logs/artifacts/nimino-evidence.json"
...
DELIVERY_RECEIPT_MARKER = "turn delivered Nimino events for channel"
...
class RuntimeLaunchError(RuntimeError):
    """Raised when a Nimino agent process cannot be launched or exits early."""
...
class NiminoContainerRuntime:
    """Launch one production Nimino agent stack per identity in the container."""
    ...
    def __init__(
        ...
        *,
        ...
        nimino_acp_binary: str = "nimino-acp",
        nimino_agent_binary: str = "nimino-agent",
        nimino_dev_mcp_binary: str = "nimino-dev-mcp",
        nimino_cli_binary: str = "nimino",
        relay_gateway: str = "",
        ...
        self.nimino_acp_binary = nimino_acp_binary
        self.nimino_agent_binary = nimino_agent_binary
        self.nimino_dev_mcp_binary = nimino_dev_mcp_binary
        # Host build used for user/provisioning operations only:
        self.nimino_cli_binary = nimino_cli_binary
    ...
    async def run(
        ...
        *,
        ...
        trial_dir = self.logs_dir / "nimino"
        ...
        try:
            ...
            # The task arrives exactly as it would in production Nimino: a
        ...
        if fixture_for(trial.task_name).requires_evidence and not evidence_exported:
            raise RuntimeLaunchError(
                "failed to export nimino-evidence.json; the trial has no "
        ...
        return RuntimeResult(
            metadata={
                ...
                "nimino_evidence_exported": evidence_exported,
    ...
    async def _install_stack(self, environment: BaseEnvironment) -> None:
        ...
        uploads = {
            f"{REMOTE_BIN}/nimino-acp": self.nimino_acp_binary,
            f"{REMOTE_BIN}/nimino-agent": self.nimino_agent_binary,
            f"{REMOTE_BIN}/nimino-dev-mcp": self.nimino_dev_mcp_binary,
        }
    ...
    async def _launch_agent(
        ...
        *,
        ...
        command = (
            f"{shlex.quote(f'{REMOTE_BIN}/nimino-acp')} </dev/null "
        ...
        *,
        ...
        return {
            **endpoint.env,
            ...
            # Desktop parity: the GUI also sets NOSTR_PRIVATE_KEY on nimino-acp
            # so nimino-dev-mcp's shim can wire git auth/signing for the agent.
            "NOSTR_PRIVATE_KEY": credential.nostr_secret_key,
            "NIMINO_AUTH_TAG": credential.nostr_auth_tag,
            "NIMINO_ACP_AGENT_COMMAND": f"{REMOTE_BIN}/nimino-agent",
            "NIMINO_ACP_AGENT_ARGS": "",
            "NIMINO_ACP_MCP_COMMAND": f"{REMOTE_BIN}/nimino-dev-mcp",
    ...
    def _rust_log(configured: str | None) -> str:
        # ``nimino_acp=info`` carries the subscription-readiness line; the turn
        ...
        required = "nimino_acp=info,pool::prompt=info"
    ...
    async def _wait_for_done(
        ...
        while True:
            ...
            polls += 1
            messages = await self._nimino_json(
                trial.user,
                trial,
                "messages",
                "get",
    ...
    async def _turn_ended(environment: BaseEnvironment, agent: _Agent) -> bool:
        _, ends = await NiminoContainerRuntime._turn_counts(environment, agent)
        return ends > 0
    ...
    async def _turn_counts(
        ...
        starts, ends, _ = await NiminoContainerRuntime._turn_status(environment, agent)
        return starts, ends
    ...
    async def _turn_status(
        ...
        return NiminoContainerRuntime._parse_turn_status(result.stdout or "")
    ...
    async def _collect_evidence(
        ...
        *,
        ...
        try:
            ...
            evidence = build_nimino_evidence(
                trial=trial,
                messages=messages,
                task_event_id=task_event_id,
            ...
            evidence_path = trial_dir / "nimino-evidence.json"
    ...
    def _record_evidence_error(trial_dir: Path, reason: str) -> None:
        ...
        try:
            (trial_dir / "nimino-evidence-error.txt").write_text(reason, encoding="utf-8")
        except OSError:
    ...
    async def _collect_observed_channels(
        ...
        for name in names:
            matches = await self._nimino_json(
                orchestrator,
                trial,
                "channels",
                "search",
            ...
            for match in matches:
                ...
                if not isinstance(channel_id, str) or not channel_id:
                    continue
                members = await self._nimino_json(
                    orchestrator,
                    trial,
                    "channels",
                    "members",
    ...
    # -- Nimino CLI as the trial user / provisioning identities -------------------
    ...
    async def _send(
        ...
        *,
        ...
        return await self._nimino_json(credential, trial, *args)
    ...
    async def _send_scripted_messages(
        ...
        *,
        ...
        Messages are sent back-to-back so Nimino's normal queueing and batching
    ...
    async def _nimino_json(
        ...
        if process.returncode != 0:
            raise RuntimeLaunchError(
                f"nimino {shlex.join(args)} exited {process.returncode}: "
                f"{stderr.decode(errors='replace').strip()}"
            )
        try:
            return json.loads(stdout)
        except json.JSONDecodeError as error:
            raise RuntimeLaunchError("nimino returned invalid JSON") from error
    ...
    def _user_relay_url(trial: TrialHandle) -> str:
        ...
        return NiminoContainerRuntime._cli_relay_url(trial.relay_ws_url)
    ...
    def _compose_system_prompt(
        ...
        *,
        ...
        The analogue of a production Nimino workspace's team context: each agent
        ...
        lines = [
            ...
            f"The team coordinates in Nimino channel `{trial.channel_id}`.",
            (
