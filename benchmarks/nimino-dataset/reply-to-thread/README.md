...
> the behavior under test, and it must come from `nimino-acp`'s production base
...
container's shell. `NiminoOrchestraAgent` launches the real `nimino-acp` /
`nimino-agent` stack against a dedicated relay, and the agent works entirely
through Nimino. Agent timeout 300s; the manifest's `trial_budget` is the
effective clock.

## Verifier

Reads the post-agent `/logs/artifacts/nimino-evidence.json` snapshot (written by
`NiminoContainerRuntime._collect_evidence` after the agent stops, so the agent
...
shipped: the Oracle agent replaces `NiminoOrchestraAgent`, so no relay trial is
...
`../nimino-orchestra/tests/test_reply_to_thread_verifier.py` (run from
`benchmarks/nimino-orchestra`: `uv run --extra dev pytest -q`).
