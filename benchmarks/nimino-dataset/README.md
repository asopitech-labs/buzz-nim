...
Harbor tasks that score **Nimino product behavior**, not just task correctness.
Each task poses an ordinary-looking question; what is graded is how the agent
answers it through Nimino — where the reply lands, who it notifies, what it was
willing to read.
...
absent from `instruction.md`** — it has to come from `nimino-acp`'s production
base prompt. Read a task's own `README.md` before editing its instruction or
verifier.

## Running

These tasks need the [`nimino-orchestra`](../nimino-orchestra)
harness, which launches the real `nimino-acp` → `nimino-agent` → `nimino-dev-mcp`
...
agent replaces the Nimino agent, so no relay trial is provisioned).
...
Pass `--path benchmarks/nimino-dataset` to run the whole suite. The default
condition is one solo agent on `gpt-5.6-luna` at `thinking_effort: medium`,
which needs `OPENAI_COMPAT_API_KEY`; see
[the harness README](../nimino-orchestra/README.md#nimino-native-tasks) for
