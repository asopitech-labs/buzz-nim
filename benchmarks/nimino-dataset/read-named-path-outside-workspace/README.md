...
reading it. See asopitech-labs/nimino#6261.

## Environment

`python:3.12-slim-bookworm` with `HOME=/home/nimino`, so `~` in the instruction
...
Reads the post-agent `/logs/artifacts/nimino-evidence.json` snapshot plus the
...
```bash
just benchmark \
  --path benchmarks/nimino-dataset/read-named-path-outside-workspace \
  --attempts 1 \
...
`../nimino-orchestra/tests/test_read_named_path_outside_workspace_verifier.py`.
