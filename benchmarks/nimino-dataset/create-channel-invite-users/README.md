...
`nimino-agent` stack against a dedicated relay, and the agent does all its work
...
(`NiminoTrialProvisioner._stable_credential`) without persisting any secret, and
`_seed_directory` skips profiles already published — so reruns are idempotent
and pubkeys are stable across trials.

## Verifier

Reads the post-agent `/logs/artifacts/nimino-evidence.json` snapshot. The
...
```bash
just benchmark \
  --path benchmarks/nimino-dataset/create-channel-invite-users \
  --attempts 1 \
...
`../nimino-orchestra/tests/test_create_channel_invite_users_verifier.py`.
