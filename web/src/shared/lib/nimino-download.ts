export const NIMINO_RELEASES_URL =
  "https://github.com/asopitech-labs/nimino/releases";
const NIMINO_RELEASES_API_URL =
  "https://api.github.com/repos/asopitech-labs/nimino/releases?per_page=10";
const CACHE_KEY = "nimino.latestDownload.v1";
const CACHE_TTL_MS = 60 * 60 * 1000;

export type NiminoDownloadPlatform = {
...
): NiminoDownloadPlatform["operatingSystem"] {
...
function normalizeArchitecture(
  value: string,
): NiminoDownloadPlatform["architecture"] {
...
export async function detectNiminoDownloadPlatform(
  navigatorValue: Navigator,
): Promise<NiminoDownloadPlatform> {
  const userAgentData = (
...
function assetPattern(platform: NiminoDownloadPlatform): RegExp | undefined {
...
export function selectNiminoDownloadUrl(
...
export async function resolveNiminoDownloadUrlForPlatform(
  platform: NiminoDownloadPlatform,
): Promise<string> {
  try {
  ...
  try {
    const response = await fetch(NIMINO_RELEASES_API_URL, {
      headers: { Accept: "application/vnd.github+json" },
    });
    if (!response.ok) return NIMINO_RELEASES_URL;
    const url = selectNiminoDownloadUrl(
      (await response.json()) as GitHubRelease[],
      platform,
    );
    if (!url) return NIMINO_RELEASES_URL;
    try {
    ...
    return NIMINO_RELEASES_URL;
  }
}

export async function resolveNiminoDownloadUrl(): Promise<string> {
  return resolveNiminoDownloadUrlForPlatform(
    await detectNiminoDownloadPlatform(navigator),
  );
}
