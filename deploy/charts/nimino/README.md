...
[Nimino](https://github.com/asopitech-labs/nimino) is a Nostr-based messaging platform for human–agent collaboration: a single relay binary serving WebSocket + REST + web UI, backed by PostgreSQL, Redis, and S3-compatible object storage.
...
```sh
helm install nimino oci://ghcr.io/asopitech-labs/nimino/charts/nimino --version 0.1.7 \
  --create-namespace --namespace nimino \
  --set quickstart=true \
  --set postgresql.enabled=true \
  --set redis.enabled=true \
  --set minio.enabled=true \
  --set relayUrl=wss://nimino.example.com \
...
`NIMINO_S3_ENDPOINT` plus autogenerated credentials automatically. No external services required. The `quickstart=true` flag is an
...
Nimino uses one URL style for both media and Git/CAS object-store requests:
...
`NIMINO_S3_ADDRESSING_STYLE` and `s3.region` as `NIMINO_S3_REGION`. The region
defaults to `us-east-1`, keeping bundled MinIO and the in-pod
`nimino-admin deletions` workflow operable without an ambient `AWS_REGION`.
...
rendering `NIMINO_S3_REGION=us-east-1` after upgrade, even if an image or
...
Store `NIMINO_S3_ACCESS_KEY=${{Object Storage.ACCESS_KEY_ID}}` and
`NIMINO_S3_SECRET_KEY=${{Object Storage.SECRET_ACCESS_KEY}}` in the Secret named by
...
`NIMINO_GIT_CONFORMANCE_PROBE` is enabled (the relay default). A probe failure is
...
relay:
  ...
  args: [/usr/local/bin/nimino-relay]
  extraVolumeMounts:
    - name: wrapper
      mountPath: /opt/wrapper
```
...
The chart can run Nimino's stateless pairing WebSocket relay as an independent
...
`pairingRelay.url` is advertised in the main relay's NIP-11 document so Nimino
...
`<release>-nimino-pairing:5000` with your platform's ingress configuration.

## HA (production)

`replicaCount > 1` hard-requires Redis:

- Redis (`redis.enabled=true`, `externalRedis.url`, or `REDIS_URL` in `existingSecret`) — for `nimino-pubsub` fan-out
...
autoscaling:
  ...
  websocketMetricName: nimino_ws_connections_active
...
`nimino_ws_connections_active` gauge as a pod metric with the name in
...
- **Bundled MinIO is eval-only.** The quickstart profile runs an in-cluster
  ...
  `minio.enabled` off and points `s3.endpoint` (or `NIMINO_S3_*` in
...
- **Minimal-mode is not yet supported.** The relay's `NIMINO_PUBSUB=local` /
...
`oci://ghcr.io/asopitech-labs/nimino/charts/nimino` by the unified Nimino relay
candidate workflow (`.github/workflows/nimino-relay-release.yml`). A reviewed
`nimino-v<version>` tag builds the native amd64/arm64 relay image, renders and
clean-installs this chart, publishes the chart and digest-pinned Compose bundle,
signs every artifact, and records them in one release set. The old independent
`chart-v*` publisher is disabled.
...
for f in ci/*-values.yaml tests/fixtures/*-values.yaml; do
  helm template nimino . -f "$f" >/dev/null && echo "ok: $f"
done
