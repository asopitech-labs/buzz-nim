...
A minimal three-agent persona pack for Nimino.
...
```bash
# Validate the pack
nimino pack validate ./examples/meadow-core

# Inspect resolved config
nimino pack inspect ./examples/meadow-core
```
...
app), not persona-pack source. `nimino pack inspect` above shows the fully-resolved per-agent
...
`crates/nimino-persona/PERSONA_PACK_SPEC.md` for the current import paths.
...
See `crates/nimino-persona/PERSONA_PACK_SPEC.md` for the full format reference.
