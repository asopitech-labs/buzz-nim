...
const BOT_ABOUT: &str =
    "A tiny non-AI Nimino reference bot that replies to !countdown and countdown-style !fib.";
    ...
    fn from_env() -> Result<Self> {
        ...
        let owner_auth_tag = match auth_mode.as_str() {
            ...
            "owner-attested" => {
                ...
                let owner = nimino_sdk::nip_oa::verify_auth_tag(&tag_json, &bot_keys.public_key())
                ...
                Some(nimino_sdk::nip_oa::parse_auth_tag(&tag_json)?)
            }
            other => {
                bail!(
...
async fn publish_profile(ws: &mut Ws, config: &Config) -> Result<()> {
    let builder = nimino_sdk::builders::build_profile(
...
async fn maybe_reply(
    ...
    let builder = nimino_sdk::builders::build_message(
