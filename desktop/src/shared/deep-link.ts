...
/**
 * Payload emitted by the Rust deep-link handler for `nimino://message?…`.
...
/**
 * Payload emitted by the Rust deep-link handler for `nimino://join?…` —
 ...
 *
 * When a `nimino://connect?relay=<url>` link is opened, the handler
 ...
 *
 * When a `nimino://join?relay=<url>&code=<invite>` link is opened (relay
 ...
 *
 * `nimino://message?…` is handled separately by `listenForNavigationDeepLinks`,
...
/**
 * Register a listener for `deep-link-entity` events — the `nimino://` share
