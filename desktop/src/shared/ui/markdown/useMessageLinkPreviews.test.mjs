...
const ENTITY_HREF = `nimino://pr?id=${EVENT_ID}&owner=${OWNER}&d=nimino-world`;
...
test("Nimino entity links do not create message preview cards", () => {
  ...
  const snapshots = parseLinkPreviewSnapshots(
    [
      snapshotTag(ENTITY_HREF, "Forged sender title", "Definitely Real Nimino"),
