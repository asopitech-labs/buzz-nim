...
function EntityMetadataTooltip({
  ...
  React.useEffect(() => {
    let cancelled = false;
    void loadNiminoEntityMetadata(href).then((value) => {
...
/**
 * Navigate to the project detail view for a `nimino://pr|issue|repo` link.
...
/**
 * Resolve an anchor href to a canonical `nimino://` entity link, accepting
 * both the deep-link scheme directly and HTTPS relay clone URLs (which the
 * preview parser normalizes onto `nimino://repo` only when the URL origin
...
/**
 * Render an inline anchor for a Nimino entity link (`nimino://pr|issue|repo` or
...
export function renderEntityLinkAnchor({
  ...
  const chip = (metadata?: LinkPreviewMetadata | null) => {
    ...
    return (
      <NiminoLinkChip
        data-nimino-link-kind={parsed.value.type}
        href={href}
