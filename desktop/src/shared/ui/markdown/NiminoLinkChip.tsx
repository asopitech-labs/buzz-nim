...
function useNiminoLinkContextMenu({
  href,
  interactive,
  onOpenLink,
}: {
  ...
  const contextMenu =
    position && href ? (
      <MediaContextMenu
        dataAttributes={["data-nimino-link-context-menu"]}
        items={[
          {
            label: "Open link",
            onSelect: () => {
...
export function NiminoLinkChip({
  children,
  className,
  href,
  icon: Icon,
  ...
  const { contextMenu, onContextMenuCapture } = useNiminoLinkContextMenu({
    href,
    interactive,
    onOpenLink,
  });
...
export function NiminoInlineLink({
  children,
  href,
  interactive,
  onOpenLink,
