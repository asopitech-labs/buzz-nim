...
const CHANNELS = ["nimino-bugs", "nimino dev", "general"];
...
test("triggers at start of string", () => {
  assert.deepEqual(at("#", "#nimino", CHANNELS), {
    query: "nimino",
    startIndex: 0,
  });
});
...
test("triggers after whitespace", () => {
  assert.deepEqual(at("#", "go to #nimino", CHANNELS), {
    query: "nimino",
    startIndex: 6,
  });
});
...
test("does NOT trigger when glued to a word character", () => {
  assert.equal(at("#", "foo#nimino", CHANNELS), null);
...
test("multi-word channel name after ( resolves via multi-word path", () => {
  assert.deepEqual(at("#", "(#nimino de", CHANNELS), {
    query: "nimino de",
    startIndex: 1,
  });
});
...
test("multi-word: glued-to-word prefix still rejected", () => {
  // `x#nimino de` — `#` preceded by `x`, no boundary → no query even though
  // "nimino de" is a prefix of a known channel.
  assert.equal(at("#", "x#nimino de", CHANNELS), null);
});
