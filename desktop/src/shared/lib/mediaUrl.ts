 ...
 *
 * Only URLs hosted on the Nimino relay are rewritten. External Blossom URLs
 ...
 * (the Nimino relay), resulting in 404s.
 */
...
/**
 * Cached relay origin (e.g. "https://staging.relay.nimino.example"),
 ...
 * (`normalize_host` in nimino-core), but the saved community relay URL keeps
...
/**
 * The relay origin (e.g. `https://staging.relay.nimino.example`) if it has
...
/**
 * If `url` is a Blossom media URL hosted on the Nimino relay, rewrite it
...
export function rewriteRelayUrl(url: string): string {
  ...
  // was typed with uppercase (e.g. wss://PENDING-SEED.communities.nimino.xyz).
  if (cachedRelayOrigin) {
