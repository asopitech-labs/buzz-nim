...
test("rewriteRelayUrl: matches relay origin case-insensitively (uppercase saved community URL)", async () => {
    ...
    __TAURI_INTERNALS__: {
      invoke(command) {
        ...
        if (command === "get_relay_http_url") {
          ...
          // emits lowercased media URLs (normalize_host in nimino-core).
          return Promise.resolve("https://PENDING-SEED.communities.nimino.xyz");
        }
        return Promise.reject(new Error(`Unexpected command: ${command}`));
      },
    },
  ...
  try {
    ...
    const relayMediaUrl = `https://pending-seed.communities.nimino.xyz/media/${HASH}.png`;
    assert.equal(
