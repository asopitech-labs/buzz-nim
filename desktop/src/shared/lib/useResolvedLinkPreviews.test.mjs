...
test("pending Nimino entity metadata remains image-less", () => {
  const entityPreview = {
    kind: "nimino-repository",
    href: `nimino://repo?owner=${"cd".repeat(32)}&d=nimino`,
    provider: "Nimino",
    title: "nimino",
    typeLabel: "repo",
  };
...
test("metadata cache keys deduplicate URL fragments", () => {
  assert.equal(
    __linkPreviewMetadataTest.metadataCacheKey(
      "https://github.com/asopitech-labs/nimino/pull/3834#issuecomment-1",
    ),
    "https://github.com/asopitech-labs/nimino/pull/3834",
  );
});
...
test("withEntityFallbacks re-adds previews dropped by null metadata", () => {
  const entityPreview = {
    ...
    href: `nimino://pr?id=${"ab".repeat(32)}&owner=${"cd".repeat(32)}&d=nimino`,
    provider: "Nimino",
    title: `nimino #${"ab".repeat(4)}`,
    typeLabel: "Review",
  };
...
test("withEntityFallbacks keeps resolved previews and preserves order", () => {
  ...
  const second = {
    kind: "nimino-issue",
    href: `nimino://issue?id=${"ef".repeat(32)}&owner=${"cd".repeat(32)}&d=nimino`,
    provider: "Nimino",
    title: `nimino #${"ef".repeat(4)}`,
    typeLabel: "Task",
  };
...
test("entity fallback eligibility is kind-scoped", () => {
  ...
  assert.equal(
    isNiminoEntityPreview({ ...preview, href: "nimino://future?id=example" }),
    false,
  );
});
...
test("withEntityFallbacks still drops unresolved external links", () => {
  ...
  assert.deepEqual(
    withEntityFallbacks(
      [{ ...preview, href: "nimino://future?id=example" }],
      [],
    ),
    [],
  );
...
test("Nimino PR metadata includes repository identity and trusted root context", async () => {
  const owner = "cd".repeat(32);
  const attacker = "ef".repeat(32);
  const id = "ab".repeat(32);
  const repoAddress = `30617:${owner}:nimino`;
  ...
  const result = await fetchNiminoEntityMetadata(
    `nimino://pr?id=${id}&owner=${owner}&d=nimino`,
    fetchEvents,
  );
  assert.equal(result?.siteName, "Nimino Desktop");
...
test("Nimino entity roots reject ambiguous repository tags", async () => {
  const owner = "cd".repeat(32);
  const attacker = "ef".repeat(32);
  const targetAddress = `30617:${owner}:nimino`;
  ...
  for (const [type, kind] of [
    ...
    const result = await fetchNiminoEntityMetadata(
      `nimino://${type}?id=${id}&owner=${owner}&d=nimino`,
      async (filter) =>
...
test("Nimino repository metadata stays image-less and exposes default branch", async () => {
  const owner = "cd".repeat(32);
  const result = await fetchNiminoEntityMetadata(
    `nimino://repo?owner=${owner}&d=relay-tools`,
    async () => [
      relayEvent({
        id: "01".repeat(32),
        kind: 30617,
...
test("Nimino project metadata resolves from the 30621 announcement", async () => {
  const owner = "cd".repeat(32);
  const result = await fetchNiminoEntityMetadata(
    `nimino://project?owner=${owner}&d=pollinator`,
    async () => [
      relayEvent({
        id: "01".repeat(32),
        kind: 30621,
...
test("Nimino project metadata declines a missing or invalid announcement", async () => {
  const owner = "cd".repeat(32);
  assert.equal(
    await fetchNiminoEntityMetadata(
      `nimino://project?owner=${owner}&d=pollinator`,
      async () => [],
    ),
    null,
