...
test("stripConfigNudgeSentinel strips the sentinel block", () => {
  ...
  assert.ok(!stripped.includes("nimino:config-nudge"), "sentinel must be gone");
...
test("authGuard_mismatchedAuthor_returnsNull", () => {
  ...
  assert.ok(
    body.includes("nimino:config-nudge"),
...
test("nudgePresent_extractNonNull_and_stripRemovesSentinel", () => {
  ...
  assert.ok(
    !stripped.includes("nimino:config-nudge"),
