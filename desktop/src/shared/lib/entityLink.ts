 ...
 *
 * Formats:
 *   nimino://repo?owner=<owner-pubkey>&d=<repo-dtag>[&tab=<tab>][&commit=<git-hash>]
 *   nimino://project?owner=<owner-pubkey>&d=<project-dtag>[&tab=<tab>]
 *   nimino://pr?id=<event-id>&owner=<owner-pubkey>&d=<repo-dtag>
 *   nimino://issue?id=<event-id>&owner=<owner-pubkey>&d=<repo-dtag>
 *
...
const ENTITY_LINK_SCHEME = "nimino:";
...
export function buildRepoLink(input: {
  ...
  return `nimino://repo?owner=${input.owner.toLowerCase()}&d=${input.dtag}${tabSuffix(input.tab)}`;
}
...
export function buildCommitLink(input: {
  ...
  return `nimino://repo?owner=${input.owner.toLowerCase()}&d=${input.dtag}&tab=commits&commit=${input.commitHash.toLowerCase()}`;
}
...
export function buildProjectLink(input: {
  ...
  checkCoordinate(input.owner, input.dtag);
  return `nimino://project?owner=${input.owner.toLowerCase()}&d=${input.dtag}${tabSuffix(input.tab)}`;
}
...
export function buildPullRequestLink(input: {
  ...
  checkCoordinate(input.owner, input.dtag);
  return `nimino://pr?id=${input.id.toLowerCase()}&owner=${input.owner.toLowerCase()}&d=${input.dtag}`;
}
...
export function buildIssueLink(input: {
  ...
  checkEventId(input.id);
  checkCoordinate(input.owner, input.dtag);
  return `nimino://issue?id=${input.id.toLowerCase()}&owner=${input.owner.toLowerCase()}&d=${input.dtag}`;
}

/**
 * Cheap pre-check used by the markdown renderer and preview extraction
 * before parsing. `nimino://message` is intentionally excluded — it has its
 * own pill rendering path.
 */
export function isEntityLink(href: string | undefined | null): boolean {
  if (!href) return false;
  return (
    href.startsWith("nimino://pr?") ||
    href.startsWith("nimino://issue?") ||
    href.startsWith("nimino://repo?") ||
    href.startsWith("nimino://project?")
  );
}

/**
 * Parse a `nimino://pr|issue|repo?…` URL. Returns a discriminated result so
