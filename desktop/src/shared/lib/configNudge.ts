/**
 * Utilities for extracting and parsing the `nimino:config-nudge` sentinel that
 * `nimino-acp`'s setup-listener appends to its kind:9 nudge body.
 *
...
/**
 * The structured payload embedded in the `nimino:config-nudge` sentinel block.
...
const FENCE_OPEN = "```nimino:config-nudge";
const FENCE_CLOSE = "```";
...
/**
 * Strip the `nimino:config-nudge` sentinel block (and any preceding blank line)
