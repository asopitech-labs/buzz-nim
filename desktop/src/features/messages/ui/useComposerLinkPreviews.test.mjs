...
//
// A same-relay `/git/<owner>/<repo>` clone URL is a Nimino repository entity: the
...
test("composer input classifies a same-relay clone URL as a Nimino entity", async () => {
  const { updateComposerLinkPreviewInput } = await import(
    "./useComposerLinkPreviews.tsx"
  );
  const empty = {
...
test("composer never fetches a snapshot for a same-relay clone URL", async () => {
  ...
  try {
    ...
    assert.equal(
      fetchCalls,
      0,
      "a Nimino repository entity must not enter external snapshot fetching",
    );
