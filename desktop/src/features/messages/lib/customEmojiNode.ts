...
export function registerCustomEmojiMarkdownIt(
  ...
  const RULE_NAME = "nimino_custom_emoji";
  const TOKEN_TYPE = "nimino_custom_emoji";
  ...
  const rule = (state: any, silent: boolean): boolean => {
    ...
    // word char. This keeps prose and URLs intact — `not:nimino:` and
    // `http://x:y:nimino:` must NOT turn the inner `:nimino:` into an image;
