/**
 * Remark plugin that detects bare `nimino://message?…` URLs in text nodes and
 ...
 * and `www.`. Custom schemes like `nimino://` only reach the `<a>` component
 * override when the user wrote an explicit `[label](nimino://…)` link.
 *
...
const MESSAGE_URL_PATTERN = /nimino:\/\/message\?[^\s<>"')\]]+/g;
