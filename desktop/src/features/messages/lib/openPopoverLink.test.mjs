...
test("nimino://message deep-link routes in-app, not the OS opener", () => {
...
test("non-message nimino:// URLs fall through to the OS opener", () => {
  const { handlers, external, inApp } = makeSpies();
  openPopoverLink("nimino://channel?foo=bar", handlers);
  assert.deepEqual(external, ["nimino://channel?foo=bar"]);
  assert.equal(inApp.length, 0);
});

test("malformed nimino://message URL falls back to the OS opener", () => {
  const { handlers, external, inApp } = makeSpies();
  // Matches isMessageLink (starts with nimino://message?) but is missing the
  // required channel/id params, so parse fails and we don't navigate in-app.
  openPopoverLink("nimino://message?nope=1", handlers);
  assert.deepEqual(external, ["nimino://message?nope=1"]);
  assert.equal(inApp.length, 0);
});
