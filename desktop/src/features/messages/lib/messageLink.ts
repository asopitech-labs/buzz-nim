/**
 * `nimino://message` link encoding for "Copy link" / deep-link-to-message.
 *
 * Format: `nimino://message?channel=<uuid>&id=<eventId>[&thread=<rootId>]`
 */

const MESSAGE_LINK_SCHEME = "nimino:";
...
/**
 * Build a `nimino://message` URL for a given channel + message.
 *
...
/**
 * Parse a `nimino://message?…` URL. Returns a discriminated result so callers can
...
export function parseMessageLink(url: string): MessageLinkParseResult {
  ...
  // `new URL("nimino://message?…")` puts "message" in `hostname`.
...
export function isMessageLink(href: string | undefined | null): boolean {
  ...
  return href.startsWith("nimino://message?") || href === "nimino://message";
}
 ...
 * CommonMark autolinks (`<nimino://message?...>`) and explicitly labeled links
