...
const EMOJI_TAGS = [["emoji", "nimino", "https://relay/nimino.png"]];
...
test("derives custom emoji and emoji-only flag from event tags", () => {
  ...
  assert.deepEqual(customEmoji, [
    { shortcode: "nimino", url: "https://relay/nimino.png" },
  ]);
  assert.equal(isEmojiOnlyMessage(":nimino:", customEmoji), true);
  assert.equal(isEmojiOnlyMessage("hi :nimino:", customEmoji), false);
});

test("messages without tags get no custom emoji and are never emoji-only", () => {
  const customEmoji = undefined;
  assert.equal(isEmojiOnlyMessage(":nimino:", customEmoji), false);
});
