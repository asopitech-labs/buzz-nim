...
const DESKTOP_NOTIFICATION_ACTION_EVENT = "nimino:desktop-notification-action";
...
type TestWindow = Window & {
  ...
  __NIMINO_E2E_APP_BADGE_STATE__?: AppBadgeState["kind"];
};
...
function notificationExtra(
  ...
  return {
    niminoNotificationTarget: target,
  };
}
...
export async function listenForDesktopNotificationActions(
  ...
  if (isTauri()) {
      ...
      try {
        pluginListener = await onAction((notification) => {
          const target = parseNotificationTarget(
            notification.extra?.niminoNotificationTarget,
          );
          if (!target) {
            return;
          }
    ...
    if (usesMacActivationQueue) {
      // Belt and suspenders for asopitech-labs/nimino#3509: the Rust delegate queues the
...
export async function setDesktopAppBadge(state: AppBadgeState): Promise<void> {
  if (typeof window !== "undefined") {
    ...
    testWindow.__NIMINO_E2E_APP_BADGE_COUNT__ =
      state.kind === "count" ? state.count : 0;
    testWindow.__NIMINO_E2E_APP_BADGE_STATE__ = state.kind;
  }
 ...
 * routing (asopitech-labs/nimino#3509).
 */
...
export async function sendDesktopNotification(
  ...
  // asopitech-labs/nimino#5081 — WebKit throws `NotificationError` from the constructor
