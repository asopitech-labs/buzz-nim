 ...
 * slot stays inside the `.nimino-onboarding-neutral-theme` subtree so
 * `--nimino-welcome-chartreuse` and the theme color tokens still resolve for the
 * docked buttons.
 */
export function OnboardingFooterProvider({
  ...
  return (
    <OnboardingFooterTargetContext.Provider value={target}>
      ...
      <div
        aria-hidden
        className="pointer-events-none fixed inset-x-0 bottom-0 z-10 h-36 bg-[linear-gradient(to_top,var(--nimino-onboarding-shell-bottom)_35%,transparent)]"
      />
      {backAction ? (
