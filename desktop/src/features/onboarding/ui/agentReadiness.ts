...
export type AgentReadinessResult =
  ...
  | { ready: true; reason: "nimino-agent" }
  | { ready: false };
 ...
 * Provider path: the preferred Nimino Agent or Goose runtime has provider and
...
export function resolveAgentReadiness(
  ...
  if (scope === "any") {
    for (const runtime of runtimes) {
      if (runtime.id === "nimino-agent") continue;
      if (
  ...
  const preferredRuntime =
    scope === "preferred"
      ...
      : runtimes.find((runtime) => runtime.id === "nimino-agent");
  ...
  if (
    preferredRuntime.id !== "nimino-agent" &&
  ...
  if (provider.length > 0 && model.length > 0) {
    ...
    if (allKeysPresent) {
      return { ready: true, reason: "nimino-agent" };
    }
  }
