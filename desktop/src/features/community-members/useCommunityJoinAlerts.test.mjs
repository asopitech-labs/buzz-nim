...
function seedCommunities(activeId) {
  ...
  storage.set("nimino-active-community-id", activeId);
}
...
function ledgerKeys() {
  return [...storage.keys()].filter((key) =>
    key.startsWith("nimino-community-join-seen.v1"),
  );
}
...
describe("useCommunityJoinAlerts — mounted subscription behaviour", () => {
  ...
  it("is completely inert for a plain member: no subscription, no ledger key", async () => {
    ...
    assert.deepEqual(
      ledgerKeys(),
      [],
      "no nimino-community-join-seen.v1 key may be created for a plain member",
    );
