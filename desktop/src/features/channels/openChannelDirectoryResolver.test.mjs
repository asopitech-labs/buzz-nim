...
beforeEach(() => {
  ipc.reset();
  localStorage.clear();
  localStorage.setItem("nimino-communities", JSON.stringify([COMMUNITY]));
  localStorage.setItem("nimino-active-community-id", COMMUNITY.id);
});
...
test("markdown message links resolve private destinations without a directory scan", async () => {
  ...
  const link = `nimino://message?channel=${channelId}&id=${messageId}`;
  const renderPaths = [
  ...
  for (const [path, content] of renderPaths) {
    ...
    assert.notEqual(
      mounted.container.querySelector(
        "span[data-message-link][data-nimino-link]",
      ),
      null,
...
test("authored-label channel and message links respect the private-destination gate", async () => {
  ...
  //   - nimino://channel/<uuid> and nimino://channel/<uuid>/<event-id> reach the
  //     gate via ChannelDeepLinkAnchor's authored branch, and
  //   - the canonical nimino://message?channel=&id= form (produced by
  ...
  const channelMessageLink = `nimino://channel/${channelId}/${messageId}`;
  const canonicalMessageLink = `nimino://message?channel=${channelId}&id=${messageId}`;
  const renderPaths = [
  ...
  for (const [path, content] of renderPaths) {
    ...
    assert.notEqual(
      mounted.container.querySelector("span[data-nimino-link]"),
      null,
