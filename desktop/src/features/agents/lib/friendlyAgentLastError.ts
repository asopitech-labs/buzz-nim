 ...
 *   nimino-agent — classifies LLM failures into `AgentError` variants with
 *                  JSON-RPC codes (`-32001` auth, `-32002` model-not-found,
 *                  `-32000` generic), defined in `crates/nimino-agent/src/types.rs`.
 *   nimino-acp   — preserves the code structurally in
...
export function friendlyAgentLastError(
  ...
  // Match either the unwrapped nimino-agent prefix or the nimino-acp v0 wrap.
  if (
