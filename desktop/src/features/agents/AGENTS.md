...
Plan of record: `Nimino/Harness-Provider-Model.md` in Morgan's Obsidian vault
...
2. **Effort reads/writes go through the descriptor.** Use the effort
   ...
   `NIMINO_AGENT_THINKING_EFFORT` literal in UI code. `currentPersistence` is
...
7. **Onboarding setup detects readiness; it does not select defaults.** The
   ...
   requires provider/model/credential config (e.g. nimino-agent with no
...
9. **The defaults modal is progressively disclosed.** An unset global config
   starts on the Nimino Agent-first deployment fallback and carries that visible
   harness into the next saved edit. The `progressive-defaults` disclosure
   preset therefore begins at Provider for Nimino Agent, then reveals Model,
...
11. **Shared agent access names the consequence where it is selected.** The
   ...
   installed `nimino-backend-*` provider, and without one `WhereToRunSection`
...
14. **Thinking effort has two surfaces: a local-only WRITE control and a
   ...
   **Cut invariant — live mid-conversation effort machinery was deliberately
   ...
   read from `NIMINO_ACP_EFFORT_LEVEL` and applies it once at session creation
   (`apply_startup_effort` in `nimino-acp/src/pool.rs`); there is no pool-level
