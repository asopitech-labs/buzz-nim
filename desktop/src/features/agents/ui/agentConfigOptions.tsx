...
import { NIMINO_AGENT_THINKING_EFFORT } from "./niminoAgentConfig";
 ...
 *
 * Mirrors the Rust `readiness::nimino_agent_requirements` /
...
export const PERSONA_LLM_PROVIDER_OPTIONS: readonly PersonaModelOption[] = [
  ...
  { id: "relay-mesh", label: "Nimino shared compute" },
...
> = {
  ...
  "nimino-agent": [DEFAULT_MODEL_OPTION],
 ...
 *
 * nimino-agent and goose use provider-specific credentials; claude and codex
...
export function requiredCredentialEnvKeys(
  ...
  if (normalizedRuntime !== "nimino-agent" && normalizedRuntime !== "goose") {
    return [];
  }
...
export function runtimeSupportsLlmProviderSelection(runtimeId: string) {
  return runtimeId === "nimino-agent" || runtimeId === "goose";
}
 ...
 *
 * Two classes of required field for provider-selection runtimes (nimino-agent,
...
export function computeLocalModeGate({
  ...
  /** Env keys that are not set in Nimino but are satisfied in the runtime's
