 ...
 * persona runtime string (e.g. `nimino-agent`). A catalog lookup then returns
 ...
 *
 * - nimino-agent / goose → "capable" (`isProviderCapable`, id-based).
 ...
 * nimino-agent/Anthropic). Persisting the local (empty) provider + credential-
 ...
 *
 * MODEL follows the same transition rule. nimino-agent/goose readiness requires a
