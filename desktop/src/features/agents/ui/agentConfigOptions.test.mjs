...
test("getPersonaProviderOptions returns databricks v1 and v2 when hideProviderIds is empty", () => {
  const options = getPersonaProviderOptions("", "nimino-agent", "", new Set());
...
test("getPersonaProviderOptions with no hideProviderIds omits the tail for a known provider", () => {
  const options = getPersonaProviderOptions("anthropic", "nimino-agent");
  const tail = options.at(-1);
...
// ── getDefaultPersonaRuntime — nimino-agent first ───────────────────────────────

test("getDefaultPersonaRuntime honors an available global preference", () => {
  const runtimes = [
    makeRuntime("nimino-agent"),
    makeRuntime("goose"),
    makeRuntime("claude"),
  ];
...
test("getDefaultPersonaRuntime returns nimino-agent over goose when both are available", () => {
  const runtimes = [
    makeRuntime("goose"),
    makeRuntime("nimino-agent"),
    makeRuntime("claude"),
  ...
  assert.equal(result?.id, "nimino-agent");
});

test("getDefaultPersonaRuntime falls back to goose when nimino-agent is unavailable", () => {
  const runtimes = [
    makeRuntime("nimino-agent", "not_installed"),
    makeRuntime("goose"),
  ];
  const result = getDefaultPersonaRuntime(runtimes);
...
test("getDefaultPersonaRuntime returns first available when neither nimino-agent nor goose is available", () => {
  const runtimes = [
    makeRuntime("nimino-agent", "adapter_missing"),
...
test("runtimeSupportsLlmProviderSelection is true for nimino-agent and goose", () => {
  assert.equal(runtimeSupportsLlmProviderSelection("nimino-agent"), true);
...
test("getPersonaModelOptions for nimino-agent with anthropic filters out zero-value default", () => {
  // anthropic requires explicit model — zero-value option is filtered out
  const options = getPersonaModelOptions("nimino-agent", "anthropic");
...
test("getPersonaModelOptions for nimino-agent with no provider returns default model", () => {
  const options = getPersonaModelOptions("nimino-agent", "");
