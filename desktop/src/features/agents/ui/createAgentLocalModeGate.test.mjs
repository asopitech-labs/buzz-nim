...
test("localMode_niminoAgent_supportsProviderSelection", () => {
  assert.equal(
    runtimeSupportsLlmProviderSelection("nimino-agent"),
    true,
    "nimino-agent must support LLM provider selection",
  );
});
...
test("localMode_niminoAgent_emptyProvider_notSatisfied", () => {
  // Scenario: user selects nimino-agent but leaves provider empty.
  // Rust readiness requires NIMINO_AGENT_PROVIDER — empty = NotReady.
...
test("localMode_niminoAgent_emptyModel_notSatisfied", () => {
  // Scenario: nimino-agent + anthropic + API key present, but model left empty.
  // Rust readiness requires NIMINO_AGENT_MODEL — empty = NotReady.
...
// ── Gate: nimino-agent / anthropic with missing key → markers shown ─────────

test("localMode_niminoAgent_anthropic_missingKey_notSatisfied", () => {
  // Scenario: user selects nimino-agent/anthropic + fills model, but hasn't
...
test("localMode_niminoAgent_anthropic_allRequired_present_allowed", () => {
...
test("localMode_requiredEnvKeys_surfaces_anthropicKey", () => {
  ...
  const allKeys = requiredCredentialEnvKeys("nimino-agent", "anthropic");
  assert.ok(
    allKeys.includes("ANTHROPIC_API_KEY"),
    "requiredCredentialEnvKeys must include ANTHROPIC_API_KEY for nimino-agent/anthropic",
  );
});
...
test("localMode_requiredEnvKeys_gate_and_envVarsEditor_share_same_key_set", () => {
  ...
  const fullKeys = requiredCredentialEnvKeys("nimino-agent", "anthropic");
...
test("localMode_goose_providerSatisfiedByFileConfig_noNormalizedFieldRequired", () => {
  // Scenario: goose, no provider in Nimino env but file config has provider + model.
...
test("providerDefaultLabel_noGlobal_returnsSelectAProvider", () => {
  ...
  const label = getDefaultLlmProviderLabel("nimino-agent", undefined);
  assert.equal(
    label,
    "Select a provider\u2026",
...
test("providerDefaultLabel_emptyGlobal_returnsSelectAProvider", () => {
  ...
  const label = getDefaultLlmProviderLabel("nimino-agent", "");
  assert.equal(
    label,
    "Select a provider\u2026",
...
test("providerDefaultLabel_globalSet_returnsInheritLabel", () => {
  ...
  const label = getDefaultLlmProviderLabel("nimino-agent", "anthropic");
  assert.equal(
    label,
...
test("providerDefaultLabel_globalSetWithWhitespace_trimsAndReturnsInherit", () => {
  ...
  const label = getDefaultLlmProviderLabel("nimino-agent", "  openai  ");
  assert.equal(
    label,
...
test("providerDefaultLabel_sharedCompute_neverLeaksInternalId", () => {
  assert.equal(
    getDefaultLlmProviderLabel("nimino-agent", "relay-mesh"),
    "Use agent defaults (Nimino shared compute)",
  );
});
...
test("bakedDefaults_emptyGlobal_usesBuildValuesForCreateAndEditLabels", () => {
  const bakedEnv = [
    { key: "NIMINO_AGENT_PROVIDER", value: "databricks_v2", masked: false },
    {
      key: "NIMINO_AGENT_MODEL",
      value: "goose-claude-opus-4-8",
      masked: false,
    },
    { key: "NIMINO_AGENT_THINKING_EFFORT", value: "high", masked: false },
  ];
  ...
  const model = resolveInheritedDefault(null, bakedEnv, "NIMINO_AGENT_MODEL");
  const effort = resolveInheritedDefault(
    null,
    bakedEnv,
    "NIMINO_AGENT_THINKING_EFFORT",
  );
  const providerOptions = getPersonaProviderOptions("", "nimino-agent");
...
test("bakedDefaults_explicitGlobalsOverrideBuildValues", () => {
  const bakedEnv = [
    { key: "NIMINO_AGENT_PROVIDER", value: "databricks_v2", masked: false },
    { key: "NIMINO_AGENT_MODEL", value: "build-model", masked: false },
    { key: "NIMINO_AGENT_THINKING_EFFORT", value: "high", masked: false },
  ];

  assert.deepEqual(
    resolveInheritedDefault("anthropic", bakedEnv, "NIMINO_AGENT_PROVIDER"),
    { source: "global", value: "anthropic" },
  );
  assert.deepEqual(
    resolveInheritedDefault("global-model", bakedEnv, "NIMINO_AGENT_MODEL"),
    { source: "global", value: "global-model" },
  );
  assert.deepEqual(
    resolveInheritedDefault("low", bakedEnv, "NIMINO_AGENT_THINKING_EFFORT"),
...
test("f3b_buildTemplateModelDropdownOptions_anthropicGlobalModelSet_containsInheritOption", () => {
  ...
  const staticOptions = getPersonaModelOptions("nimino-agent", "anthropic");
...
test("f3b_buildTemplateModelDropdownOptions_blankProviderGlobalModelSet_noDoubleSeed", () => {
  ...
  const staticOptions = getPersonaModelOptions("nimino-agent", "");
...
// include ANTHROPIC_API_KEY for explicit nimino-agent/anthropic so that the
...
test("providerConfig_explicitAnthropic_requiredKeysIncludesApiKey", () => {
  ...
  const required = requiredCredentialEnvKeys("nimino-agent", "anthropic");
  assert.ok(
    required.includes("ANTHROPIC_API_KEY"),
    "nimino-agent + explicit anthropic must list ANTHROPIC_API_KEY as required",
  );
});
...
test("providerConfig_inheritThenExplicitAnthropic_sameRequiredKeys", () => {
  ...
  const inheritKeys = requiredCredentialEnvKeys("nimino-agent", "");
  const explicitKeys = requiredCredentialEnvKeys("nimino-agent", "anthropic");
  assert.ok(
...
test("providerConfig_databricks_requiredKeyIsHost_noSecretClear", () => {
  ...
  assert.ok(
    ...
    "nimino-agent + databricks must list DATABRICKS_HOST as required",
  );
