...
import {
  ...
  isNiminoAgentRuntime,
} from "./niminoAgentConfig.ts";

// ---------------------------------------------------------------------------
// Thinking effort values
// ---------------------------------------------------------------------------

test("NIMINO_AGENT_THINKING_EFFORT_VALUES contains exactly the 7 accepted values", () => {
  assert.deepEqual(
    [...NIMINO_AGENT_THINKING_EFFORT_VALUES],
...
test("NIMINO_AGENT_THINKING_EFFORT_VALUES has no duplicates", () => {
  const set = new Set(NIMINO_AGENT_THINKING_EFFORT_VALUES);
  assert.equal(set.size, NIMINO_AGENT_THINKING_EFFORT_VALUES.length);
});
...
test("env var key constants match expected NIMINO_AGENT_* names", () => {
  assert.equal(NIMINO_AGENT_THINKING_EFFORT, "NIMINO_AGENT_THINKING_EFFORT");
  assert.equal(
    NIMINO_AGENT_MAX_OUTPUT_TOKENS,
    "NIMINO_AGENT_MAX_OUTPUT_TOKENS",
  );
  ...
  assert.equal(NIMINO_AGENT_MAX_ROUNDS, "NIMINO_AGENT_MAX_ROUNDS");
});
...
test("isNiminoAgentRuntime returns true only for nimino-agent id", () => {
  assert.equal(isNiminoAgentRuntime("nimino-agent"), true);
});

test("isNiminoAgentRuntime returns false for other runtimes", () => {
  assert.equal(isNiminoAgentRuntime("goose"), false);
  assert.equal(isNiminoAgentRuntime("custom"), false);
  assert.equal(isNiminoAgentRuntime(""), false);
  assert.equal(isNiminoAgentRuntime("nimino-agent-v2"), false);
});
...
test("setting a thinking effort value writes the key into envVars", () => {
  ...
  assert.equal(result[NIMINO_AGENT_THINKING_EFFORT], "high");
});

test("clearing thinking effort removes the key so the agent inherits", () => {
  const initial = { [NIMINO_AGENT_THINKING_EFFORT]: "high" };
  const result = applyEnvVarChange(initial, NIMINO_AGENT_THINKING_EFFORT, "");
  assert.equal(Object.hasOwn(result, NIMINO_AGENT_THINKING_EFFORT), false);
});

test("setting max output tokens writes the exact NIMINO_AGENT_MAX_OUTPUT_TOKENS key", () => {
  const initial = {};
  const result = applyEnvVarChange(
    initial,
    NIMINO_AGENT_MAX_OUTPUT_TOKENS,
  ...
  assert.equal(result[NIMINO_AGENT_MAX_OUTPUT_TOKENS], "4096");
  // Must not affect other keys
  assert.equal(Object.keys(result).length, 1);
});

test("clearing max output tokens removes the key", () => {
  const initial = { [NIMINO_AGENT_MAX_OUTPUT_TOKENS]: "4096" };
  const result = applyEnvVarChange(initial, NIMINO_AGENT_MAX_OUTPUT_TOKENS, "");
  assert.equal(Object.hasOwn(result, NIMINO_AGENT_MAX_OUTPUT_TOKENS), false);
});

test("setting context limit writes the exact NIMINO_AGENT_MAX_CONTEXT_TOKENS key", () => {
  const initial = {};
  const result = applyEnvVarChange(
    initial,
    NIMINO_AGENT_MAX_CONTEXT_TOKENS,
  ...
  assert.equal(result[NIMINO_AGENT_MAX_CONTEXT_TOKENS], "100000");
});

test("clearing context limit removes the key", () => {
  const initial = { [NIMINO_AGENT_MAX_CONTEXT_TOKENS]: "100000" };
  const result = applyEnvVarChange(
    initial,
    NIMINO_AGENT_MAX_CONTEXT_TOKENS,
    "",
  );
  assert.equal(Object.hasOwn(result, NIMINO_AGENT_MAX_CONTEXT_TOKENS), false);
});

test("setting max rounds writes the exact NIMINO_AGENT_MAX_ROUNDS key", () => {
  const initial = {};
  const result = applyEnvVarChange(initial, NIMINO_AGENT_MAX_ROUNDS, "50");
  assert.equal(result[NIMINO_AGENT_MAX_ROUNDS], "50");
});

test("clearing max rounds removes the key", () => {
  const initial = { [NIMINO_AGENT_MAX_ROUNDS]: "50" };
  const result = applyEnvVarChange(initial, NIMINO_AGENT_MAX_ROUNDS, "");
  assert.equal(Object.hasOwn(result, NIMINO_AGENT_MAX_ROUNDS), false);
});

test("changing one field does not disturb other env vars", () => {
  const initial = {
    SOME_OTHER_KEY: "value",
    [NIMINO_AGENT_MAX_OUTPUT_TOKENS]: "2048",
  };
  const result = applyEnvVarChange(initial, NIMINO_AGENT_MAX_ROUNDS, "20");
  assert.equal(result.SOME_OTHER_KEY, "value");
  assert.equal(result[NIMINO_AGENT_MAX_OUTPUT_TOKENS], "2048");
  assert.equal(result[NIMINO_AGENT_MAX_ROUNDS], "20");
});
...
test("thinking effort select is bounded: all 7 accepted values are present in the constant", () => {
  ...
  for (const v of expected) {
    assert.ok(
      NIMINO_AGENT_THINKING_EFFORT_VALUES.includes(v),
      `missing value: ${v}`,
    );
  }
  assert.equal(NIMINO_AGENT_THINKING_EFFORT_VALUES.length, expected.length);
});
...
test("non-numeric string is stored as-is (validation is at the backend)", () => {
  ...
  assert.equal(result[NIMINO_AGENT_MAX_OUTPUT_TOKENS], "not-a-number");
});
...
// Mirrors the `isNiminoAgent` derivation in CreateAgentRuntimeFields.
...
test("isNiminoAgentRuntime(prospectiveRuntimeId) shows fields when Edit resolves nimino-agent even though selectedRuntimeId sentinel is 'inherit'", () => {
  // Simulates Edit dialog state: inheritHarness=true, persona is nimino-agent.
  // selectedRuntimeId would be "inherit" (sentinel for custom-command hiding),
  // but prospectiveRuntimeId correctly resolves to "nimino-agent".
  const selectedRuntimeIdSentinel = "inherit"; // what Edit passes to selectedRuntimeId
  const prospectiveRuntimeId = "nimino-agent"; // what Edit passes to modelTuningRuntimeId

  assert.equal(
    isNiminoAgentRuntime(selectedRuntimeIdSentinel),
    false,
    "sentinel 'inherit' must NOT trigger model-tuning fields",
  );
  assert.equal(
    isNiminoAgentRuntime(prospectiveRuntimeId),
    true,
    "prospectiveRuntimeId 'nimino-agent' MUST trigger model-tuning fields",
  );
});

test("isNiminoAgentRuntime(prospectiveRuntimeId) shows fields when Edit has a pinned nimino-agent (selectedRuntimeId sentinel is also 'inherit')", () => {
  // Simulates Edit dialog with a pinned non-custom runtime:
  // selectedRuntimeId sentinel = "inherit" (non-custom known runtime),
  // prospectiveRuntimeId = "nimino-agent" (selectedRuntime?.id).
  const selectedRuntimeIdSentinel = "inherit";
  const prospectiveRuntimeId = "nimino-agent";

  assert.equal(isNiminoAgentRuntime(prospectiveRuntimeId), true);
  assert.equal(isNiminoAgentRuntime(selectedRuntimeIdSentinel), false);
});

test("isNiminoAgentRuntime(prospectiveRuntimeId) hides fields when Edit resolves to non-nimino-agent", () => {
  // E.g. user switches from nimino-agent to goose in Edit — prospectiveRuntimeId = "goose"
  const prospectiveRuntimeId = "goose";
  assert.equal(isNiminoAgentRuntime(prospectiveRuntimeId), false);
});

test("isNiminoAgentRuntime(prospectiveRuntimeId) hides fields when Edit has no resolved runtime (empty string)", () => {
...
// useEffect in NiminoAgentModelTuningFields and AgentDefaultsSettingsCard
