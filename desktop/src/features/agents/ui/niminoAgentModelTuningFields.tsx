/**
 * Tier-1 nimino-agent model-tuning UI fields.
 *
 ...
 *
 * Used by both `NiminoAgentModelTuningFields` (per-agent/persona dialogs) and
...
export function EffortSelectField({
   ...
   * Defaults to `"Inherit"`.
   *
   * Per-agent callers (NiminoAgentModelTuningFields) pass `"Inherit (agent default)"`
 ...
 *
 * - `maxOutputTokens` / `contextLimit`: minimum 1 — the nimino-agent runtime
 *   rejects 0 for these fields (crates/nimino-agent/src/config.rs:921-928).
...
export function NiminoAgentModelTuningFields({
  envVars,
  inheritedEnvVars,
  model,
  onEnvVarChange,
