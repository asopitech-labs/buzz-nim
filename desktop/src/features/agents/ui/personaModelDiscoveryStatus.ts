...
function isEmptySharedComputeError(message: string): boolean {
  ...
  return (
    ...
    normalized.includes("no nimino shared compute serving members") ||
    normalized.includes("no live nimino shared compute models") ||
...
export function formatModelDiscoveryErrorStatus(
  ...
  if (provider.trim() === "relay-mesh") {
    if (message.includes("waiting for the current member roster")) {
      return {
        message:
          "Nimino is waiting for the relay's member roster. Try again shortly; if this persists, check the relay's membership configuration.",
        tone: "warning",
      };
    }
    ...
    if (message.includes("shared compute is not available in this build")) {
      return {
        message:
          "This version of Nimino cannot use shared compute. Update Nimino or choose another provider.",
        tone: "warning",
      };
    }
    ...
    if (message.includes("shared compute status is malformed")) {
      return {
        message:
          "Nimino received an invalid shared compute status. Check the member machine, then try again.",
        tone: "warning",
      };
    }
    ...
    return {
      message:
        "Nimino couldn't check shared compute through the relay. Check your relay connection and try again.",
      tone: "warning",
    };
  }

  // Spec-reserved auth error text (agent-client-protocol ErrorCode::AuthRequired),
  // surfaced verbatim through nimino-acp's stderr — generic across conformant
  ...
  if (
    ...
    message.includes("NIMINO_AGENT_PROVIDER is required")
  ) {
    return null;
  }
...
function formatDatabricksAuthStatus(
  ...
  if (message.includes("Databricks sign-in is required")) {
    return {
      message:
        "Databricks sign-in is required. Open the model picker to sign in, or run `nimino-agent auth databricks` in a terminal.",
      tone: "muted",
    };
  }
  ...
  if (
    ...
    return {
      message:
        "Databricks sign-in didn't complete. Open the model picker to retry, or run `nimino-agent auth databricks` in a terminal.",
      tone: "warning",
    };
  }
