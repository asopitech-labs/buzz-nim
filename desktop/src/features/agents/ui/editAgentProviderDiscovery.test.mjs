...
test("editAgent_providerFieldVisible_forNiminoAgent", () => {
  assert.equal(
    runtimeSupportsLlmProviderSelection("nimino-agent"),
    true,
    "nimino-agent runtime must expose the provider picker",
  );
});
...
test("editAgent_providerOptions_includesDatabricksV2AndV1OnOSS", () => {
  ...
  const options = getPersonaProviderOptions("", "nimino-agent");
...
test("editAgent_providerOptions_includesCurrentIfCustom", () => {
  const options = getPersonaProviderOptions("my-custom-llm", "nimino-agent");
  const ids = options.map((o) => o.id);
  assert.ok(
...
//
// Switching to nimino-agent runtime (which supports LLM provider selection)
// must make the provider field visible, enabling live discovery.

test("editAgent_runtimeSwitch_toNiminoAgentEnablesProvider", () => {
  // Simulate: user switches from "claude" to "nimino-agent"
  const previousRuntime = "claude";
  const nextRuntime = "nimino-agent";
...
test("editAgent_modelClearedOnRuntimeChange", () => {
  const previousRuntime = "nimino-agent";
...
test("editAgent_catalogArrival_rederivesRuntimeIdWhenNotTouched", () => {
  ...
  const agentCommand = "/usr/local/bin/nimino-agent";
  const catalog = [
    { id: "nimino-agent", command: agentCommand, defaultArgs: [] },
...
test("editAgent_noOpSavePreservesProvider_whenCatalogLate", () => {
  ...
  // If the catalog-arrival effect correctly sets selectedRuntimeId = "nimino-agent",
  // then llmProviderFieldVisible = true and the provider is preserved on save.
  const selectedRuntimeId = "nimino-agent"; // correctly derived after catalog arrival
  const savedProvider = "databricks_v2";
  ...
  assert.equal(
    llmProviderFieldVisible,
    true,
    "provider field must be visible once catalog derives nimino-agent runtime",
  );
  assert.equal(
    providerUpdate,
    undefined,
...
test("editAgent_runtimeDropdown_pinsHarnessWhenConcreteCatalogRuntimeSelected", () => {
  ...
  const _nextRuntimeId = "nimino-agent";
  const catalogRuntime = {
    id: "nimino-agent",
    command: "/usr/local/bin/nimino-agent",
    defaultArgs: [],
  };
...
test("editAgent_runtimeDropdown_keepsInheritWhenCatalogEntryHasNoCommand", () => {
  ...
  const NO_RUNTIME_DROPDOWN_VALUE = "__none__";
  const nextValue = "nimino-agent"; // catalog entry, but adapter missing
  const nextRuntimeId =
    nextValue === NO_RUNTIME_DROPDOWN_VALUE ? "" : nextValue;
  const resolvedRuntimeId = nextRuntimeId || "custom";
  const nextRuntime = { id: "nimino-agent", command: null, defaultArgs: [] };
...
test("editAgent_missingRequiredEnvKey_blocksSaveViaValidity", () => {
  ...
  const base = {
    ...
    selectedRuntimeId: "nimino-agent",
    inheritHarness: false,
...
test("editAgent_inheritedAgentRuntimeSwitch_producesConsistentCommandProviderPair", () => {
  ...
  //
  // After fix: selecting nimino-agent sets inheritHarness=false, so agentCommandUpdate
  // resolves to the nimino-agent command, and provider persists consistently.

  // Initial state: inherited Claude agent
  const inheritHarness = false; // after fix: pinned by runtime switch
  const selectedRuntimeCommand = "/usr/local/bin/nimino-agent";
  ...
  const selectedRuntimeId = "nimino-agent";
  const llmProviderFieldVisible =
    runtimeSupportsLlmProviderSelection(selectedRuntimeId);
...
test("editAgent_inheritCheckboxRoundTrip_doesNotPersistProviderOnInheritedRuntime", () => {
  ...
  // → user picks nimino-agent (inheritHarness→false, selectedRuntimeId='nimino-agent')
  // → user picks databricks_v2
  // → user RE-CHECKS inherit (inheritHarness→true, selectedRuntimeId STAYS 'nimino-agent')
  ...
  const selectedRuntimeId = "nimino-agent"; // dropdown state (stale after re-check)
  ...
  const chosenProvider = "databricks_v2"; // chosen while dropdown was nimino-agent

  // llmProviderFieldVisible is driven by the live dropdown (nimino-agent → true).
  ...
  assert.equal(
    llmProviderFieldVisible,
    true,
    "provider field is visible (dropdown shows nimino-agent) — this is the UX state",
  );
...
test("editAgent_inheritCheckboxRoundTrip_clearsStaleSavedProviderWhenRevertingToInherit", () => {
  // Variant: agent previously had a provider saved (e.g. was pinned to nimino-agent
  ...
  const selectedRuntimeId = "nimino-agent"; // dropdown state
  const selectedRuntime = runtimes.find((r) => r.id === selectedRuntimeId);
...
// persona's runtime is nimino-agent/Goose legitimately carries a provider
...
test("editAgent_inheritedNiminoAgentProvider_preservedOnNameOnlySave", () => {
  // Inherited nimino-agent persona with databricks_v2 snapshot.
  // User makes a name-only edit (never touches runtime or provider).
  // The catalog-arrival effect correctly derived selectedRuntimeId="nimino-agent".

  const inheritHarness = true; // agentCommandOverride == null → inheriting
  const agentCommand = "/usr/local/bin/nimino-agent"; // inherited nimino-agent command
  const runtimes = [
    {
      id: "nimino-agent",
      command: "/usr/local/bin/nimino-agent",
  ...
  const selectedRuntimeId = "nimino-agent"; // correctly derived by catalog-arrival effect
  const selectedRuntime = runtimes.find((r) => r.id === selectedRuntimeId);
  const savedProvider = "databricks_v2"; // valid provider snapshot
  const currentProvider = "databricks_v2"; // unchanged by user

  // llmProviderFieldVisible (UX) is true since dropdown shows nimino-agent.
  const llmProviderFieldVisible =
    runtimeSupportsLlmProviderSelection(selectedRuntimeId);
  assert.equal(
    llmProviderFieldVisible,
    true,
    "provider field must be visible for inherited nimino-agent",
  );

  // The effective runtime for submit: inherited → match agentCommand in catalog → nimino-agent.
  const llmProviderCanPersistAtSubmit = computeCanPersistAtSubmit({
    inheritHarness,
    agentCommand,
    runtimes,
  ...
  assert.equal(
    llmProviderCanPersistAtSubmit,
    true,
    "inherited nimino-agent runtime IS provider-capable — provider must be persistable",
  );
  ...
  assert.equal(
    providerUpdate,
    undefined,
    "name-only save on inherited nimino-agent must omit provider (not send null to clear it)",
  );
});

test("editAgent_inheritedNiminoAgentProvider_clearsWhenUserSwitchesToInheritedClaude", () => {
  // An agent inheriting nimino-agent with databricks_v2, but the persona was
...
test("editAgent_findingE_emptyCatalog_providerOmittedNotCleared", () => {
  ...
  // Inherited nimino-agent agent with saved databricks_v2.
  // A name-only save must omit provider (not send null).

  const inheritHarness = true;
  const agentCommand = "nimino-agent"; // inherited command (short form)
...
test("editAgent_findingE_commandNullCatalogEntry_providerPreservedByIdMatch", () => {
  // Form 2: catalog loaded, but nimino-agent's entry has command:null (adapter
  ...
  const inheritHarness = true;
  const agentCommand = "nimino-agent"; // inherited command (short form = runtime id)
  const runtimes = [
    { id: "nimino-agent", command: null, defaultArgs: [] }, // adapter missing
  ...
  assert.equal(
    capability,
    "capable",
    "command:null nimino-agent entry must resolve to 'capable' via id-based fallback",
  );
  ...
  assert.equal(
    providerUpdate,
    undefined,
    "name-only save on inherited nimino-agent (command:null) must omit provider, not clear it",
  );
});
...
test("editAgent_findingE_lockedRuntimeStillClears", () => {
  ...
  const runtimes = [
    { id: "nimino-agent", command: "nimino-agent", defaultArgs: [] },
    { id: "claude", command: "claude-agent-acp", defaultArgs: [] },
  ];
  const selectedRuntimeId = "nimino-agent"; // stale dropdown state (irrelevant)
  const selectedRuntime = runtimes.find((r) => r.id === selectedRuntimeId);
...
test("editAgent_findingE_capableNiminoAgentLoadedCatalog_preservedOnNoOpSave", () => {
  // Confirm loaded-catalog inherited nimino-agent still preserves provider.
  // This is Finding D's good path — must not regress with the tri-state change.

  const inheritHarness = true;
  const agentCommand = "nimino-agent";
  const runtimes = [
    { id: "nimino-agent", command: "nimino-agent", defaultArgs: [] },
    { id: "claude", command: "claude-agent-acp", defaultArgs: [] },
  ];
  ...
  assert.equal(capability, "capable", "loaded nimino-agent must be 'capable'");
  ...
  assert.equal(
    providerUpdate,
    undefined,
    "no-op save on inherited nimino-agent (loaded catalog) must omit provider",
  );
});
...
//
// Problem: nimino-agent stores agentCommand="nimino-agent" (short name) while the
// catalog entry has command="/Applications/Nimino.app/.../nimino-agent" (resolved path).
...
test("editAgent_bugA_inheritedShortName_resolvesViaIdFallback", () => {
  // The core regression: agentCommand is the short name "nimino-agent" but the
  // catalog's command is the resolved binary path. Command match fails; id
  // fallback must rescue it.

  const agentCommand = "nimino-agent"; // short name stored by effective_agent_command
  const runtimes = [
    {
      id: "nimino-agent",
      command: "/Applications/Nimino.app/Contents/MacOS/nimino-agent", // resolved path
  ...
  assert.equal(
    selectedRuntimeId,
    "nimino-agent",
    "short-name agentCommand must resolve to nimino-agent id via id-fallback when command path differs",
  );
});
...
test("editAgent_bugA_inheritedShortName_commandMatchStillWinsWhenPresent", () => {
  ...
  const agentCommand = "/usr/local/bin/nimino-agent"; // full path (explicit pin)
  const runtimes = [
    {
      id: "nimino-agent",
      command: "/usr/local/bin/nimino-agent",
...
test("editAgent_bugA_inheritedShortName_discoveryGatePasses", () => {
  // Once selectedRuntimeId is correctly resolved to "nimino-agent" via id-fallback,
  ...
  const agentCommand = "nimino-agent"; // short name
  const runtimes = [
    {
      id: "nimino-agent",
      command: "/Applications/Nimino.app/Contents/MacOS/nimino-agent",
      availability: "available",
      defaultArgs: [],
    },
  ];
  ...
  assert.equal(
    selectedRuntimeId,
    "nimino-agent",
    "id-fallback must resolve selectedRuntimeId to nimino-agent",
  );
  assert.ok(
  ...
  assert.equal(
    ...
    "/Applications/Nimino.app/Contents/MacOS/nimino-agent",
    "discoveryAgentCommand must be the resolved path from the catalog entry",
  );
  assert.equal(
    canDiscoverModelOptions,
    true,
    "discovery gate must pass for inherited nimino-agent with databricks_v2 provider once id-fallback resolves the runtime",
  );
});
...
test("requiredCredentialEnvKeys: nimino-agent + anthropic → ANTHROPIC_API_KEY", () => {
  const keys = requiredCredentialEnvKeys("nimino-agent", "anthropic");
  assert.deepEqual(keys, ["ANTHROPIC_API_KEY"]);
});

test("requiredCredentialEnvKeys: nimino-agent + openai → OPENAI_COMPAT_API_KEY", () => {
  const keys = requiredCredentialEnvKeys("nimino-agent", "openai");
  assert.deepEqual(keys, ["OPENAI_COMPAT_API_KEY"]);
});

test("requiredCredentialEnvKeys: nimino-agent + databricks → DATABRICKS_HOST only (no token)", () => {
  const keys = requiredCredentialEnvKeys("nimino-agent", "databricks");
...
test("requiredCredentialEnvKeys: nimino-agent + databricks_v2 → DATABRICKS_HOST only", () => {
  const keys = requiredCredentialEnvKeys("nimino-agent", "databricks_v2");
  assert.deepEqual(keys, ["DATABRICKS_HOST"]);
});
...
test("requiredCredentialEnvKeys: nimino-agent + no provider → empty (provider not yet selected)", () => {
  const keys = requiredCredentialEnvKeys("nimino-agent", "");
...
test("blockSave_niminoAgentAnthropicMissingKey_blocked", () => {
  // Will's exact case: nimino-agent / anthropic / opus / no ANTHROPIC_API_KEY
  const requiredKeys = requiredCredentialEnvKeys("nimino-agent", "anthropic");
...
test("blockSave_niminoAgentAnthropicKeyProvided_allowed", () => {
  const requiredKeys = requiredCredentialEnvKeys("nimino-agent", "anthropic");
...
test("blockSave_niminoAgentAnthropicEmptyStringKey_blocked", () => {
...
test("blockSave_niminoAgentDatabricksMissingHost_blocked", () => {
  const requiredKeys = requiredCredentialEnvKeys("nimino-agent", "databricks");
  const envVars = {};
  assert.equal(
    hasRequiredEnvKeyMissing(requiredKeys, envVars),
    true,
...
test("blockSave_niminoAgentDatabricksHostProvided_allowed", () => {
  const requiredKeys = requiredCredentialEnvKeys("nimino-agent", "databricks");
...
//   FALSE-ALLOW: claude pin → inherit nimino-agent persona → missing ANTHROPIC_API_KEY
//     → must be BLOCKED (prospective runtime is nimino-agent/anthropic, key absent)
//   FALSE-BLOCK: nimino-agent pin → inherit claude persona
//     → must NOT be blocked (claude has no dialog-fixable credential requirement)

test("blockSave_inheritTransition_claudePin_toNiminoAgentPersona_missingKey_blocked", () => {
  ...
  // from persona" where the persona uses nimino-agent/anthropic.
  // prospectiveRuntimeId resolves to "nimino-agent"; providerForRequiredKeys must
  // use the PROSPECTIVE runtime's provider-field visibility (nimino-agent supports
  // provider selection) rather than the current locked runtime's suppression.
  const prospectiveRuntimeId = "nimino-agent"; // resolved from persona's agentCommand
  ...
  // providerForRequiredKeys must be "anthropic" (nimino-agent supports selection)
  ...
  assert.equal(
    missing,
    true,
    "inheriting nimino-agent/anthropic persona with no key must BLOCK save (false-allow prevented)",
  );
});

test("blockSave_inheritTransition_niminoAgentPin_toClaudePersona_notBlocked", () => {
  // Scenario: agent is pinned to nimino-agent/anthropic. The user checks
