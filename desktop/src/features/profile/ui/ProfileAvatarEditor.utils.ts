...
const EMOJI_MART_SHADOW_CSS = `
  ...
  #root {
    --padding: var(--nimino-emoji-picker-padding, 16px);
    --nimino-emoji-picker-search-control-height: 48px;
    --sidebar-width: 0px;
    display: flex;
  ...
  #root::after {
    background: linear-gradient(
      to bottom,
      rgba(var(--nimino-emoji-picker-rgb-background), 0),
      rgba(var(--nimino-emoji-picker-rgb-background), 0.98)
    );
    bottom: calc(var(--nimino-emoji-picker-nav-button-size, 40px) + 24px);
    content: "";
    height: var(--nimino-emoji-picker-fade-height, 0px);
    left: 0;
    opacity: var(--nimino-emoji-picker-fade-opacity, 0);
    pointer-events: none;
    position: absolute;
    right: 0;
    z-index: 3;
  ...
  .scroll {
    ...
    padding-top: var(--nimino-emoji-picker-scroll-padding-top, 28px);
    width: 100%;
  }
  ...
  .search input[type="search"] {
    border-radius: 12px;
    height: var(--nimino-emoji-picker-search-control-height);
    padding-bottom: 0;
    padding-top: 0;
  }
  ...
  .search + .flex {
    border-radius: 12px;
    flex: 0 0 auto;
    height: var(--nimino-emoji-picker-search-control-height) !important;
    margin-left: 8px;
    width: var(--nimino-emoji-picker-search-control-height) !important;
  }
  ...
  .skin-tone-button {
    ...
    height: calc(var(--nimino-emoji-picker-search-control-height) - 8px) !important;
    width: calc(var(--nimino-emoji-picker-search-control-height) - 8px) !important;
  }
  ...
  .category button[data-nimino-selected="true"] .background {
  ...
  #nav {
    ...
    flex: 0 0 auto;
    justify-content: space-between;
    padding: 8px var(--nimino-emoji-picker-nav-padding-x, 24px) 16px;
  }
  ...
  #nav button {
    ...
    display: flex;
    flex: 0 0 var(--nimino-emoji-picker-nav-button-size, 40px);
    height: var(--nimino-emoji-picker-nav-button-size, 40px);
    justify-content: center;
    transition:
      background-color var(--duration) var(--easing),
      color var(--duration) var(--easing),
      transform var(--duration) var(--easing);
    width: var(--nimino-emoji-picker-nav-button-size, 40px);
  }
  ...
  #nav svg,
  #nav img {
    height: var(--nimino-emoji-picker-category-icon-size, 24px);
    width: var(--nimino-emoji-picker-category-icon-size, 24px);
  }
`;
...
export function useEmojiMartStyles(
  ...
  React.useEffect(() => {
    ...
    const installEmojiMartStyles = () => {
      ...
      if (!shadowRoot.querySelector("#nimino-emoji-mart-style")) {
        const style = document.createElement("style");
        style.id = "nimino-emoji-mart-style";
...
export function useEmojiMartThemeVars() {
  ...
  React.useEffect(() => {
    const updateThemeVars = () => {
      ...
      setThemeVars({
        "--nimino-emoji-picker-rgb-background": muted ?? "54, 58, 79",
        "--nimino-emoji-picker-rgb-color": foreground ?? "245, 247, 255",
        "--nimino-emoji-picker-rgb-input": input ?? "47, 51, 68",
