...
test("buildNostrBindCallbackUrl returns a UTF-8 base64url payload in the fragment", () => {
  const response = JSON.stringify({ content: "Nimino ⚡", sig: "+/=" });
  const result = buildNostrBindCallbackUrl(
    "https://example.com/nimino?source=bind",
    response,
  );
  const url = new URL(result);

  assert.equal(url.origin, "https://example.com");
  assert.equal(url.pathname, "/nimino");
  assert.equal(url.search, "?source=bind");
  assert.match(url.hash, /^#nimino_bind=v1\.[A-Za-z0-9_-]+$/);
  assert.equal(decodePayload(result), response);
  assert.equal(url.searchParams.has("nimino_bind"), false);
});

test("buildNostrBindCallbackUrl replaces an existing fragment", () => {
  const result = buildNostrBindCallbackUrl(
    "https://example.com/nimino#stale-fragment",
    "signed",
  );

  assert.equal(new URL(result).hash, "#nimino_bind=v1.c2lnbmVk");
});
