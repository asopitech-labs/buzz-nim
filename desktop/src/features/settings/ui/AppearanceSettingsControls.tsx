...
/** Nimino navigation can use either its production tint or a stronger tab. */
...
/**
 * Build the sample thumbnail as an SVG data URL from the Nimino gradient
 * tokens. Data-URL images cannot resolve CSS variables, so the token values
 * are read from the live stylesheet and baked in per render — if the Nimino
 * gradient ever changes in `theme.css`, this preview follows automatically.
 */
function niminoGradientSampleImage(isDark: boolean): string {
  ...
  const top = isDark
    ? readToken("--nimino-gradient-dark-top", "#4a4616")
    : readToken("--nimino-gradient-light-top", "#e6e6b6");
  const bottom = isDark
    ? readToken("--nimino-gradient-dark-bottom", "#0a1423")
    : readToken("--nimino-gradient-light-bottom", "#c4d0da");
...
function LinkPreviewSample({ style }: { style: LinkPreviewStyle }) {
  ...
  const preview = React.useMemo<ResolvedLinkPreview>(
    () => ({
      ...
      imageDataUrl: niminoGradientSampleImage(isDark),
    }),
    [isDark],
  );
  return (
...
function ThreadLayoutDiagram({ mode }: { mode: ThreadViewMode }) {
  ...
  // Nimino gradient tokens directly and follows theme.css automatically.
  const gradientTop = isDark
    ? "var(--nimino-gradient-dark-top, #4a4616)"
    : "var(--nimino-gradient-light-top, #e6e6b6)";
  const gradientBottom = isDark
    ? "var(--nimino-gradient-dark-bottom, #0a1423)"
    : "var(--nimino-gradient-light-bottom, #c4d0da)";
    ...
    <SettingsOptionRow className="items-start">
      <div className="min-w-0">
        ...
        <p
          ...
          data-settings-subcopy
        >
          Choose the highlight color used throughout Nimino.
        </p>
      </div>
      <div
