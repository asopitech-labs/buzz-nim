...
function buildProjectPatchTemplate({
  ...
  // (d, name, description, nimino-channel, nimino-visibility, relay hints embedded
  ...
  return {
    ...
    tags: [
      ...repository.eventTags
        .filter((tag) => tag[0] !== "nimino-channel")
        .map((tag) => [...tag]),
      ["nimino-channel", normalizedChannelId],
    ],
  };
}
...
export function buildAddedRepositoryEventTemplatesFromHead({
  ...
  repositoryTags.push(["nimino-channel", normalizedAccessChannelId]);
