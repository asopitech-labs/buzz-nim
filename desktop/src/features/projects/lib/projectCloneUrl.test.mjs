...
test("projectRepoHost recognizes a canonical repository on the relay", () => {
  assert.deepEqual(projectRepoHost(`${ORIGIN}/git/${OWNER}/nimino`, ORIGIN), {
    kind: "nimino",
  });
});
...
test("projectRepoHost identifies an external repository by host", () => {
  assert.deepEqual(
    projectRepoHost("https://github.com/asopitech-labs/nimino.git", ORIGIN),
...
test("projectRepoHost fails closed while either URL is unresolved", () => {
  ...
  assert.deepEqual(projectRepoHost(`${ORIGIN}/git/${OWNER}/nimino`, null), {
    kind: "unresolved",
  });
...
test("projectRepoHostForProject recognizes an implicit relay repository", () => {
  assert.deepEqual(
    projectRepoHostForProject(
      { cloneUrls: [], dtag: "nimino", owner: OWNER },
      ORIGIN,
    ),
    { kind: "nimino" },
  );
...
test("repositoryDisplayPath renders an external repo as host/path without .git", () => {
  assert.equal(
    repositoryDisplayPath(
      {
        cloneUrls: ["https://github.com/asopitech-labs/nimino.git"],
        dtag: "nimino",
        owner: OWNER,
      },
      ORIGIN,
    ),
    "github.com/asopitech-labs/nimino",
  );
});
...
test("repositoryDisplayPath fails closed without a resolvable clone URL", () => {
  ...
  assert.equal(
    repositoryDisplayPath(
      { cloneUrls: ["not a URL"], dtag: "nimino", owner: OWNER },
      ORIGIN,
    ),
    null,
  );
