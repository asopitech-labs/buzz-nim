...
import { markAllReadSources } from "@/app/AppShell.helpers";
...
import { useAppShellNavigationState } from "@/app/navigation/useAppShellNavigationState";
...
export function AppShell() {
  ...
  const shellNavigation = useAppShellNavigationState(
    location.pathname,
    location.href,
  );
  const { selectedChannelId, selectedView } = shellNavigation;
  const {
  ...
  // Dispatch `nimino://` deep links only from the main window; the companion is dedicated to its active Huddle route.
  useAppDeepLinks(!isHuddleRoom);
  const handleOpenCreateChannel = React.useCallback(
    () => setIsCreateChannelOpen(true),
    [],
  );
  const handleOpenNewMessage = React.useCallback(() => {
    setIsChannelManagementOpen(false);
    shellNavigation.openNewMessage();
  }, [shellNavigation.openNewMessage]);
            ...
            >
              <AppProfilePanelProvider>
                {!settingsOpen && !isHuddleRoom ? (
                  <AppTopChrome
                    canGoBack={shellNavigation.canGoBack}
                    canGoForward={shellNavigation.canGoForward}
                    hasCommunityRail={hasCommunityRail}
                    onGoBack={shellNavigation.goBack}
                    onGoForward={shellNavigation.goForward}
                  />
                ) : null}
                {settingsOpen ? (
                  ...
                  <div className="relative flex min-h-0 flex-1 overflow-visible">
                    {!isHuddleRoom ? (
                      <AppSidebar
                        ...
                        onSelectHome={() => {
                          shellNavigation.closeNewMessage();
                          void goHome();
                        }}
                    ...
                    >
                      ...
                      >
                        {shellNavigation.isNewMessageOpen ? (
                          <NewMessageScreen
                            onClose={shellNavigation.closeNewMessage}
                          />
                        ) : (
                          <Outlet />
                        )}
