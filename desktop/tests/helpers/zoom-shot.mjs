...
await page.addInitScript(() => {
  ...
  window.localStorage.setItem("nimino-communities", JSON.stringify([ws]));
  window.localStorage.setItem("nimino-active-community-id", id);
});
await page.addInitScript(
  ({ prefix, pubkeys }) => {
...
await page.addInitScript(() => {
  ...
  window.__NIMINO_E2E__ = { mode: "mock" };
