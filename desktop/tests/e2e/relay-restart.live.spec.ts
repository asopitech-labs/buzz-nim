...
async function runCli(args: string[], relayUrl: string, privateKey: string) {
  const binary = required("NIMINO_E2E_CLI_BIN", process.env.NIMINO_E2E_CLI_BIN);
...
async function connectionState(page: Page): Promise<string> {
  return page.evaluate(() => {
    ...
    return win.__NIMINO_E2E_GET_RELAY_CONNECTION_STATE__?.() ?? "uninstalled";
  });
}
...
test.describe("relay restart live gate", () => {
  test.skip(!enabled, "set NIMINO_E2E_RELAY_RESTART=1 to run live gate");
