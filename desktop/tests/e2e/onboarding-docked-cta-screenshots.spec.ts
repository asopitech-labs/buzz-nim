...
test("machine onboarding: landing, backup, setup docked CTAs", async ({
  ...
  await expect(
    page.getByTestId("backup-options").locator(".nimino-card-textured"),
  ).toHaveCount(0);
  ...
  await expect(passwordPanel).not.toHaveClass(/nimino-card-textured/);
  ...
  await expect(generatorPopover).not.toHaveClass(/nimino-card-textured/);
  await waitForAnimations(page);
