  ...
  await page.evaluate((seeded) => {
    window.__NIMINO_E2E_SEED_MOCK_REMINDERS__?.(
      seeded as Parameters<
        NonNullable<typeof window.__NIMINO_E2E_SEED_MOCK_REMINDERS__>
      >[0],
    );
    window.__NIMINO_E2E_QUERY_CLIENT__?.invalidateQueries({
      queryKey: ["reminders"],
    });
  }, events);
}
...
const NOTIFICATION_SETTINGS_KEY = `nimino-notification-settings.v2:${MOCK_PUBKEY}`;
