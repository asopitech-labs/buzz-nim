...
test("Share compute chooses a model before sharing", async ({ page }) => {
  ...
  await expect
    .poll(() =>
      page.evaluate(() => (window as E2eWindow).__NIMINO_E2E_COMMANDS__ ?? []),
    )
    .toContain("mesh_start_node");
  await expect
    .poll(() =>
      page.evaluate(
        () => (window as E2eWindow).__NIMINO_E2E_COMMAND_PAYLOADS__ ?? [],
      ),
    )
    .toContainEqual({
...
test("a consuming client can switch to sharing its saved local model", async ({
  ...
  await page.addInitScript((model) => {
    window.localStorage.setItem("nimino.mesh-compute.share.model.v1", model);
  }, localModel);
  await installMockBridge(page);
  await page.goto("/");
  ...
  await page.waitForFunction(
    () => typeof (window as E2eWindow).__NIMINO_E2E_SET_MESH__ === "function",
  );
  await page.evaluate(() => {
    (window as E2eWindow).__NIMINO_E2E_SET_MESH__?.({
      nodeState: "running",
      nodeMode: "client",
    });
  });
  ...
  await expect(card).toContainText("Nimino may briefly restart");
  await expect(toggle).not.toBeChecked();
  await expect(
    page.getByTestId("mesh-share-compute-options-motion"),
  ).toHaveCount(0);
  ...
  const commands = await page.evaluate(() => ({
    names: (window as E2eWindow).__NIMINO_E2E_COMMANDS__ ?? [],
    payloads: (window as E2eWindow).__NIMINO_E2E_COMMAND_PAYLOADS__ ?? [],
  }));
