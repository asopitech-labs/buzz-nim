...
const SHOTS = "test-results/nimino-theme";
...
async function expectNiminoSidebarPalette(page: Page, mode: "light" | "dark") {
  const mutedColor =
  ...
  await expect(
    page.locator("[data-nimino-sidebar-secondary]").first(),
...
async function expectNiminoContentShadow(page: Page, mode: "light" | "dark") {
  const effects = await page.evaluate(() => {
    const shell = document.querySelector(".nimino-huddle-shell");
    const content = document.querySelector("[data-nimino-content-surface]");
...
async function expectNiminoGradientPaint(
  page: Page,
  mode: "light" | "dark",
): Promise<string> {
  const paint = await page.evaluate(() => {
    const root = document.documentElement;
    const appSurface = document.querySelector(".nimino-huddle-app-surface");
    const lightLayer = document.querySelector('[data-nimino-gradient="light"]');
    const darkLayer = document.querySelector('[data-nimino-gradient="dark"]');
    ...
    return {
      ...
      theme: root.getAttribute("data-nimino-theme"),
  ...
  expect(paint.theme).toBe(mode === "light" ? "nimino" : "nimino-dark");
...
async function expectNiminoSettingsPalette(page: Page, mode: "light" | "dark") {
  const mutedColor =
    mode === "light" ? "rgba(0, 0, 0, 0.4)" : "rgba(255, 255, 255, 0.4)";
  ...
  await expectNiminoGradientPaint(page, mode);
...
async function expectAppliedNiminoTheme(
  page: Page,
  themeName: "nimino" | "nimino-dark",
  storedTheme: "nimino" | "nimino-dark" = themeName,
) {
  const isDark = themeName === "nimino-dark";
  await expect
    .poll(() =>
      page.evaluate((storageKey) => {
        ...
        return {
          ...
          isDark: root.classList.contains("dark"),
          niminoTheme: root.getAttribute("data-nimino-theme"),
          gradientTop: styles.getPropertyValue("--nimino-gradient-top").trim(),
          gradientBottom: styles
            .getPropertyValue("--nimino-gradient-bottom")
            .trim(),
        };
      }, THEME_STORAGE_KEY),
    )
...
test("nimino light sidebar gradient", async ({ page }) => {
  await seedTheme(page, "nimino");
  await installMockBridge(page);
  await openChannel(page);
  await expectNiminoGradientPaint(page, "light");
  await expectNiminoSidebarPalette(page, "light");
  await expectNiminoContentShadow(page, "light");
  await expectIconlessSectionTitleAligned(page, "stream-list");
  await expectIconlessSectionTitleAligned(page, "dm-list");
  await waitForAnimations(page);
  await page
    .getByTestId("app-sidebar")
    .screenshot({ path: `${SHOTS}/01-nimino-light-sidebar.png` });
});

test("nimino dark sidebar gradient", async ({ page }) => {
  await seedTheme(page, "nimino-dark");
  await installMockBridge(page);
  await openChannel(page);
  await expectNiminoGradientPaint(page, "dark");
  await expectNiminoSidebarPalette(page, "dark");
  await expectNiminoContentShadow(page, "dark");
  await expectIconlessSectionTitleAligned(page, "stream-list");
  await expectIconlessSectionTitleAligned(page, "dm-list");
  await expect(page.locator("[data-nimino-content-surface]")).toHaveCSS(
    "background-color",
    "rgb(26, 26, 26)",
  );
  await waitForAnimations(page);
  await page
    .getByTestId("app-sidebar")
    .screenshot({ path: `${SHOTS}/02-nimino-dark-sidebar.png` });
});
...
test("appearance groups theme and preferences into labeled rows", async ({
  ...
  await themeCard.getByTestId("theme-option-nimino").click();
  await expect(themeStyleTrigger).toHaveAttribute("aria-expanded", "true");
  await expect(themeStyleOptions).toBeVisible();
  await expect(
    themeCard.getByTestId("prominent-active-tab-toggle"),
...
test("app font size and conversation density apply independently", async ({
  ...
  // type tokens ride on `--nimino-type-rem`, which itself derives from the
  ...
  const readScale = () =>
    root.evaluate((element) => {
      const PROBE_ID = "nimino-e2e-conversation-scale-probe";
      const tokens = {
  ...
  await expect(densityDescription).toHaveText(
    "Spacing in conversations and Markdown content across Nimino",
  );
...
test("appearance picker — system tab (Nimino follows OS)", async ({ page }) => {
  await seedTheme(page, "nimino");
  await installMockBridge(page);
...
test("appearance picker — light tab (Nimino)", async ({ page }) => {
  await seedTheme(page, "nimino");
  await installMockBridge(page);
...
test("appearance picker — dark tab (Nimino Dark)", async ({ page }) => {
  await seedTheme(page, "nimino-dark");
  await installMockBridge(page);
...
test("settings nav uses Nimino active pill + hover (light)", async ({
  page,
}) => {
  await seedTheme(page, "nimino");
  await installMockBridge(page);
  ...
  // Appearance is the active section here; its nav row should carry the Nimino
  ...
  await expectNiminoSettingsPalette(page, "light");
...
test("settings nav uses Nimino active pill + hover (dark)", async ({
  page,
}) => {
  await seedTheme(page, "nimino-dark");
  await installMockBridge(page);
  ...
  await expect(sidebar).toBeVisible({ timeout: 10_000 });
  await page.getByTestId("settings-nav-appearance").click();
  await expectNiminoSettingsPalette(page, "dark");
...
for (const { mode, theme } of [
  ...
  test(`${theme} ignores the Nimino prominent preference`, async ({ page }) => {
    await seedTheme(page, theme);
    await page.addInitScript(
      ({ key }) => window.localStorage.setItem(key, "true"),
      { key: PROMINENT_ACTIVE_TAB_STORAGE_KEY },
...
test("appearance hides accent picker under Nimino", async ({ page }) => {
  await seedTheme(page, "nimino");
  await installMockBridge(page);
  const panel = await openAppearance(page, "light");
  // The accent picker is hidden while a Nimino theme is active. Its neutral
...
test("glass background keeps the content panel solid", async ({ page }) => {
  ...
  await expect(page.locator(".nimino-theme-gradient-underlay")).not.toHaveCSS(
    "background-image",
    "none",
  );
  ...
  await expect(opacitySlider).toHaveClass(/nimino-avatar-framing-slider/);
  ...
  await expect(
    opacitySlider.locator(".nimino-avatar-framing-slider-handle"),
  ).toHaveCSS("opacity", "1");
  await expect(root).toHaveAttribute("data-glass-background", "");
  const niminoSettingOrder = await page
  ...
  expect(niminoSettingOrder).toEqual([
  ...
  await expect(page.locator(".nimino-theme-gradient-underlay")).toHaveCSS(
    "background-image",
    "none",
  );
...
test("non-Nimino glass preserves the selected theme sidebar tint", async ({
  page,
}) => {
  ...
  const tint = await page
    .locator(".nimino-theme-gradient-layer")
    .evaluate((element) => {
...
test("accent picker reveals/hides when toggling Nimino", async ({ page }) => {
  // Start on a non-Nimino theme so the accent picker is present, then select the
  // Nimino tile — the picker should animate out and unmount. Reselecting a
  // non-Nimino tile brings it back. Asserts the presence toggle (the motion
  ...
  const nonNiminoSettingOrder = await page
    .getByTestId("appearance-theme-card")
    .locator(
    ...
    .evaluateAll((rows) => rows.map((row) => row.getAttribute("data-testid")));
  expect(nonNiminoSettingOrder).toEqual([
    "appearance-color-mode-row",
    "theme-style-row",
  ...
  // Switch to Nimino — picker should leave (allow the exit animation to settle).
  await page.getByTestId("theme-style-trigger").click();
  await page.getByTestId("theme-option-nimino").click();
  ...
  // Back to a non-Nimino theme — picker returns.
...
test("Nimino light and dark modes apply live without a reload", async ({
  page,
}) => {
  await seedTheme(page, "nimino");
  await installMockBridge(page);
  await openAppearance(page, "light");
  await expectAppliedNiminoTheme(page, "nimino");
  const lightGradient = await expectNiminoGradientPaint(page, "light");

  await page.getByTestId("appearance-mode-dark").click();
  await expectAppliedNiminoTheme(page, "nimino-dark");
  const darkGradient = await expectNiminoGradientPaint(page, "dark");
...
test("Nimino follows native system theme changes without a reload", async ({
  page,
}) => {
  await seedTheme(page, "nimino");
  await page.addInitScript(() => {
  ...
  await expectAppliedNiminoTheme(page, "nimino-dark", "nimino");
  await expectNiminoGradientPaint(page, "dark");

  await emitNativeThemeChange(page, "light");
  await expectAppliedNiminoTheme(page, "nimino", "nimino");
  await expectNiminoGradientPaint(page, "light");
});
