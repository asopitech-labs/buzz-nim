 ...
 *
 * Covers:
 *   1. Global defaults Advanced shows numeric inputs for nimino-agent.
...
test("global_advanced_nimino_agent_shows_all_numeric_controls", async ({
  page,
}) => {
  // The mock bridge's withMockRuntimeConfigMetadata injects the numeric env var
  // fields for nimino-agent. When nimino-agent is selected and Advanced is opened,
  ...
  // All three numeric inputs must be present for nimino-agent.
...
test("delayed_catalog_per_agent_saved_tuning_values_visible_then_structured_controls_appear", async ({
    ...
    managedAgents: [
      {
        ...
        channelNames: ["agents"],
        envVars: {
          NIMINO_AGENT_MAX_OUTPUT_TOKENS: "4096",
          NIMINO_AGENT_MAX_ROUNDS: "25",
        },
      },
    ],
  });
  ...
  // while the catalog hasn't settled: NIMINO_AGENT_MAX_OUTPUT_TOKENS and
  // NIMINO_AGENT_MAX_ROUNDS should appear in the env-vars editor.
  await expect(
    page.locator(
      'input[data-testid="env-vars-key"][value="NIMINO_AGENT_MAX_OUTPUT_TOKENS"]',
    ),
  ).toBeVisible();
  await expect(
    page.locator(
      'input[data-testid="env-vars-key"][value="NIMINO_AGENT_MAX_ROUNDS"]',
    ),
  ).toBeVisible();
