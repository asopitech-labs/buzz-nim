...
/**
 * SCROLL-BACK latency profile for one channel (#nimino-bugs) against a LIVE
 ...
 *   NIMINO_COMMUNITY_HOST=relay.nimino.example \
 *   NIMINO_PERF_NSEC=nsec1... \
 *   npx playwright test --config=playwright.perf.config.ts scrollback-niminobugs.perf.ts
 */

const RELAY_HTTP =
  process.env.NIMINO_E2E_RELAY_URL ?? "https://relay.nimino.example";
const NSEC = process.env.NIMINO_PERF_NSEC ?? "";
const COMMUNITY_HOST = process.env.NIMINO_COMMUNITY_HOST ?? "";
const TARGET_CHANNEL = process.env.NIMINO_PERF_CHANNEL ?? "nimino-bugs";
