 ...
 *       empty baked env, simulating an OSS install with no NIMINO_AGENT_PROVIDER).
 *
 ...
 *       runtime (nimino-agent) and model discovery runs → model combobox is non-empty.
...
test.describe("agent provider dropdown screenshots", () => {
  ...
  // NIMINO_AGENT_PROVIDER is baked and hideProviderIds is empty → v1 appears.
  ...
  // default runtime (nimino-agent via getDefaultPersonaRuntime) and model
  ...
  test("03-builtin-edit-runtime-seeded", async ({ page }) => {
    ...
    // must have run and selected the app default (nimino-agent in the mock catalog).
