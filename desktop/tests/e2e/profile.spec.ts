...
test("keeps the saved profile description after a community round trip", async ({
  ...
  await page.addInitScript((seed) => {
    window.localStorage.setItem("nimino-communities", JSON.stringify(seed));
    window.localStorage.setItem("nimino-active-community-id", seed[0].id);
  }, communities);
  await page.goto("/");
...
test("notification settings drive the Inbox badge and desktop alerts", async ({
  ...
  async function getAppBadgeCount() {
    return page.evaluate(() => {
      ...
      return win.__NIMINO_E2E_APP_BADGE_COUNT__ ?? 0;
    });
  }
  ...
  await page.evaluate(() => {
    ...
    win.__NIMINO_E2E_PUSH_MOCK_FEED_ITEM__?.({
      category: "mention",
  ...
  await expect
    .poll(() =>
      page.evaluate(() => {
        ...
        return win.__NIMINO_E2E_NOTIFICATIONS__?.length ?? 0;
      }),
    )
    .toBe(1);
  ...
  const notifications = await page.evaluate(() => {
    ...
    return win.__NIMINO_E2E_NOTIFICATIONS__ ?? [];
  });
  ...
  const clickedNotification = await page.evaluate(() => {
    ...
    return win.__NIMINO_E2E_CLICK_NOTIFICATION__?.(0) ?? false;
  });
...
test("opens settings with the keyboard shortcut and updates theme", async ({
  ...
  await expect
    .poll(() => page.evaluate(() => localStorage.getItem("nimino-theme")))
    .toBe("github-light");
...
test("supports webview zoom keyboard shortcuts", async ({ page }) => {
  ...
  const getTextScaleState = () =>
    page.evaluate(() => ({
      ...
      storedScale: localStorage.getItem("nimino:text-scale"),
      webviewZoom: (window as Window & { __NIMINO_E2E_WEBVIEW_ZOOM__?: number })
        .__NIMINO_E2E_WEBVIEW_ZOOM__,
    }));
  ...
  const readTypographyState = () =>
    page.evaluate(() => {
      ...
      return {
        ...
        rootFontSize: getComputedStyle(document.documentElement).fontSize,
        typeRemPx,
        textScale: localStorage.getItem("nimino:text-scale"),
      };
    });
