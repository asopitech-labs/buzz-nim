...
//!
//! Nimino's own process holds `NIMINO_PRIVATE_KEY` (an nsec), `NIMINO_AUTH_TAG`, and
...
/// Keys the child is allowed to inherit from Nimino's own environment.
///
...
/// Values Nimino sets on the child unconditionally, overriding any inherited
...
/// it is injected rather than inherited. Nimino's own `SHELL` and the shell we
...
pub fn fence_env(cmd: &mut CommandBuilder, path: &str, shell: &str) {
    ...
    // 3. Apply Nimino's own terminal identity.
