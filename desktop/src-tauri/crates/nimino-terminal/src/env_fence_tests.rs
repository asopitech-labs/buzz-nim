...
/// Secrets Nimino's own process holds. Sourced from the desktop crate's
...
const SECRET_KEYS: &[&str] = &[
    ...
    "NIMINO_AUTH_TAG",
    "NIMINO_API_TOKEN",
    "NIMINO_ACP_PRIVATE_KEY",
    "NIMINO_ACP_API_TOKEN",
    "NIMINO_RELAY_URL",
];

const CANARY: &str = "SAMI_CANARY_MUST_NOT_LEAK";

/// Uniquely-named executable seeded into Nimino's own PATH; the child must not
/// be able to run it.
const CANARY_BIN: &str = "nimino-hermit-canary-tool";
...
/// canary value they set themselves, so a real `NIMINO_PRIVATE_KEY` in the
...
fn fence_still_delivers_the_terminal_contract() {
    ...
    for (key, value) in [("TERM", "xterm-256color"), ("TERM_PROGRAM", "Nimino")] {
        assert!(
...
fn fence_excludes_keys_it_has_never_heard_of() {
    std::env::set_var("NIMINO_SOME_FUTURE_CREDENTIAL", CANARY);
    let out = fenced_child_environment();

    assert!(
        !out.contains("NIMINO_SOME_FUTURE_CREDENTIAL"),
        "an unknown key reached the child:\n{out}"
    );
}

/// `PATH` is constructed, not inherited, so Nimino's Hermit build toolchain
...
#[test]
fn child_path_is_free_of_nimino_toolchain() {
    let dir = std::env::temp_dir().join("nimino-terminal-path-canary");
    ...
    // Stand in for Hermit activation: Nimino's own PATH leads with a directory
    ...
    assert!(
        !path_line.contains("nimino-terminal-path-canary"),
        "Nimino's toolchain leaked into the child PATH: {path_line}"
    );
    ...
    assert!(
        ...
        "a Nimino-only executable was reachable from the child shell: {resolved}"
    );
}
...
fn resolve_shell_rejects_a_file_the_user_cannot_execute() {
    ...
    let path = fixture_file(
        "nimino-terminal-group-only-exec",
        "#!/bin/sh\ntrue\n",
        0o010,
    );
...
fn resolve_shell_rejects_a_non_executable_file() {
    let path = fixture_file("nimino-terminal-not-a-shell", "not a shell", 0o644);
    assert_ne!(resolve_shell(path.to_str()), path.to_str().unwrap());
}
...
/// The child must be told the shell we actually spawned, not the one Nimino
