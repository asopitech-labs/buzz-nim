...
#[test]
fn media_directives_allow_the_nimino_media_scheme() {
    // `rewriteRelayUrl` emits `nimino-media://localhost/...` until the loopback
    // proxy port resolves, so cold-start media renders through the custom
    // scheme (mapped to `http://nimino-media.localhost` on Windows).
    for directive in ["img-src", "media-src", "connect-src"] {
        let allowed = sources(directive);
        assert!(
            allowed.contains(&"nimino-media:".to_owned()),
            "{directive} must allow nimino-media:"
        );
        assert!(
            allowed.contains(&"http://nimino-media.localhost".to_owned()),
            "{directive} must allow http://nimino-media.localhost"
        );
    }
}
