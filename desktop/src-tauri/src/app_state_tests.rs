...
/// `NIMINO_PRIVATE_KEY` is process-global; serialize the env-mutating tests
/// so they don't race each other under the parallel test runner.
static ENV_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());

/// Run `body` with `NIMINO_PRIVATE_KEY` set to `value` (or unset when `None`),
...
fn with_env_key<T>(value: Option<&str>, body: impl FnOnce() -> T) -> T {
    ...
    match value {
        Some(v) => std::env::set_var("NIMINO_PRIVATE_KEY", v),
