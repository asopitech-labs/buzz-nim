...
//!
//! `NIMINO_TTS_STREAMING=1` streams PCM deltas out of Pocket as they are
//! generated instead of waiting for the full first-chunk synthesis.
//! `NIMINO_TTS_EMIT_FRAMES` tunes the delta size in Flow LM frames (80 ms of
...
/// when `NIMINO_TTS_STREAMING=1`, `None` for the production batch path.
pub(super) fn streaming_emit_frames() -> Option<usize> {
    std::env::var("NIMINO_TTS_STREAMING")
        .is_ok_and(|v| v == "1")
        .then(|| {
            std::env::var("NIMINO_TTS_EMIT_FRAMES")
                .ok()
...
pub(super) fn synthesize_streaming(
    ...
    match stream_result {
        ...
        Ok(false) => {
            eprintln!(
                "nimino-desktop: tts stage=synthesis status=cancelled reason=stream_callback route_id={route_id}"
            );
            Some("cancelled")
        }
        Err(_) => {
            eprintln!(
                "nimino-desktop: tts stage=synthesis status=failed reason=inference route_id={route_id}"
            );
            Some("failed")
        }
    }
