...
//!
//! Mental model:
//!   app launch → start_stt_download (background) → ~/.nimino/models/parakeet-tdt-ctc-110m-en/
//!   app launch → start_tts_download (background) → ~/.nimino/models/pocket-tts/
//!   STT pipeline → is_stt_ready() → stt_model_dir() → run inference
//!   TTS pipeline → is_tts_ready() → tts_model_dir() → run synthesis
//!
//! Models are downloaded once and cached. A version manifest (`.nimino-model-manifest`)
...
//! `~/.nimino/models/moonshine-tiny/` is removed best-effort once the new STT
...
const MANIFEST_FILENAME: &str = ".nimino-model-manifest";
...
(https://github.com/k2-fsa/sherpa-onnx); Nimino ships this conversion
unmodified.
...
#[derive(Clone)]
struct ModelSlot {
    dir_name: &'static str,                  // subdir under ~/.nimino/models/
    ...
    fn recover_interrupted_install(&self, models_dir: &Path) {
        ...
        if self.is_ready(models_dir) {
            if let Err(error) = std::fs::remove_dir_all(&backup_dir) {
                eprintln!(
                    "nimino-desktop: could not remove stale {} backup: {error}",
                    self.dir_name
                );
            }
            return;
        ...
        if final_dir.exists() {
            if let Err(error) = std::fs::remove_dir_all(&final_dir) {
                eprintln!(
                    "nimino-desktop: could not remove incomplete {} install: {error}",
                    self.dir_name
                );
                return;
            }
        ...
        if let Err(error) = std::fs::rename(&backup_dir, &final_dir) {
            eprintln!(
                "nimino-desktop: could not restore interrupted {} install: {error}",
                self.dir_name
            );
        }
    }
        ...
        Fut: std::future::Future<Output = Result<(), String>> + Send,
    {
        ...
        tauri::async_runtime::spawn(async move {
            if let Err(e) = download_fn(http_client).await {
                eprintln!("nimino-desktop: {name} download failed: {e}");
...
impl ModelManager {
    /// Create a new `ModelManager` rooted at `~/.nimino/models/`.
    ///
    /// Returns `None` if the home directory cannot be resolved.
    pub fn new() -> Option<Self> {
        let models_dir = dirs::home_dir()?.join(".nimino").join("models");
        let manager = Self {
            models_dir,
    ...
    pub fn start_tts_download(&self, http_client: reqwest::Client) {
        if let Err(error) = voice_upgrade::install_vctk_presets_into_v4_model(&self.models_dir) {
            eprintln!("nimino-desktop: could not upgrade existing Pocket voices in place: {error}");
        }
        let manager = self.clone();
    ...
    async fn download_stt_model(&self, http_client: reqwest::Client) -> Result<(), String> {
        ...
        eprintln!("nimino-desktop: downloading STT model from {STT_DOWNLOAD_URL}");
        ...
        .await?;
        eprintln!("nimino-desktop: downloaded {bytes} bytes, wrote to disk");
        ...
        eprintln!("nimino-desktop: extracting STT archive…");
        ...
        eprintln!(
            "nimino-desktop: STT model ready at {}",
    ...
    ///
    /// Downloads files into `~/.nimino/models/pocket-tts/`:
    ///   - five ONNX sessions selected by the April INT8 bundle
    ///   - bundle metadata, SentencePiece tokenizer, and learned voice BOS
    ///   - upstream `LICENSE` plus Nimino's `MODEL_LICENSE.txt` attribution sidecar
    ...
    async fn download_tts_model(&self, http_client: reqwest::Client) -> Result<(), String> {
        ...
        for (i, (url, artifact)) in downloads.iter().enumerate() {
            ...
            eprintln!("nimino-desktop: downloading Pocket TTS {filename} from {url}");
            ...
            eprintln!("nimino-desktop: downloaded {bytes} bytes ({filename}), wrote to disk");
        ...
        eprintln!(
            "nimino-desktop: Pocket TTS model ready at {}",
...
///
/// Removes `~/.nimino/models/moonshine-tiny/` if present (~70 MB on disk).
...
async fn cleanup_legacy_moonshine_dir(models_dir: &Path) {
    ...
    match tokio::fs::remove_dir_all(&legacy).await {
        Ok(()) => eprintln!(
            "nimino-desktop: removed legacy STT model dir {}",
            legacy.display()
        ),
        Err(e) => eprintln!(
            "nimino-desktop: could not remove legacy STT model dir {}: {e} \
