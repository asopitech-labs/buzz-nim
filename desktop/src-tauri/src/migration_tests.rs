...
#[test]
fn reconcile_mcp_commands_clears_stale_nimino_mcp_server() {
    let dir = tempfile::tempdir().unwrap();
    write_agents_json(
        dir.path(),
        &serde_json::json!([{
...
#[test]
fn reconcile_mcp_commands_sets_canonical_for_nimino_agent() {
    let dir = tempfile::tempdir().unwrap();
    write_agents_json(
        dir.path(),
        &serde_json::json!([{
    ...
    reconcile_mcp_commands_in_file(&dir.path().join("agents/managed-agents.json"));
    let records = read_agents_json(dir.path());
    assert_eq!(records[0]["mcp_command"], "nimino-dev-mcp");
}
...
fn reconcile_mcp_commands_handles_mixed_agents() {
    ...
    write_agents_json(
        dir.path(),
        &serde_json::json!([
            {"name": "Stale Goose", "agent_command": "goose", "mcp_command": "nimino-mcp-server"},
            {"name": "Clean Goose", "agent_command": "goose", "mcp_command": ""},
            {"name": "Custom Agent", "agent_command": "goose", "mcp_command": "my-custom-mcp"},
            {"name": "Stale Nimino", "agent_command": "nimino-agent", "mcp_command": "nimino-mcp-server"}
        ]),
    );
    reconcile_mcp_commands_in_file(&dir.path().join("agents/managed-agents.json"));
    let records = read_agents_json(dir.path());
    ...
    assert_eq!(records[3]["mcp_command"], "nimino-dev-mcp");
}

#[test]
fn reconcile_mcp_commands_resolves_persona_runtime_over_stale_snapshot() {
    // The frozen snapshot is nimino-agent (wants nimino-dev-mcp), but the linked
    // persona's runtime is goose (wants no mcp). The reconcile must follow the
    // EFFECTIVE harness (persona-wins) and clear the stale nimino-mcp-server.
    let dir = tempfile::tempdir().unwrap();
    write_agents_json(
        dir.path(),
        &serde_json::json!([{
...
fn reconcile_mcp_commands_sees_team_dir_runtime_edit_same_launch() {
    ...
    // (goose → nimino-agent) into personas.json. The reader runs AFTER, so it
    // must derive the NEW nimino-agent mcp_command without a second launch.
    write_personas_json(
        dir.path(),
        &serde_json::json!([{"id": "p1", "runtime": "nimino-agent"}]),
    );
    reconcile_mcp_commands_in_file(&dir.path().join("agents/managed-agents.json"));
    assert_eq!(
        read_agents_json(dir.path())[0]["mcp_command"],
...
fn reconcile_mcp_commands_honors_explicit_override_over_persona() {
    ...
    // runtime: persona is goose (no mcp) but the pin is nimino-agent, so the
    // reconcile sets the nimino-agent mcp_command.
    let dir = tempfile::tempdir().unwrap();
    write_agents_json(
        dir.path(),
        &serde_json::json!([{
...
fn migrate_dev_nest_carries_knowledge_and_skips_repos() {
    ...
    std::fs::create_dir_all(legacy.join("REPOS/nimino")).unwrap();
    std::fs::write(legacy.join("REPOS/nimino/huge.bin"), "checkout").unwrap();

    let migrated = super::copy_nest_knowledge_at(&legacy, &current);

    assert!(migrated, "migration ran because source nest existed");
    assert!(
...
fn migrate_dev_nest_noops_when_legacy_absent() {
    ...
    assert!(!migrated, "no migration when source nest is absent");
    assert!(
        !current.exists(),
...
#[test]
fn migrate_dev_nest_respects_deliberate_dev_reset() {
    let dir = tempfile::tempdir().unwrap();
    let legacy = dir.path().join(".nimino");
    ...
    assert!(!migrated, "reset marker opts out of source nest imports");
