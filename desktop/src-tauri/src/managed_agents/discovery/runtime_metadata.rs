...
pub(crate) struct KnownAcpRuntime {
    ...
    /// `Some(dir)` → Nimino creates a symlink at `<nest>/<dir>/nimino-cli`
    /// pointing to the canonical `.agents/skills/nimino-cli`. `None` → this
    ...
    /// authenticated. `None` for runtimes that have no login step (goose, nimino-agent).
