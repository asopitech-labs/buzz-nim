...
//! # Exit-code fidelity
//!
//! [#2892](https://github.com/asopitech-labs/nimino/pull/2892) established that an install
...
///
/// `slug` names the downloaded script (`nimino-install-<slug>.ps1`) so concurrent
...
macro_rules! windows_install_command {
    ...
    ($slug:literal, $url:literal, $env_prefix:literal) => {
        concat!(
            ...
            "$installer=Join-Path $env:TEMP 'nimino-install-",
            $slug,
            ".ps1'; ",
            "Invoke-RestMethod ",
            $url,
...
mod tests {
    ...
    fn test_every_windows_install_command_downloads_to_a_file_then_executes_it() {
        ...
        for (id, command) in commands {
            ...
            assert!(
                command.contains(&format!("nimino-install-{id}.ps1")),
