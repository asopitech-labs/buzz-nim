...
fn provider_id_from_filename(name: &str) -> Option<&str> {
    let raw = name.strip_prefix("nimino-backend-")?;
    let id = [".exe", ".bat", ".cmd"]
        .into_iter()
        .find_map(|extension| {
...
/// Enumerate PATH for nimino-backend-* executables. Returns (id, path) pairs.
...
/// `resolve_command(format!("nimino-backend-{id}"))` to prevent a compromised
...
pub fn resolve_provider_binary(provider_id: &str) -> Result<PathBuf, String> {
    ...
    match found {
        ...
        None => Err(format!(
            "provider 'nimino-backend-{provider_id}' not found on PATH"
        )),
    }
}
