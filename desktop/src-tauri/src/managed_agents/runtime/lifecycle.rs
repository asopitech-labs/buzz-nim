...
pub fn kill_stale_tracked_processes(
    ...
    kill_stale_tracked_processes_with(
        records,
        runtimes,
        |pid| process_has_nimino_marker(pid, instance_id),
        terminate_process,
    )
}
...
/// `NIMINO_MANAGED_AGENT` marker; `kill(pid)` performs the termination.
pub(crate) fn kill_stale_tracked_processes_with(
    ...
    for record in records.iter_mut() {
        ...
        if !runtimes.keys().any(|key| key.pubkey == record.pubkey) {
            ...
            // binary names not in KNOWN_AGENT_BINARIES. NIMINO_MANAGED_AGENT is the
