...
//! or that predates `NIMINO_MANAGED_AGENT` injection. This sweep derives the
//! expected `nimino-acp` path from the running executable and kills any process
...
///    intermediates run in their own process groups (e.g. nimino-acp ->
///    node shim -> codex-acp).
#[cfg(target_os = "macos")]
...
///
/// On Linux, `read_link("/proc/<pid>/exe")` returns `…/nimino-acp (deleted)`
...
/// `extract_nimino_marker_value` / `process_has_nimino_marker` which must skip
...
/// binary name (`nimino-acp`) proceed to the full exe-path fetch.  This mirrors
...
/// processes but at most a handful of `nimino-acp` instances).
#[cfg(target_os = "macos")]
...
fn collect_process_snapshots(harness_name: &str) -> Vec<ProcessSnapshot> {
    ...
    for entry in entries.flatten() {
        ...
        // "nimino-acp" is 8 chars so it's always preserved).
...
/// Derive the expected path of the `nimino-acp` harness binary next to the
/// current executable. Returns `None` if `current_exe()` fails or has no
/// parent directory.
///
/// In a `.app` bundle: `.../Contents/MacOS/nimino-acp`.
/// In a dev checkout: `<target-dir>/debug/nimino-acp` or similar.
...
/// scopes by `NIMINO_MANAGED_AGENT` instance ID rather than exe path.
pub fn expected_harness_exe_path() -> Option<PathBuf> {
    let exe = std::env::current_exe().ok()?;
    let dir = exe.parent()?;
    let raw = dir.join("nimino-acp");
...
const HARNESS_BINARY_NAME: &str = "nimino-acp";

// ── sweep_untracked_bundle_harnesses ─────────────────────────────────────

/// Sweep and kill harness processes that share this bundle's exact `nimino-acp`
/// executable path but are not in `skip_pids`.
///
/// Complements the env-var-based `sweep_system_agent_processes`: this sweep
/// catches orphans that predate the `NIMINO_MANAGED_AGENT` env var injection
...
/// where possible) equals `<this bundle>/nimino-acp` are candidates. Dev builds
...
pub(crate) fn sweep_untracked_bundle_harnesses(skip_pids: &[u32]) {
    ...
    eprintln!(
        "nimino-desktop: sweep_untracked_bundle_harnesses: reaping {} stale harness process(es) {:?} (exe: {})",
        to_kill.len(),
        to_kill,
        harness_exe.display(),
    );
...
mod tests {
    ...
    fn strip_deleted_suffix_removes_kernel_suffix() {
        ...
        let p = PathBuf::from("/Applications/Nimino.app/Contents/MacOS/nimino-acp (deleted)");
        assert_eq!(
            strip_deleted_suffix(p),
            PathBuf::from("/Applications/Nimino.app/Contents/MacOS/nimino-acp")
        );
    }

    #[test]
    fn strip_deleted_suffix_leaves_normal_path_unchanged() {
        let p = PathBuf::from("/Applications/Nimino.app/Contents/MacOS/nimino-acp");
    ...
    fn strip_deleted_suffix_does_not_strip_partial_match() {
        ...
        let p = PathBuf::from("/some/path/nimino-acp(deleted)");
        assert_eq!(strip_deleted_suffix(p.clone()), p,);
    }

    // ── select_untracked_bundle_harnesses ────────────────────────────────

    const BUNDLE_HARNESS: &str = "/Applications/Nimino.app/Contents/MacOS/nimino-acp";
    const DEV_HARNESS: &str = "/Users/dev/nimino/.worktrees/main/target/debug/nimino-acp";
    ...
    fn child_of_tracked_parent_not_directly_targeted() {
        ...
        let snapshots = vec![snap(1004, "/Applications/Nimino.app/Contents/MacOS/goose")];
        let result =
            select_untracked_bundle_harnesses(&snapshots, &PathBuf::from(BUNDLE_HARNESS), &[]);
        assert!(result.is_empty());
    }
    ...
    fn deleted_suffix_stripped_path_matches_expected() {
        ...
        let raw = PathBuf::from("/Applications/Nimino.app/Contents/MacOS/nimino-acp (deleted)");
