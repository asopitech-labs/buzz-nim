...
/// A1: NIMINO_ACP_MODEL must NOT be present in the spawned-child env after
/// `apply_claude_model_env`, even if it was set before (dual-authority defect).
/// ANTHROPIC_MODEL must be set to the resolved model.
#[test]
fn a1_nimino_acp_model_absent_anthropic_model_present_after_env_apply() {
    let mut cmd = std::process::Command::new("true");
    // Simulate descriptor.env writing NIMINO_ACP_MODEL (the pre-A1 path).
    cmd.env("NIMINO_ACP_MODEL", "claude-opus-4");
    apply_claude_model_env(&mut cmd, Some("claude-opus-4"));

    let env_map: std::collections::HashMap<_, _> = cmd.get_envs().collect();

    // NIMINO_ACP_MODEL must be removed. Command::get_envs returns None for
    // explicitly-removed keys.
    let nimino_acp = env_map.get(std::ffi::OsStr::new("NIMINO_ACP_MODEL"));
    assert!(
        nimino_acp.is_none() || nimino_acp.unwrap().is_none(),
        "NIMINO_ACP_MODEL must be absent (or explicitly removed) after A1 policy"
    );
...
fn a1_anthropic_model_removed_when_no_effective_model() {
    ...
    cmd.env("NIMINO_ACP_MODEL", "claude-3-5-sonnet");
    ...
    assert!(
        nimino_acp.is_none() || nimino_acp.unwrap().is_none(),
        "NIMINO_ACP_MODEL must always be absent after A1 policy"
    );
}
...
/// Simulates the defect scenario: descriptor.env wrote NIMINO_ACP_EFFORT_LEVEL=low,
...
fn b5_canonical_effort_wins_over_user_env_collision() {
    ...
    cmd.env("NIMINO_ACP_EFFORT_LEVEL", "low");

    // Post-loop canonical application — the fix.
    apply_effort_env(&mut cmd, Some("high"));

    let env_map: std::collections::HashMap<_, _> = cmd.get_envs().collect();
    let effort = env_map.get(std::ffi::OsStr::new("NIMINO_ACP_EFFORT_LEVEL"));
    assert!(effort.is_some(), "NIMINO_ACP_EFFORT_LEVEL must be present");
    assert_eq!(
...
/// Simulates: descriptor.env wrote NIMINO_ACP_EFFORT_LEVEL=low (already in command),
...
fn b5_canonical_effort_injected_when_no_user_collision() {
    let mut cmd = std::process::Command::new("true");
    // No user-supplied NIMINO_ACP_EFFORT_LEVEL in descriptor.env.
