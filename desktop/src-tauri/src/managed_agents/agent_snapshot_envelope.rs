...
//!
//! A locked card carries the same `nimino_agent_snapshot` tEXt chunk as a plain
...
//!
//! Wire contract (agreed with Wren, nimino-agent-trading-cards thread):
...
use nimino_core_pkg::engram::NIP44_PLAINTEXT_MAX;
...
/// `nimino-agent-snapshot` so detection never guesses.
pub const LOCKED_FORMAT: &str = "nimino-agent-snapshot-encrypted";
...
/// Typed outer envelope stored (base64 JSON) in the `nimino_agent_snapshot`
...
///
/// - `nimino-agent-snapshot` → full plain-manifest decode + validation.
/// - `nimino-agent-snapshot-encrypted` → size caps, typed envelope parse,
...
mod tests {
    ...
    fn unknown_format_version_scheme_rejected() {
        ...
        let unknown = serde_json::json!({"format": "nimino-agent-snapshot-v9", "version": 1});
