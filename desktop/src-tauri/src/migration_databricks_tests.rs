...
fn reconcile_databricks_v1_to_v2_rewrites_v1_provider_on_block_build() {
    ...
    // NIMINO_AGENT_PROVIDER=databricks_v2). The structured provider field
    ...
    // shadowed by NIMINO_AGENT_MODEL at spawn time (last-write-wins in Command::env).
    assert!(
...
fn reconcile_databricks_v1_to_v2_preserves_v1_provider_on_oss_build() {
    ...
    write_agents_json(
        ...
        &serde_json::json!([{
            ...
            "provider": "databricks",
            "model": "dbrx-instruct",
            "env_vars": { "NIMINO_AGENT_PROVIDER": "databricks" }
        }]),
    );
    ...
    assert!(
        records[0]["env_vars"]
            ...
            .is_none(),
        "NIMINO_AGENT_PROVIDER must be stripped even when provider rewrite is disabled"
    );
}
...
fn reconcile_databricks_v1_to_v2_clears_model_on_provider_rewrite() {
    ...
    // via NIMINO_AGENT_MODEL at spawn time would shadow the baked DATABRICKS_MODEL
...
fn reconcile_databricks_v1_to_v2_strips_stale_buzz_agent_provider_from_env_vars() {
    ...
    assert!(
        records[0]["env_vars"]
            .get("NIMINO_AGENT_PROVIDER")
            .is_none(),
        "NIMINO_AGENT_PROVIDER must be stripped from env_vars"
    );
...
fn reconcile_databricks_v1_to_v2_handles_multiple_records_block_build() {
    ...
    write_agents_json(
        ...
        &serde_json::json!([
            ...
            },
            {
                "name": "Agent B",
                "provider": "anthropic",
                "env_vars": { "NIMINO_AGENT_MODEL": "claude-3-5-sonnet" }
            },
            {
                "name": "Agent C",
                "provider": "databricks_v2",
    ...
    assert!(records[0]["env_vars"]
        .get("NIMINO_AGENT_PROVIDER")
        .is_none());
    // B: provider untouched, stale NIMINO_AGENT_MODEL stripped.
    assert_eq!(records[1]["provider"], "anthropic");
    assert!(records[1]["env_vars"].get("NIMINO_AGENT_MODEL").is_none());
...
fn reconcile_databricks_v1_to_v2_strips_derived_keys_from_keyless_persona_definition() {
    ...
    assert!(
        env_vars.get("NIMINO_AGENT_PROVIDER").is_none(),
        "NIMINO_AGENT_PROVIDER must be stripped from keyless persona definition env_vars"
    );
    assert!(
        env_vars.get("NIMINO_AGENT_MODEL").is_none(),
        "NIMINO_AGENT_MODEL must be stripped from keyless persona definition env_vars"
    );
