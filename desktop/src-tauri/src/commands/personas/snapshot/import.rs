//! Import-side helpers for `nimino-agent-snapshot v1`.
//!
...
/// Decode a `nimino-agent-snapshot v1` manifest from raw bytes.
///
...
/// Import a `nimino-agent-snapshot v1` file as a brand-new agent.
///
/// Phase sequence:
...
pub async fn confirm_agent_snapshot_import(
    ...
    if memory_total > 0 {
        ...
        for (idx, entry) in snapshot.memory.entries.iter().enumerate() {
            let body = if entry.slug == engram::CORE_SLUG {
                engram::Body::Core {
            ...
            match engram::build_event(&agent_keys, &owner_pubkey, &body, created_at) {
                Ok(event) => {
...
fn retain_agent_pending(app: &AppHandle, state: &AppState, record: &ManagedAgentRecord) {
    ...
    if let Err(e) = result {
        eprintln!("nimino-desktop: snapshot-import retain-agent: {e}");
    }
}
