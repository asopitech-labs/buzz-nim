...
fn resolve_config_surface(
    ...
    // model/provider/prompt so they can never masquerade as NiminoExplicit and
...
/// the binary at compile time via `NIMINO_BUILD_AGENT_ENV`. The backend readiness
...
/// record; at spawn `runtime.rs` injects it as `NIMINO_ACP_EFFORT_LEVEL` and the
