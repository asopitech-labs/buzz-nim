...
pub(crate) fn retain_managed_agent_pending(
    ...
    if let Err(e) = result {
        eprintln!("nimino-desktop: agent-retain: {e}");
    }
}
...
pub(crate) fn tombstone_managed_agent_pending(
    ...
    if let Err(e) = result {
        eprintln!("nimino-desktop: agent-tombstone: {e}");
    }
}
...
pub(crate) fn build_agent_archive_request(
        ...
        .eq_ignore_ascii_case(agent_pubkey)
    {
        ...
        let tag_json = nimino_sdk_pkg::nip_oa::compute_auth_tag(keys, &agent, "")
...
pub(crate) fn archive_managed_agent_pending(
    ...
    use nimino_core_pkg::kind::KIND_IA_ARCHIVE_REQUEST;
    use nostr::JsonUtil;
    ...
    if let Err(e) = result {
        eprintln!("nimino-desktop: agent-archive: {e}");
    }
}
