...
pub(crate) fn validate_clone_url(clone_url: &str) -> Result<(), String> {
    ...
    // Nimino git remotes are served at `…/git/<owner-pubkey>/<repo-id>` — a
    ...
    let segments = parsed
        ...
        .unwrap_or_default();
    let is_nimino_repo_path = segments
        .iter()
        ...
        .unwrap_or(false);
    if !is_nimino_repo_path {
        return Err("clone URL must point at a Nimino git repository".into());
    }
    Ok(())
}
...
pub(crate) fn validate_local_clone_url(clone_url: &str) -> Result<(), String> {
    ...
    Err("clone URL must point at a Nimino repository or public GitHub repository".into())
}
...
mod tests {
    ...
    fn credential_helper_config_value_uses_forward_slashes() {
        let path =
            std::path::PathBuf::from(r"C:\Users\x\AppData\Local\Nimino\git-credential-nostr.exe");
        assert_eq!(
            credential_helper_config_value(&path),
            "C:/Users/x/AppData/Local/Nimino/git-credential-nostr.exe",
        );
    }
    ...
    #[test]
    fn validate_clone_url_requires_nimino_repo_shape() {
    ...
    fn local_clone_url_allows_only_public_github_https_urls() {
        assert!(validate_local_clone_url("https://github.com/asopitech-labs/nimino").is_ok());
        assert!(validate_local_clone_url("https://github.com/asopitech-labs/nimino.git").is_ok());
        assert!(validate_local_clone_url("http://github.com/asopitech-labs/nimino").is_err());
        assert!(
            validate_local_clone_url("https://github.com/asopitech-labs/nimino/issues").is_err()
        );
        assert!(validate_local_clone_url("https://user@github.com/asopitech-labs/nimino").is_err());
        assert!(
            validate_local_clone_url("https://github.com.evil.test/asopitech-labs/nimino").is_err()
        );
        assert!(validate_local_clone_url("https://gitlab.com/asopitech-labs/nimino").is_err());
    }
}
