...
/// Query available models from an agent via `nimino-acp models --json`.
///
...
pub async fn discover_agent_models(
    ...
    // Nimino shared compute discovery must not depend on the local OpenAI ingress: that
        ...
        == Some(crate::managed_agents::RELAY_MESH_PROVIDER_ID)
    {
        ...
        .await
        .map_err(|error| format!("Nimino shared compute model discovery failed: {error}"))?;
        ...
        if availability.models.is_empty() {
            return Err(availability.reason.unwrap_or_else(|| {
                "No live Nimino shared compute models are available".to_string()
            }));
        }
    ...
    if input.provider.as_deref().map(str::trim)
        == Some(crate::managed_agents::RELAY_MESH_PROVIDER_ID)
    {
        return Err("Nimino shared compute is not available in this build".to_string());
    }
...
/// Normalize raw `nimino-acp models --json` output into a typed DTO for the frontend.
///
