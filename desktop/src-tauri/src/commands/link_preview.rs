...
async fn resolve_public_addresses(host: &str) -> Result<Vec<IpAddr>, String> {
    ...
    if addresses
        .iter()
        .any(nimino_core_pkg::network::is_private_ip)
    {
...
async fn send_pinned_request(url: &Url, accept: &str) -> Result<reqwest::Response, String> {
    ...
    let request = client
        ...
        .header(USER_AGENT, "Nimino Desktop link preview");
...
mod tests {
    ...
    fn metadata_prefers_open_graph_and_reads_site_name() {
        let html = r#"<meta content="Nimino" property="og:site_name">
        ...
        assert_eq!(
            ...
            Some(LinkPreviewMetadata {
                ...
                site_name: Some("Nimino".to_string()),
    ...
    fn favicon_metadata_prefers_a_supported_raster_candidate() {
        let page = Url::parse("https://github.com/asopitech-labs/nimino").unwrap();
