...
impl From<String> for ProjectPullRequestMergeError {
    fn from(message: String) -> Self {
        // Relay push-policy denial for a repo with no `nimino-channel` binding.
        // The stable token is declared in `nimino-core::git_perms`
        ...
        if message.contains(nimino_core_pkg::git_perms::GIT_NO_CHANNEL_BINDING_TOKEN) {
            return Self::new(
                nimino_core_pkg::git_perms::GIT_NO_CHANNEL_BINDING_TOKEN,
...
mod tests {
    ...
    fn no_channel_binding_denial_converts_to_structured_code() {
        ...
        let remote_stderr = format!(
            ...
            nimino_core_pkg::git_perms::GIT_NO_CHANNEL_BINDING_BODY
        );
