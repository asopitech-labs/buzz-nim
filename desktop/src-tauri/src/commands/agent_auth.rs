...
pub struct AcpAuthMethod {
    ...
    #[serde(default)]
    pub args: Vec<String>,
    /// Full terminal command advertised by the adapter. Nimino never guesses
...
fn discover_acp_auth_methods_blocking(runtime_id: &str) -> Result<AcpAuthMethodsResult, String> {
    let output = run_nimino_acp_auth_command(runtime_id, ["auth-methods", "--json"])?;
    if !output.status.success() {
        return Err(command_error("nimino-acp auth-methods", &output));
    }
...
fn connect_acp_runtime_blocking(
    ...
    let output = run_nimino_acp_auth_command(
        &request.runtime_id,
        ["authenticate", "--method-id", request.method_id.as_str()],
    )?;
    if !output.status.success() {
        return Err(command_error("nimino-acp authenticate", &output));
    }

    Ok(ConnectAcpRuntimeResult { launched: true })
}

fn run_nimino_acp_auth_command<const N: usize>(
    runtime_id: &str,
    args: [&str; N],
    ...
    let acp_path = std::env::current_exe()
        .map(|path| path.with_file_name(format!("nimino-acp{}", std::env::consts::EXE_SUFFIX)))
        .ok()
        .filter(|path| path.exists())
        .or_else(|| resolve_command("nimino-acp"))
        .ok_or_else(|| "nimino-acp helper not found".to_string())?;

    let augmented_path = auth_command_path();
    run_nimino_acp_auth_command_with_paths(
        &acp_path,
        adapter_command.0,
        &adapter_command.1,
        args,
...
/// PATH for the nimino-acp auth helper child process.
///
/// Uses the augmented agent PATH so `#!/usr/bin/env node` adapter shims
/// resolve the Nimino-managed Node runtime — the same PATH normal agent
/// launches and readiness probes use.
///
/// On Windows, `login_shell_path()` is intentionally `None`, so the augmented
/// PATH contains only Nimino-managed directories and the exe parent. Nimino does
...
fn run_nimino_acp_auth_command_with_paths<const N: usize>(
    acp_path: &Path,
    adapter_name: &str,
    adapter_path: &Path,
    args: [&str; N],
    ...
    command
        .args(args)
        .env("NIMINO_ACP_AGENT_COMMAND", adapter_path.as_os_str())
        .env("NIMINO_ACP_AGENT_ARGS", agent_args.join(","))
        .stdout(Stdio::piped())
        .stderr(Stdio::piped());
    ...
    command
        .output()
        .map_err(|error| format!("failed to run nimino-acp auth helper: {error}"))
}
...
mod tests {
    use super::{
        ...
        run_nimino_acp_auth_command_with_paths, shell_escape, shell_join, uses_terminal_auth,
        windows_terminal_args, AcpAuthMethod,
    };

    /// Windows regression: the augmented PATH there holds only Nimino-managed
    ...
    fn append_inherited_path_appends_after_augmented() {
        ...
        let augmented = format!("{0}nimino-bin{1}{0}exe-dir", std::path::MAIN_SEPARATOR, sep);
...
"#
    ...
    fn terminal_argv_uses_adapter_declared_command() {
        ...
        let method = AcpAuthMethod {
            ...
            command: vec![
                "definitely-not-on-path-nimino-test".into(),
                "auth".into(),
                "login".into(),
            ],
            meta: None,
        ...
        assert_eq!(
            ...
            vec![
                "definitely-not-on-path-nimino-test".to_string(),
                "auth".to_string(),
                "login".to_string()
            ]
        );
    ...
    fn terminal_argv_falls_back_to_adapter_command() {
        ...
        assert_eq!(
            adapter_terminal_argv("Claude Code", &method, "definitely-not-on-path-nimino-test")
                .unwrap(),
            vec!["definitely-not-on-path-nimino-test".to_string()]
        );
    }
}
