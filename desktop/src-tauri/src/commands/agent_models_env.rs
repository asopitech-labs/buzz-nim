...
/// with the key in goose's own keyring rather than Nimino's env) would otherwise
...
impl DiscoveryProvider {
    ...
    /// the caller must fall through to the `nimino-acp models` subprocess rather
...
/// — even on an internal build that bakes `NIMINO_AGENT_PROVIDER=databricks_v2`
/// and a `DATABRICKS_HOST`. Discovery then degrades to the `nimino-acp models`
