...
pub async fn spawn_media_proxy(http_client: reqwest::Client, app_handle: tauri::AppHandle) -> u16 {
    ...
    eprintln!("nimino-desktop: media proxy listening on 127.0.0.1:{port}");
    port
}
...
/// This handler routes `nimino-media://localhost/{path}` through reqwest, which
/// runs in the Tauri process and goes through the VPN.
pub async fn handle_nimino_media(
    app: &tauri::AppHandle,
