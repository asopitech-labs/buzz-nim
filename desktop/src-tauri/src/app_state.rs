    ...
    match store.probe(IDENTITY_KEY_NAME) {
                    ...
                    Ok(keyring_keys) => {
                        ...
                        if legacy_path.exists() {
                            match load_key_file(legacy_path) {
                                ...
                                {
                                    eprintln!(
                                        "nimino-desktop: identity.key differs from keyring; \
                                    ...
                                    let storage = if let Err(e) = persist_identity_to_keyring(
                                        ...
                                        eprintln!(
                                            "nimino-desktop: keyring adoption of identity.key \
                                ...
                                Err(e) => {
                                    eprintln!(
                                        "nimino-desktop: leftover identity.key is corrupt ({e}); \
                        ...
                        if !legacy_path.exists() && !migration_marker_path(data_dir).exists() {
                            ...
                            {
                                eprintln!(
                                    "nimino-desktop: keyring present but marker missing; \
        ...
        KeyringProbe::ReachableButEmpty => {
            ...
            if legacy_path.exists() {
                ...
                eprintln!(
                    "nimino-desktop: identity lost — keyring was empty despite migration marker; \
        ...
        KeyringProbe::Unreachable => {
            ...
            if !legacy_path.exists() && migration_marker_path(data_dir).exists() {
                let ephemeral = Keys::generate();
                eprintln!(
                    "nimino-desktop: keyring unreachable but migration marker present; \
...
fn recover_from_keyring(
    ...
    eprintln!("nimino-desktop: corrupt keyring nsec ({error}); recovering from file");
    if let Err(e) = store.delete(IDENTITY_KEY_NAME) {
        eprintln!("nimino-desktop: failed to clear corrupt keyring value: {e}");
    }
    if legacy_path.exists() {
        if let Some(keys) = migrate_identity_file(store, legacy_path, data_dir)? {
            return Ok(ResolvedIdentity {
    ...
    if migration_marker_path(data_dir).exists() {
        let ephemeral = Keys::generate();
        eprintln!(
            "nimino-desktop: identity lost — keyring had corrupt data and no valid identity.key \
...
fn migrate_identity_file(
    ...
    let keys = match load_key_file(legacy_path) {
        Ok(keys) => keys,
        Err(error) => {
            eprintln!("nimino-desktop: corrupt identity.key during migration ({error}), skipping");
            return Ok(None);
        }
    };
    let nsec = keys
    ...
    if let Err(e) = write_migration_marker(&marker_path) {
        eprintln!(
            "nimino-desktop: keyring import ok but failed to write migration marker ({e}); \
             keeping identity.key so the key is not stranded"
        );
        return Ok(Some(keys));
    }
    if let Err(e) = std::fs::remove_file(legacy_path) {
        eprintln!("nimino-desktop: keyring import ok but failed to delete identity.key: {e}");
    } else {
        eprintln!("nimino-desktop: migrated identity key into OS keyring");
    }
    Ok(Some(keys))
}
...
fn persist_identity_to_keyring(
    ...
    if let Err(e) = write_migration_marker(&marker_path) {
        ...
        if !legacy_path.exists() {
            if let Err(write_err) = save_key_file(legacy_path, keys) {
                eprintln!(
                    "nimino-desktop: keyring ok but marker write failed ({e}) and \
                ...
                eprintln!(
                    "nimino-desktop: keyring ok but marker write failed ({e}); \
    ...
    if legacy_path.exists() {
        if let Err(e) = std::fs::remove_file(legacy_path) {
            eprintln!("nimino-desktop: keyring write ok but failed to delete identity.key: {e}");
        }
    }
...
fn persist_imported_identity_impl(
    ...
    match persist_identity_to_keyring(store, keys, legacy_path, data_dir) {
        ...
        Err(e) => {
            eprintln!(
                "nimino-desktop: keyring write failed during import ({e}), \
...
fn generate_and_persist(
    ...
    if storage == IdentityStorage::SystemKeyring {
        ...
        if let Err(e) = write_migration_marker(&marker_path) {
            eprintln!(
                "nimino-desktop: stored identity in keyring but failed to write migration marker \
...
fn store_key_preferring_keyring(
    ...
    match store.store(IDENTITY_KEY_NAME, &nsec) {
        Ok(()) => Ok(IdentityStorage::SystemKeyring),
        Err(keyring_err) => {
            eprintln!("nimino-desktop: keyring write failed ({keyring_err}), using file fallback");
            save_key_file(legacy_path, keys)?;
            Ok(IdentityStorage::LocalFile)
        }
    }
...
fn cleanup_leftover_identity_file(legacy_path: &std::path::Path) {
    ...
    match std::fs::remove_file(legacy_path) {
        Ok(()) => eprintln!("nimino-desktop: removed leftover identity.key (key is in keyring)"),
        Err(e) => eprintln!("nimino-desktop: failed to remove leftover identity.key: {e}"),
    }
}
...
fn quarantine_corrupt_key(key_path: &std::path::Path, data_dir: &std::path::Path, error: &str) {
    ...
    eprintln!("nimino-desktop: corrupt identity.key ({error}), quarantining to {bad_name}");
