...
fn main() {
    println!("cargo:rerun-if-env-changed=NIMINO_RELAY_URL");
    println!("cargo:rerun-if-env-changed=NIMINO_RELAY_HTTP");
    println!("cargo:rerun-if-env-changed=NIMINO_UPDATER_PUBLIC_KEY");
    println!("cargo:rerun-if-env-changed=NIMINO_UPDATER_ENDPOINT");
    println!("cargo:rerun-if-env-changed=NIMINO_BUILD_NIMINO_AGENT_PROVIDER");
    println!("cargo:rerun-if-env-changed=NIMINO_BUILD_NIMINO_AGENT_MODEL");
    println!("cargo:rerun-if-env-changed=NIMINO_BUILD_AGENT_ENV");
    ...
    if std::env::var("NIMINO_BUILD_AGENT_ACCESS_OWNER_ONLY").is_ok() {
        println!("cargo:rustc-env=NIMINO_DESKTOP_BUILD_AGENT_ACCESS_OWNER_ONLY=1");
    }

    if let Ok(relay_url) = std::env::var("NIMINO_RELAY_URL") {
        println!("cargo:rustc-env=NIMINO_DESKTOP_BUILD_RELAY_URL={relay_url}");
    }

    if let Ok(relay_http) = std::env::var("NIMINO_RELAY_HTTP") {
        println!("cargo:rustc-env=NIMINO_DESKTOP_BUILD_RELAY_HTTP={relay_http}");
    }

    if let Ok(provider) = std::env::var("NIMINO_BUILD_NIMINO_AGENT_PROVIDER") {
        println!("cargo:rustc-env=NIMINO_DESKTOP_BUILD_NIMINO_AGENT_PROVIDER={provider}");
    }

    if let Ok(model) = std::env::var("NIMINO_BUILD_NIMINO_AGENT_MODEL") {
        println!("cargo:rustc-env=NIMINO_DESKTOP_BUILD_NIMINO_AGENT_MODEL={model}");
    }
    ...
    if let Ok(raw) = std::env::var("NIMINO_BUILD_AGENT_ENV") {
        for (line_no, line) in raw.lines().enumerate() {
            ...
            let eq = line.find('=').unwrap_or_else(|| {
                panic!(
                    "NIMINO_BUILD_AGENT_ENV line {}: missing '=' separator in {:?}",
                    line_no + 1,
                    line
                )
            });
            ...
            if key.is_empty() {
                panic!(
                    "NIMINO_BUILD_AGENT_ENV line {}: key must not be empty in {:?}",
                    line_no + 1,
                    line
                );
            }
        ...
        println!("cargo:rustc-env=NIMINO_DESKTOP_BUILD_AGENT_ENV={encoded}");
    }

    if let Ok(val) = std::env::var("NIMINO_BUILD_RELAY_RECONNECT_CMD") {
        let parsed: serde_json::Value = serde_json::from_str(&val)
            .unwrap_or_else(|e| panic!("NIMINO_BUILD_RELAY_RECONNECT_CMD is not valid JSON: {e}"));
        serde_json::from_value::<ReconnectHookConfig>(parsed).unwrap_or_else(|e| {
            panic!("NIMINO_BUILD_RELAY_RECONNECT_CMD doesn't match ReconnectHookConfig: {e}")
        });
        println!("cargo:rustc-env=NIMINO_DESKTOP_BUILD_RELAY_RECONNECT_CMD={val}");
    }
    ...
    if std::env::var("NIMINO_BUILD_AUTO_CONNECT_DEFAULT_RELAY").is_ok() {
        println!("cargo:rustc-env=NIMINO_DESKTOP_BUILD_AUTO_CONNECT_DEFAULT_RELAY=1");
    }

    let updater_public_key = std::env::var("NIMINO_UPDATER_PUBLIC_KEY")
        .ok()
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty());
    let updater_endpoint = std::env::var("NIMINO_UPDATER_ENDPOINT")
        .ok()
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty());
