...
2. Copy the encoded files into this fixture directory:
   ...
   ```sh
   cp uikit-encoded.png crates/nimino-media/tests/fixtures/ios/
   cp uikit-encoded.jpg crates/nimino-media/tests/fixtures/ios/
   ```

3. Run `cargo test -p nimino-media test_ios_uikit` to verify that UIKit's encoded output is rejected and the matching sanitizer output is accepted by the relay contract.
