# Trace Schema (`nimino-conformance`)
...
- **`sanitized_error { reason }`** where `reason ∈ { restricted,
  ...
  `crates/nimino-relay/src/conformance/mod.rs`) goes non-exhaustive and
  CI catches it.
...
| File | What it emits |
|------|---------------|
| `crates/nimino-relay/src/conformance/mod.rs` | helpers + `EmitGuard` + `sanitized_reason_for` |
| `crates/nimino-relay/src/conformance/tracers.rs` | `NoopTracer` (prod default), `JsonlTracer` |
| `crates/nimino-relay/src/handlers/ingest.rs` | `AuthCheck`, `WriteInsert`, `WriteInsertGlobal`, `WriteDuplicate`, outer-wrapper `SanitizedError` |
| `crates/nimino-relay/src/handlers/req.rs` | **held back** — additive patch for integration onto Max's req.rs work |

## Where the checker lives

| File | What it does |
|------|--------------|
| `crates/nimino-conformance/src/lib.rs` | schema + `Tracer` trait |
| `crates/nimino-conformance/src/transitions.rs` | spec `Next` re-implementation |
| `crates/nimino-conformance/src/checker.rs` | replay engine: `IllegalTransition` / `StateMismatch` / `NonInterference` / `CoverageBreach` |
...
`crates/nimino-conformance/src/checker.rs::tests` proving the gate bites
