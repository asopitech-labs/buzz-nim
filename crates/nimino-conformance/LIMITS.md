...
boundary** in `crates/nimino-relay/src/handlers/{ingest,req,event}.rs`.
...
3. Every other layer (DB filter SQL, object metadata, socket delivery) is
...
- **Cross-process leaks.** The harness traces one process. A multi-node
  leak (NIP-98 replay across nodes, fanout to the wrong node) shows up
...
- **Socket fan-out.** Fan-out is **not** a spec action (see the
...
cargo test -p nimino-conformance --lib

# 2. Replay fixtures (5 tests). Three JSONL traces in
#    crates/nimino-conformance/tests/fixtures/ are committed for reviewer
...
#    NIMINO_CONFORMANCE_UPDATE=1 cargo test -p nimino-conformance --test replay_fixtures
cargo test -p nimino-conformance --test replay_fixtures

# 3. EmitGuard coverage-breach self-test (2 tests in
#    crates/nimino-relay/src/conformance/mod.rs). Proves the Drop guard
#    records `ImplBug` when no emit reaches the tracer, and stays
#    silent when an emit did. The seam-name string flows through.
cargo test -p nimino-relay --lib conformance::
