...
/// trace schema.
///
/// This deliberately does NOT reuse `nimino_core::CommunityId`. Two reasons:
///
/// 1. **Production fence preservation.** `nimino_core::CommunityId` has no
...
impl CommunityLabel {
    /// Wrap a UUID into a community label. Unlike `nimino_core::CommunityId`
