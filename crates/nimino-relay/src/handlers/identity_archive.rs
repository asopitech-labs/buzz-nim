...
use nimino_core::kind::{KIND_IA_ARCHIVE_REQUEST, KIND_IA_UNARCHIVE_REQUEST, KIND_PROFILE};
...
fn verify_auth_tag_owner(auth_tag_json: &str, target_hex: &str) -> Result<String, String> {
    let target_pubkey =
        PublicKey::from_hex(target_hex).map_err(|e| format!("invalid target pubkey: {e}"))?;
    nimino_sdk::nip_oa::verify_auth_tag(auth_tag_json, &target_pubkey)
...
mod tests {
    ...
    fn auth_tag(owner_keys: &Keys, target_pubkey: &nostr::PublicKey) -> Tag {
        let tag_json = nimino_sdk::nip_oa::compute_auth_tag(owner_keys, target_pubkey, "")
            .expect("compute auth tag");
        nimino_sdk::nip_oa::parse_auth_tag(&tag_json).expect("parse auth tag")
    }
    ...
    async fn seed_test_community(pool: &sqlx::PgPool) -> nimino_core::tenant::TenantContext {
    ...
    async fn archival_snapshot_advances_timestamp_for_rapid_state_replacement() {
        ...
        let archived_snapshot = state
            .db
            .query_events(&EventQuery {
                kinds: Some(vec![nimino_core::kind::KIND_IA_ARCHIVED_LIST as i32]),
