...
use nimino_boundary::{
    ModerationEffect, ModerationEnforcementOperation, ModerationEnforcementRequest,
    ModerationPolicyError, ModerationPolicyRequest, ModerationPolicyResult,
};
...
/// the JSON-array string that [`nimino_sdk::nip_oa::verify_auth_tag`] expects.
///
...
pub async fn handle_auth(event: nostr::Event, conn: Arc<ConnectionState>, state: Arc<AppState>) {
    ...
    metrics::counter!("nimino_auth_attempts_total", "method" => "nip42").increment(1);
        ...
        .await
    {
        Ok(mut auth_ctx) => {
            ...
            {
                let owner = crate::api::relay_members::extract_nip_oa_owner(
                    pubkey.as_bytes(),
                ...
                let (principal, owner_restriction) = tokio::join!(
                    state
                        .db
                        .moderation_restriction_facts(conn.tenant.community(), pubkey.as_bytes(),),
                    async {
                        match owner.as_ref() {
                            Some(owner) => {
                                state
                                    .db
                                    .moderation_restriction_facts(
                ...
                let denial = match (principal, owner_restriction) {
                    (Ok(principal), Ok(owner_restriction)) => {
                        let result = super::ingest::call_moderation_policy(
                            &state,
                            ModerationPolicyRequest::Enforcement {
                                request: ModerationEnforcementRequest {
                                    operation: ModerationEnforcementOperation::Authenticate,
                                    request_community: *conn.tenant.community().as_uuid(),
                                    principal_restriction_community: principal
                                        .exists
                                        .then_some(*conn.tenant.community().as_uuid()),
                                    owner_restriction_community: owner_restriction
                                        .exists
                                        .then_some(*conn.tenant.community().as_uuid()),
                                    principal_restriction_exists: principal.exists,
                                    principal_ban_set: principal.ban_set,
                                    principal_ban_expires_at: principal
                                        .ban_expires_at
                                        .map(|value| value.timestamp()),
                                    principal_muted_until: principal
                                        .muted_until
                                        .map(|value| value.timestamp()),
                                    owner_attested: owner.is_some(),
                                    owner_restriction_exists: owner_restriction.exists,
                                    owner_ban_set: owner_restriction.ban_set,
                                    owner_ban_expires_at: owner_restriction
                                        .ban_expires_at
                                        .map(|value| value.timestamp()),
                        ...
                        .await;
                        match result {
                            Ok(ModerationPolicyResult::Enforcement {
                            ...
                            Err(error) => {
                                warn!(conn_id = %conn_id, ?error, "Nim ban policy failed, denying");
                                Some((
                                    "ban_policy_error",
                                    "error: internal error checking restriction state",
                                ))
                    ...
                    (principal, owner_restriction) => {
                        let error = principal.err().or_else(|| owner_restriction.err());
                        warn!(conn_id = %conn_id, ?error, "ban-state DB lookup failed, denying");
                        Some((
                            "ban_check_error",
                            "error: internal error checking restriction state",
                        ))
                ...
                if let Some((metric_reason, deny_reason)) = denial {
                    ...
                    metrics::counter!("nimino_auth_failures_total", "reason" => metric_reason)
                        .increment(1);
            ...
            if state.config.pubkey_allowlist_enabled
                && auth_ctx.auth_method == nimino_auth::AuthMethod::Nip42
            {
                ...
                if !allowed {
                    ...
                    metrics::counter!("nimino_auth_failures_total", "reason" => "allowlist_denied")
                        .increment(1);
                    *conn.auth_state.write().await = AuthState::Failed;
                    conn.send(RelayMessage::ok(
                        &event_id_hex,
            ...
            .await
            {
                ...
                Err(e) => {
                    ...
                    metrics::counter!("nimino_auth_failures_total", "reason" => "not_relay_member")
                        .increment(1);
                    *conn.auth_state.write().await = AuthState::Failed;
                    conn.send(RelayMessage::ok(
                        &event_id_hex,
            ...
            let authenticated_owner = auth_ctx
                .agent_owner_pubkey
        ...
        Err(e) => {
            ...
            metrics::counter!("nimino_auth_failures_total", "reason" => "nip42_invalid")
                .increment(1);
            *conn.auth_state.write().await = AuthState::Failed;
            conn.send(RelayMessage::ok(
                &event_id_hex,
