...
use nimino_boundary::{
    MembershipAction, MembershipCommand, MembershipPolicyError, MembershipPolicyRequest,
    MembershipPolicyResult, MembershipRole, RelayMembershipRequest,
};
...
fn membership_role_name(role: MembershipRole) -> Result<&'static str, String> {
    match role {
        MembershipRole::Admin => Ok("admin"),
        MembershipRole::Member => Ok("member"),
        _ => Err("membership policy returned an invalid relay role".to_owned()),
    }
}

async fn decide_relay_membership(
    state: &AppState,
    ...
    let MembershipPolicyResult::Relay {
        action,
        error,
        effective_role,
    } = result
    else {
        return Err("Nim membership policy returned an unexpected result".to_owned());
    };
    if error != MembershipPolicyError::None || action == MembershipAction::Reject {
        return Err(format!(
            "membership policy rejected the relay command ({error:?})"
        ));
    }
    Ok((action, effective_role))
}
...
async fn execute_relay_admin_command(
    ...
    let target_member = state
        ...
        .await
        .map_err(|e| format!("database error: {e}"))?;
    let target_role = target_member.as_ref().map(|member| member.role.as_str());

    match kind {
        // kind:9030 — Add relay member
        k if k == RELAY_ADMIN_ADD_MEMBER => {
            let requested_role = extract_tag_value(event, "role");
            let (action, effective_role) = decide_relay_membership(
                state,
                Some(sender_role),
                target_role,
                MembershipCommand::Add,
                requested_role.as_deref(),
                target_hex == sender_hex,
            )
            .await?;
            let role = membership_role_name(effective_role)?;
        ...
        k if k == RELAY_ADMIN_REMOVE_MEMBER => {
            ...
            match remove_result {
                ...
                RemoveResult::RoleMismatch => return Err("membership changed concurrently".into()),
            }
        ...
        k if k == RELAY_ADMIN_CHANGE_ROLE => {
            ...
            let new_role = membership_role_name(effective_role)?;
...
mod tests {
    ...
    fn banned_actor_is_not_admitted_to_a_relay_admin_command() {
        let banned = nimino_db::moderation::RestrictionState {
            banned: true,
            muted_until: None,
        };
        assert_eq!(
    ...
    fn timed_out_actor_is_still_admitted() {
        let timed_out = nimino_db::moderation::RestrictionState {
            banned: false,
    ...
    fn unrestricted_actor_is_admitted() {
        assert!(
            admits_relay_admin_command(&nimino_db::moderation::RestrictionState::default()).is_ok()
        );
    }
