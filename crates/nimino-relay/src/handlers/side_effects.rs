...
use nimino_boundary::{
    AgentAddPolicy, ChannelMembershipRequest, ChannelVisibility, MembershipAction,
    MembershipCommand, MembershipPolicyError, MembershipPolicyRequest, MembershipPolicyResult,
    MembershipRole,
};
...
/// The per-fire authority gate in `nimino-workflow` is the fail-closed backstop;
...
///
/// Nimino accepts standard deletions for self-authored events, plus the owning
...
async fn actor_owns_any_owner_agent(
    state: &Arc<AppState>,
    community_id: nimino_core::CommunityId,
    members: &[nimino_db::channel::MemberRecord],
    actor_bytes: &[u8],
) -> anyhow::Result<bool> {
...
fn member_role(role: MembershipRole) -> anyhow::Result<MemberRole> {
    match role {
        MembershipRole::Owner => Ok(MemberRole::Owner),
        MembershipRole::Admin => Ok(MemberRole::Admin),
        MembershipRole::Member => Ok(MemberRole::Member),
        MembershipRole::Guest => Ok(MemberRole::Guest),
        MembershipRole::Bot => Ok(MemberRole::Bot),
        MembershipRole::None => Err(anyhow::anyhow!(
            "Nim membership policy returned no effective role"
        )),
    }
}

/// Collect scoped membership facts and ask Nim for the exact channel effect.
pub(crate) async fn decide_channel_membership(
    tenant: &TenantContext,
    state: &AppState,
    channel_id: Uuid,
    actor: &[u8],
...
) -> anyhow::Result<(MembershipAction, MemberRole)> {
    ...
    let target_role = members
        .iter()
        .find(|member| member.pubkey == target)
        .map(|member| member.role.as_str());
    let agent_policy = state
        .db
        .get_agent_channel_policy(tenant.community(), target)
        .await?;
    let (target_add_policy, actor_owns_target_agent) = match agent_policy.as_ref() {
        Some((policy, owner)) => (
            match policy.as_str() {
                "owner_only" => AgentAddPolicy::OwnerOnly,
                "nobody" => AgentAddPolicy::Nobody,
                _ => AgentAddPolicy::Anyone,
            },
            owner.as_deref() == Some(actor),
        ),
        None => (AgentAddPolicy::Anyone, false),
    };
    let result = super::ingest::call_membership_policy(
        state,
        MembershipPolicyRequest::Channel {
            request: ChannelMembershipRequest {
                command,
                visibility: if channel.visibility == "open" {
                    ChannelVisibility::Open
                } else {
                    ChannelVisibility::Private
                },
                actor_role: membership_role(actor_role),
                target_role: membership_role(target_role),
                requested_role: requested_role
                    .map(|role| membership_role(Some(role.as_str())))
                    .unwrap_or(MembershipRole::None),
                actor_is_target: actor == target,
                actor_owns_target_agent,
                target_is_agent: agent_policy.is_some(),
                target_add_policy,
                owner_count: i32::try_from(
                    members
                        .iter()
    ...
    .await
    .map_err(|error| anyhow::anyhow!(super::ingest::ingest_error_message(error)))?;
    let MembershipPolicyResult::Channel {
        action,
        error,
        effective_role,
    } = result
        ...
        return Err(anyhow::anyhow!(
            "Nim membership policy returned an unexpected result"
        ));
    };
    if error != MembershipPolicyError::None || action == MembershipAction::Reject {
        return Err(anyhow::anyhow!(
            "membership policy rejected the request ({error:?})"
        ));
    }
    Ok((action, member_role(effective_role)?))
}
    ...
    match kind {
        9000 => {
            ...
            let requested_role = match role_str {
                Some(ref s) => match s.parse::<nimino_db::channel::MemberRole>() {
                    Ok(r) => Some(r),
            ...
            let target_role = members
                ...
                .and_then(|member| member.role.parse::<MemberRole>().ok());
            let command = if actor_bytes == target_pubkey {
                if requested_role.is_some_and(|role| Some(role) != target_role)
                    && target_role.is_some()
                {
                    MembershipCommand::ChangeRole
                } else {
                    MembershipCommand::Join
                }
            } else if requested_role.is_some_and(|role| Some(role) != target_role)
                && target_role.is_some()
            {
                MembershipCommand::ChangeRole
            } else {
        ...
        9002 => {
            ...
            for t in event.tags.iter() {
                if t.kind().to_string() == "name" {
                    match t.content() {
                        Some(v)
                            if !nimino_core::channel::canonical_channel_name(v)
                                .trim()
                                .is_empty() => {}
                        _ => {
        ...
        9022 => decide_channel_membership(
            tenant,
            state,
            channel_id,
            &actor_bytes,
...
pub async fn emit_membership_notification(
    ...
    // through the guarded send path; the access gate no-ops
...
async fn emit_addressable_discovery_event(
    ...
    let min_ts = {
        let existing = state
            .db
            .query_events(&nimino_db::event::EventQuery {
                kinds: Some(vec![kind as i32]),
                channel_id: Some(channel_id),
                limit: Some(1),
                ..nimino_db::event::EventQuery::for_community(tenant.community())
            })
            .await
            .unwrap_or_default();
        existing
...
async fn store_group_members_event(
    ...
    state: &Arc<AppState>,
    channel_id: Uuid,
    member_snapshot: &mut nimino_db::channel::LockedMemberSnapshot,
) -> anyhow::Result<Option<nimino_core::StoredEvent>> {
...
async fn dispatch_group_members_event(
    tenant: &TenantContext,
    state: &Arc<AppState>,
    stored: Option<nimino_core::StoredEvent>,
    relay_pubkey_hex: &str,
) {
...
pub async fn emit_group_discovery_events(
...

    {
        ...
        // Nimino channels always require explicit membership
...
async fn handle_put_user(
    ...
    let requested_role = match extract_tag_value(event, "role") {
    ...
    let target_role = members
        ...
        .and_then(|member| member.role.parse::<MemberRole>().ok());
    let command = if actor_bytes == target_pubkey {
        if requested_role.is_some_and(|role| Some(role) != target_role) && target_role.is_some() {
            MembershipCommand::ChangeRole
        } else {
            MembershipCommand::Join
        }
    } else if requested_role.is_some_and(|role| Some(role) != target_role) && target_role.is_some()
    {
        MembershipCommand::ChangeRole
    } else {
        MembershipCommand::Add
    };
    let (action, role) = decide_channel_membership(
        tenant,
        state,
        channel_id,
        &actor_bytes,
    ...
    .await?;
    if action == MembershipAction::Noop {
        return Ok(());
    }
        ...
        if let Some(val) = tag.content() {
            match key.as_str() {
                "name" => {
                    state
                        ...
                        .update_channel(
                            tenant.community(),
                            channel_id,
                            nimino_db::channel::ChannelUpdate {
                ...
                "visibility" => {
                    ...
                    // filter is the send-time correctness backstop.
...
async fn handle_create_group(
    ...
    let channel_type: nimino_db::channel::ChannelType = channel_type_str
        .parse()
    ...
    if visibility == nimino_db::channel::ChannelVisibility::Open {
        state.invalidate_all_accessible_channels(tenant);
    }
...
async fn handle_join_request(
    ...
    if let Err(e) = emit_membership_notification(
        ...
        &actor_bytes,
        &actor_bytes,
        nimino_core::kind::KIND_MEMBER_ADDED_NOTIFICATION,
    )
    .await
    {
...
async fn handle_a_tag_deletion(
    ...
    match kind_num {
        nimino_core::kind::KIND_WORKFLOW_DEF => {
...
async fn handle_git_repo_announcement(
    ...
    use nimino_db::git_repo::ReserveOutcome;
...
/// carrying the announcer's pubkey in the `p` tag (nimino extension).
...
pub async fn reconcile_nip43_membership_snapshots(state: &Arc<AppState>) -> anyhow::Result<usize> {
    ...
    for community in communities {
        let community_id = nimino_core::CommunityId::from_uuid(community.id);
        ...
        match result {
            ...
            Ok(false) => {}
            Err(error) => {
                metrics::counter!("nimino_nip43_membership_reconciliation_failures_total")
                    .increment(1);
    ...
    metrics::counter!("nimino_nip43_membership_reconciliations_total").increment(reconciled as u64);
    Ok(reconciled)
}
...
pub async fn publish_nip43_membership_list(
    ...
    metrics::counter!("nimino_nip43_membership_publications_total", "result" => "attempted")
        .increment(1);
    let result = publish_nip43_membership_list_inner(tenant, state).await;
    metrics::histogram!("nimino_nip43_membership_publication_seconds")
        .record(started_at.elapsed().as_secs_f64());
    metrics::counter!(
        "nimino_nip43_membership_publications_total",
