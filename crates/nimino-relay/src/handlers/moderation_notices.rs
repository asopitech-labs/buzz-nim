...
//!    participant-hash-idempotent DM model (`nimino-db/src/dm.rs`).
...
use nimino_core::kind::{event_kind_u32, KIND_STREAM_MESSAGE};
...
mod tests {
    ...
    fn tenant() -> TenantContext {
        TenantContext::resolved(
            nimino_core::CommunityId::from_uuid(Uuid::new_v4()),
            "example.org",
        )
    }
