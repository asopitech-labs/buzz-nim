...
use nimino_boundary::{
    DmAction, DmCommand, DmMutationRequest, DmPolicyError, DmPolicyRequest, DmPolicyResult,
};
use nimino_core::kind::*;
use nimino_core::tenant::{CommunityId, TenantContext};
use nimino_datastore_tracing::datastore_span;
use nimino_db::workflow::{ApprovalStatus, RunStatus};
use nimino_db::DbError;
use nimino_workflow::executor::TriggerContext;

use crate::state::AppState;
use crate::webhook_secret;

use super::ingest::{call_dm_policy, extract_channel_id, IngestAuth, IngestError, IngestResult};
...
async fn persist_command_event(
    db: &nimino_db::Db,
    domain: Option<&crate::cluster_runtime::RelayDomainAdapters>,
    tenant: &TenantContext,
    event: &Event,
    ...
    if let Some(domain) = domain {
        crate::canonical_event::commit(domain, tenant.community(), event, channel_id, None, None)
            .await
            .map_err(|e| {
                IngestError::Internal(format!("error: canonical command commit failed: {e}"))
            })?;
    }
    ...
    let d_tag = nimino_db::event::extract_d_tag(event);
...
fn validate_workflow_revision(
    ...
    match (expected_id.as_deref(), existing_id) {
        ...
        (None, Some(_)) => Err(IngestError::Rejected(
            "conflict: expected workflow revision is required for updates".into(),
        )),
...
async fn decide_dm_mutation(
    state: &AppState,
    request: DmMutationRequest,
) -> Result<DmAction, IngestError> {
    let DmPolicyResult::Mutation { action, error } =
        call_dm_policy(state, DmPolicyRequest::Mutation { request }).await?
    else {
        return Err(IngestError::Internal(
            "error: Nim DM policy returned an unexpected result".to_owned(),
        ));
    };
    if error == DmPolicyError::None && action != DmAction::Reject {
        Ok(action)
    } else {
        Err(IngestError::Rejected(format!(
            "invalid: DM policy rejected the request ({error:?})"
        )))
    }
}
...
async fn handle_dm_open(
    ...
    // 2. Decode transport values; participant policy belongs to Nim.
    ...
    let participant_hash = nimino_db::dm::compute_participant_hash(&all_refs);
    let destination = state
        .db
        .find_dm_by_participants(tenant.community(), &participant_hash)
        .await
        .map_err(|error| IngestError::Internal(format!("error: DM lookup failed: {error}")))?;
    let destination_hidden = if let Some(destination) = &destination {
        state
            .db
            .list_hidden_dms(tenant.community(), &self_bytes)
            .await
            .map_err(|error| {
                IngestError::Internal(format!("error: DM visibility lookup failed: {error}"))
            })?
            .contains(&destination.id)
    } else {
        false
    };
    let action = decide_dm_mutation(
        state,
        DmMutationRequest {
            command: DmCommand::Open,
            request_community: *tenant.community().as_uuid(),
            source_community: None,
            destination_community: destination.as_ref().map(|_| *tenant.community().as_uuid()),
            source_exists: false,
            source_is_dm: false,
            actor_is_source_participant: false,
            source_actor_hidden: false,
            actor_included: all_bytes.contains(&self_bytes),
            participant_count: i32::try_from(all_bytes.len()).unwrap_or(i32::MAX),
            new_participant_count: i32::try_from(other_bytes.len()).unwrap_or(i32::MAX),
            destination_exists: destination.is_some(),
            destination_is_dm: destination.is_some(),
            destination_actor_hidden: destination_hidden,
        },
    )
    .await?;
    ...
    let tx =
        match persist_command_event(&state.db, Some(&state.domain), tenant, event, None).await? {
    ...
    // 4. Execute the exact create/reuse/unhide effect selected by Nim.
    if !matches!(
        action,
        DmAction::Create | DmAction::Reuse | DmAction::Unhide
    ) {
        return Err(IngestError::Internal(
            "error: Nim selected an invalid DM open effect".to_owned(),
        ));
    }
...
async fn handle_dm_add_member(
    ...
    let p_tags = extract_p_tags(event);
    let source = match state.db.get_channel(tenant.community(), channel_id).await {
        Ok(channel) => Some(channel),
        Err(DbError::ChannelNotFound(_)) | Err(DbError::NotFound(_)) => None,
        Err(error) => {
            return Err(IngestError::Internal(format!(
                "error: DM lookup failed: {error}"
            )))
        }
    };
    let actor_is_source_participant = if source.is_some() {
        state
            .is_member_cached(tenant.community(), channel_id, &self_bytes)
            .await
            .map_err(|error| {
                IngestError::Internal(format!("error: membership check failed: {error}"))
            })?
    } else {
        false
    };
    let existing_members = if source.is_some() {
        state
            .db
            .get_members(tenant.community(), channel_id)
            .await
            .map_err(|error| {
                IngestError::Internal(format!("error: DM member lookup failed: {error}"))
            })?
    } else {
        Vec::new()
    };
    let mut all_bytes: Vec<Vec<u8>> = existing_members
        .iter()
        .map(|member| member.pubkey.clone())
        .collect();
    let mut new_participant_count = 0i32;
    for hex_str in &p_tags {
        let bytes = decode_pubkey(hex_str)?;
        if !all_bytes.iter().any(|b| b == &bytes) {
            all_bytes.push(bytes);
            new_participant_count = new_participant_count.saturating_add(1);
        }
    }
    let all_refs: Vec<&[u8]> = all_bytes.iter().map(Vec::as_slice).collect();
    let destination = if source.is_some() {
        let hash = nimino_db::dm::compute_participant_hash(&all_refs);
        state
            .db
            .find_dm_by_participants(tenant.community(), &hash)
            .await
            .map_err(|error| {
                IngestError::Internal(format!("error: destination DM lookup failed: {error}"))
            })?
    } else {
        None
    };
    ...
    let action = decide_dm_mutation(
        state,
        DmMutationRequest {
            command: DmCommand::Add,
            request_community: *tenant.community().as_uuid(),
            source_community: source.as_ref().map(|_| *tenant.community().as_uuid()),
            destination_community: destination.as_ref().map(|_| *tenant.community().as_uuid()),
            source_exists: source.is_some(),
            source_is_dm: source
                .as_ref()
                .is_some_and(|channel| channel.channel_type == "dm"),
            actor_is_source_participant,
            source_actor_hidden: hidden.contains(&channel_id),
            actor_included: all_bytes.contains(&self_bytes),
            participant_count: i32::try_from(all_bytes.len()).unwrap_or(i32::MAX),
            new_participant_count,
            destination_exists: destination.is_some(),
            destination_is_dm: destination.is_some(),
            destination_actor_hidden: destination
                .as_ref()
                .is_some_and(|channel| hidden.contains(&channel.id)),
        },
    )
    .await?;
    ...
    // 6. Execute the exact immutable-destination effect selected by Nim.
    if !matches!(
        action,
        DmAction::Create | DmAction::Reuse | DmAction::Unhide
    ) {
        return Err(IngestError::Internal(
            "error: Nim selected an invalid DM add effect".to_owned(),
        ));
    }
...
async fn handle_dm_hide(
    ...
    // 2. Load scoped facts; Nim owns the participant and channel-type decision.
    ...
    let is_member = if channel.is_some() {
        state
            .is_member_cached(tenant.community(), channel_id, &self_bytes)
            .await
            .map_err(|error| {
    ...
    let action = decide_dm_mutation(
        state,
        DmMutationRequest {
            command: DmCommand::Hide,
            request_community: *tenant.community().as_uuid(),
            source_community: channel.as_ref().map(|_| *tenant.community().as_uuid()),
            destination_community: None,
            source_exists: channel.is_some(),
            source_is_dm: channel
                .as_ref()
                .is_some_and(|channel| channel.channel_type == "dm"),
            actor_is_source_participant: is_member,
            source_actor_hidden: hidden,
            actor_included: false,
            participant_count: 0,
            new_participant_count: 0,
            destination_exists: false,
            destination_is_dm: false,
            destination_actor_hidden: false,
        },
    )
    .await?;
    ...
    // 4. Execute only the effect selected by Nim.
    match action {
        DmAction::Hide => state
            .db
            .hide_dm(tenant.community(), channel_id, &self_bytes)
            .await
            .map_err(|error| IngestError::Internal(format!("error: db hide_dm: {error}")))?,
        DmAction::Noop => {}
        _ => {
            return Err(IngestError::Internal(
                "error: Nim selected an invalid DM hide effect".to_owned(),
            ))
        }
    }
...
async fn handle_workflow_def(
    ...
    let (def, definition_json_str) = nimino_workflow::schema::parse_yaml_codec(&event.content)
        .map_err(|e| IngestError::Rejected(format!("invalid: workflow YAML parse error: {e}")))?;
    let requires_elevated = state
        .workflow_engine
        .validate_definition_policy(&def)
        .await
        .map_err(|e| IngestError::Rejected(format!("invalid: workflow definition: {e}")))?;
    ...
    let webhook_secret = if matches!(def.trigger, nimino_workflow::TriggerDef::Webhook) {
...
async fn handle_workflow_trigger(
    ...
    if !workflow.enabled || workflow.status != nimino_db::workflow::WorkflowStatus::Active {
        return Err(IngestError::Rejected(
            "forbidden: workflow is disabled or inactive".into(),
        ));
    }
    let def: nimino_workflow::WorkflowDef = serde_json::from_value(workflow.definition.clone())
    ...
    let tx = match persist_command_event(
        &state.db,
        Some(&state.domain),
        tenant,
        event,
    ...
    tokio::spawn(async move {
        let def: nimino_workflow::WorkflowDef = match serde_json::from_value(def_value) {
            Ok(d) => d,
            Err(e) => {
                ...
                if let Err(db_err) = engine
                    .transition_run(
                        community_id,
                        run_id,
                        "fail:0:invalid_definition",
                        nimino_boundary::WorkflowTransitionCommand::Fail,
                        0,
                        0,
                        &serde_json::json!([]),
        ...
        let result = nimino_workflow::executor::execute_from_step(
            &engine,
            community_id,
            run_id,
            &def,
...
async fn handle_approval_grant(
    ...
    let updated = nimino_db::workflow::update_approval_by_stored_hash_in_transaction(
        &mut tx,
        tenant.community(),
        &token_hash,
...
async fn handle_approval_deny(
    ...
    tokio::spawn(async move {
        ...
        let step_count = match db.get_workflow(community_id, run.workflow_id).await {
            Ok(workflow) => workflow
                ...
                .map_or(0, Vec::len),
            Err(e) => {
                tracing::error!("approval_deny: failed to fetch workflow for run {run_id}: {e}");
                return;
            }
        };
        let Ok(step_index) = usize::try_from(run.current_step) else {
            tracing::error!("approval_deny: run {run_id} has a negative current step");
            return;
        };
        ...
        if let Err(e) = engine
            .transition_run(
                community_id,
                run_id,
                &format!("cancel:{step_index}:approval_denied"),
                nimino_boundary::WorkflowTransitionCommand::Cancel,
                step_count,
                step_index,
                &run.execution_trace,
                Some(nimino_db::workflow::WorkflowRunFailure {
...
async fn resume_workflow_after_approval(
    engine: Arc<nimino_workflow::WorkflowEngine>,
    db: nimino_db::Db,
    ...
    let def: nimino_workflow::WorkflowDef =
        match serde_json::from_value(workflow.definition.clone()) {
            Ok(d) => d,
            Err(e) => {
                tracing::error!("resume_workflow: failed to parse workflow definition: {e}");
                let step_index = usize::try_from(run.current_step).unwrap_or_default();
                let step_count = workflow
                    .definition
                    .get("steps")
                    .and_then(serde_json::Value::as_array)
                    .map_or(step_index, Vec::len);
                if let Err(db_err) = engine
                    .transition_run(
                        community_id,
                        run_id,
                        &format!("fail:{step_index}:invalid_definition"),
                        nimino_boundary::WorkflowTransitionCommand::Fail,
                        step_count,
                        step_index,
                        &run.execution_trace,
...
mod tests {
    ...
    async fn persistence_test_context() -> (nimino_db::Db, TenantContext) {
    ...
    fn workflow_revision_rejects_stale_and_malformed_updates() {
        ...
        assert_eq!(
            ...
            "conflict: expected workflow revision is required for updates",
        );
    ...
    async fn workflow_persistence_preserves_replays_and_rejects_dominated_cas_updates() {
        ...
        let PersistResult::Inserted(tx) = persist_command_event(&db, None, &tenant, &create, None)
            .await
        ...
        assert!(matches!(
            persist_command_event(&db, None, &tenant, &create, None)
                .await
        ...
        let PersistResult::Inserted(tx) = persist_command_event(&db, None, &tenant, &update, None)
            .await
        ...
        assert!(matches!(
            persist_command_event(&db, None, &tenant, &update, None)
                .await
                .expect("replay update"),
            PersistResult::Duplicate
        ));

        let error = match persist_command_event(&db, None, &tenant, &dominated_update, None).await {
            Err(error) => error,
