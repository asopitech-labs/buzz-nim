...
use nimino_core::filter::filters_match;
...
/// live WS subscription, a registry entry plus local delivery topic retain. Bound the
...
/// Remove DM channels rejected by the Nim product policy from a DB-derived
/// accessible-channel set. Non-DM channels are unchanged.
pub(crate) async fn retain_dm_read_access(
    state: &AppState,
    ...
    let facts = state
        .db
        .list_dm_access_facts(community_id, pubkey)
        .await
        .map_err(|error| format!("DM access facts failed: {error}"))?;
    for fact in facts {
        if !accessible_channels.contains(&fact.channel_id) {
            continue;
        }
        // ponytail: one boundary call per accessible DM; batch the contract only
        // if measured communities make this request-latency significant.
        let allowed = matches!(
            super::ingest::call_dm_policy(
                ...
                DmPolicyRequest::Access {
                    request: DmAccessRequest {
                        ...
                        channel_is_dm: true,
                        actor_is_participant: fact.is_member,
        ...
        if !allowed {
            accessible_channels.retain(|channel_id| *channel_id != fact.channel_id);
        }
    }
    Ok(())
}
...
pub async fn handle_req(
    ...
    let mut accessible_channels = if filters_are_nip43_membership_only(&filters) {
        metrics::counter!("nimino_req_global_access_resolution_skips_total", "kind" => "13534")
            .increment(1);
        Vec::new()
    } else {
        match state
    ...
    let trace_state = nimino_core::PublicKey::from_slice(&pubkey_bytes)
        .ok()
    ...
    // cache-negative may be stale until its 10s TTL expires, so on a miss we confirm uncached against
    ...
    if let Err(error) = retain_dm_read_access(
        &state,
    ...
    .await
    {
        warn!(conn_id = %conn_id, %error, "DM access policy failed");
        conn.send(RelayMessage::closed(
            &sub_id,
            "error: DM access policy failed",
        ));
        return;
    }
...
/// the response.
const MAX_SEARCH_PAGES: u32 = (nimino_db::DEFAULT_MAX_PAGE_LIMIT as u32).div_ceil(SEARCH_PAGE_SIZE);
...
/// search scope, and COUNT. A cache-negative can be stale before its TTL
/// expires, so the branch confirms against the DB uncached and passes the
/// result here.
///
...
/// [`nimino_search::ChannelScope`] enum that the Postgres-FTS search layer takes.
///
...
/// 1-to-1 with the table in [`nimino_search::ChannelScope`]'s docs:
///
...
pub(crate) fn build_search_channel_scope_filter(
    ...
    include_global: bool,
) -> Option<nimino_search::ChannelScope> {
    use nimino_search::ChannelScope;
...
async fn handle_search_req(
    ...
    for filter in filters {
        ...
        let limit = filter
            .limit
            .map(|l| (l as u32).min(nimino_db::DEFAULT_MAX_PAGE_LIMIT as u32))
            .unwrap_or(nimino_db::DEFAULT_MAX_PAGE_LIMIT as u32);
        ...
        for page in 1..=MAX_SEARCH_PAGES {
            ...
            let search_query = nimino_search::SearchQuery {
                ...
                mode: nimino_search::SearchMode::FullText,
            };
...
pub fn filter_fully_pushable(filter: &Filter) -> bool {
    ...
    let is_nip33_only = filter.kinds.as_ref().is_some_and(|ks| {
        !ks.is_empty()
            && ks
                .iter()
                .all(|k| nimino_core::kind::is_parameterized_replaceable(k.as_u16() as u32))
    });
    ...
    !filters.is_empty()
        && filters.iter().all(|filter| {
            filter.kinds.as_ref().is_some_and(|kinds| {
                !kinds.is_empty()
                    && kinds.iter().all(|kind| {
                        kind.as_u16() as u32 == nimino_core::kind::KIND_NIP43_MEMBERSHIP_LIST
                    })
            })
        })
}
...
fn filter_to_query_params(
    ...
    let kinds: Option<Vec<i32>> = filter.kinds.as_ref().map(|ks| {
        if ks.is_empty() {
            ...
            // Cast to i32 for Postgres INT column; safe because all Nimino kinds fit in i32.
    ...
    let limit = filter
        .limit
        .map(|l| (l as i64).min(nimino_db::DEFAULT_MAX_PAGE_LIMIT))
        .unwrap_or(nimino_db::DEFAULT_MAX_PAGE_LIMIT);
    ...
    let filter_is_nip33_only = kinds.as_ref().is_some_and(|ks| {
        !ks.is_empty()
            && ks
                .iter()
                .all(|&k| nimino_core::kind::is_parameterized_replaceable(k as u32))
    });
...
/// following hold:
///
/// 1. **Author-only kinds** (`AUTHOR_ONLY_KINDS`, e.g. kind 30300): only
...
pub(crate) async fn event_visible_to_reader(
    ...
    if kind == KIND_DM_VISIBILITY {
        let p = nostr::SingleLetterTag::lowercase(nostr::Alphabet::P);
        let actor_is_viewer = event
            .tags
            .filter(nostr::TagKind::SingleLetter(p))
            .any(|tag| tag.content() == Some(requester_pubkey_hex.as_str()));
        return matches!(
            super::ingest::call_dm_policy(
                state,
                DmPolicyRequest::Access {
                    request: DmAccessRequest {
                        operation: DmAccessOperation::Visibility,
                        request_community: *community_id.as_uuid(),
                        resource_community: Some(*community_id.as_uuid()),
                        resource_exists: true,
                        channel_is_dm: false,
                        actor_is_participant: false,
                        actor_is_viewer,
                    },
                },
            )
    ...
    kind != KIND_AGENT_TURN_METRIC
        || nimino_core::filter::reader_authorized_for_event(event, &requester_pubkey_hex)
}
...
mod tests {
    ...
    fn global_queries_push_access_scope_before_limit() {
        ...
        let mut query = EventQuery::for_community(nimino_core::tenant::CommunityId::from_uuid(
            uuid::Uuid::new_v4(),
        ));
    ...
    fn req_filter_limit_clamps_to_advertised_nip11_max_limit() {
        ...
        let community = nimino_core::tenant::CommunityId::from_uuid(uuid::Uuid::new_v4());
        ...
        assert_eq!(nimino_db::DEFAULT_MAX_PAGE_LIMIT, advertised);
    ...
    fn extract_channel_id_from_multi_value_filter_returns_none() {
        ...
        assert_eq!(
            filter_to_query_params(
                ...
                nimino_core::tenant::CommunityId::from_uuid(uuid::Uuid::nil()),
            )
            .channel_id,
            None,
    ...
    fn dm_visibility_requires_p_tag_even_with_ids() {
        ...
        let dm_vis = nostr::Kind::Custom(nimino_core::kind::KIND_DM_VISIBILITY as u16);
        ...
        let member_notif_ids = Filter::new()
            .kind(nostr::Kind::Custom(
                nimino_core::kind::KIND_MEMBER_ADDED_NOTIFICATION as u16,
            ))
            .id(nostr::EventId::from_hex(snapshot_id).unwrap());
    ...
    fn agent_turn_metric_requires_p_tag_even_with_ids() {
        ...
        let metric_kind = nostr::Kind::Custom(nimino_core::kind::KIND_AGENT_TURN_METRIC as u16);
        ...
        // `reader_authorized_for_event` (nimino-core/src/filter.rs), which gates
    ...
    fn agent_observer_subscription_requires_matching_p_tag() {
        ...
        let missing_p = Filter::new().kind(nostr::Kind::Custom(
            nimino_core::kind::KIND_AGENT_OBSERVER_FRAME as u16,
        ));
    ...
    fn restricted_search_scope_excludes_global_results() {
        ...
        match scope {
            nimino_search::ChannelScope::Channels(ids) => assert_eq!(ids, vec![channel_id]),
    ...
    fn result_gated_explicit_44200_can_match() {
        let f = Filter::new().kind(nostr::Kind::Custom(
            nimino_core::kind::KIND_AGENT_TURN_METRIC as u16,
        ));
        assert!(filter_can_match_result_gated_kinds(&f));
    }
    ...
    fn result_gated_explicit_30622_can_match() {
        let f = Filter::new().kind(nostr::Kind::Custom(
            nimino_core::kind::KIND_DM_VISIBILITY as u16,
        ));
        assert!(filter_can_match_result_gated_kinds(&f));
    }
