...
/// fail closed. This send-time check prevents stale subscriptions from leaking
/// events after an open→private flip.
///
...
pub async fn filter_fanout_by_access(
    ...
    // local fan-out chokepoint shared by all author-only kinds, so no delivery can bypass
    ...
    let matches = if nimino_core::kind::is_shared_gated_kind(event_kind_u32(&stored_event.event)) {
        let author = stored_event.event.pubkey.to_bytes();
        matches
            .into_iter()
            .filter(|(conn_id, _)| {
    ...
    let channel_is_dm = match state.db.get_channel(community_id, channel_id).await {
        Ok(channel) => channel.channel_type == "dm",
        #[cfg(test)]
        Err(_) if state.domain.policy().is_none() => false,
        Err(error) => {
            warn!(%channel_id, "fan-out access filter: channel lookup failed: {error}");
            return Vec::new();
        }
    };
    ...
    match visibility {
        Ok(v) if v != "private" && !channel_is_dm => return matches,
        Ok(_) => {}
        Err(e) => {
    ...
    for (conn_id, sub_id) in matches {
        ...
        if channel_is_dm {
            if matches!(
                super::ingest::call_dm_policy(
                    state,
                    DmPolicyRequest::Access {
                        request: DmAccessRequest {
                            operation: DmAccessOperation::Read,
                            request_community: *community_id.as_uuid(),
                            resource_community: Some(*community_id.as_uuid()),
...
/// open→private flip or a stale cache) cannot leak events.
///
/// Persistent ingest calls `filter_fanout_by_access` inline because it layers
/// an additional per-recipient DM-visibility-owner gate on top.
pub async fn fan_out_event_to_local_subscribers(
    state: &AppState,
    community_id: CommunityId,
    stored: &StoredEvent,
) {
    ...
    metrics::histogram!("nimino_fanout_recipients").record(matches.len() as f64);
    if matches.is_empty() {
        return;
    }
...
/// NIP-01 `OK` means the event was durably accepted, not that local delivery publish,
...
pub(crate) async fn dispatch_persistent_event(
    ...
    metrics::counter!("nimino_post_commit_dispatch_scheduled_total").increment(1);
...
/// Fan out a canonical event projected from another node without triggering
/// the workflow engine a second time.
pub(crate) async fn dispatch_projected_event(
    tenant: &TenantContext,
    state: &Arc<AppState>,
    stored_event: &StoredEvent,
    kind_u32: u32,
) {
    let _ = dispatch_persistent_event_inner(
        tenant,
        state,
        stored_event,
        kind_u32,
...
async fn dispatch_persistent_event_inner(
    ...
    // ingest seam (`crates/nimino-relay/src/handlers/ingest.rs`'s
    ...
    let event_json = match serde_json::to_string(&stored_event.event) {
        ...
        Err(e) => {
            ...
            metrics::counter!("nimino_post_commit_dispatch_errors_total", "stage" => "serialize")
                .increment(1);
            return 0;
        }
    };
    ...
    let mut recipients = Vec::with_capacity(matches.len());
    for (target_conn_id, sub_id) in &matches {
        let Some(reader) = state.conn_manager.pubkey_for_conn(*target_conn_id) else {
            continue;
        };
        ...
        recipients.push((*target_conn_id, sub_id.as_str()));
    }
    ...
    let is_relay_workflow_msg = stored_event.event.pubkey == state.relay_keypair.public_key()
        && stored_event
            ...
            .tags
            .iter()
            .any(|t| t.as_slice().first().map(|s| s.as_str()) == Some("nimino:workflow"));

    if trigger_workflows
        && !nimino_core::kind::is_workflow_execution_kind(kind_u32)
        && !nimino_core::kind::is_command_kind(kind_u32)
...
async fn enqueue_event_created_audit(
    ...
    let audit_entry = nimino_audit::NewAuditEntry {
        community_id: tenant.community(),
        action: nimino_audit::AuditAction::EventCreated,
    ...
    if let Err(e) = audit_tx.send(audit_entry).await {
        ...
        metrics::counter!("nimino_audit_send_errors_total").increment(1);
    }
}
...
pub async fn handle_event(event: Event, conn: Arc<ConnectionState>, state: Arc<AppState>) {
    ...
    metrics::counter!("nimino_events_received_total", "kind" => kind_str).increment(1);
    ...
    metrics::counter!(
        "nimino_community_events_received_total",
    ...
    if kind_u32 == KIND_AGENT_OBSERVER_FRAME {
        if !scopes.is_empty() && !scopes.contains(&nimino_auth::Scope::MessagesWrite) {
            reject("scope");
            conn.send(RelayMessage::ok(
                &event_id_hex,
                false,
    ...
    let classification = super::ingest::classify_event(&state, &event).await;
    if matches!(
        classification,
        Ok(nimino_boundary::EventDisposition::Ephemeral)
    ) {
        if !scopes.is_empty() && !scopes.contains(&nimino_auth::Scope::MessagesWrite) {
            reject("scope");
            conn.send(RelayMessage::ok(
        ...
        match nimino_deletion::store(&state.db)
        ...
        match handle_ephemeral_event(event, pubkey_bytes, auth_pubkey, Arc::clone(&conn), state)
            .await
        {
            Ok(()) => {
    ...
    match super::ingest::ingest_classified_event(
        &state,
        &conn.tenant,
        event,
        ingest_auth,
    ...
    .await
    {
        Ok(result) => {
            if result.accepted {
                // nimino_events_stored_total is emitted inside ingest_event()
            ...
            metrics::histogram!("nimino_event_processing_seconds")
...
mod tests {
    ...
    #[test]
    fn fanout_event_frame_matches_nostr_format_byte_for_byte() {
        let sub_id = "sub-id";
    ...
    async fn observer_frame_rate_limiter_is_scoped_by_community() {
        ...
        let community_a =
            nimino_core::tenant::CommunityId::from_uuid(uuid::Uuid::from_u128(0xAAAA));
        let community_b =
            nimino_core::tenant::CommunityId::from_uuid(uuid::Uuid::from_u128(0xBBBB));
    ...
    async fn observer_owner_cache_is_scoped_to_community() {
        ...
        let community_a = nimino_core::CommunityId::from_uuid(Uuid::new_v4());
        let community_b = nimino_core::CommunityId::from_uuid(Uuid::new_v4());
    ...
    mod local_fanout {
        ...
        use nimino_core::kind::{KIND_MEMBER_ADDED_NOTIFICATION, KIND_PRESENCE_UPDATE};
        use nostr::{EventBuilder, Filter, Keys, Kind};
        use tokio::sync::{mpsc, Mutex};
        use tokio_util::sync::CancellationToken;
        use uuid::Uuid;

        use crate::handlers::event::fan_out_event_to_local_subscribers;
        use crate::state::AppState;
        ...
        #[tokio::test]
        async fn global_presence_event_fans_out_to_local_subscribers() {
            ...
            fan_out_event_to_local_subscribers(
                &state,
                nimino_core::tenant::CommunityId::from_uuid(Uuid::nil()),
                &nimino_core::StoredEvent::new(event, None),
            )
            .await;
        ...
        #[tokio::test]
        async fn global_membership_event_fans_out_by_p_tag() {
            let state = test_state().await;
        ...
        async fn audit_records_caller_actor_not_relay_signer_for_relay_signed_event() {
            ...
            let Some((state, audit_shutdown, pool)) = super::fanout_access::audit_state().await
                ...
                eprintln!(
                    "skipping audit-actor provenance test: Postgres/local delivery unavailable"
                );
                return;
            };
            ...
            // Relay-signed event tagged nimino:workflow so workflow triggering is
            ...
            let event = EventBuilder::new(Kind::from(KIND_PRESENCE_UPDATE as u16), "online")
                .tags([nostr::Tag::parse(["nimino:workflow", "true"]).expect("workflow tag")])
        ...
        /// call that `nimino_audit::service::tests::chains_are_independent_per_community`
        ...
        async fn audit_chain_is_isolated_per_tenant_through_relay_ingest() {
            ...
            let Some((state, audit_shutdown, pool)) = super::fanout_access::audit_state().await
                ...
                eprintln!("skipping audit isolation test: Postgres/local delivery unavailable");
                return;
            };
    ...
    mod fanout_access {
        ...
        pub(super) async fn test_state() -> Arc<AppState> {
            ...
            let media_storage =
                nimino_media::MediaStorage::new(&config.media).expect("media storage");
        ...
        /// Real-PG state that hands back the audit shutdown handle so
        /// a test can drain queued audit entries before asserting on `audit_log`.
        /// `None` when Postgres is unavailable (test skips).
        ...
        async fn open_channel_event_passes_through_unfiltered() {
            ...
            let community_id = nimino_core::tenant::CommunityId::from_uuid(Uuid::nil());
            state
        ...
        async fn author_only_reminder_delivers_to_author_only() {
            ...
            let reminder = EventBuilder::new(
                Kind::Custom(nimino_core::kind::KIND_EVENT_REMINDER as u16),
                "{}",
            )
    ...
    // Cross-community fan-out regression.
    mod redteam {
        use std::collections::HashMap;
        use std::sync::atomic::AtomicU8;
        use std::sync::Arc;

        use nimino_core::kind::KIND_PRESENCE_UPDATE;
        use nimino_core::StoredEvent;
        use nostr::{EventBuilder, Keys, Kind};
        use tokio::sync::{mpsc, Mutex};
        use tokio_util::sync::CancellationToken;
        ...
        fn register_conn(
            state: &AppState,
            community_id: nimino_core::tenant::CommunityId,
            pubkey: Option<Vec<u8>>,
        ) -> Uuid {
            let conn_id = Uuid::new_v4();
            let (tx, _rx) = mpsc::channel(1);
        ...
        /// A connection bound to A must not receive a B-labelled global event.
        #[tokio::test]
