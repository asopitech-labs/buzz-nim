...
use nimino_boundary::{
    ModerationEffect, ModerationPolicyError, ModerationPolicyRequest, ModerationPolicyResult,
    ModerationReportRequest, ModerationReportTarget, ModerationReportType,
};
use nimino_core::tenant::TenantContext;
use nimino_db::moderation::{NewReport, ReportTarget};
use nostr::Event;
...
pub async fn handle_report_event(
    ...
    let (target, channel_id, target_kind, target_pubkey) = match parsed.target {
        ParsedReportTarget::Event { event_id, .. } => {
            ...
            (
                ...
                stored.channel_id,
                ModerationReportTarget::Event,
                stored.event.pubkey.to_bytes().to_vec(),
            )
        }
        ParsedReportTarget::Blob {
            ...
            (
                ReportTarget::Blob(sha256),
                None,
                ModerationReportTarget::Blob,
                author_pubkey,
            )
        }
        ParsedReportTarget::Pubkey { pubkey } => (
            ReportTarget::Pubkey(pubkey.clone()),
            None,
            ModerationReportTarget::Pubkey,
            pubkey,
        ),
    };
    ...
    let duplicate = state
        .db
        .get_moderation_report_by_event(tenant.community(), event.id.as_bytes())
        .await
        .map_err(|error| format!("error: database error checking report: {error}"))?
        .is_some();
    let report_type = match parsed.report_type {
        "illegal" => ModerationReportType::Illegal,
        "nudity" => ModerationReportType::Nudity,
        "malware" => ModerationReportType::Malware,
        "spam" => ModerationReportType::Spam,
        "impersonation" => ModerationReportType::Impersonation,
        "profanity" => ModerationReportType::Profanity,
        "other" => ModerationReportType::Other,
        _ => return Err("invalid: unsupported report type".to_owned()),
    };
    let decision = super::ingest::call_moderation_policy(
        state,
        ModerationPolicyRequest::Report {
            request: ModerationReportRequest {
                request_community: *tenant.community().as_uuid(),
                target_community: Some(*tenant.community().as_uuid()),
                target_exists: true,
                reporter_is_target: reporter_pubkey.as_slice() == target_pubkey,
                duplicate,
                target_kind,
                report_type,
            },
    ...
    match decision {
        ModerationPolicyResult::Report {
            effect: ModerationEffect::QueueReport,
            error: ModerationPolicyError::None,
            ..
        } => {}
        ModerationPolicyResult::Report { error, .. } => {
            return Err(format!(
                "invalid: moderation policy rejected the report ({error:?})"
            ));
        }
        _ => {
            return Err("error: Nim moderation policy returned an unexpected result".to_owned());
        }
    }
...
mod tests {
    use super::*;
    use nimino_core::kind::KIND_REPORT;
