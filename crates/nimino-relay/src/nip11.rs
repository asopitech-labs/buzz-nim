...
///
/// `max_limit` is [`nimino_db::DEFAULT_MAX_PAGE_LIMIT`], the same constant the
...
fn relay_limitation(max_message_length: usize) -> RelayLimitation {
    ...
    RelayLimitation {
        ...
        max_filters: Some(10),
        max_limit: Some(nimino_db::DEFAULT_MAX_PAGE_LIMIT as u32),
...
impl RelayInfo {
    ...
    /// kinds 39000/39001/39002, which Nimino signs with `state.relay_keypair`
    ...
    pub fn build(
        ...
        Self {
            name: "Nimino Relay".to_string(),
            description: "Nimino — private team communication relay".to_string(),
            icon: icon.filter(|s| !s.is_empty()).map(|s| s.to_string()),
            pubkey: None,
            contact: None,
            supported_nips,
            supported_extensions: Some(vec!["nip-er".to_string()]),
            software: "https://github.com/asopitech-labs/nimino".to_string(),
...
mod tests {
    ...
    #[test]
    fn build_advertises_nimino_repository_url() {
        let info = RelayInfo::build(None, None, false, DEFAULT_MAX_FRAME_BYTES, None);
        assert_eq!(info.software, "https://github.com/asopitech-labs/nimino");
    }
    ...
    fn auth_required_is_advertised_true() {
        ...
        // `AuthState::Authenticated` (see `crates/nimino-relay/src/handlers/`),
