//! Smart HTTP git transport for Nimino.
//!
...
impl axum::extract::FromRequestParts<Arc<AppState>> for GitAuth {
    ...
    async fn from_request_parts(
        ...
        let auth_header = parts
            ...
            .ok_or_else(|| {
                Response::builder()
                    ...
                    .header(
                        "WWW-Authenticate",
                        format!("Nostr realm=\"nimino\", method=\"{method}\""),
                    )
        ...
        let pubkey =
            nimino_auth::nip98::verify_nip98_event(&event_json, &expected_url, &event_method, None)
                .map_err(|e| {
...
async fn git_restriction_state(
    ...
    community: nimino_core::CommunityId,
    pubkey: &nostr::PublicKey,
) -> Result<nimino_db::moderation::RestrictionState, Response> {
...
fn enforce_git_ban(
    restriction: &nimino_db::moderation::RestrictionState,
) -> Result<(), StatusCode> {
...
fn enforce_git_ban_cascade(
    agent: &nimino_db::moderation::RestrictionState,
    owner: Option<&nimino_db::moderation::RestrictionState>,
) -> Result<(), StatusCode> {
...
///    resolve;
/// 2. its `nimino-channel` tag → channel UUID;
/// 3. [`nimino_db::Db::get_member_role`] for the caller — a read is allowed
///    only on `Ok(Some(role))` with a role the relay recognizes.
///
/// Fail-closed: missing/deleted announcement, invalid owner, missing or
/// malformed `nimino-channel` binding, non-member, unknown role, and every DB
...
/// `nimino-channel` tag, and the repo then 404s forever with no explanation
...
async fn authorize_git_read(
    ...
    let query = nimino_db::EventQuery {
    ...
    let channel_id = match resolve_repo_binding(&repo_event.event) {
        ...
        RepoBinding::NotBound => {
            ...
            warn!(repo = %repo_name, "git read gate: missing nimino-channel binding (deny)");
            return Err(denied());
        }
        RepoBinding::Broken => {
            warn!(repo = %repo_name, "git read gate: malformed nimino-channel binding (deny)");
            return Err(denied());
        }
    };
...
fn read_role_allows(role: Option<&str>) -> bool {
    match role {
        Some(r) => r.parse::<nimino_core::channel::MemberRole>().is_ok(),
        None => false,
    }
}
...
/// 0000
/// <pkt><head-oid> HEAD\0<caps> symref=HEAD:<head-ref> object-format=<fmt> agent=nimino-git\n
...
async fn finalize_push_inner(
    ...
    let serving_write = match nimino_deletion::acquire_serving_write(
        &state.db,
...
mod track_c_tests {
    ...
    fn receive_pack_config_allows_deleting_current_branch() {
        ...
        assert_git_success(
            run_test_git(
                source.as_path(),
                &["config", "user.email", "nimino-test@example.com"],
                &[],
            ),
    ...
    async fn approved_deletion(
        ...
        nimino_db::deletion::DeletionRequest,
        nimino_db::deletion::ClaimedDeletion,
    ) {
        ...
        let inventory = FrozenInventory {
            ...
            storage: StorageManifest {
                version: 4,
                prefixes: nimino_media::tenant_prefixes(*request.community_id.as_uuid())
                    .into_iter()
    ...
    async fn finalize_push_holds_serving_lease_through_post_cas_publication() {
        ...
        assert!(matches!(
            error,
            nimino_db::DbError::ServingWritesNotDrained { .. }
        ));
        assert!(!state
            .db
            .deletion_store()
        ...
        let mut query = nimino_db::event::EventQuery::for_community(community);
        ...
        assert_eq!(
            state
                ...
                .stage,
            nimino_db::deletion::DeletionStage::Fenced
        );
        drop(state);
        pool.close().await;
    }
    ...
    fn git_nip98_rejects_token_signed_for_wrong_community_host() {
        ...
        let err = nimino_auth::nip98::verify_nip98_event(&event_json, &expected_for_b, "GET", None)
    ...
    fn git_nip98_accepts_token_signed_for_matching_community_host() {
        ...
        let pubkey =
            nimino_auth::nip98::verify_nip98_event(&event_json, &expected_for_a, "GET", None)
...
mod sec005_read_gate_tests {
    ...
    fn durable_ban_denies_git_even_with_otherwise_valid_auth() {
        let restriction = nimino_db::moderation::RestrictionState {
            banned: true,
            muted_until: None,
        };
    ...
    fn restriction(banned: bool) -> nimino_db::moderation::RestrictionState {
        nimino_db::moderation::RestrictionState {
            banned,
            muted_until: None,
        }
    }
    ...
    async fn setup_db() -> nimino_db::Db {
        ...
        nimino_db::Db::from_pool(pool)
    }

    /// How the fixture's kind:30617 binds (or fails to bind) a channel.
    enum Binding {
        /// `nimino-channel` tag carrying the fixture channel's UUID.
        Channel,
        /// No `nimino-channel` tag at all.
        Missing,
        /// `nimino-channel` tag whose value is not a UUID.
        Malformed,
        /// `nimino-channel` tag carrying a well-formed UUID that names no
    ...
    async fn setup_repo(binding: Binding) -> RepoFixture {
        ...
        db.add_member(
            ...
            channel,
            &member_pk,
            nimino_core::channel::MemberRole::Member,
            Some(&creator_pk),
        )
        .await
        .expect("member");
        ...
        match binding {
            Binding::Channel => {
                tags.push(Tag::parse(["nimino-channel", &channel.to_string()]).unwrap());
            }
            Binding::Missing => {}
            Binding::Malformed => {
                tags.push(Tag::parse(["nimino-channel", "not-a-uuid"]).unwrap());
            }
            Binding::UnknownChannel => {
                tags.push(
                    Tag::parse(["nimino-channel", &uuid::Uuid::new_v4().to_string()]).unwrap(),
                );
            }
        }
    ...
    async fn read_gate_denies_missing_or_malformed_binding_and_absent_repo() {
        // Missing nimino-channel tag → deny even for a channel member, with
        ...
        // Malformed nimino-channel tag → deny with the generic body EVEN FOR
    ...
    async fn setup_ban_community() -> (nimino_db::Db, nimino_core::CommunityId, Vec<u8>) {
        let db = setup_db().await;
    ...
    async fn ban_gate_cascades_to_a_banned_nip_oa_owner() {
        ...
        let auth_tag = nimino_sdk::nip_oa::compute_auth_tag(&owner, &agent.public_key(), "kind=9")
            .expect("auth tag");
    ...
    async fn ban_gate_fails_closed_with_503_when_the_store_is_unreachable() {
        ...
        let community = nimino_core::CommunityId::from_uuid(uuid::Uuid::new_v4());
        let (status, body) = denial_parts(
