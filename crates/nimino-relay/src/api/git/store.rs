...
//! ## The 412 sharp edge
//!
//! `rust-s3 = "0.37"` is shared across the workspace with `nimino-media`. The
...
impl GitStore {
    ...
    ///
    /// Credential selection mirrors [`nimino_media::MediaStorage::new`]:
    ...
    pub fn new(
        ...
        bucket_name: &str,
        region: &str,
        addressing_style: nimino_media::config::S3AddressingStyle,
        ...
        let bucket = match addressing_style {
            nimino_media::config::S3AddressingStyle::Path => bucket.with_path_style(),
            nimino_media::config::S3AddressingStyle::Virtual => bucket,
        };
        Ok(Self {
...
mod tests {
    ...
    fn constructor_applies_both_addressing_styles() {
        for (style, expected_url, path_style) in [
            (
                nimino_media::config::S3AddressingStyle::Path,
                "https://storage.example/nimino-git",
                true,
            ),
            (
                nimino_media::config::S3AddressingStyle::Virtual,
                "https://nimino-git.storage.example",
                false,
            ),
        ] {
            let store = GitStore::new(
...
mod probe {
    ...
    //!
    //! Run manually:
    //!   NIMINO_GIT_S3_PROBE=1 cargo test -p nimino-relay --lib \
    //!     api::git::store::probe -- --nocapture --test-threads=1
    //!
    //! Pre-req: `docker compose up minio` and the `nimino-git` bucket exists.
    ...
    fn store() -> GitStore {
        ...
        let bucket = std::env::var("NIMINO_S3_BUCKET").unwrap_or_else(|_| "nimino-git".into());
