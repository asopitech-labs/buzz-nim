...
use base64::Engine;
use nimino_audit::{AuditAction, NewAuditEntry};
use nimino_core::tenant::TenantContext;
use nimino_media::{BlobDescriptor, MediaError, UploadAttribution, UploadNetworkInfo};
...
fn should_stream_as_video(sniff: &[u8]) -> bool {
    infer::get(sniff).is_some_and(|kind| kind.mime_type() == "video/mp4")
        || nimino_media::looks_like_iso_bmff(sniff)
}
...
impl FromRequestParts<Arc<AppState>> for AuthenticatedUpload {
    ...
    async fn from_request_parts(
        ...
        nimino_media::auth::verify_blossom_auth_event(&auth_event, Some(tenant.host()), 3600)?;
        ...
        if upload_rate_limited(state, tenant.community(), &auth_event.pubkey) {
            metrics::counter!("nimino_media_upload_rejections_total", "reason" => "rate_limit")
                .increment(1);
        ...
        let upload_permit = acquire_upload_permit(state, tenant.community(), &auth_event.pubkey)
            .inspect_err(|_| {
                metrics::counter!("nimino_media_upload_rejections_total", "reason" => "concurrency")
                    .increment(1);
            })?;
...
/// (`NIMINO_MEDIA_UPLOAD_RECORDS`). Returns `None` when the feature is off —
...
///   (`NIMINO_MEDIA_UPLOAD_IP_HEADER`) and validated as a public IP —
...
async fn upload_attribution(
    ...
    let ip = header_value(&cfg.upload_ip_header).and_then(nimino_media::parse_public_ip);
    let port = ip.and(header_value(&cfg.upload_port_header).and_then(nimino_media::parse_port));
...
fn serving_write_error(error: anyhow::Error) -> MediaError {
    if nimino_deletion::ServingWriteGuard::acquisition_is_fenced(&error) {
...
pub async fn upload_blob(
    ...
    let serving_write =
        nimino_deletion::acquire_serving_write(&state.db, auth.tenant.community(), "media_upload")
            .await
    ...
    let mut descriptor = serving_write
        .protect(async {
            Ok::<_, MediaError>(if should_stream_as_video(&sniff) {
                // Video path: stream body directly to disk — never fully buffered in RAM.
                let content_length = headers
                    .get("content-length")
                    .and_then(|v| v.to_str().ok())
                    .and_then(|v| v.parse::<u64>().ok());
                nimino_media::process_video_upload(
                    &state.media_storage,
                    &state.config.media,
                    &auth.tenant,
                    &auth.auth_event,
                ...
                if is_image {
                    ...
                    nimino_media::process_file_upload(
                        &state.media_storage,
                        &state.config.media,
                        &auth.tenant,
                        &auth.auth_event,
        ...
        .await
        .map_err(|error| {
            if nimino_deletion::ServingWriteGuard::is_lease_lost(&error) {
...
async fn authenticate_media_read(
    ...
    nimino_media::auth::verify_blossom_get_auth(&auth_event, sha256, Some(tenant.host()), 3600)?;
...
pub(crate) async fn serve_blob_for_tenant(
    ...
    let disposition = if nimino_media::serve_inline(&content_type) {
        "inline"
    } else {
        "attachment"
    };
...
async fn resolve_s3_key(
    storage: &nimino_media::MediaStorage,
    tenant: &TenantContext,
    sha256_ext: &str,
...
mod tests {
    ...
    fn serving_write_error_taxonomy_separates_fence_from_backend_failure() {
        let fenced = anyhow::Error::from(nimino_db::DbError::AccessDenied("fenced".to_string()));
        assert!(matches!(
            serving_write_error(fenced),
            MediaError::CommunityWriteFenced
        ));
        let backend = anyhow::Error::from(nimino_db::DbError::Sqlx(sqlx::Error::PoolTimedOut));
        assert!(matches!(
