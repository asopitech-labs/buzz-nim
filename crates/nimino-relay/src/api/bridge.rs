...
use nimino_auth::{LimitType, Nip98ReplayGuard, DEFAULT_REPLAY_TTL_SECS};
...
pub(crate) fn verify_bridge_auth_with_options(
        ...
        .and_then(|s| s.strip_prefix("Nostr "))
    {
        ...
        let pubkey = nimino_auth::verify_nip98_event(&event_json, url, method, body)
...
///
/// The correctness boundary is the shared, community-scoped local delivery seen-set on
/// `AppState`, not process-local memory. Any local delivery/guard error fails closed:
...
fn extract_search_mode(raw: &Value) -> nimino_search::SearchMode {
    match raw
        .get("search_mode")
        .or_else(|| raw.get("searchMode"))
        .and_then(Value::as_str)
    {
        Some("prefix") => nimino_search::SearchMode::Prefix,
        _ => nimino_search::SearchMode::FullText,
    }
}
...
const WINDOW_AUX_KINDS: [u32; 4] = [
    nimino_core::kind::KIND_DELETION,
    nimino_core::kind::KIND_REACTION,
    nimino_core::kind::KIND_NIP29_DELETE_EVENT,
    nimino_core::kind::KIND_STREAM_MESSAGE_EDIT,
];
...
async fn handle_channel_window_filter(
    state: &AppState,
    tenant: &nimino_core::TenantContext,
    raw: &Value,
    filter: &nostr::Filter,
    accessible_channels: &[uuid::Uuid],
    events: &mut Vec<Value>,
) -> Result<(), (StatusCode, Json<Value>)> {
    use nimino_core::kind::{KIND_THREAD_SUMMARY, KIND_WINDOW_BOUNDS};
    ...
    if extension_flag(raw, "include_aux") && !row_ids_hex.is_empty() {
        ...
        for hop_kinds in [&WINDOW_AUX_KINDS[..], &WINDOW_AUX_DELETE_KINDS[..]] {
            let mut aux_query = nimino_db::EventQuery::for_community(tenant.community());
...
fn event_in_accessible_channel(se: &nimino_core::StoredEvent, accessible: &[uuid::Uuid]) -> bool {
    match se.channel_id {
...
async fn submit_event_authed(
    ...
    let kind_u32 = nimino_core::kind::event_kind_u32(&event);
    let auth = IngestAuth::Http {
        pubkey,
        scopes: nimino_auth::Scope::all_known(), // Pure Nostr: full scopes, channel access via membership
...
async fn query_events_authed(
    ...
    .await?;
    crate::handlers::req::retain_dm_read_access(
        state,
        tenant.community(),
        &pubkey_bytes,
        &mut accessible_channels,
    ...
    for (idx, (raw, filter)) in raw_filters.iter().zip(filters.iter()).enumerate() {
        ...
        for feed_type in &feed_types {
            ...
            for se in type_events {
                ...
                if !nimino_core::filter::reader_authorized_for_event(&se.event, &authed_pubkey_hex)
                {
                    continue;
                }
    ...
    let mut catchall_queries: Vec<(usize, nimino_db::EventQuery)> = Vec::new();
    for (idx, (raw, filter)) in raw_filters.iter().zip(filters.iter()).enumerate() {
        if handled.contains(&idx) {
            continue;
        }
        ...
        match filter_events {
            Ok(stored_events) => {
                for se in stored_events {
                    ...
                    if !nimino_core::filter::filters_match(std::slice::from_ref(filter), &se) {
                        continue;
                    }
                ...
                && !needs_shared_gate_filtering
            {
                    ...
                    .await
                {
                    Ok(stored_events) => {
                        ...
                        for se in stored_events {
                            if !nimino_core::filter::filters_match(
                                std::slice::from_ref(filter),
                                &se,
                            ) {
                                continue;
...
fn search_hit_accepted(
    filter: &nostr::Filter,
    stored: &nimino_core::StoredEvent,
    accessible_channels: &[uuid::Uuid],
    reader_pubkey_hex: &str,
) -> bool {
    if !nimino_core::filter::filters_match(std::slice::from_ref(filter), stored) {
        return false;
    }
    ...
    if !nimino_core::filter::reader_authorized_for_event(&stored.event, reader_pubkey_hex) {
        return false;
    }
    true
}
...
async fn handle_bridge_search(
    ...
    filters: &[nostr::Filter],
    accessible_channels: &[uuid::Uuid],
    tenant: &nimino_core::tenant::TenantContext,
    reader_pubkey_hex: &str,
    pubkey_bytes: &[u8],
) -> Result<Json<Value>, (StatusCode, Json<Value>)> {
    ...
    for (raw, filter) in raw_filters.iter().zip(filters) {
        ...
        let filter_channel_scope =
            if let Some(vs) = filter.generic_tags.get(&h_tag).filter(|vs| !vs.is_empty()) {
                ...
                nimino_search::ChannelScope::Channels(valid)
            } else {
                channel_scope.clone()
            };
        ...
        let search_query = nimino_search::SearchQuery {
            community: tenant.community(),
            q: search_text,
        ...
        let event_map: std::collections::HashMap<[u8; 32], &nimino_core::StoredEvent> =
            stored_events
                .iter()
...
pub async fn workflow_webhook(
    ...
    if !matches!(def.trigger, nimino_workflow::TriggerDef::Webhook) {
        return Err(api_error(
            StatusCode::BAD_REQUEST,
    ...
    let mut trigger_ctx = nimino_workflow::executor::TriggerContext {
        channel_id: workflow
            .channel_id
            .map(|ch| ch.to_string())
            .unwrap_or_default(),
...
/// presence from local delivery instead of querying the DB (ephemeral events are never
...
async fn synthesize_presence(
    ...
    use nimino_core::kind::{KIND_PRESENCE_SNAPSHOT, KIND_PRESENCE_UPDATE};
...
fn report_json(r: &nimino_db::moderation::ReportRecord) -> Value {
    let (target_kind, target) = match &r.target {
        nimino_db::moderation::ReportTarget::Event(id) => ("event", hex::encode(id)),
        nimino_db::moderation::ReportTarget::Pubkey(pk) => ("pubkey", hex::encode(pk)),
        nimino_db::moderation::ReportTarget::Blob(sha) => ("blob", hex::encode(sha)),
    };
    serde_json::json!({
        "id": r.id,
...
fn action_json(a: &nimino_db::moderation::ActionRecord) -> Value {
    serde_json::json!({
        "id": a.id,
...
fn ban_json(b: &nimino_db::moderation::BanRecord) -> Value {
    serde_json::json!({
...
mod tests {
    ...
    fn fresh_tenant(host: &str) -> TenantContext {
        TenantContext::resolved(
            nimino_core::CommunityId::from_uuid(uuid::Uuid::new_v4()),
            host,
        )
    }
    ...
    fn bridge_search_mode_extension_defaults_to_full_text() {
        assert_eq!(
            ...
            nimino_search::SearchMode::FullText
        );
        assert_eq!(
    ...
    fn bridge_search_mode_extension_accepts_prefix_snake_or_camel_case() {
        assert_eq!(
            ...
            nimino_search::SearchMode::Prefix
        );
        assert_eq!(
    ...
    /// A single node-local guard, called twice with the same
    /// `TenantContext` and the same event id, MUST reject the second call.
    #[tokio::test]
    async fn nip98_replay_guard_rejects_same_node_same_community_replay() {
        let guard = nimino_local_delivery::LocalReplayGuard::new();
        let tenant = fresh_tenant("relay-a.example");
        let event_id_bytes = fresh_nip98_event_id_bytes();

        check_nip98_replay_with_guard(&guard, &tenant, event_id_bytes)
            .await
            .expect("first claim on a fresh event id must succeed");

        let (status, _) = check_nip98_replay_with_guard(&guard, &tenant, event_id_bytes)
            .await
            .expect_err("same-node replay of the same id+community must reject");
        assert_eq!(status, StatusCode::UNAUTHORIZED);
    }

    /// A replay-guard failure must reject the request, never admit it. The seen-set is the
    /// freshness fence; degrading to "best effort, allow on error" forfeits
    /// the proof (per the `Nip98ReplayGuard` trait contract,
    /// `nimino-auth/src/nip98_replay.rs:70-73`).
    ///
    /// This test does not require local delivery — it injects a guard that always
    ...
    async fn nip98_replay_check_fails_closed_when_guard_errors() {
        ...
        impl Nip98ReplayGuard for AlwaysErrGuard {
            fn try_mark_in_scope<'a>(
                ...
                Box::pin(async {
                    Err(AuthError::Internal("simulated replay guard failure".into()))
                })
            }
        }
    ...
    /// `nimino_auth::nip98::tests::make_nip98_event` so the bridge tests don't
    /// reach into nimino-auth's test scope.
    ...
    /// `nimino moderation reports` / `audit` 401'd on a NIP-98 URL mismatch in
    ...
    fn verify_nip42_rejects_event_signed_for_wrong_communitys_host() {
        ...
        assert!(
            matches!(err, nimino_auth::AuthError::RelayUrlMismatch),
    ...
    fn engram_envelope(agent: &Keys, owner_hex: &str) -> nimino_core::StoredEvent {
        ...
        let ev = EventBuilder::new(Kind::Custom(30174), "engram body")
            ...
            .expect("sign engram");
        nimino_core::StoredEvent::new(ev, None)
    }
    ...
    fn extract_page_offset_sizes_pages_from_clamped_limit() {
        let clamped = nimino_db::DEFAULT_MAX_PAGE_LIMIT;
    ...
    fn event_accessible_no_channel() {
        ...
        let ev = EventBuilder::new(Kind::Custom(1), "test")
            .sign_with_keys(&keys)
            .unwrap();
        let se = nimino_core::StoredEvent::new(ev, None);
    ...
    fn event_accessible_matching_channel() {
        ...
        let mut se = nimino_core::StoredEvent::new(ev, None);
    ...
    fn search_hit_rejects_dm_visibility_for_kindless_ids_third_party() {
        ...
        let ev = EventBuilder::new(
            Kind::Custom(nimino_core::kind::KIND_DM_VISIBILITY as u16),
            "",
        )
        .tags([d_tag, p_tag])
        .sign_with_keys(&relay)
        .expect("sign snapshot");
        let stored = nimino_core::StoredEvent::new(ev.clone(), None);
    ...
    // increments nimino_events_rejected_total{transport="http"}.  They are
    ...
    // (needs a real communities row) and enforce_http_admission (needs local delivery).
    // Both services run locally in dev.  The admission check succeeds for a
    // freshly generated pubkey (first request, far below quota), so no local delivery
    ...
    /// - `nip98_replay` replaced with an always-fresh guard → no local delivery needed
    ///   for replay detection.
    /// - admission uses the process-local limiter.
    ///
    ...
    /// Collect nimino_events_rejected_total with (transport, reason) labels from
    ...
    /// increment nimino_events_rejected_total{transport="http",reason="invalid"}.
    ///
    ...
    fn submit_event_relay_only_kind_increments_http_transport_counter() {
        ...
        let relay_only_event = EventBuilder::new(
            Kind::Custom(nimino_core::kind::KIND_NIP43_MEMBERSHIP_LIST as u16),
            "",
        )
