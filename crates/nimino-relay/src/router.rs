...
/// Readiness probe — checks shutdown state and durable-store catalogs.
async fn readiness_handler(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    if !state.cluster_ready.load(Ordering::Relaxed) {
        return (
            StatusCode::SERVICE_UNAVAILABLE,
            Json(json!({"status": "not_ready", "cluster_sync": false})),
        )
            .into_response();
    }

    let check = async {
        let (pg_ok, deletion_catalog_ok) = tokio::join!(state.db.ping(), async {
            state.db.validate_deletion_serving_catalog().await.is_ok()
        },);
        (pg_ok, deletion_catalog_ok)
    };

    let (pg_ok, deletion_catalog_ok) = tokio::time::timeout(Duration::from_secs(2), check)
        .await
        .unwrap_or((false, false));

    if pg_ok && deletion_catalog_ok {
...
async fn status_handler(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    Json(json!({
        ...
        "cluster_sync_ready": state.cluster_ready.load(Ordering::Relaxed),
    }))
}
...
mod tests {
    ...
    async fn http_and_datastore_spans_are_exported_in_the_same_trace() {
        let _subscriber_lock = crate::telemetry::TEST_SUBSCRIBER_LOCK
            .lock()
            .expect("subscriber test lock");
