...
use nimino_core::{filter::filters_match, CommunityId, StoredEvent};
...
pub struct SubscriptionRegistry {
    ...
    /// gives lifecycle code the exact local delivery topic to release.
...
impl SubscriptionRegistry {
    ...
    ///
    /// Used by the usage poller to emit `nimino_community_subscriptions{community}`.
...
mod tests {
    ...
    fn test_global_p_kind_index_fan_out_targets_matching_p() {
        ...
        let conn_a = Uuid::new_v4();
        let conn_b = Uuid::new_v4();
        let kind = Kind::Custom(nimino_core::kind::KIND_AGENT_OBSERVER_FRAME as u16);
