...
struct AdmissionGuard {
    ...
    /// budget.
    next_candidate: u8,
    /// Routing identities held by currently connected peers.
...
mod tests {
    ...
    fn manager_isolates_same_channel_uuid_across_communities() {
        ...
        assert!(Arc::ptr_eq(
            &manager
                ...
                .expect("community A room exists"),
            &room_a
        ));
        ...
        assert_eq!(room_a.peers.len(), 1);
        assert!(room_b.peers.is_empty(), "A room peers must stay out of B");
    }
    ...
    fn reused_peer_index_gets_a_distinct_epoch() {
        ...
        room.guard.lock().expect("admission guard").next_candidate = 0;
