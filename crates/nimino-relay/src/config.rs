...
pub struct Config {
    ...
    /// [`nimino_db::DbConfig::replica_read_max_age_ms`].
    ...
    ///
    /// The `nimino-db` default of 20 was sized for a handful of pods against
    ...
    pub auth: nimino_auth::AuthConfig,
    ...
    /// cross pods via local delivery). Under horizontal scaling (any-pod-any-connection,
    ...
    pub media: nimino_media::MediaConfig,
...
fn rate_limit_config_from_env() -> Result<nimino_auth::RateLimitConfig, ConfigError> {
    let defaults = nimino_auth::RateLimitConfig::default();
    Ok(nimino_auth::RateLimitConfig {
...
impl Config {
    ...
    pub fn from_env() -> Result<Self, ConfigError> {
        ...
        let auth = nimino_auth::AuthConfig {
        ...
        let s3_addressing_style = match std::env::var("NIMINO_S3_ADDRESSING_STYLE") {
            ...
            Err(std::env::VarError::NotPresent) => {
                nimino_media::config::S3AddressingStyle::default()
            }
        ...
        let media = nimino_media::MediaConfig {
...
mod tests {
    ...
    fn defaults_are_valid() {
        ...
        assert_eq!(
            ...
            nimino_media::config::S3AddressingStyle::Path,
    ...
    fn s3_addressing_style_env_accepts_virtual_and_rejects_invalid_values() {
        ...
        assert_eq!(configured, nimino_media::config::S3AddressingStyle::Virtual);
        assert!(matches!(
            invalid,
    ...
    fn pairing_relay_url_accepts_websocket_urls_and_rejects_http() {
        let _guard = ENV_MUTEX.lock().unwrap();
        std::env::set_var("NIMINO_PAIRING_RELAY_URL", "wss://pairing.nimino.xyz");
        let config = Config::from_env().expect("config");
        assert_eq!(
            config.pairing_relay_url.as_deref(),
            Some("wss://pairing.nimino.xyz")
        );

        std::env::set_var("NIMINO_PAIRING_RELAY_URL", "https://pairing.nimino.xyz");
        let result = Config::from_env();
