...
//! from `nimino-conformance`. See `crates/nimino-conformance/src/lib.rs` for
...
///   [`nimino_db::Nimino::communities_of_channels`]). On a hit, return the
...
fn project_row_community(
    ...
    channel_communities: &std::collections::HashMap<Uuid, nimino_core::CommunityId>,
...
/// [`nimino_db::Nimino::communities_of_channels`] over the distinct
/// channel ids in `rows`.
