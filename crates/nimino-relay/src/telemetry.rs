...
#[cfg(test)]
pub(crate) static TEST_SUBSCRIBER_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());
...
pub fn otel_env_filter(configured: Option<&str>) -> EnvFilter {
    EnvFilter::new(configured.unwrap_or("nimino_relay=info,nimino_datastore=info"))
}
...
mod tests {
    ...
    fn trace_context_json_correlates_nested_span_logs() {
        let _subscriber_lock = TEST_SUBSCRIBER_LOCK.lock().expect("subscriber test lock");
