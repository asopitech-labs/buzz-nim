use nimino_auth::{LimitType, RateLimiter};
...
// is still a fixed-window limiter, so a local delivery-backed token bucket would be a
...
mod tests {
    ...
    use nimino_auth::{AuthError, RateLimitResult, RateLimiter};
    ...
    impl RateLimiter for StubLimiter {
        async fn check_and_increment(
            ...
            match self.outcome {
                ...
                StubOutcome::Failed => {
                    Err(AuthError::Internal("local delivery unavailable".to_owned()))
                }
            }
        }
