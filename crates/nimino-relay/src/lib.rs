#![deny(unsafe_code)]
#![warn(missing_docs)]
//! NIP-01 WebSocket relay for Nimino private team communication.
...
pub mod audio;
/// Canonical signed-event persistence shared by all accepted write paths.
pub mod canonical_event;
/// Production Nim worker, Chirps, per-node store, and sync composition.
...
/// `docs/spec/MultiTenantRelay.tla` by the independent `nimino-conformance`
/// checker.
pub mod conformance;
...
/// Workflow action sink — relay-side implementation of [`nimino_workflow::ActionSink`].
