...
fn server_authority(url: &reqwest::Url) -> Option<String> {
    let authority = nimino_core::tenant::relay_url_authority(url.as_str());
...
/// media and `NIMINO_PRIVATE_KEY` is available; `None` otherwise.
///
...
fn relay_media_get_auth(url: &reqwest::Url) -> Option<String> {
    let relay = std::env::var("NIMINO_RELAY_URL").ok()?;
    let relay = reqwest::Url::parse(&relay).ok()?;
    if !is_relay_media_url(url, &relay) {
        return None;
    }
    let key = std::env::var("NIMINO_PRIVATE_KEY").ok()?;
    let keys = match nostr::Keys::parse(&key) {
        Ok(k) => k,
        Err(e) => {
            tracing::warn!("NIMINO_PRIVATE_KEY invalid; fetching relay media unauthenticated: {e}");
            return None;
        }
    };
...
/// `NIMINO_RELAY_URL` + `NIMINO_PRIVATE_KEY` are configured.
async fn fetch_url(url: &str) -> Result<Vec<u8>, ErrorData> {
    ...
    if auth.is_some() {
        // Do not forward Nimino auth headers to a redirect target. reqwest strips
        ...
        if matches!(status.as_u16(), 401 | 403) && !authed {
            return Err(invalid_params(format!(
                "fetch {url} returned HTTP {status} — this relay requires authenticated media \
                 reads; set NIMINO_PRIVATE_KEY (and NIMINO_RELAY_URL) to a member identity"
            )));
        }
        return Err(invalid_params(format!(
