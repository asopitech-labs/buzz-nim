...
//!
//! Defines [`ChannelAccessChecker`] so `nimino-auth` can enforce access
//! without depending on `nimino-db` directly.
...
///
/// Implemented by the database layer (`nimino-db`) in production. The `nimino-auth`
