#![deny(unsafe_code)]
#![warn(missing_docs)]
//! `nimino-auth` — Authentication and authorization for the Nimino relay.
//!
//! ## Auth paths
//!
...
///
/// Uses `SHA-256("nimino-test-key:{username}")` as the secret key material.
...
pub fn derive_pubkey_from_username(username: &str) -> Result<nostr::PublicKey, AuthError> {
    ...
    let seed = format!("nimino-test-key:{username}");
