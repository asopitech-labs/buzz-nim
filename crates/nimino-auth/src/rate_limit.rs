...
//!
//! Defines the [`RateLimiter`] trait. The process-local implementation lives in
//! `nimino-local-delivery` and uses fixed-window counters.
//!
...
///
/// Each variant maps to a distinct counter-key suffix so limits are tracked
...
impl LimitType {
    /// Short suffix used in counter-key construction (e.g. `"msg"`, `"api"`).
...
///
/// The process-local production implementation lives in `nimino-local-delivery`.
/// A no-op `AlwaysAllowRateLimiter` is provided for unit tests.
///
/// ## Tenant scoping
///
/// Pubkey-keyed limits ([`check_and_increment`]) take `&TenantContext` and the
/// key is community-prefixed (`nimino:{community}:ratelimit:{pubkey}:{suffix}`). The
...
///
/// ⚠️ The fixed-window algorithm allows up to 2×
...
/// Counter key for a pubkey-based rate limit:
/// `nimino:{community}:ratelimit:{pubkey_hex}:{suffix}`.
///
...
/// Counter key for an IP-based rate limit: `nimino:ratelimit:ip:{ip}:conn`.
///
/// Operator-global by design — see [`RateLimiter`] docs.
pub fn ip_rate_limit_key(ip: &IpAddr) -> String {
    format!("nimino:ratelimit:ip:{}:conn", ip)
}
...
mod tests {
    ...
    fn rate_limit_key_includes_community_prefix() {
        ...
        let expected_prefix = format!("nimino:{}:ratelimit:", ctx.community());
        assert!(
    ...
    fn rate_limit_key_isolates_communities_for_same_pubkey() {
        ...
        // same pubkey, two communities -> two distinct local delivery keys -> independent quotas.
        let keys = Keys::generate();
    ...
    fn rate_limit_key_components_are_lowercase() {
        ...
        // produce two distinct local delivery keys → effective 2× quota. Pin the
    ...
    fn ip_rate_limit_key_format() {
        ...
        assert_eq!(
            ...
            "nimino:ratelimit:ip:192.168.1.1:conn"
        );
    }
