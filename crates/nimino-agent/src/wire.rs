...
/// one. `expected_run_id` must match the run id nimino-agent advertised via
...
/// session use `model_id` instead of the configured `NIMINO_AGENT_MODEL`.
#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
...
pub fn classify(msg: &Value) -> Inbound {
    ...
    match (method, id) {
        ...
        // Bare responses (id present, no method) are unexpected — nimino-agent
...
/// method goose uses for custom usage and status events.  Used by nimino-agent
/// to emit the `usage_update` payload so nimino-acp's `UsageTracker` can treat
/// nimino-agent and goose symmetrically.
...
/// exists to prevent.
///
/// All counts are SESSION-cumulative, matching goose, so nimino-acp's
...
/// | `contextLimit` | `u64` | `0` — nimino-agent has no context limit tracking |
...
pub fn usage_update_payload(
    ...
    let mut update = json!({
        ...
        // contextLimit: 0 (nimino-agent has no context limit tracking).
