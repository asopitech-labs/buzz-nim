...
const PASSTHROUGH_ENV: &[&str] = &[
    ...
    "SSL_CERT_FILE",
    "SSL_CERT_DIR",
    // Nimino identity — dev-mcp writes NOSTR_PRIVATE_KEY to a keyfile then
    // removes it from its own env (children never see it). NIMINO_PRIVATE_KEY
    // and NIMINO_RELAY_URL are kept for the nimino CLI. NIMINO_AUTH_TAG is a
    ...
    // spawn nimino-agent without declaring it.
...
mod content_tests {
    ...
    #[test]
    fn passthrough_includes_nimino_owner_attestation() {
