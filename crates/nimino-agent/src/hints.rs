...
mod tests {
    ...
    fn build_hints_section_combined() {
        ...
        let skill_dir = cwd.join(".agents/skills/nimino-cli");
        std::fs::create_dir_all(&skill_dir).unwrap();
        std::fs::write(
            skill_dir.join("SKILL.md"),
            "---\nname: nimino-cli\ndescription: CLI reference for Nimino managed agents\n---\nUse `nimino` to manage agents.\n",
        )
        .unwrap();
        ...
        assert!(
            result.contains("nimino-cli: CLI reference for Nimino managed agents"),
            "missing skill bullet"
        );
        // Body must NOT be inlined — lazy loading only.
        assert!(
            !result.contains("Use `nimino` to manage agents."),
