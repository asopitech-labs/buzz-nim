...
const REPLY_GUARD_SERVER: &str = "nimino-agent";
...
/// Whether `call` is a recognized attempt to publish a reply to Nimino.
///
...
fn is_nimino_reply_call(call: &ToolCall, mcp: &McpRegistry) -> bool {
    mcp.has(&call.name) && !mcp.is_hook(&call.name) && is_reply_shaped(&call.name, &call.arguments)
}

/// Whether a tool name and arguments have the shape of a Nimino publish command.
///
/// Split from [`is_nimino_reply_call`] only so the matcher is testable without a
...
impl RunCtx<'_> {
    ...
    pub async fn run(&mut self, prompt: Vec<ContentBlock>) -> Result<StopReason, AgentError> {
        ...
        *self.turn_total_state = TurnTotalState::Unseen;
        ...
        // successful publish. See `is_nimino_reply_call`.
        let mut nimino_reply_call_seen = false;
        ...
        loop {
                ...
                || response.output_tokens.is_some()
            {
                ...
                *self.turn_cache_write_tokens = self
                ...
                // the consumer: nimino-acp's UsageTracker advances its committed
            ...
            if self.cfg.require_reply && !nimino_reply_call_seen {
                nimino_reply_call_seen = calls.iter().any(|c| is_nimino_reply_call(c, self.mcp));
            }
            self.history.push(HistoryItem::Assistant {
                text: response.text,
...
fn synthetic_hook_id(hook: &str, server: &str, ordinal: u64) -> String {
    format!("nimino_hook_{hook}_{server}_{ordinal}")
}
...
mod tests {
    ...
    fn reply_shape_matches_documented_send_forms() {
        for cmd in [
            ...
            "nimino reactions add --event E --emoji +",
    ...
    fn reply_shape_rejects_non_reply_commands() {
        for cmd in [
            ...
            "nimino reactions remove --event E",
            "nimino pr open --title T",
            "nimino social publish --content hi",
            "nimino notes set --name n",
    ...
    fn reply_shape_requires_the_qname_separator() {
        ...
        assert!(is_reply_shaped("nimino-dev-mcp__shell", &args));
    }
