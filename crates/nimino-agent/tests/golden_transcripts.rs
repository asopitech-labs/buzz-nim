...
async fn handshake(h: &mut Harness) -> String {
    ...
    assert_eq!(init["result"]["agentInfo"]["name"], "nimino-agent");
    assert_eq!(
...
/// nimino-agent must emit a `usage_update` notification with `accumulatedInputTokens`
...
/// When the Anthropic parser produces an input-sum overflow, nimino-agent must
