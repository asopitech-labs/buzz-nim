...
//! exercised manually via the `nimino-agent auth databricks` subcommand
//! (see `lib.rs::auth_subcommand`).
//!
//! The second test module (further down) is an ACP-level envelope
//! regression: it spawns the real `nimino-agent` binary with
...
use nimino_agent::auth::{PkceOAuthConfig, PkceOAuthTokenSource, TokenSource};
...
//
// Boots the real nimino-agent binary with `DATABRICKS_TOKEN` set (so the
...
impl AgentHarness {
    ...
    async fn spawn_provider_with_options(
        ...
        cmd.env("NIMINO_AGENT_PROVIDER", provider)
            ...
            .env("NIMINO_AGENT_MAX_ROUNDS", "2")
            .env("NIMINO_AGENT_MAX_SESSIONS", max_sessions.to_string())
...
async fn model_discovery_surfaces_rejected_static_token_as_auth_failure() {
    ...
    use nimino_agent::config::{Config, Provider};
    use nimino_agent::discover_databricks_models;
