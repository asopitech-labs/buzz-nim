//! Integration test: fake LLM HTTP server + nimino-agent subprocess.
//!
...
async fn init_session(h: &mut Harness) -> String {
    ...
    assert_eq!(r["result"]["agentInfo"]["name"], "nimino-agent");
...
async fn rejects_oversized_line() {
    ...
    cmd.env("NIMINO_AGENT_PROVIDER", "openai")
        ...
        .env("NIMINO_AGENT_MAX_LINE_BYTES", "256")
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::null())
        .kill_on_drop(true);
...
async fn system_prompt_reaches_llm_system_role() {
    ...
    let default_prompt = "You are nimino-agent";
    assert!(
...
async fn system_prompt_absent_no_canary() {
    ...
    assert!(
        system_content.contains("You are nimino-agent"),
...
/// Wait for the `activeRunId` advert nimino-agent emits at prompt start and
...
/// completion_tokens). nimino-agent maps these to `accumulatedInputTokens` /
...
/// nimino-agent must treat this turn's total as Unknown and poison the cumulative.
...
/// nimino-agent must emit `_goose/unstable/session/update` with `sessionUpdate:
...
/// When the provider returns a response with no `usage` block, nimino-agent must
...
/// (so token counts are observed), nimino-agent must still emit the usage
