...
//!
//! Run with a local PG: `NIMINO_TEST_DATABASE_URL=postgres://nimino:nimino_dev@localhost:5432/nimino cargo test -p nimino-search --tests -- --include-ignored`
//!
...
use nimino_search::{ChannelScope, SearchQuery, SearchService};
...
async fn setup() -> (PgPool, String) {
    let url = std::env::var("NIMINO_TEST_DATABASE_URL").unwrap_or_else(|_| TEST_DB_URL.to_string());
...
async fn search_finds_event_in_same_community() {
    ...
    insert_event(
        ...
        pk,
        9,
        "hello wonderland — nimino everyone",
        None,
        1700000000,
    )
    .await;
...
async fn kind0_search_by_display_name_works_without_flattening() {
    ...
    insert_event(
        ...
        pk,
        0,
        r#"{"display_name":"Alice Wonderland","name":"alice","nip05":"alice@nimino.app","about":"hello"}"#,
        None,
        1700000000,
    )
    .await;
    ...
    let svc = SearchService::new(pool.clone());
    for q in ["wonderland", "alice", "alice@nimino.app"] {
        let r = svc
            .search(&SearchQuery {
                community: c,
                q: q.to_string(),
...
async fn short_kind0_prefix_prioritizes_exact_lexeme_on_a_noisy_page() {
    ...
    let first_page = svc
        .search(&SearchQuery {
            ...
            page: 1,
            per_page: 3,
            mode: nimino_search::SearchMode::Prefix,
        })
        .await
...
async fn p_gated_persistent_kinds_have_storage_null_tsvector() {
    ...
    let persistent: Vec<u32> = P_GATED_KINDS
        .iter()
        .copied()
        .filter(|&k| !nimino_core::kind::is_ephemeral(k))
        .collect();
    assert!(
        !persistent.is_empty(),
