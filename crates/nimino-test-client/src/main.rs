//! `nimino-test-cli` — Manual testing CLI for the Nimino relay.
//!
//! # Usage
//!
//! ```text
//! nimino-test-cli [OPTIONS]
//!
//! Options:
...
//! Send a message:
//! ```text
//! nimino-test-cli --channel my-channel --send "Hello, Nimino!"
//! ```
//!
//! Subscribe and watch events:
//! ```text
//! nimino-test-cli --channel my-channel --subscribe
//! ```
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            std::env::var("RUST_LOG")
                .unwrap_or_else(|_| "nimino_test_client=debug".to_string())
                .as_str(),
        )
        .init();
...
async fn run_send(url: &str, keys: &Keys, channel: &str, message: &str, kind: u16) {
    ...
    let mut client = match NiminoTestClient::connect(url, keys).await {
        Ok(c) => c,
        Err(e) => {
...
async fn run_subscribe(url: &str, keys: &Keys, channel: &str, kind: u16) {
    ...
    loop {
        match client.recv_event(Duration::from_secs(30)).await {
            ...
            Ok(_) => {}
            Err(nimino_test_client::TestClientError::Timeout) => {
                // Keep waiting.
            }
            Err(e) => {
                eprintln!("Error: {e}");
...
fn print_help() {
    println!(
        r#"nimino-test-cli — Manual testing CLI for the Nimino relay
...
EXAMPLES:
    ...
    nimino-test-cli --url ws://relay.example.com --channel test --subscribe
"#
    );
}
