...
//!
//! ```text
//! RELAY_URL=ws://localhost:3001 cargo test -p nimino-test-client --test e2e_event_reminder -- --ignored
//! ```
...
async fn test_other_user_subscription_closed_for_author_only_kind_ws() {
    ...
    let mut ws = NiminoTestClient::connect(&url, &other_keys)
        .await
        .expect("connect");
...
async fn test_mixed_kind_filter_omits_other_authors_reminders_ws() {
    ...
    let mut ws_reader = NiminoTestClient::connect(&url, &reader_keys)
        .await
...
async fn test_fanout_isolation_other_user_does_not_receive_reminder() {
    ...
    let mut ws_other = NiminoTestClient::connect(&url, &other_keys)
        .await
...
/// `NIMINO_REMINDER_SCHEDULER_INTERVAL_SECS=1`.
#[tokio::test]
#[ignore]
