//! End-to-end acceptance tests for Nimino shared compute.
//!
//! These tests require a membership-gated nimino-relay and a mesh-enabled desktop
...
//!   cargo test --test e2e_cluster_llm trust -- --ignored --nocapture
...
//!   cargo test --test e2e_cluster_llm live_agent_completes -- --ignored --nocapture
...
//! | 5 | dropped member → typed auth failure reaches lastError | runbook (desktop harness) | nimino-agent `-32001` unit |
...
const KIND_NIMINO_CLUSTER_MEMBER_STATUS: u16 = 30003;
const MESH_STATUS_D_TAG_PREFIX: &str = "nimino-mesh-member-status:";
const MESH_STATUS_TYPE: &str = "nimino-mesh-status";
...
fn mesh_status_filter() -> Filter {
    Filter::new()
        .kind(Kind::Custom(KIND_NIMINO_CLUSTER_MEMBER_STATUS))
...
async fn trust_member_reads_mesh_status() {
    ...
    let mut client = NiminoTestClient::connect(&url, &member)
        .await
    ...
    let status = events
        .iter()
        .find(|e| {
            e.kind == Kind::Custom(KIND_NIMINO_CLUSTER_MEMBER_STATUS)
...
///
/// Requires a relay with `NIMINO_REQUIRE_RELAY_MEMBERSHIP=true` and a published
...
async fn trust_nonmember_read_denied() {
    ...
    let mut client = match NiminoTestClient::connect(&url, &stranger).await {
        Ok(c) => c,
    ...
    let leaked = events
        .iter()
        .any(|e| e.kind == Kind::Custom(KIND_NIMINO_CLUSTER_MEMBER_STATUS));
    assert!(
        !leaked,
...
/// B running a mesh client + a launched nimino-agent pointed at B's local
...
/// split route. Auto-split is mesh runtime behavior (no Nimino code); this row
