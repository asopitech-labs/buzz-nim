...
//! cargo test -p nimino-test-client --test conformance_multitenant -- --ignored
//! ```
//!
//! Both URLs MUST address the same relay process and Postgres; only the `Host`
//! header differs. That is the whole point: one binary,
...
mod row_zero_host_binding {
    ...
    /// properties of it (see channel: `nimino-relay-rewrite`, 2026-06-27):
    ///
    ///   * [`super::channels_membership::same_channel_uuid_in_two_communities_is_isolated`]
    ///     (Mari, `nimino-db`) asserts **coexistence**: a channel UUID that exists
...
mod nip11_relay_info {
    ...
    /// (`crates/nimino-relay/src/nip11.rs::_RELAY_INFO_BUILD_STATIC_INPUT_FENCE`):
...
// ---------------------------------------------------------------------------
// API tokens and NIP-98 replay (Sami — nimino-auth)
// ---------------------------------------------------------------------------
mod api_tokens_nip98_replay {
    ...
    /// in `nimino-relay`** — there is no route by which a client can bind a token
    /// to a community over HTTP, so the "mint in A, present to B" precondition
    /// has no entry point.
    ///
    /// Verified on PR head:
    ///   * `crates/nimino-relay/src/router.rs:52-79` — full route list is `/`,
    ...
    ///   * `crates/nimino-relay/src/api/` directory is `{bridge, events, media,
    ...
    ///     `crates/nimino-relay/src/api/media.rs:638`, which extracts the
    ///     `X-Auth-Token: nimino_*` header and looks up
    ...
    /// contract (its deps are `nimino-ws-client`/`reqwest`/`tokio-tungstenite`/
    /// `s3` — no `sqlx`, no `nimino-db`).
    ///
    ...
    ///
    ///   1. `crates/nimino-db/src/api_token.rs:425
    ...
    ///   2. `crates/nimino-db/src/api_token.rs:488
    ...
    fn token_minted_in_a_does_not_authorize_in_b() {
        ...
        // If anyone adds a `/tokens` route to `nimino-relay`, this row's shape
    ...
    /// in `crates/nimino-relay/src/api/bridge.rs:79` (return `Ok(())` before
    ...
    ///
    /// The replay key shape is `nimino:{community}:nip98:{event_id_hex}` (see
    /// `crates/nimino-auth/src/nip98_replay.rs:103 nip98_replay_key`). The
    ...
    /// (`crates/nimino-auth/src/nip98_replay.rs:163
    ...
    /// "simplifying" the key to just `nimino:nip98:{event_id}`, or
    ...
    /// (relay-membership is open under `NIMINO_REQUIRE_AUTH_TOKEN=false`).
    #[tokio::test]
    #[ignore]
    ...
    /// the pattern in `crates/nimino-auth/src/nip98.rs`; kept local to this
...
// ---------------------------------------------------------------------------
// Membership / allowlist / archived identities (Sami — nimino-auth)
// ---------------------------------------------------------------------------
...
mod users_profiles_nip05 {
    ...
    use nimino_test_client::NiminoTestClient;
    ...
    /// relay's ingest side-effect at `crates/nimino-relay/src/handlers/side_effects.rs::handle_kind0_profile`
    /// also syncs the parsed fields into `users` via
    /// `update_user_profile(tenant.community(), pubkey, ...)`.
    async fn publish_kind0(
        client: &mut NiminoTestClient,
        keys: &Keys,
        content_json: &str,
    ) -> String {
    ...
    /// bridge endpoint; with `NIMINO_REQUIRE_AUTH_TOKEN=false` the dev-mode
    ...
    /// `crates/nimino-relay/src/api/nip05.rs::extract_domain` — strips the
    ...
    /// `crates/nimino-db/src/user.rs:140` (`UPDATE users SET ... WHERE
    /// community_id = $N AND pubkey = $M`) AND the event-storage fence at
    /// `crates/nimino-db/src/event.rs::get_events_by_ids` (or the equivalent
    ...
    async fn same_pubkey_distinct_profiles_in_two_communities() {
        ...
        let mut client_a = NiminoTestClient::connect(&ws_a, &keys)
            .await
            .expect("connect A");
        let _id_a = publish_kind0(&mut client_a, &keys, &content_a).await;

        let mut client_b = NiminoTestClient::connect(&ws_b, &keys)
            .await
            .expect("connect B");
    ...
    /// `crates/nimino-relay/src/handlers/side_effects.rs::handle_kind0_profile`
    /// validates the handle via `crates/nimino-relay/src/api/nip05.rs::canonicalize_nip05`
    ...
    /// from `crates/nimino-db/src/user.rs::get_user_by_nip05` (line 185 form
    ...
    async fn same_nip05_local_part_on_two_hosts_is_independent() {
        ...
        let mut client_a = NiminoTestClient::connect(&ws_a, &keys_a)
            .await
            .expect("connect A");
        let _ = publish_kind0(&mut client_a, &keys_a, &content_a).await;

        let mut client_b = NiminoTestClient::connect(&ws_b, &keys_b)
            .await
            .expect("connect B");
...
// ---------------------------------------------------------------------------
// Channel-less global events and DMs (Mari+Max — db+local_delivery)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Feed read-side isolation (Perci — nimino-db/relay bridge)
// ---------------------------------------------------------------------------
mod feed_read_side_isolation {
    ...
    /// `crates/nimino-db/src/feed.rs`; this row is the wire-level conformance
...
// ---------------------------------------------------------------------------
// Channels and channel membership (Mari — nimino-db)
// ---------------------------------------------------------------------------
mod channels_membership {
    ...
    /// `crates/nimino-relay/src/handlers/side_effects.rs::handle_create_group`.
    ...
    /// `NIMINO_REQUIRE_AUTH_TOKEN=false` recipe). Returns the events as their
    ...
    /// `crates/nimino-db/src/event.rs::query_events` (the non-p-tag branch,
    ...
    async fn same_channel_uuid_in_two_communities_is_isolated() {
        ...
        // synchronously with the OK response, but `received_at`/local fan-out
...
mod workflows {
    ...
    /// the REST bridge (`POST /events`). In dev mode (`NIMINO_REQUIRE_AUTH_TOKEN
    ...
    /// `crates/nimino-relay/src/handlers/command_executor.rs:703`
    ...
    /// (`crates/nimino-workflow/src/lib.rs` — "approval gates not yet implemented,
    ...
    /// minting), **not** nimino-db — the scoping fence it will exercise is
    /// already landed.
    #[tokio::test]
    #[ignore]
...
// ---------------------------------------------------------------------------
// Search / FTS (Quinn — nimino-search)
// ---------------------------------------------------------------------------
mod search_fts {
    use super::*;

    use nimino_test_client::{NiminoTestClient, RelayMessage};
    ...
    ///   - `crates/nimino-search/src/query.rs::search` lines 160-161 (the FTS
    ///     `WHERE community_id = $ctx` predicate); and
    ///   - `crates/nimino-db/src/event.rs::get_events_by_ids` lines 870-872
...
// ---------------------------------------------------------------------------
// Process-local presence and live fan-out
// ---------------------------------------------------------------------------
mod pubsub_presence_typing {
    ...
    async fn create_open_channel(http_base: &str, keys: &Keys, channel_uuid: uuid::Uuid) -> String {
        ...
        let event = EventBuilder::new(Kind::Custom(9007), "")
            .tags(vec![
                ...
                Tag::parse([
                    "name",
                    &format!("conformance-local_delivery-{channel_uuid}"),
                ])
                .unwrap(),
                Tag::parse(["channel_type", "stream"]).unwrap(),
                Tag::parse(["visibility", "open"]).unwrap(),
    ...
    async fn publish_presence(client: &mut NiminoTestClient, keys: &Keys, status: &str) {
    ...
    /// process-local state via `get_presence_bulk`; there is no DB fallback.
    ...
    async fn subscribe_typing(client: &mut NiminoTestClient, sub_id: &str, channel_id: &str) {
        let filter = Filter::new()
    ...
    /// Obligation: local state is community-scoped and fan-out never delivers
    /// an A event to a B subscription, even for the same channel UUID;
    /// the same pubkey can be online in A and away in B independently.
    ///
    /// Wire-observable shape, split across two tenant fences:
    ///
    /// 1. **Presence fence.** The same keypair publishes kind:20001
    ///    presence with distinct statuses in A and B. Presence is ephemeral, so
    ///    `/query` is synthesized from process-local state (`api/bridge.rs::synthesize_presence`
    ///    → `nimino_local_delivery::get_presence_bulk`). A's query must return only A's
    ///    status and B's only B's. The map key must include `ctx.community()`.
    ///    The same pubkey proves the isolation coordinate is community, not key.
    ///
    ...
    ///
    /// Mutate-bites:
    ///   - Presence: drop/neutralize `ctx.community()` in the local map key. The B
    ...
    async fn fanout_and_presence_do_not_cross_communities() {
        ...
        // Presence key must be `(community, pubkey)`, not just `pubkey`.
        let keys = Keys::generate();
        let pubkey_hex = keys.public_key().to_hex();
        ...
        // Let presence writes and the bridge's subsequent read settle.
...
// ---------------------------------------------------------------------------
// Media / Blossom / S3 (Perci — nimino-media)
// ---------------------------------------------------------------------------
mod media_blossom {
    use super::*;
...
// ---------------------------------------------------------------------------
// Git hosting / NIP-34 / object storage (Perci — nimino-media/git)
// ---------------------------------------------------------------------------
mod git_hosting {
    use super::*;
...
// ---------------------------------------------------------------------------
// Audit log and observability (Dawn — nimino-audit)
// ---------------------------------------------------------------------------
mod audit_log {
    ...
    //! `crates/nimino-relay/src/router.rs` (the route list is `/`, `/info`,
    ...
    //! `nimino_audit::AuditService::{verify_chain, get_entries}`, which are
    //! operator-internal (consumed by `nimino-admin`). `crates/nimino-audit/src/
    ...
    //! (its deps are `nimino-ws-client`/`reqwest`/`tokio-tungstenite`/`s3` — no
    //! `sqlx`, no `nimino-audit`), and a DB-direct read can never catch a
    ...
    //!      `nimino_audit::service::tests::chains_are_independent_per_community`
    ...
    //!      `nimino_relay::handlers::event::tests::
    ...
    //!   2. **Errors don't leak** —
    //!      `nimino_audit::error::tests::audit_error_text_carries_no_community_id_or_constraint`
    //!      asserts no `AuditError` variant's rendered text embeds a
    //!      `community_id`, constraint name, or cross-community object id.
    //!
    //! Substrate on PR head: `crates/nimino-audit/src/entry.rs` keys `AuditEntry`
