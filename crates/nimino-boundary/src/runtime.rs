...
fn ready_matches_contract(ready: &ReadyPayload) -> bool {
    ready.protocol_version == PROTOCOL_VERSION
        ...
        && ready
            .capabilities
            .iter()
            .any(|name| name == "domain.moderation.policy")
        && ready
            .capabilities
            .iter()
            .any(|name| name == "domain.workflow.policy")
        && ready
            .capabilities
            .iter()
            .any(|name| name == "domain.cluster.lifecycle")
        && ready
            .capabilities
            .iter()
            .any(|name| name == "domain.admission.policy")
        && ready
            .capabilities
            .iter()
            .any(|name| name == "domain.ephemeral.policy")
...
mod tests {
    ...
    fn ready() -> ReadyPayload {
        ReadyPayload {
            ...
            capabilities: vec![
                ...
                "domain.moderation.policy".to_owned(),
                "domain.workflow.policy".to_owned(),
                "domain.cluster.lifecycle".to_owned(),
                "domain.admission.policy".to_owned(),
                "domain.ephemeral.policy".to_owned(),
            ],
        }
    }
    ...
    fn handshake_requires_every_exact_contract_fact() {
        ...
        candidate
            .capabilities
            .retain(|name| name != "domain.moderation.policy");
        assert!(!ready_matches_contract(&candidate));
        let mut candidate = ready();
        candidate
            .capabilities
            .retain(|name| name != "domain.workflow.policy");
        assert!(!ready_matches_contract(&candidate));
        let mut candidate = ready();
        candidate
            .capabilities
            .retain(|name| name != "domain.cluster.lifecycle");
        assert!(!ready_matches_contract(&candidate));
        let mut candidate = ready();
        candidate
            .capabilities
            .retain(|name| name != "domain.admission.policy");
        assert!(!ready_matches_contract(&candidate));
        let mut candidate = ready();
        candidate
            .capabilities
            .retain(|name| name != "domain.ephemeral.policy");
        assert!(!ready_matches_contract(&candidate));
        let mut candidate = ready();
