...
///
/// Priority: NOSTR_PRIVATE_KEY > NIMINO_PRIVATE_KEY > git config nostr.keyfile
///
...
fn load_key() -> Result<zeroize::Zeroizing<String>, Error> {
    ...
    // 2. NIMINO_PRIVATE_KEY
    if let Ok(mut val) = std::env::var("NIMINO_PRIVATE_KEY") {
        // Cap at 128 bytes: nsec1 bech32 is ~63 chars, hex is 64 chars.
        // 128 bytes is generous headroom; anything larger is malformed input.
        if val.len() > 128 {
            val.zeroize();
            std::env::remove_var("NIMINO_PRIVATE_KEY");
            return Err(Error::Fatal(
                "NIMINO_PRIVATE_KEY exceeds 128-byte size limit".to_string(),
            ));
        }
        let trimmed = val.trim().to_string();
        val.zeroize();
    ...
    let path = git_config("nostr.keyfile").ok_or_else(|| {
        Error::Fatal(
            "no key available: set NOSTR_PRIVATE_KEY, NIMINO_PRIVATE_KEY, \
...
///
/// Priority per NIP-GS spec: `NIMINO_AUTH_TAG` env var > `nostr.authtag` git config.
...
fn load_auth_tag() -> Result<Option<(String, String, String)>, Error> {
    ...
    let json_str = match std::env::var("NIMINO_AUTH_TAG")
        .ok()
        .filter(|s| !s.is_empty())
    {
        Some(val) => Some(val),
    ...
    let arr: serde_json::Value = serde_json::from_str(&json_str)
        .map_err(|e| Error::Fatal(format!("NIMINO_AUTH_TAG is not valid JSON: {e}")))?;
    let arr = arr
        .as_array()
        .ok_or_else(|| Error::Fatal("NIMINO_AUTH_TAG must be a JSON array".to_string()))?;
    if arr.len() != 4 {
        return Err(Error::Fatal(
            "NIMINO_AUTH_TAG must have exactly 4 elements".to_string(),
        ));
    }
    if arr[0].as_str() != Some("auth") {
        return Err(Error::Fatal(
            "NIMINO_AUTH_TAG[0] must be \"auth\"".to_string(),
        ));
    }

    let owner = arr[1]
        .as_str()
        .ok_or_else(|| Error::Fatal("NIMINO_AUTH_TAG[1] must be a string".to_string()))?
        .to_string();
    let conditions = arr[2]
        .as_str()
        .ok_or_else(|| Error::Fatal("NIMINO_AUTH_TAG[2] must be a string".to_string()))?
        .to_string();
    let sig = arr[3]
        .as_str()
        .ok_or_else(|| Error::Fatal("NIMINO_AUTH_TAG[3] must be a string".to_string()))?
        .to_string();
    ...
    if !validate_conditions(&conditions) {
        return Err(Error::Fatal(
            "NIMINO_AUTH_TAG conditions contain invalid characters".to_string(),
        ));
    }
    ...
    {
        return Err(Error::Fatal(
            "NIMINO_AUTH_TAG owner must be 64 lowercase hex chars".to_string(),
        ));
    }
    if sig.len() != 128
        || !sig
    ...
    {
        return Err(Error::Fatal(
            "NIMINO_AUTH_TAG signature must be 128 lowercase hex chars".to_string(),
        ));
    }
        ...
        if !verify_oa(&pk_hex, oa_val) {
            return Err(Error::Fatal(
                "auth tag owner signature (oa[2]) verification failed — \
                 the configured NIMINO_AUTH_TAG is invalid or stale"
                    .to_string(),
            ));
        }
    }
