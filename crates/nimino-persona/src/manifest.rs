...
//! (OPS metadata) and tells Nimino where to find personas, hooks, and MCP
//! config.
//!
//! ```json
//! {
...
pub struct Engines {
    /// Semver range the Nimino runtime must satisfy (e.g. `">=0.9.0"`).
    #[serde(skip_serializing_if = "Option::is_none", alias = "nimino")]
    pub nimino: Option<String>,
}
...
/// validator issues advisory warnings for Nimino-unknown keys.
...
mod tests {
    ...
    fn parse_full_manifest() {
        ...
        assert_eq!(m.engines.unwrap().nimino.as_deref(), Some(">=0.9.0"));
