...
//! - Nimino-agent personas emit NIMINO_AGENT_MODEL, NIMINO_AGENT_PROVIDER
...
use nimino_persona::resolve::resolve_pack;
...
#[test]
fn resolve_pack_nimino_agent_persona_emits_nimino_agent_vars() {
    let tmp = tempfile::tempdir().unwrap();
    let root = tmp.path();
...
"#,
    ...
    assert_eq!(
        env.get("NIMINO_AGENT_MODEL"),
        Some(&"gpt-4o"),
        "should emit NIMINO_AGENT_MODEL=gpt-4o"
    );
    assert_eq!(
        env.get("NIMINO_AGENT_PROVIDER"),
        Some(&"openai"),
        "should emit NIMINO_AGENT_PROVIDER=openai"
    );

    // Must NOT contain GOOSE_* keys
    assert!(
        !env.contains_key("GOOSE_MODEL"),
        "nimino-agent runtime must not emit GOOSE_MODEL"
    );
    assert!(
        !env.contains_key("GOOSE_PROVIDER"),
        "nimino-agent runtime must not emit GOOSE_PROVIDER"
    );
}
...
"#,
    ...
    // Nimino-agent persona
    fs::write(
        root.join("agents/nimino-bot.persona.md"),
        r#"---
name: "nimino-bot"
display_name: "Nimino Bot"
description: "A nimino-agent runtime bot"
runtime: "nimino-agent"
model: "openai:gpt-4o"
---
You are a nimino bot.
"#,
    )
    .unwrap();
    ...
    let nimino = pack
        .personas
        .iter()
        .find(|p| p.name == "nimino-bot")
        .expect("nimino-bot should exist");
    ...
    assert!(
        !goose_env.contains_key("NIMINO_AGENT_MODEL"),
        "goose persona must not emit NIMINO_AGENT_MODEL"
    );
    assert!(
        !goose_env.contains_key("NIMINO_AGENT_PROVIDER"),
        "goose persona must not emit NIMINO_AGENT_PROVIDER"
    );

    // Nimino-agent persona gets NIMINO_AGENT_* env vars
    let nimino_env: std::collections::HashMap<_, _> = nimino
        .runtime_env_vars
        .iter()
        .map(|(k, v)| (k.as_str(), v.as_str()))
        .collect();
    assert_eq!(nimino_env.get("NIMINO_AGENT_MODEL"), Some(&"gpt-4o"));
    assert_eq!(nimino_env.get("NIMINO_AGENT_PROVIDER"), Some(&"openai"));
    assert!(
        !nimino_env.contains_key("GOOSE_MODEL"),
        "nimino-agent persona must not emit GOOSE_MODEL"
    );
    assert!(
        !nimino_env.contains_key("GOOSE_PROVIDER"),
        "nimino-agent persona must not emit GOOSE_PROVIDER"
    );
}
