...
///
/// Shortcodes are case-insensitive in Nimino's relay-global set; lowercase
...
/// Build a Nimino-native delete event (kind 9005).
...
/// Build a Nimino-native delete event (kind 9005) with optional moderation metadata.
...
pub const CUSTOM_EMOJI_SET_D_TAG: &str = "nimino:custom-emoji";
...
/// "nimino:custom-emoji")`. Replaces the caller's prior set. The workspace
...
pub fn build_update_channel(
    ...
    if name
        .map(nimino_core::channel::canonical_channel_name)
...
/// Maximum byte length of a project `nimino-channel` tag value.
pub const PROJECT_CHANNEL_MAX: usize = 256;
/// Maximum byte length of a project `nimino-visibility` tag value.
...
///
/// Rules enforced (matches relay `nimino-db` ingest logic):
...
/// 7. Singleton metadata: each of `name`, `description`, `nimino-channel`,
///    `nimino-visibility` appears at most once.
/// 8. Metadata byte lengths: `name` ≤256, `description` ≤2048,
///    `nimino-channel` ≤256, `nimino-visibility` ≤256.
...
pub fn build_project(
    ...
    if let Some(ch) = channel {
        uuid::Uuid::parse_str(ch).map_err(|_| {
            SdkError::InvalidInput(format!("nimino-channel must be a valid UUID (got {ch:?})"))
        })?;
    }
    ...
    if let Some(vis) = visibility {
        if vis != "listed" && vis != "unlisted" {
            return Err(SdkError::InvalidInput(format!(
                "nimino-visibility must be 'listed' or 'unlisted' (got {vis:?})"
            )));
        }
    }
    ...
    if let Some(ch) = channel {
        tags.push(tag(&["nimino-channel", ch])?);
    }
    if let Some(vis) = visibility {
        tags.push(tag(&["nimino-visibility", vis])?);
    }
...
mod tests {
    ...
    fn agent_observer_frame_happy_path() {
        ...
        let recipient = keys();
        let agent = keys();
        let encrypted = nimino_core::observer::encrypt_observer_payload(
            &sender,
    ...
    fn repo_announcement_with_tags_preserves_metadata_and_canonicalizes_d() {
        let tags = vec![
            ...
            Tag::parse(["nimino-channel", "channel-id"]).unwrap(),
        ...
        assert!(has_tag(&ev, "nimino-channel", "channel-id"));
    ...
    fn git_patch_rejects_whitespace_only_content() {
        // Regression: a failed `git format-patch | nimino patches send
    ...
    fn build_project_emitted_envelope_has_correct_shape() {
        ...
        // name, description, nimino-channel, nimino-visibility present.
        ...
        let vis_tags: Vec<_> = all_tags
            .iter()
            .filter(|t| t[0] == "nimino-visibility")
            .collect();
        ...
        assert_eq!(a_tags[0][1], format!("30617:{OWNER64}:nimino"));
    }
