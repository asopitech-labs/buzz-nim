...
use nimino_media::{is_tenant_owned_key, tenant_prefixes, MediaStorage};
use serde::Serialize;
...
pub struct ServingWriteGuard {
    store: DeletionStore,
    lease: nimino_db::deletion::ServingWriteLease,
...
impl ServingWriteGuard {
    ...
    pub fn acquisition_is_fenced(error: &anyhow::Error) -> bool {
        matches!(
            error.downcast_ref::<nimino_db::DbError>(),
            Some(nimino_db::DbError::AccessDenied(_))
        )
    }
    ...
    pub fn lease(&self) -> &nimino_db::deletion::ServingWriteLease {
        &self.lease
    }
...
/// durable proof that deletion can drain object-store work across replicas.
...
fn is_permanent_error(error: &anyhow::Error) -> bool {
    error.chain().any(|cause| {
        cause.is::<PermanentSource>()
            || matches!(
                cause.downcast_ref::<nimino_db::DbError>(),
                Some(nimino_db::DbError::DeletionSafety(_))
            )
            || cause
...
async fn run_with_services(command: Command, services: Services) -> Result<i32> {
    match command {
        ...
        Command::Sweep => {
            ...
            let outcome =
                nimino_media::sweep_bucket_taxonomy(cap, SWEEP_UNKNOWN_KEY_SAMPLE, move |token| {
...
fn resolve_submit_host(host: Option<&str>, relay_url: Option<&str>) -> Result<String> {
    ...
    let host = nimino_core::tenant::relay_url_authority(relay_url);
    if host.is_empty() {
        anyhow::bail!(
...
async fn connect_store() -> Result<DeletionStore> {
    ...
    let db = Db::new(&DbConfig {
        database_url,
        max_connections: env_parse("NIMINO_DB_POOL_SIZE", 20),
        ..DbConfig::default()
    })
    .await?;
    Ok(store(&db))
...
fn resolve_s3_region(nimino_region: Option<String>, aws_region: Option<String>) -> String {
    nimino_region
...
async fn connect_services_with_store(store: DeletionStore) -> Result<Services> {
    ...
    let media_config = nimino_media::MediaConfig {
        s3_endpoint: required_env("NIMINO_S3_ENDPOINT")?,
        s3_access_key,
        s3_secret_key,
        s3_bucket: required_env("NIMINO_S3_BUCKET")?,
        s3_region: s3_region_from_env(),
        s3_addressing_style: std::env::var("NIMINO_S3_ADDRESSING_STYLE")
    ...
    Ok(Services { store, media })
}

fn s3_region_from_env() -> String {
    resolve_s3_region(
        std::env::var("NIMINO_S3_REGION").ok(),
...
fn s3_key_pair_from(get_env: impl Fn(&str) -> Option<String>) -> (String, String) {
    (
        optional_env_from(&get_env, "NIMINO_S3_ACCESS_KEY"),
        optional_env_from(&get_env, "NIMINO_S3_SECRET_KEY"),
    )
}
...
fn validate_storage_ownership(request: &DeletionRequest, manifest: &StorageManifest) -> Result<()> {
    nimino_db::deletion::validate_storage_manifest(manifest)?;
...
async fn record_stage_failure(
    ...
    match result {
        Ok(()) => Ok(true),
        Err(error) if nimino_db::deletion::is_stale_deletion_lease(&error) => Ok(false),
    ...
    match request.stage {
        DeletionStage::Approved => {
            ...
            match services.store.fence(&token).await {
                Ok(_) => {}
                Err(nimino_db::DbError::ServingWritesNotDrained {
                    active_count,
                    operations,
                    ..
                }) => {
...
fn sweep_object_cap() -> u64 {
    std::env::var("NIMINO_DELETION_SWEEP_MAX_OBJECTS")
        .ok()
        .and_then(|value| value.parse().ok())
...
fn manifest_chunk_keys() -> usize {
    std::env::var("NIMINO_DELETION_MANIFEST_CHUNK_KEYS")
        .ok()
...
fn default_executor_id() -> String {
    let hostname = std::env::var("HOSTNAME").unwrap_or_else(|_| "nimino-admin".to_string());
...
mod tests {
    ...
    fn empty_storage_manifest(community: nimino_core::CommunityId) -> StorageManifest {
        StorageManifest {
            version: 4,
    ...
    async fn claimed_test_deletion(prefix: &str) -> (Db, Services, ClaimedDeletion) {
        let database_url = std::env::var("NIMINO_TEST_DATABASE_URL")
            ...
            .expect("NIMINO_TEST_DATABASE_URL or DATABASE_URL is required");
        ...
        let services = Services {
            store,
            media: Arc::new(
                MediaStorage::new(&nimino_media::MediaConfig {
                    ...
                    s3_addressing_style: nimino_media::S3AddressingStyle::Path,
                    max_image_bytes: 1,
                    max_gif_bytes: 1,
                    max_video_bytes: 1,
                    max_file_bytes: 1,
    ...
    fn deletion_s3_key_pair_normalizes_missing_and_blank_pairs_for_default_credentials() {
        ...
        assert_eq!(
            s3_key_pair_from(env_of(&[
                ("NIMINO_S3_ACCESS_KEY", ""),
                ("NIMINO_S3_SECRET_KEY", "   "),
            ])),
            (String::new(), String::new())
        );
    }
    ...
    fn deletion_s3_key_pair_trims_static_and_preserves_partial_pairs() {
        assert_eq!(
            ...
            ("nimino_dev".to_string(), "nimino_dev_secret".to_string())
        );

        for (env, expected) in [
            (
                &[("NIMINO_S3_ACCESS_KEY", " nimino_dev ")][..],
                ("nimino_dev".to_string(), String::new()),
            ),
            (
                &[("NIMINO_S3_SECRET_KEY", " nimino_dev_secret ")][..],
                (String::new(), "nimino_dev_secret".to_string()),
            ),
            (
                &[
                    ("NIMINO_S3_ACCESS_KEY", " nimino_dev "),
    ...
    fn deletion_test_media_storage() -> Arc<MediaStorage> {
        let endpoint = std::env::var("NIMINO_TEST_S3_ENDPOINT")
            .or_else(|_| std::env::var("NIMINO_S3_ENDPOINT"))
            .expect("NIMINO_TEST_S3_ENDPOINT or NIMINO_S3_ENDPOINT is required");
        let access_key = std::env::var("NIMINO_TEST_S3_ACCESS_KEY")
            .or_else(|_| std::env::var("NIMINO_S3_ACCESS_KEY"))
            .expect("NIMINO_TEST_S3_ACCESS_KEY or NIMINO_S3_ACCESS_KEY is required");
        let secret_key = std::env::var("NIMINO_TEST_S3_SECRET_KEY")
            .or_else(|_| std::env::var("NIMINO_S3_SECRET_KEY"))
            .expect("NIMINO_TEST_S3_SECRET_KEY or NIMINO_S3_SECRET_KEY is required");
        let bucket = std::env::var("NIMINO_TEST_S3_BUCKET")
            .or_else(|_| std::env::var("NIMINO_S3_BUCKET"))
            .expect("NIMINO_TEST_S3_BUCKET or NIMINO_S3_BUCKET is required");
        Arc::new(
            MediaStorage::new(&nimino_media::MediaConfig {
                ...
                s3_bucket: bucket,
                s3_region: std::env::var("NIMINO_TEST_S3_REGION")
                    .or_else(|_| std::env::var("NIMINO_S3_REGION"))
    ...
    async fn frozen_inventory_digest_and_storage_ownership_fail_closed() {
        ...
        let foreign_manifest =
            empty_storage_manifest(nimino_core::CommunityId::from_uuid(Uuid::new_v4()));
    ...
    fn permanent_failures_are_typed_not_string_classified() {
        ...
        let db_permanent = anyhow::Error::from(nimino_db::DbError::DeletionSafety(
            "typed catalog drift".to_string(),
        ));
        let db_transient = anyhow::Error::from(nimino_db::DbError::Sqlx(sqlx::Error::PoolTimedOut));
    ...
    fn deletion_configuration_requires_every_destructive_dependency() {
        let variable = format!("NIMINO_DELETION_REQUIRED_TEST_{}", Uuid::new_v4().simple());
