...
use client::NiminoClient;
use error::CliError;
use nostr::Keys;
use uuid::Uuid;

/// Run the Nimino CLI from raw arguments (including `argv[0]`).
///
...
///
/// ```ignore
/// let code = nimino_cli::run_from_args(std::env::args()).await;
    ...
    S: Into<std::ffi::OsString> + Clone,
{
    ...
    // features across the workspace and enables *both* ring (from nimino-acp/nimino-dev-mcp)
    ...
    // nimino-dev-mcp delegates to run_from_args, it has already installed ring; the
...
#[derive(Subcommand)]
pub enum AgentsCmd {
    /// Open a prefilled create-agent form in the owner's Nimino Desktop
    DraftCreate {
    ...
    /// Open a prefilled edit-agent form in the owner's Nimino Desktop
    DraftUpdate {
...
pub enum MessagesCmd {
    ...
    /// Get the containing thread for a message or Nimino message link
    #[command(
...
pub enum ReposCmd {
    ...
    Create {
        ...
        relays: Vec<String>,
        /// Channel UUID to bind the repo to. The `nimino-channel` tag is the
    ...
    ///
    /// The `nimino-channel` tag on the announcement is the git ACL: the relay
...
pub enum ProjectsCmd {
    ...
    Create {
        ...
        slug: String,
        /// Member repository coordinate: bare Nimino repo id (e.g. `nimino`) or full
        ...
        #[arg(long)]
        description: Option<String>,
        /// Associated Nimino channel UUID
        #[arg(long)]
        channel: Option<String>,
    ...
    Update {
        ...
        /// Set the associated Nimino channel UUID
        #[arg(long, group = "mutation")]
        channel: Option<String>,
...
async fn run(cli: Cli) -> Result<(), CliError> {
    ...
    let (auth_tag, auth_tag_json) = match cli.auth_tag {
        Some(ref input) if !input.is_empty() => {
            ...
            let tag = nimino_sdk::nip_oa::parse_auth_tag(&json)
                .map_err(|e| CliError::Auth(format!("NIMINO_AUTH_TAG is malformed: {e}")))?;
            nimino_sdk::nip_oa::verify_auth_tag(&json, &keys.public_key()).map_err(|e| {
            ...
            // (same shape as nimino-acp's RestClient), never from raw input.
    ...
    let client = NiminoClient::new(relay_url, keys, auth_tag, auth_tag_json)?;
