...
use nimino_core::kind::KIND_PROJECT;
use nimino_sdk::{
...
// ── Nimino repo-ID grammar (bare --repo shorthand) ─────────────────────────────

/// Pattern for a Nimino-hosted repo identifier (bare `--repo` shorthand).
...
async fn fetch_own_project(client: &NiminoClient, slug: &str) -> Result<Option<Event>, CliError> {
...
/// which renders as a rich preview card in Nimino Desktop when included in a
...
pub async fn cmd_update(
    ...
    let singleton_fields = ["name", "description", "nimino-channel", "nimino-visibility"];
            ...
            if let Some(field) = tag_name(t) {
                if singleton_fields.contains(&field) {
                    let clear = match field {
                        ...
                        "nimino-channel" => clear_channel || channel.is_some(),
                        "nimino-visibility" => clear_visibility || visibility.is_some(),
                        _ => false,
                    };
                    return !clear;
                }
    ...
    if let Some(ch) = channel {
        tags.push(make_tag(&["nimino-channel", ch])?);
    }
    if let Some(vis) = visibility {
        tags.push(make_tag(&["nimino-visibility", vis])?);
    }
...
pub async fn cmd_delete(client: &NiminoClient, slug: &str) -> Result<(), CliError> {
    validate_project_slug(slug)?;
...
/// Does NOT impose the Nimino repo-ID grammar — project slugs are more permissive.
...
/// Validate a `nimino-visibility` value at the writer level.
...
pub async fn dispatch(cmd: crate::ProjectsCmd, client: &NiminoClient) -> Result<(), CliError> {
...
#[cfg(test)]
mod tests {
    use nimino_sdk::{validate_project_envelope, PROJECT_MEMBER_CAP};
    use nostr::Tag;
    ...
    fn expand_repo_coord_rejects_uppercase_owner() {
        let upper = "30617:AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA:nimino";
    ...
    fn rebuild_project_preserves_hinted_member_tags() {
        ...
        let coord = format!("30617:{OWNER_HEX}:nimino");
    ...
    fn apply_update_tags(
        ...
        if let Some(ch) = channel {
            tags.push(make_test_tag(&["nimino-channel", ch]));
        }
        if let Some(vis) = visibility {
            tags.push(make_test_tag(&["nimino-visibility", vis]));
        }
        tags
    }
    ...
    fn update_clear_visibility_drops_tag() {
        let head = make_head_tags(&[make_test_tag(&["nimino-visibility", "unlisted"])]);
        let result = apply_update_tags(head, None, false, None, false, None, false, None, true);
        assert!(!result
            .iter()
            .any(|t| tag_name(t) == Some("nimino-visibility")));
    }

    #[test]
    fn update_exactly_one_singleton_after_replace() {
        // Start with a nimino-channel; replace with a new one; must have exactly one.
        let uuid1 = "3580ca9b-47b4-4af9-b22a-1068778f26c6";
        let uuid2 = "00000000-0000-0000-0000-000000000000";
        let head = make_head_tags(&[make_test_tag(&["nimino-channel", uuid1])]);
        let result = apply_update_tags(
            head,
            None,
            false,
        ...
        let channels: Vec<_> = result
            .iter()
            .filter(|t| tag_name(t) == Some("nimino-channel"))
            .collect();
    ...
    fn project_head_at(created_at: u64) -> Event {
        ...
        let tags = vec![
            make_test_tag(&["d", "platform"]),
            make_test_tag(&["a", &format!("30617:{OWNER_HEX}:nimino")]),
        ];
    ...
    async fn empty_update_returns_usage_error_before_any_network_call() {
        ...
        let client =
            crate::client::NiminoClient::new("http://127.0.0.1:9".into(), keys, None, None)
    ...
    fn discard_client() -> crate::client::NiminoClient {
        let keys = nostr::Keys::generate();
        crate::client::NiminoClient::new("http://127.0.0.1:9".into(), keys, None, None)
            .expect("client construction")
    }
    ...
    // (step 7: nimino already present → exit=5). No relay mock is available
