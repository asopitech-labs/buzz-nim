...
use crate::client::{normalize_write_response, NiminoClient};
use crate::error::CliError;
use nimino_sdk::CustomEmoji;
...
const CUSTOM_EMOJI_SET_D_TAG: &str = nimino_sdk::CUSTOM_EMOJI_SET_D_TAG;
...
/// own kind:30030 set (d=`nimino:custom-emoji`).
async fn cmd_list(client: &NiminoClient) -> Result<(), CliError> {
    let filter = serde_json::json!({
        "kinds": [nimino_sdk::kind::KIND_EMOJI_SET],
...
async fn fetch_own_emoji(client: &NiminoClient) -> Result<Vec<CustomEmoji>, CliError> {
...
async fn publish_own_set(client: &NiminoClient, emojis: &[CustomEmoji]) -> Result<(), CliError> {
    let builder = nimino_sdk::build_custom_emoji_set(emojis)
...
async fn cmd_set(client: &NiminoClient, shortcode: &str, url: &str) -> Result<(), CliError> {
    let normalized = nimino_sdk::normalize_custom_emoji_shortcode(shortcode)
...
async fn cmd_rm(client: &NiminoClient, shortcode: &str) -> Result<(), CliError> {
    let normalized = nimino_sdk::normalize_custom_emoji_shortcode(shortcode)
        .map_err(|e| CliError::Other(format!("invalid shortcode: {e}")))?;
    let mut emojis = fetch_own_emoji(client).await?;
...
pub async fn dispatch(cmd: crate::EmojiCmd, client: &NiminoClient) -> Result<(), CliError> {
    use crate::EmojiCmd;
    match cmd {
