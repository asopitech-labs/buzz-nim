...
fn resolve_owner(client: &NiminoClient, owner_flag: Option<&str>) -> Result<PublicKey, CliError> {
...
async fn submit_engram(client: &NiminoClient, event: nostr::Event) -> Result<(), CliError> {
...
pub async fn dispatch(cmd: crate::MemCmd, client: &NiminoClient) -> Result<(), CliError> {
    use crate::MemCmd;
    match cmd {
...
mod tests {
    ...
    fn test_client(keys: nostr::Keys) -> NiminoClient {
        NiminoClient::new("http://127.0.0.1:9".into(), keys, None, None).unwrap()
    }
