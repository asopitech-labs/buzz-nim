...
//! a crate, since nimino-cli and desktop-tauri are independent crates and the
...
const PROD_BUNDLE_IDENTIFIER: &str = "xyz.block.nimino.app";
...
/// `<platform-data-dir>/xyz.block.nimino.app/templates/channel-templates.json`.
...
fn load_templates(path: &Path) -> Result<Vec<ChannelTemplateRecord>, CliError> {
    if !path.exists() {
        return Err(CliError::NotFound(format!(
            "no channel templates store found at {} (create a template in Nimino Desktop first, \
...
mod tests {
    ...
    fn resolve_templates_path_defaults_to_prod_bundle() {
        ...
        assert!(path.ends_with("xyz.block.nimino.app/templates/channel-templates.json"));
    }

    #[test]
    fn find_template_matches_case_insensitive() {
        let f =
            write_store(r#"[{"id":"t1","name":"Nimino Team","createdAt":"x","updatedAt":"x"}]"#);
        let t = find_template(f.path(), "nimino team").expect("found");
        assert_eq!(t.name, "Nimino Team");
    ...
    fn load_templates_parses_full_roster() {
        let f = write_store(
            r##"[{
                "id":"t1","name":"Nimino Team","channel_type":"forum","visibility":"private",
