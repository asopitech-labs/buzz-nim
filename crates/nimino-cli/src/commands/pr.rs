...
use nimino_sdk::{GitPrUpdateMeta, GitPullRequestMeta, GitRepoCoord, GitStatusMeta};
...
pub async fn cmd_open_pr(
    ...
    let builder = with_git_provenance(
        nimino_sdk::build_git_pull_request(&repo, &content, &meta).map_err(sdk_err)?,
    )?;
...
pub async fn cmd_update_pr(
    ...
    let builder = with_git_provenance(
        nimino_sdk::build_git_pr_update(&repo, &content, &meta).map_err(sdk_err)?,
    )?;
    let event = client.sign_event(builder)?;
    let resp = client.submit_event(event).await?;
    println!("{resp}");
...
pub async fn cmd_get_pr(client: &NiminoClient, event: &str) -> Result<(), CliError> {
...
pub async fn cmd_pr_status(
    ...
    let builder = with_git_provenance(
        nimino_sdk::build_git_status(status, &content, &meta).map_err(sdk_err)?,
    )?;
    let event = client.sign_event(builder)?;
    let resp = client.submit_event(event).await?;
    println!("{resp}");
...
pub async fn dispatch(cmd: crate::PrCmd, client: &NiminoClient) -> Result<(), CliError> {
    use crate::PrCmd;
    match cmd {
        PrCmd::Open {
            repo_owner,
