...
Agent-first command-line interface for Nimino relay. JSON in, JSON out.

## Install

```bash
cargo install --path crates/nimino-cli
```
...
# Reactions
nimino reactions add --event <event-id> --emoji "👍"
nimino reactions get --event <event-id>
...
# Canvas
nimino canvas get --channel <uuid>
nimino canvas set --channel <uuid> --content "# Welcome"
...
```
nimino <group> <subcommand> [flags]
    │
    ├─ main.rs ──▶ commands/*.rs ──▶ client.rs ──▶ Nimino Relay REST API
