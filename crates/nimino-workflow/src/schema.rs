...
pub fn parse_yaml(yaml: &str) -> Result<(WorkflowDef, String), WorkflowError> {
    let (def, json) = parse_yaml_codec(yaml)?;
    def.validate()?;
    Ok((def, json))
}

/// Decode YAML and produce canonical JSON without choosing product policy.
/// Production callers pass the returned definition to the Nim policy worker.
pub fn parse_yaml_codec(yaml: &str) -> Result<(WorkflowDef, String), WorkflowError> {
