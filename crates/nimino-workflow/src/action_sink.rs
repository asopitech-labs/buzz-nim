...
pub trait ActionSink: Send + Sync {
    ...
    /// Persist an approval gate and publish its kind:46010 notification.
    ///
    /// The implementation owns token generation and the DB/event transaction so
    /// a crash cannot leave a waiting run without a discoverable approval.
