#![deny(unsafe_code)]
#![warn(missing_docs)]
//! `nimino-workflow` — Workflow engine for Nimino.
//!
...
//!
//! ```rust,ignore
//! let engine = Arc::new(WorkflowEngine::new_with_policy(db, config, policy));
//!
...
use dashmap::DashMap;
use nimino_boundary::{
    BoundaryClient, BoundaryRequest, BoundaryResult, CallContext, WorkflowDirective,
    WorkflowPolicyError, WorkflowPolicyRequest, WorkflowPolicyResult, WorkflowPortEffect,
    WorkflowRunState, WorkflowRunStatus, WorkflowTransitionCommand, WorkflowTransitionRequest,
};
use nimino_core::kind::{event_kind_u32, is_workflow_execution_kind, KIND_REACTION};
use nimino_core::tenant::CommunityId;
use nimino_db::workflow::RunStatus;
use nimino_db::Db;
...
pub struct WorkflowEngine {
    ...
    /// Production write-before-effect adapter.
    pub(crate) effect_ledger: OnceLock<effect_ledger::EffectLedgerAdapter>,
    effects_required: bool,
    ...
    pub(crate) workflow_cache:
        moka::sync::Cache<(CommunityId, Uuid), Arc<Vec<nimino_db::workflow::WorkflowRecord>>>,
}

impl WorkflowEngine {
    /// Construct an engine without a policy client for non-workflow relay tests.
    /// Any attempted policy decision fails closed.
    #[doc(hidden)]
    pub fn new_without_policy_for_tests(db: Db, config: WorkflowConfig) -> Self {
        Self::build(db, config, None, false)
    }

    /// Create the production engine with its supervised Nim policy client.
    pub fn new_with_policy(db: Db, config: WorkflowConfig, policy: BoundaryClient) -> Self {
        Self::build(db, config, Some(policy), true)
    }
    ...
    ) -> Result<WorkflowPolicyResult, WorkflowError> {
        let Some(policy) = &self.policy else {
            return Err(WorkflowError::InvalidDefinition(
                "Nim workflow policy is not configured".to_owned(),
            ));
        };
        match policy
            .call(
                BoundaryRequest::workflow_policy(request),
                CallContext::with_timeout(std::time::Duration::from_secs(2)),
            )
            .await
            .map_err(|error| WorkflowError::InvalidDefinition(error.to_string()))?
        {
            BoundaryResult::WorkflowPolicy(result) => Ok(result),
            _ => Err(WorkflowError::InvalidDefinition(
                "Nim worker returned an unexpected workflow result".to_owned(),
            )),
        }
    }

    /// Validate a decoded definition through the Nim policy and return whether
    /// the action set requires owner/admin authority.
    pub async fn validate_definition_policy(
        &self,
        def: &WorkflowDef,
    ) -> Result<bool, WorkflowError> {
        match self
            .call_policy(WorkflowPolicyRequest::Definition {
                definition: policy::definition(def),
            })
            .await?
        {
            WorkflowPolicyResult::Definition {
                valid: true,
                error: WorkflowPolicyError::None,
                requires_elevated_authority,
            } => Ok(requires_elevated_authority),
            WorkflowPolicyResult::Definition { error, .. } => {
                Err(WorkflowError::InvalidDefinition(format!(
                    "Nim workflow policy rejected definition ({error:?})"
                )))
            }
            _ => Err(WorkflowError::InvalidDefinition(
                "Nim worker returned an unexpected definition result".to_owned(),
            )),
        }
    }

    pub(crate) async fn evaluate_condition_policy(
        &self,
        expression: &str,
        ...
        match self
            .call_policy(WorkflowPolicyRequest::Condition {
                expression: expression.to_owned(),
                values: policy::condition_values(trigger, outputs),
            })
            .await?
        {
            WorkflowPolicyResult::Condition {
                value,
                error: WorkflowPolicyError::None,
            } => Ok(value),
            WorkflowPolicyResult::Condition { error, .. } => Err(WorkflowError::ConditionError(
                format!("Nim workflow condition rejected input ({error:?})"),
            )),
            _ => Err(WorkflowError::ConditionError(
                "Nim worker returned an unexpected condition result".to_owned(),
            )),
        }
    }

    pub(crate) async fn plan_step_policy(
        &self,
        def: &WorkflowDef,
        trigger: &executor::TriggerContext,
        outputs: &HashMap<String, serde_json::Value>,
        step_index: usize,
    ) -> Result<policy::PlannedStep, WorkflowError> {
        match self
            .call_policy(WorkflowPolicyRequest::Plan {
                request: policy::plan_request(def, trigger, outputs, step_index),
            })
            .await?
        {
            WorkflowPolicyResult::Plan {
                directive: WorkflowDirective::SkipStep,
                error: WorkflowPolicyError::None,
                ..
            } => Ok(policy::PlannedStep::Skip),
            WorkflowPolicyResult::Plan {
                directive: WorkflowDirective::CompleteRun,
                error: WorkflowPolicyError::None,
                ..
            } => Ok(policy::PlannedStep::Complete),
            WorkflowPolicyResult::Plan {
                directive: WorkflowDirective::ExecuteEffect,
                error: WorkflowPolicyError::None,
                step_id,
                effect: Some(effect),
            } if def
                .steps
                .get(step_index)
                .is_some_and(|step| step.id == step_id) =>
            {
                Ok(policy::PlannedStep::Execute(policy::adapter_action(
                    *effect,
                )))
            }
            WorkflowPolicyResult::Plan { error, .. } => Err(WorkflowError::InvalidDefinition(
                format!("Nim workflow planner rejected step ({error:?})"),
            )),
            _ => Err(WorkflowError::InvalidDefinition(
                "Nim worker returned an unexpected workflow plan".to_owned(),
            )),
        }
    }

    /// Apply one durable workflow transition decided by the supervised Nim core.
    ...
    pub async fn transition_run(
        ...
        failure: Option<nimino_db::workflow::WorkflowRunFailure<'_>>,
    ) -> Result<(), WorkflowError> {
        let step_count = u32::try_from(step_count).map_err(|_| {
            WorkflowError::InvalidDefinition("workflow has too many steps".to_owned())
        })?;
        let step_index = u32::try_from(step_index).map_err(|_| {
            WorkflowError::InvalidDefinition("workflow step index is too large".to_owned())
        })?;
        let transition_id_owned = transition_id.to_owned();
        let outcome = self
            .db
            .transition_workflow_run(
                ...
                failure,
                |facts| async move {
                    let current_step = u32::try_from(facts.current_step).map_err(|_| {
                        nimino_db::DbError::InvalidData(
                            "workflow run has a negative current step".to_owned(),
                        )
                    })?;
                    let revision = u64::try_from(facts.revision).map_err(|_| {
                        nimino_db::DbError::InvalidData(
                            "workflow run has a negative revision".to_owned(),
                        )
                    })?;
                    let result = self
                        .call_policy(WorkflowPolicyRequest::Transition {
                            request: WorkflowTransitionRequest {
                                state: WorkflowRunState {
                                    status: boundary_run_status(&facts.status),
                                    current_step,
                                    revision,
                                },
                                expected_revision: revision,
                                transition_id: transition_id_owned,
                                transition_already_applied: facts.transition_already_applied,
                                command,
                                step_count,
                                step_index,
                            },
                        ...
                        .await
                        .map_err(|error| nimino_db::DbError::InvalidData(error.to_string()))?;
                    match result {
                        WorkflowPolicyResult::Transition {
                            allowed: true,
                            error: WorkflowPolicyError::None,
                            next_state,
                            port_effect: WorkflowPortEffect::PersistTransition,
                        } => Ok(nimino_db::workflow::WorkflowTransitionAction::Persist(
                            nimino_db::workflow::WorkflowTransitionState {
                                status: db_run_status(next_state.status),
                                current_step: i32::try_from(next_state.current_step).map_err(
                                    |_| {
                                        nimino_db::DbError::InvalidData(
                                            "Nim returned an oversized workflow step".to_owned(),
                                        )
                                    },
                                )?,
                                revision: i64::try_from(next_state.revision).map_err(|_| {
                                    nimino_db::DbError::InvalidData(
                                        "Nim returned an oversized workflow revision".to_owned(),
                                    )
                                })?,
                            },
                        )),
                        WorkflowPolicyResult::Transition { allowed: false, .. } => {
                            Ok(nimino_db::workflow::WorkflowTransitionAction::Reject)
                        }
                        _ => Err(nimino_db::DbError::InvalidData(
                            "Nim returned an invalid workflow transition result".to_owned(),
                        )),
                    }
                },
            )
        ...
        match outcome {
            nimino_db::workflow::WorkflowTransitionOutcome::Applied
            | nimino_db::workflow::WorkflowTransitionOutcome::Duplicate => Ok(()),
            nimino_db::workflow::WorkflowTransitionOutcome::Rejected => Err(
                WorkflowError::InvalidDefinition("Nim rejected workflow transition".to_owned()),
            ),
        }
    }
    ...
    pub async fn check_owner_authority(
        ...
        let requires_elevated = self.validate_definition_policy(def).await?;
        if owner_authority_allows(role.as_deref(), requires_elevated) {
            Ok(())
        } else {
    ...
    /// Installs the production canonical effect ledger and lease adapter once.
    pub fn set_effect_ledger(
        &self,
        store: Arc<dyn nimino_store::NodeStorePort>,
        lease: nimino_control::LeaseClient,
    ) -> Result<(), WorkflowError> {
        let policy = self.policy.clone().ok_or_else(|| {
            WorkflowError::InvalidDefinition("Nim workflow policy is not configured".into())
        })?;
        self.effect_ledger
            .set(effect_ledger::EffectLedgerAdapter::new(
                policy, store, lease,
            ))
            .map_err(|_| {
                WorkflowError::InvalidDefinition("effect ledger already initialized".into())
            })
    }

    pub(crate) fn effect_ledger(
        &self,
    ) -> Result<&effect_ledger::EffectLedgerAdapter, WorkflowError> {
        self.effect_ledger.get().ok_or_else(|| {
            WorkflowError::InvalidDefinition("effect ledger is not initialized".into())
        })
    }
    ...
    pub(crate) async fn prepare_effect(
        ...
        step_id: &str,
        action: &ActionDef,
    ) -> Result<Option<effect_ledger::EffectPermit>, WorkflowError> {
        match self.effect_ledger.get() {
            Some(adapter) => adapter
                .prepare(effect_ledger::EffectIdentity {
                    community_id,
                    workflow_id,
                    run_id,
                    step_id,
                    resolved_effect: action,
                })
                .await
                .map(Some),
            None if !self.effects_required => Ok(None),
            None => Err(WorkflowError::InvalidDefinition(
                "effect ledger is not initialized".into(),
            )),
        }
    }

    pub(crate) async fn record_effect<T: std::fmt::Debug>(
        &self,
        permit: Option<effect_ledger::EffectPermit>,
        result: &Result<T, WorkflowError>,
    ) -> Result<(), WorkflowError> {
        if let Some(permit) = permit {
            self.effect_ledger()?.record(permit, result).await?;
        }
        Ok(())
    }
    ...
    pub async fn finalize_run(
        ...
        match result {
                ...
                if result.approval_token.is_some() {
                    ...
                    if let Err(e) = self
                        .transition_run(
                            ...
                            run_id,
                            "complete",
                            WorkflowTransitionCommand::Complete,
                            result.step_count,
                            result.step_count,
                            &trace_json,
                            None,
            ...
            Err((e, progress)) => {
                ...
                let trace_json = serde_json::Value::Array(full_trace);
                let transition_id = format!("fail:{}:{}", progress.step_index, e.code());
                if let Err(db_err) = self
                    .transition_run(
                        community_id,
                        run_id,
                        &transition_id,
                        WorkflowTransitionCommand::Fail,
                        progress.step_index,
                        progress.step_index,
                        &trace_json,
                        Some(nimino_db::workflow::WorkflowRunFailure {
                            code: e.code(),
                            message: &e.to_string(),
                        }),
                    )
    ...
    pub async fn on_event(
        ...
        for workflow in workflows.iter() {
            ...
            if !should_fire_workflow(self, &def, &trigger_ctx, workflow.id).await {
                continue;
            }
...
fn boundary_run_status(status: &RunStatus) -> WorkflowRunStatus {
    match status {
        RunStatus::Pending => WorkflowRunStatus::Pending,
        RunStatus::Running => WorkflowRunStatus::Running,
        RunStatus::WaitingApproval => WorkflowRunStatus::WaitingApproval,
...
fn db_run_status(status: WorkflowRunStatus) -> RunStatus {
    match status {
        WorkflowRunStatus::Pending => RunStatus::Pending,
        WorkflowRunStatus::Running => RunStatus::Running,
        WorkflowRunStatus::WaitingApproval => RunStatus::WaitingApproval,
...
async fn should_fire_workflow(
    ...
    if let Some(expr) = filter {
        let decision = engine
            .evaluate_condition_policy(expr, trigger_ctx, &HashMap::new())
            .await;
        match decision {
            Ok(true) => {}
            Ok(false) => {
...
/// Build a [`executor::TriggerContext`] from a [`nimino_core::StoredEvent`].
///
...
pub fn build_trigger_context(event: &nimino_core::StoredEvent) -> executor::TriggerContext {
    let kind_u32 = event_kind_u32(&event.event);
...
/// marker. Delegates to the shared [`nimino_core::nip10`] parser so this stays in
...
fn event_is_reply(event: &nostr::Event) -> bool {
    nimino_core::nip10::parse_thread_markers(&event.tags)
        .reply
        .is_some()
}
...
fn trigger_matches_event(trigger: &TriggerDef, kind_u32: u32) -> bool {
    use nimino_core::kind::{KIND_REACTION, KIND_STREAM_MESSAGE, KIND_STREAM_MESSAGE_DIFF};
    match trigger {
...
"#;
    ...
    fn trigger_matches_stream_message() {
        ...
        assert!(trigger_matches_event(
            &trigger,
            nimino_core::kind::KIND_STREAM_MESSAGE
        ));
        assert!(!trigger_matches_event(
            &trigger,
            nimino_core::kind::KIND_REACTION
        ));
    }
    ...
    fn schedule_trigger_never_matches_events() {
        ...
        assert!(!trigger_matches_event(
            &trigger,
            nimino_core::kind::KIND_WORKFLOW_TRIGGERED
        ));
    }
    ...
    fn workflow_execution_kinds_do_not_match_any_trigger() {
        ...
        for kind in nimino_core::kind::KIND_WORKFLOW_TRIGGERED
            ..=nimino_core::kind::KIND_WORKFLOW_APPROVAL_DENIED
        {
            assert!(
    ...
    fn make_message_event() -> nimino_core::StoredEvent {
        ...
        let event = EventBuilder::new(Kind::Custom(9), "hello world")
            ...
            .sign_with_keys(&keys)
            .expect("sign");
        nimino_core::StoredEvent::new(event, Some(Uuid::new_v4()))
    }

    /// Create a reaction event with an `e` tag pointing to a target message.
    fn make_reaction_event() -> (nimino_core::StoredEvent, String) {
    ...
    fn build_trigger_context_is_reply_true_for_threaded_message() {
        ...
        let event = EventBuilder::new(Kind::Custom(9), "a threaded reply")
            ...
            .sign_with_keys(&keys)
            .expect("sign");
        let stored = nimino_core::StoredEvent::new(event, Some(Uuid::new_v4()));
        let ctx = build_trigger_context(&stored);
    ...
    fn build_trigger_context_no_channel_id() {
        ...
        let stored = nimino_core::StoredEvent::new(event, None);
        let ctx = build_trigger_context(&stored);
    ...
    fn build_trigger_context_ignores_actor_tag() {
        ...
        let event = EventBuilder::new(Kind::Custom(9), "forged actor")
            ...
            .expect("sign");
        let stored = nimino_core::StoredEvent::new(event, Some(uuid::Uuid::new_v4()));
    ...
    async fn setup_db() -> nimino_db::Db {
        ...
        nimino_db::Db::new(&nimino_db::DbConfig {
            database_url,
            ..Default::default()
        })
        .await
    ...
    async fn setup_policy_engine(
        db: nimino_db::Db,
    ) -> (Arc<WorkflowEngine>, nimino_boundary::BoundaryRuntime) {
        let worker = std::env::var_os("NIMINO_BOUNDARY_PRODUCTION_WORKER")
            .expect("NIMINO_BOUNDARY_PRODUCTION_WORKER must be set");
        let runtime =
            nimino_boundary::BoundaryRuntime::start(nimino_boundary::BoundaryConfig::new(worker))
                .await
                .expect("start production Nim worker");
        (
            Arc::new(WorkflowEngine::new_with_policy(
                db,
    ...
    async fn setup_channel(
        ...
        {
            nimino_db::CreateCommunityWithOwnerResult::Created(rec) => rec.id,
        ...
        db.create_channel_with_id(
            ...
            nimino_db::channel::ChannelType::Stream,
            nimino_db::channel::ChannelVisibility::Open,
            None,
            creator,
            None,
        )
        ...
        db.add_member(
            ...
            channel_id,
            member,
            nimino_db::channel::MemberRole::Member,
            Some(creator),
        )
        .await
        .expect("add member");
    ...
    fn message_event(channel_id: Uuid) -> nimino_core::StoredEvent {
        let keys = nostr::Keys::generate();
        let event = nostr::EventBuilder::new(nostr::Kind::Custom(9), "hello")
            .sign_with_keys(&keys)
            .expect("sign");
        nimino_core::StoredEvent::new(event, Some(channel_id))
    }
    ...
    #[tokio::test]
    #[ignore = "requires Postgres and the production Nim worker"]
    async fn on_event_denies_run_after_owner_removed() {
        ...
        let (engine, runtime) = setup_policy_engine(db.clone()).await;
        ...
        runtime
            .shutdown()
            .await
            .expect("stop production Nim worker");
    }
    ...
    #[tokio::test]
    #[ignore = "requires Postgres and the production Nim worker"]
    async fn nim_transition_policy_commits_once_through_db_cas() {
        let db = setup_db().await;
        ...
        let runtime =
            nimino_boundary::BoundaryRuntime::start(nimino_boundary::BoundaryConfig::new(worker))
                .await
                .expect("start production Nim worker");
        let engine = WorkflowEngine::new_with_policy(
            db.clone(),
            WorkflowConfig::default(),
            runtime.client(),
        );
        ...
        engine
            .transition_run(
                ...
                run_id,
                "start",
                WorkflowTransitionCommand::Start,
                1,
                0,
                &trace,
                None,
        ...
        engine
            .transition_run(
                ...
                run_id,
                "effect:0",
                WorkflowTransitionCommand::EffectCompleted,
                1,
                0,
                &trace,
                None,
        ...
        engine
            ...
            .await
            .expect("duplicate transition is idempotent");
        engine
            .transition_run(
                community,
                run_id,
        ...
        let run = db
            ...
            .await
            .expect("read transitioned run");
        assert_eq!(run.status, RunStatus::Completed);
        assert_eq!(run.current_step, 1);
        assert_eq!(run.revision, 3, "duplicate must not advance revision");
        runtime
            .shutdown()
            .await
            .expect("stop production Nim worker");
