...
use nimino_boundary::WorkflowTransitionCommand;
...
pub async fn dispatch_action(
    ...
    let workflow_id = engine
        ...
        .workflow_id;
    let permit = engine
        .prepare_effect(community_id, workflow_id, run_id, step_id, action)
        .await?;
    let result = async {
            match action {
                SendMessage {
        ...
        .await;
    engine.record_effect(permit, &result).await?;
    result
}
...
// is_private_ip is provided by nimino_core::network::is_private_ip
...
async fn check_ssrf(host: &str, port: u16) -> Result<std::net::IpAddr, WorkflowError> {
    ...
    for ip in &addrs {
        if nimino_core::network::is_private_ip(ip) {
...
pub struct ExecutionResult {
    ...
    /// Total number of steps in the validated definition.
...
pub async fn execute_from_step(
    ...
    let durable_step = usize::try_from(run.current_step).unwrap_or_default();
    // Fail fast if all concurrency permits are in use — no queuing.
    let _permit = engine.run_semaphore.try_acquire().map_err(|_| {
        (
            WorkflowError::CapacityExceeded,
            crate::error::PartialProgress {
                step_index: durable_step,
                trace: Vec::new(),
            },
        )
    })?;
    ...
    let (transition_id, command, transition_step) = match run.status {
        nimino_db::workflow::RunStatus::Pending if start_index == 0 => {
            ("start".to_owned(), WorkflowTransitionCommand::Start, 0)
        }
        nimino_db::workflow::RunStatus::WaitingApproval
            if usize::try_from(run.current_step)
                .ok()
                .and_then(|step| step.checked_add(1))
                == Some(start_index) =>
        {
            (
                format!("resume:{}", run.current_step),
                WorkflowTransitionCommand::Resume,
                usize::try_from(run.current_step).unwrap_or_default(),
            )
        }
        _ => {
            return Err((
                WorkflowError::InvalidDefinition(format!(
                    "cannot execute workflow from step {start_index} while run is {}:{}",
                    run.status, run.current_step
                )),
                crate::error::PartialProgress {
                    step_index: usize::try_from(run.current_step).unwrap_or_default(),
                    trace: Vec::new(),
                },
            ));
        }
    ...
    engine
        .transition_run(
            ...
            def.steps.len(),
            transition_step,
            &JsonValue::Array(existing_trace.clone()),
            None,
        )
        .await
        .map_err(|e| {
...
async fn execute_steps(
    ...
    start_index: usize,
    initial_outputs: Option<HashMap<String, JsonValue>>,
    durable_trace_prefix: &[JsonValue],
) -> Result<ExecutionResult, (WorkflowError, crate::error::PartialProgress)> {
    ...
    for (i, step) in def.steps.iter().enumerate() {
        ...
        let resolved_action = match engine
            .plan_step_policy(def, trigger_ctx, &step_outputs, i)
            .await
        {
            Ok(crate::policy::PlannedStep::Execute(action)) => action,
            Ok(crate::policy::PlannedStep::Skip) => {
                info!(run_id = %run_id, step = %step.id, "Policy skipped workflow step");
                trace.push(serde_json::json!({
                    "step_id": step.id,
                    "status": "skipped",
                }));
                let durable_trace = combined_trace(durable_trace_prefix, &trace);
                if let Err(e) = engine
                    .transition_run(
                        community_id,
                        run_id,
                        &format!("skip:{i}"),
                        WorkflowTransitionCommand::SkipStep,
                        def.steps.len(),
                        i,
                        &durable_trace,
                        None,
            ...
            Ok(crate::policy::PlannedStep::Complete) => break,
            Err(e) => {
            ...
            StepResult::Suspended { approval_token } => {
                ...
                if let Err(e) = engine
                    .transition_run(
                        ...
                        run_id,
                        &format!("approval:{i}"),
                        WorkflowTransitionCommand::AwaitApproval,
                        def.steps.len(),
                        i,
                        &durable_trace,
                        None,
...
fn combined_trace(prefix: &[JsonValue], trace: &[JsonValue]) -> JsonValue {
    JsonValue::Array(prefix.iter().chain(trace).cloned().collect())
}
...
mod tests {
    ...
    fn trigger_context_get_field_webhook_field() {
        let mut ctx = make_trigger();
        ctx.webhook_fields
            .insert("repo".to_owned(), "nimino".to_owned());
        assert_eq!(ctx.get_field("repo"), Some("nimino"));
    }
