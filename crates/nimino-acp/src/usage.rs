...
//! turn.  Both goose and nimino-agent use this same wire format.  The payload
...
/// ```
///
/// `used` and `contextLimit` are optional because nimino-agent does not track a
...
pub(crate) struct UsageUpdatePayload {
    /// Total tokens used (context-usage proxy). Optional — nimino-agent omits
    /// this field or sends 0 because it does not track a context window limit.
    #[serde(default)]
    #[allow(dead_code)]
    pub used: u64,
    /// Context window size. Optional — nimino-agent omits this field.
    #[serde(default)]
    #[allow(dead_code)]
    pub context_limit: u64,
    /// Session-cumulative inclusive input tokens.
    ///
    /// `None` when nimino-agent omitted the field — this happens when the
    ...
    /// emitted by nimino-agent when every turn in the session so far supplied a
    ...
    #[serde(default)]
    pub pricing_identity: Option<nimino_core::agent_turn_metric::PricingIdentity>,
}
...
pub struct TurnUsage {
    ...
    /// Session-cumulative genuine provider total tokens as reported by nimino-agent;
    ...
    /// Session-cumulative cache-read input tokens as reported by nimino-agent.
    ...
    /// Session-cumulative cache-write tokens as reported by nimino-agent.
...
pub(crate) struct UsageTracker {
    ...
    pending_identity: Option<Option<nimino_core::agent_turn_metric::PricingIdentity>>,
...
impl UsageTracker {
    ...
    /// Seed a zero baseline for a session that nimino-acp just spawned.
    ///
    /// When nimino-acp creates a session itself via `session/new`, the session's
...
mod tests {
    ...
    /// The camelCase key nimino-agent actually puts on the wire must land on the
    ...
    fn notification_deserializes_without_used_and_context_limit() {
        // nimino-agent emits usage_update without used/contextLimit.
        let raw = serde_json::json!({
            "sessionId": "nimino-sess",
            "update": {
                "sessionUpdate": "usage_update",
    ...
    #[test]
    fn nimino_agent_notification_flows_through_tracker() {
        // End-to-end: a nimino-agent-shaped usage_update (no used/contextLimit)
    ...
    #[test]
    fn nimino_agent_payload_no_context_fields_processes_correctly() {
    ...
    fn pool_omitted_cache_field_publishes_no_cache_read_tokens_in_kind44200() {
        // End-to-end: a nimino-agent or goose payload that omits the cache field
    ...
    fn pool_reported_cache_field_publishes_nonzero_cache_read_tokens_in_kind44200() {
        // End-to-end: a nimino-agent payload with a nonzero cache count must
    ...
    fn make_pricing_identity_payload(
        ...
        UsageUpdatePayload {
            ...
            pricing_identity: Some(nimino_core::agent_turn_metric::PricingIdentity {
    ...
    /// Old harnesses (goose, older nimino-agent) that do not emit `pricingIdentity`
    ...
    // The ACP tracker has a DISTINCT multi-notification path: nimino-agent sends
