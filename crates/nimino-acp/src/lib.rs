...
/// name (e.g., `nimino-acp --verbose models`) are not supported.
...
/// Timeout for `nimino-acp authenticate`. Browser-based vendor auth can require
...
/// the desktop client's format. The relay stores this in local delivery and synthesizes
...
fn resolve_agent_owner(config: &Config) -> Option<String> {
    ...
    if let Ok(auth_tag) = std::env::var("NIMINO_AUTH_TAG") {
        if !auth_tag.is_empty() {
            ...
            match nimino_sdk::nip_oa::verify_auth_tag(&auth_tag, &agent_pk) {
                Ok(owner_pk) => {
...
async fn check_sibling_via_profile(
    ...
    for tag in tags {
        ...
        match nimino_sdk::nip_oa::verify_auth_tag(&tag_json, &agent_pk) {
            Ok(_) => {
...
/// keyed to `OBSERVER_MAX_PLAINTEXT_LEN` in nimino-core/observer.rs:25) is what actually
...
/// already-small frame. Reusing that string would mean changing nimino-core's
/// `encrypt_observer_payload` signature or adding a parallel encrypt path; both
/// are out of this change's scope (nimino-core stays untouched). The clean `&mut`
...
async fn publish_relay_observer_event(
    ...
    let builder = match nimino_sdk::build_agent_observer_frame(
        owner_pubkey_hex,
        agent_pubkey_hex,
...
fn handle_relay_observer_control_event(
    ...
    if let Err(e) = nimino_core::verify_event(&event) {
...
async fn tokio_main() -> Result<()> {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            EnvFilter::try_from_default_env().unwrap_or_else(|_| EnvFilter::new("nimino_acp=info")),
        )
        .compact()
        .init();
    ...
    // model, or provider), it spawns nimino-acp with NIMINO_ACP_SETUP_PAYLOAD set.
    ...
    {
        tracing::info!("nimino-acp: setup payload present, entering setup-listener mode");
        return setup_mode::run_setup_listener(config, payload).await;
    }

    tracing::info!("nimino-acp starting: {}", config.summary());
    ...
    let relay_auth_tag: Option<nostr::Tag> = std::env::var("NIMINO_AUTH_TAG")
        .ok()
        .filter(|s| !s.is_empty())
        .and_then(|s| nimino_sdk::nip_oa::parse_auth_tag(&s).ok());
    ...
    loop {
        ...
        let pool_event: Option<PoolEvent> = {
            ...
            tokio::select! {
                ...
                nimino_event = relay.next_event() => {
                    let _ = result_rx; // end split borrow before relay handling
                    match nimino_event {
                        Some(nimino_event) => {
                            let kind_u32 = nimino_event.event.kind.as_u16() as u32;

                            if kind_u32 == KIND_MEMBER_ADDED_NOTIFICATION
                                || kind_u32 == KIND_MEMBER_REMOVED_NOTIFICATION
                            {
                                let ch = nimino_event.channel_id;
                                let ts = nimino_event.event.created_at.as_secs();
                                let eid = nimino_event.event.id.to_hex();
                            ...
                            if config.ignore_self && nimino_event.event.pubkey.to_hex() == pubkey_hex {
                                tracing::debug!(channel_id = %nimino_event.channel_id, "dropping self-authored event");
                                continue;
                            }
                            ...
                            if is_shutdown {
                                ...
                                if let Some(owner) = owner {
                                    if nimino_event.event.pubkey.to_hex() == *owner {
                                        tracing::info!(
                                            channel_id = %nimino_event.channel_id,
                                            sender = %nimino_event.event.pubkey.to_hex(),
                                ...
                                if let Some(owner) = owner_cache.get() {
                                    if nimino_event.event.pubkey.to_hex() == *owner {
                                        ...
                                        if fired {
                                            ...
                                            let invalidated = pool.invalidate_channel_sessions(nimino_event.channel_id);
                                            tracing::info!(
                                                channel_id = %nimino_event.channel_id,
                                                invalidated,
                            ...
                            {
                                let author = nimino_event.event.pubkey.to_hex();
                                // DM hardening: resolve channel type (fail-closed
                                // to DM) so allowlist/anyone modes cannot be
                                // exercised by non-owner authors inside DMs.
                                let is_dm =
                                    is_dm_channel(nimino_event.channel_id, &ctx.channel_info).await;
                                ...
                                if !allowed {
                                    tracing::debug!(
                                        channel_id = %nimino_event.channel_id,
                                        author = %nimino_event.event.pubkey.to_hex(),
                            ...
                            let matched = filter::match_event(&nimino_event.event, nimino_event.channel_id, &rules, &pubkey_hex).await;
                            let prompt_tag = match matched {
                                Some(m) => m.prompt_tag,
                                None => {
                                    tracing::debug!(channel_id = %nimino_event.channel_id, kind = nimino_event.event.kind.as_u16(), "event matched no rule — dropping");
                                    continue;
                                }
                            };
                            // Capture author pubkey before queue.push() moves
                            // nimino_event.event (needed for mode gate below).
                            let author_hex = nimino_event.event.pubkey.to_hex();
                            let event_id_hex = nimino_event.event.id.to_hex();
                            ...
                            let event_for_steer = nimino_event.event.clone();
                            let prompt_tag_for_steer = prompt_tag.clone();
                            let accepted = queue.push(QueuedEvent {
                                channel_id: nimino_event.channel_id,
                                event: nimino_event.event,
                            ...
                            if accepted && queue.is_channel_in_flight(nimino_event.channel_id) {
    ...
    tracing::info!("nimino-acp stopped");
    Ok(())
}
...
fn try_native_steer(
    ...
    // `queue::format_prompt` uses internally for `[Nimino event: …]`
...
fn handle_prompt_result(
    ...
    // which live in nimino-agent's session state and are what `llm: (model) …`
...
mod agent_draft_prompt_tests {
    ...
    fn shared_base_prompt_teaches_single_command_mentions_and_preflight() {
        let prompt = include_str!("base_prompt.md");
        assert!(prompt.contains("use the person's **exact display name as shown in Nimino**"));
...
/// `nimino-acp auth-methods` — spawn an adapter, initialize it, print authMethods.
...
/// `nimino-acp authenticate` — invoke one adapter-owned auth method.
...
mod build_mcp_servers_tests {
    ...
    fn test_config() -> Config {
        Config {
            ...
            config_path: std::path::PathBuf::from("./nimino-acp.toml"),
    ...
    #[test]
    fn session_new_mcp_server_forwards_nimino_auth_tag() {
    ...
    #[test]
    fn session_new_mcp_server_skips_empty_nimino_auth_tag() {
        let _guard = ENV_LOCK.lock().unwrap();
...
mod observer_payload_trim_tests {
    ...
    fn test_multi_block_prompt_retains_every_section_header_after_elision() {
        ...
        // {text: "[Agent Memory — core]…"}, … {text: "[Nimino event: …]…<huge>"}].
        ...
        // [Nimino event] header fell into the elided middle and the count collapsed
        // to 1).
        let sections = [
        ...
        let event_block = texts
            .iter()
            .find(|t| t.starts_with("[Nimino event: @mention]"))
            .unwrap();
        assert!(
