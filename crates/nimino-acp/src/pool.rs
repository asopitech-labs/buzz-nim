...
pub struct ChannelDeliveryState {
    ...
    /// Nimino event IDs already delivered to this ACP session, either as trigger
...
pub struct OwnedAgent {
    ...
    /// Persisted startup effort value from `NIMINO_ACP_EFFORT_LEVEL` (carried from
...
pub struct PromptContext {
    ...
    /// `--no-memory` / `NIMINO_ACP_NO_MEMORY`.
    pub memory_enabled: bool,
    /// Harness identity string for NIP-AM `harness` field. Derived from the
    /// configured `agent_command` at startup (e.g. `"goose"`, `"nimino-agent"`).
...
async fn create_session_and_apply_model(
    ...
    // application above. The held value comes from `NIMINO_ACP_EFFORT_LEVEL` and
...
fn mcp_servers_with_git_origin(
    ...
    let origin = match (channel_id, channel_type) {
        (Some(channel_id), Some("stream")) => Some(EnvVar {
            name: "NIMINO_GIT_ORIGIN_CHANNEL_ID".into(),
            value: channel_id.to_string(),
        }),
        (Some(_), _) => agent_name
            .filter(|name| !name.trim().is_empty())
            .map(|name| EnvVar {
                name: "NIMINO_GIT_ORIGIN_AGENT_NAME".into(),
...
pub async fn run_prompt_task(
    ...
    //
    // Operator opt-out: `--no-memory` / `NIMINO_ACP_NO_MEMORY` skips the fetch.
            ...
            if let Some(sid) = agent.state.sessions.get(cid) {
                ...
                .await
                {
                    Ok(sid) => {
                        ...
                        // Seed a zero usage baseline: nimino-acp spawned this session
    ...
    // (`prompt[0].text.startsWith("/")`) fires; the wrapped Nimino context
                ...
                mode = rx => {
                    ...
                    if agent.acp.has_in_flight_prompt() {
                            ...
                            .await
                        {
                            Ok(stop_reason) => {
                                ...
                                publish_agent_turn_metric(
                                    ...
                                    &session_id,
                                    &turn_id,
                                    Some(nimino_core::agent_turn_metric::StopReason::Cancelled),
                                )
                                .await;
                                send_prompt_result(
                                    &result_tx,
                            ...
                            Err(error) => {
                                ...
                                publish_agent_turn_metric(
                                    ...
                                    &session_id,
                                    &turn_id,
                                    Some(nimino_core::agent_turn_metric::StopReason::Error),
                                )
                                .await;
                                send_prompt_result(
                                    &result_tx,
                        ...
                        publish_agent_turn_metric(
                            ...
                            &session_id,
                            &turn_id,
                            Some(nimino_core::agent_turn_metric::StopReason::EndTurn),
                        )
                        .await;
                        send_prompt_result(
                            &result_tx,
...
pub(crate) async fn fetch_channel_info(
    ...
    let filter = nostr::Filter::new()
        .kind(nostr::Kind::Custom(
            nimino_core::kind::KIND_NIP29_GROUP_METADATA as u16,
        ))
...
async fn fetch_huddle_instructions(
    ...
    let filter = nostr::Filter::new()
        .kind(nostr::Kind::Custom(
            nimino_core::kind::KIND_HUDDLE_GUIDELINES as u16,
        ))
        .author(*owner)
...
fn huddle_instructions_from_query_response(
    ...
    if event.pubkey != *owner
        || event.kind.as_u16() as u32 != nimino_core::kind::KIND_HUDDLE_GUIDELINES
        || !event
            .tags
            .iter()
...
async fn fetch_canvas_section(channel_id: Uuid, rest: &RestClient) -> Option<String> {
    ...
    let h_tag = SingleLetterTag::lowercase(Alphabet::H);
    let filter = nostr::Filter::new()
        .kind(nostr::Kind::Custom(nimino_core::kind::KIND_CANVAS as u16))
        .custom_tags(h_tag, [channel_id.to_string()])
        .limit(1);
...
pub(crate) fn canvas_section_from_query_response(
    ...
    if event.kind != nostr::Kind::Custom(nimino_core::kind::KIND_CANVAS as u16) {
        tracing::warn!(
            target: "canvas::fetch",
            channel = %channel_uuid,
...
pub(crate) fn render_canvas_section(event_id: &str, timestamp: &str, channel_uuid: &str) -> String {
    format!(
        "[Channel Canvas]\n\
         ...
         Fetch current content with: nimino canvas get --channel {channel_uuid}"
    )
}
...
/// IDs are compared in Nimino's canonical 64-character lowercase hex form: relay
    ...
    CountFut: std::future::Future<Output = Result<serde_json::Value, crate::relay::RelayError>>,
{
    ...
    let replies_filter = nostr::Filter::new()
        .kinds([
            nostr::Kind::Custom(nimino_core::kind::KIND_STREAM_MESSAGE as u16),
            nostr::Kind::Custom(nimino_core::kind::KIND_STREAM_MESSAGE_V2 as u16),
        ])
...
fn delivery_receipt_line(channel_id: Uuid, event_ids: &HashSet<String>) -> String {
    ...
    format!(
        "turn delivered Nimino events for channel {channel_id}: {}",
        event_ids.join(",")
    )
}
...
fn acp_stop_to_core(r: &StopReason) -> nimino_core::agent_turn_metric::StopReason {
    use nimino_core::agent_turn_metric::StopReason as CoreStop;
    match r {
...
pub(crate) fn build_turn_metric_counts(
    ...
    Option<nimino_core::agent_turn_metric::TokenCounts>,
    Option<nimino_core::agent_turn_metric::TokenCounts>,
) {
    use nimino_core::agent_turn_metric::TokenCounts;
    ...
    let cumulative_counts = Some(TokenCounts {
        ...
        // reported this field (e.g. goose or older nimino-agent sessions).
...
async fn publish_agent_turn_metric(
    ...
    session_id: &str,
    turn_id: &str,
    stop_reason: Option<nimino_core::agent_turn_metric::StopReason>,
) {
    use nimino_core::agent_turn_metric::AgentTurnMetricPayload;
    ...
    let ciphertext = match nimino_core::agent_turn_metric::encrypt_agent_turn_metric(
        &ctx.agent_keys,
        owner_pk,
        &payload,
    ) {
    ...
    let event = match EventBuilder::new(
        Kind::Custom(nimino_core::kind::KIND_AGENT_TURN_METRIC as u16),
        ciphertext,
    )
    .tags([
...
///
/// Builds a reaction event with `nimino_sdk::build_reaction`, signs it with
...
pub(crate) async fn reaction_add(rest: &crate::relay::RestClient, event_id: &str, emoji: &str) {
    ...
    let builder = match nimino_sdk::build_reaction(target_id, emoji) {
        Ok(b) => b,
        Err(e) => {
...
pub(crate) async fn post_failure_notice(
    ...
    let thread_ref = thread_tags.root_event_id.as_deref().and_then(|root| {
        ...
        let parent_id = thread_tags
            ...
            .unwrap_or(root_id);
        Some(nimino_sdk::ThreadRef {
            root_event_id: root_id,
            parent_event_id: parent_id,
        })
    });
    let builder = match nimino_sdk::build_message(
        channel_id,
        content,
        thread_ref.as_ref(),
        &[],
...
pub(crate) async fn reaction_remove(rest: &crate::relay::RestClient, event_id: &str, emoji: &str) {
    ...
    let builder = match nimino_sdk::build_remove_reaction(target_id) {
        Ok(b) => b,
        Err(e) => {
...
mod tests {
    ...
    fn delivery_receipt_line_sorts_event_ids() {
        ...
        assert_eq!(
            delivery_receipt_line(channel_id, &event_ids),
            format!("turn delivered Nimino events for channel {channel_id}: alpha,beta")
        );
    }
    ...
    fn public_session_forwards_channel_origin_to_mcp() {
        ...
        assert!(servers[0].env.iter().any(|entry| {
            entry.name == "NIMINO_GIT_ORIGIN_CHANNEL_ID" && entry.value == channel_id.to_string()
        }));
        assert!(!servers[0]
            .env
            .iter()
            .any(|entry| entry.name == "NIMINO_GIT_ORIGIN_AGENT_NAME"));
    }
    ...
    fn private_session_forwards_agent_name_without_channel_id() {
        ...
        assert!(servers[0].env.iter().any(|entry| {
            entry.name == "NIMINO_GIT_ORIGIN_AGENT_NAME" && entry.value == "Builder"
        }));
        assert!(!servers[0]
            .env
            .iter()
            .any(|entry| entry.name == "NIMINO_GIT_ORIGIN_CHANNEL_ID"));
    }
    ...
    fn goose_uses_system_prompt_only_after_custom_method_succeeds() {
        ...
        assert!(has_system_prompt_support(2, "nimino-agent", None));
        ...
        assert_eq!(
            session_new_system_prompt(false, 2, "nimino-agent", Some("instructions")),
    ...
    fn test_workspace_section_preserves_windows_cwd() {
        assert_eq!(
            workspace_section(r"C:\Users\me\nimino"),
            "[Workspace]\nCurrent working directory: C:\\Users\\me\\nimino"
        );
    }
    ...
    async fn run_prompt_task_commits_standing_context_only_after_acp_success() {
        let capture = std::env::temp_dir().join(format!(
            "nimino-acp-standing-lifecycle-{}.ndjson",
            Uuid::new_v4()
        ));
...
while IFS= read -r line; do
  ...
  if [ "$count" -eq 1 ]; then
    ...
    async fn channel_prompt_commits_delivery_state_only_after_acp_success() {
        let capture = std::env::temp_dir().join(format!(
            "nimino-acp-channel-delivery-lifecycle-{}.ndjson",
            Uuid::new_v4()
        ));
        let quoted_capture = capture.to_string_lossy().replace('\'', "'\\''");
        let script = format!(
...
while IFS= read -r line; do
  ...
  if [ "$count" -eq 1 ]; then
    ...
    async fn merged_cancel_prompt_commits_and_deduplicates_all_rendered_event_ids() {
        ...
        let capture = std::env::temp_dir().join(format!(
            "nimino-acp-merged-delivery-wire-{}.ndjson",
            Uuid::new_v4()
        ));
        let quoted_capture = capture.to_string_lossy().replace('\'', "'\\''");
        let script = format!(
...
while IFS= read -r line; do
  ...
  count=$((count + 1))
    ...
    async fn late_successful_steer_ack_excludes_event_from_next_channel_wire_prompt() {
        ...
        let capture = std::env::temp_dir().join(format!(
            "nimino-acp-late-steer-wire-{}.ndjson",
            Uuid::new_v4()
        ));
        let quoted_capture = capture.to_string_lossy().replace('\'', "'\\''");
        let script = format!(
...
printf '%s\n' '{{"jsonrpc":"2.0","id":0,"result":{{"stopReason":"end_turn"}}}}'"#
    ...
    /// A nimino-agent-commanded context must not panic — verifies the harness
    /// field flows through encrypt/sign without error.
    #[tokio::test]
    async fn test_publish_agent_turn_metric_nimino_agent_harness_name() {
        let agent_keys = nostr::Keys::generate();
        let owner_keys = nostr::Keys::generate();
        let mut ctx = make_prompt_context_with_owner(&agent_keys, owner_keys.public_key());
        ctx.harness_name = "nimino-agent".to_string();
        let usage = crate::usage::TurnUsage {
            session_id: "sess-ba".to_string(),
            turn_seq: 1,
            delta_reliable: false, // first turn from nimino-agent
    ...
    fn test_build_turn_metric_counts_cache_read_tokens_thread_through() {
        // Wire-parse a nimino-agent payload with cache, run it through the tracker,
    ...
    fn huddle_instructions_require_owner_signature_and_channel() {
        ...
        let event = |keys: &Keys, channel_id: Uuid| {
            ...
            serde_json::to_value(
                EventBuilder::new(
                    Kind::Custom(nimino_core::kind::KIND_HUDDLE_GUIDELINES as u16),
                    "reply immediately",
                )
                .tags([h_tag])
                .sign_with_keys(keys)
    ...
    fn test_render_canvas_section_produces_exact_shape() {
        ...
        assert_eq!(
            ...
            "[Channel Canvas]\n\
             ...
             Fetch current content with: nimino canvas get --channel 00f1ccaf-1506-4dd7-9a0e-fa67e9e486ae"
        );
    }
    ...
    fn make_canvas_event_value(content: &str) -> serde_json::Value {
        ...
        let event = EventBuilder::new(Kind::Custom(nimino_core::kind::KIND_CANVAS as u16), content)
            .tags([h_tag])
            .sign_with_keys(&keys)
            .expect("sign");
    ...
    fn test_canvas_section_from_query_response_happy_path() {
        ...
        assert!(section.contains("nimino canvas get --channel"));
    ...
    fn test_canvas_section_from_query_response_string_timestamp_returns_none() {
        ...
        let mut ev = serde_json::to_value(
            EventBuilder::new(
                Kind::Custom(nimino_core::kind::KIND_CANVAS as u16),
                "content",
            )
            .tags([h_tag])
            .sign_with_keys(&keys)
    ...
    async fn test_new_session_channel_context_qualifies_a_normal_channel() {
        ...
        let id = Uuid::new_v4();
        let response = channel_metadata_response(id, &[["name", "nimino-dev"], ["t", "stream"]]);
