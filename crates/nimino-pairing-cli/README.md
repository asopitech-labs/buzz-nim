...
CLI tool for testing the [NIP-AB device pairing protocol](../nimino-core/src/pairing/NIP-AB.md) end-to-end. Exercises the full protocol over a live Nostr relay — designed for interop testing and NIP submission, not production use.

## Quick Start

```bash
cargo build --release -p nimino-pairing-cli

# Terminal 1 — source (holds the secret)
./target/release/nimino-pair source --relay wss://relay.damus.io

# Terminal 2 — target (receives the secret)
./target/release/nimino-pair target --show-secret
...
```
nimino-pair source --relay <RELAY_URL> [--nsec <BECH32_NSEC>]
```
...
```
nimino-pair target [--relay <OVERRIDE_URL>] [--show-secret]
```
...
```
nimino-pair test-vectors
```

## Testing Against a Local Nimino Relay

The CLI supports NIP-42 authentication, so it works with Nimino relays out of the box.

### Prerequisites

- Docker running (for Postgres and object storage)
- Nimino relay built: `cargo build --release -p nimino-relay`
...
screen -dmS relay bash -c "./target/release/nimino-relay 2>&1 | tee /tmp/nimino-relay.log"
...
```bash
# Terminal 1
./target/release/nimino-pair source --relay ws://localhost:3000
