//! `nimino-pair` — NIP-AB device pairing interop testing CLI.
//!
//! # Usage
//!
//! ```text
//! nimino-pair source --relay wss://relay.example.com [--nsec nsec1...]
//! nimino-pair target [--relay wss://relay.example.com]
...
use nimino_core::kind::KIND_PAIRING;
use nimino_core::pairing::session::PairingSession;
use nimino_core::pairing::{
...
async fn cmd_source(relay_url: String, nsec: Option<String>) -> Result<(), CliError> {
    ...
    if !confirmed {
        ...
        if let Some(abort_event) =
            session.abort(nimino_core::pairing::types::AbortReason::SasMismatch)?
        {
