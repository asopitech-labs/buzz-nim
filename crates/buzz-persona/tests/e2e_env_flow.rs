...
//! - Buzz-agent personas emit NIMINO_AGENT_MODEL, NIMINO_AGENT_PROVIDER
...
"#,
    ...
    // Buzz-agent persona gets NIMINO_AGENT_* env vars
    let buzz_env: std::collections::HashMap<_, _> = buzz
        .runtime_env_vars
        .iter()
        .map(|(k, v)| (k.as_str(), v.as_str()))
        .collect();
    assert_eq!(buzz_env.get("NIMINO_AGENT_MODEL"), Some(&"gpt-4o"));
    assert_eq!(buzz_env.get("NIMINO_AGENT_PROVIDER"), Some(&"openai"));
    assert!(
