...
mod tests {
            ...
            .body(Body::from(
                serde_json::to_vec(&serde_json::json!({
                    ...
                    "items": [{
                        "apiVersion": "v1",
                        "kind": "Pod",
                        "metadata": {"name": "nimino-agent"},
                        "spec": {"containers": [{"name": "agent", "image": "example.invalid/nimino-agent@sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"}]}
                    }]
                }))
                .unwrap(),
            ))
    ...
    async fn list_through_real_request_path(
        ...
        let result = cluster
            .list_with_date::<Pod>("app.kubernetes.io/managed-by=nimino-backend-kubernetes")
            .await
            .unwrap();
    ...
    async fn list_with_date_uses_a_quorum_request_and_returns_the_server_clock() {
        ...
        assert_eq!(pods[0].metadata.name.as_deref(), Some("nimino-agent"));
        assert!(uri.starts_with("/api/v1/namespaces/owned-ns/pods?"));
        assert!(uri
            .contains("labelSelector=app.kubernetes.io%2Fmanaged-by%3Dnimino-backend-kubernetes"));
        assert!(
