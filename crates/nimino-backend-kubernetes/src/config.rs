...
/// `NIMINO_ACP_EXIT_AFTER_INACTIVITY` are one knob, not two.
...
/// A fresh `nimino-agents-<rand6>` namespace default.
///
    ...
    serde_json::json!({
        ...
        "properties": {
            ...
            "image": {
                "type": "string",
                "title": "Agent image",
                "description": "Digest-pinned image containing the four-file Nimino agent bundle, e.g. ghcr.io/asopitech-labs/nimino-agent@sha256:<digest>. Tags alone are not accepted: this pod holds the agent's private key."
            },
            "cpu_request": {
...
mod tests {
    ...
    fn digest_ref() -> String {
        format!(
            "ghcr.io/asopitech-labs/nimino-agent@sha256:{}",
            "a".repeat(64)
        )
    }

    fn minimal() -> serde_json::Value {
        serde_json::json!({"namespace": "nimino-agents-abc123", "image": digest_ref()})
    }
    ...
    fn image_is_required_and_must_be_digest_pinned() {
        ...
        cfg["image"] = "ghcr.io/asopitech-labs/nimino-agent:latest".into();
    ...
    fn generated_namespaces_are_fresh_and_valid() {
        ...
        assert!(a.starts_with("nimino-agents-"));
        assert_eq!(a.len(), "nimino-agents-".len() + 6);
    }
    ...
    #[test]
    fn schema_has_no_mutable_or_stale_image_default() {
        let schema = config_schema();
