//! Nimino V2 kind number registry.
//!
//! This module is the authoritative source for Nimino kind numbers.
...
/// NIP-01: Channel metadata (replaceable). Not used by Nimino today.
...
/// Nimino custom one-time identity binding proof (ephemeral, not stored).
...
// NEW: Nimino command kinds (Pure Nostr plan)
...
/// `crates/nimino-search/tests/fts_integration.rs`).
///
...
/// visibility pushdown in `nimino-db`.
///
...
/// Nimino product feedback submission. Accepted at ingest, sidecarred to the
...
// Nimino community moderation commands (mod-signed, processed like 9030-series:
...
/// Nimino: Set the workspace profile (icon). Admin/owner-signed command.
...
// Ephemeral events (20000–29999) — process-local delivery only, never stored.
...
// Not using NIP-90 kinds (5000–6999) — Nimino requires auth chains (depth ≤ 3, breadth ≤ 10).
...
/// or the Nimino workspace-profile admin command (9033).
...
/// Returns `true` if `kind` is a Nimino command kind that requires transactional execution.
...
/// Safe: all Nimino kinds fit in i32 (max 65535 < i32::MAX).
...
// Compile-time: all Nimino kind constants fit in nostr's u16-backed Kind.
