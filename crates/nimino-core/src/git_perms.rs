...
//!
//! This module defines the core data types for the Nimino git permission system.
//! The permission model: channel role = repo role; `nimino-protect` tags on
...
/// announcement with no `nimino-channel` binding.
///
...
/// Maximum number of `nimino-protect` tags per repo.
...
/// A single protection rule parsed from a `nimino-protect` tag on kind:30617.
///
/// Format: `["nimino-protect", "<ref-pattern>", "<rule>", ...]`
...
/// Errors from parsing a `nimino-protect` tag.
#[derive(Debug, Clone, PartialEq, Eq)]
...
impl fmt::Display for RuleParseError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::TooFewValues => write!(f, "nimino-protect tag needs pattern + at least one rule"),
...
/// Parse a single `nimino-protect` tag into a `ProtectionRule`.
///
/// Tag format: `["nimino-protect", "<pattern>", "<rule1>", "<rule2>", ...]`
/// The first element ("nimino-protect") should already be stripped — pass
/// the remaining values starting with the pattern.
/// Parse a single `nimino-protect` tag (simple API, discards unknown rules).
...
/// Parse a single `nimino-protect` tag, returning unknown rules for logging.
...
/// Parse all `nimino-protect` tags from a kind:30617 event's tag list.
///
/// Returns an error if any `nimino-protect` tag is structurally malformed.
...
pub fn parse_protection_tags(tags: &[Vec<String>]) -> Result<ParsedProtection, RuleParseError> {
    ...
    for tag in tags {
        if tag.first().map(|s| s.as_str()) != Some("nimino-protect") {
            continue;
        }
...
/// Built-in default minimum role for an operation when no `nimino-protect` tag matches.
