...
//!
//! These types live in `nimino-core` (zero I/O deps) so the DB, auth, pub/sub,
...
impl CommunityId {
    ...
    /// The underlying UUID, for DB binds and local delivery key construction.
...
/// the `nimino-admin` CLI's tenant resolution. All of these must derive the
...
mod tests {
    ...
    fn relay_url_authority_keeps_explicit_nondefault_port() {
        ...
        // nimino-admin must all derive `localhost:3000` (NOT bare `localhost`),
