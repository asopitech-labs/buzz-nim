#![deny(unsafe_code)]
#![warn(missing_docs)]
//! `nimino-core` — zero-I/O foundation types for the Nimino relay.
//!
//! Provides [`StoredEvent`], filter matching, kind constants, and event
//! verification. All other Nimino crates depend on this one.
...
pub mod invite;
/// Nimino kind number registry — custom event type constants.
pub mod kind;
