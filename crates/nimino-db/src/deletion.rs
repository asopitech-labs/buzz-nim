...
/// the instant it is checked. The value is arbitrary (ASCII `niminodel1`) but
...
impl DeletionStage {
    ...
    pub const fn next(self) -> Option<Self> {
        match self {
            ...
            Self::PostgresPurged => Some(Self::LogicallyVerified),
...
impl DeletionStore {
    ...
    ///
    /// Probe the deployed catalog rather than SQLx's migration ledger. Nimino also
    ...
    ///
    /// The caller must verify object storage too, then call
    ...
    pub async fn verify_postgres_logically_deleted(&self, token: &LeaseToken) -> Result<()> {
        ...
        lock_schema_destruction_shared(&mut tx).await?;
        validate_catalog_on(&mut tx).await?;
        verify_lease_and_fence(&mut tx, token, DeletionStage::PostgresPurged, generation).await?;
    ...
    pub async fn guard_transaction_with_serving_lease(
        ...
        sqlx::query(
            "SELECT set_config('nimino.serving_write_community', $1, true), \
                    set_config('nimino.serving_write_lease_id', $2, true), \
                    set_config('nimino.serving_write_owner', $3, true), \
                    set_config('nimino.serving_write_generation', $4, true), \
                    set_config('nimino.serving_write_fence_generation', $5, true)",
        )
...
async fn set_executor_gucs(
    ...
    sqlx::query(
        "SELECT set_config('nimino.deletion_executor_community', $1, true), \
                set_config('nimino.deletion_fence_generation', $2, true)",
    )
...
mod postgres_tests {
    ...
    async fn preclaim_setup_failure_is_durable_without_a_lease() {
        ...
        let blocked = store
            .block_preclaim_setup(
                ...
                "NIMINO_S3_ENDPOINT is required",
            )
            .await
        ...
        assert_eq!(
            ...
            Some("NIMINO_S3_ENDPOINT is required")
        );
        assert_eq!(
    ...
    async fn serving_lease_reaper_remains_global_across_tombstoned_tenant() {
        ...
        sqlx::query(
            "SELECT set_config('nimino.deletion_executor_community', $1, true), \
                    set_config('nimino.deletion_fence_generation', '1', true)",
        )
    ...
    async fn destructive_stages_serialize_with_migrations_and_fail_closed_on_new_scoped_tables() {
        ...
        let base_url = std::env::var("NIMINO_TEST_DATABASE_URL")
            .or_else(|_| std::env::var("DATABASE_URL"))
            .unwrap_or_else(|_| "postgres://nimino:nimino_dev@localhost:5432/nimino".to_string());
        let admin = PgPool::connect(&base_url)
            .await
            .expect("connect admin database");
        let probe_db = format!("nimino_lock_probe_{}", Uuid::new_v4().simple());
    ...
    async fn desired_state_schema_bootstrap_progresses_beyond_fencing() {
        ...
        let admin = PgPool::connect(&base_url)
            .await
            .expect("connect admin database");
        let probe_db = format!("nimino_desired_state_{}", Uuid::new_v4().simple());
        sqlx::query(AssertSqlSafe(format!("CREATE DATABASE {probe_db}")))
            .execute(&admin)
            .await
            .expect("create probe database");
