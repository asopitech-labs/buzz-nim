...
/// Ownership facts observed while the current-owner row and transferee quota
/// are locked by the transfer transaction.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct OwnershipTransferFacts {
    /// Whether the community has a current owner.
    pub owner_present: bool,
    /// Whether the caller's expected owner matches a locked owner row.
    pub expected_owner_matches: bool,
    /// Whether the transferee is already the sole current owner.
    pub new_owner_is_current_owner: bool,
    /// Whether the transferee has reached the configured ownership limit.
    pub new_owner_at_limit: bool,
}

/// Exact transaction effect selected by the Nim ownership policy.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum OwnershipTransferAction {
    /// Reject without changing ownership.
    Reject,
    /// Preserve the existing sole owner.
    Noop,
    /// Promote the transferee and demote every other owner.
    Transfer,
}
...
///
/// Reads `NIMINO_MAX_COMMUNITIES_PER_OWNER` once (cached for the process
...
pub fn max_communities_per_owner() -> i64 {
    ...
    *LIMIT.get_or_init(|| {
        effective_owner_limit(
            std::env::var("NIMINO_MAX_COMMUNITIES_PER_OWNER")
                .ok()
                .as_deref(),
        )
    })
...
/// 2. Locks the current owner row `FOR UPDATE` and reads compare-and-swap facts.
/// 3. Reads the transferee's ownership count under the same transaction.
/// 4. Asks the supplied policy callback to reject, no-op, or transfer.
/// 5. On transfer, upserts `new_owner_pubkey` as `owner` and demotes every
///    other owner in this community to `member` — **not**
...
where
    F: FnOnce(OwnershipTransferFacts) -> Fut,
    Fut: Future<Output = Result<OwnershipTransferAction>>,
{
    ...
    // 2. Lock the current owner row FOR UPDATE and capture compare-and-swap facts.
    ...
    // 3. Capture the transferee's community ownership count inside the same
    ...
    let facts = OwnershipTransferFacts {
        owner_present: !existing_owners.is_empty(),
        expected_owner_matches: existing_owners.iter().any(|p| p == &expected_owner),
        new_owner_is_current_owner: existing_owners.len() == 1 && existing_owners[0] == pubkey,
        new_owner_at_limit: owned_count >= max_communities_per_owner(),
    };
    let action = decide(facts).await?;
    if action == OwnershipTransferAction::Reject {
        tx.rollback().await?;
        return if !facts.owner_present {
            Ok(TransferResult::NoOwner)
        } else if !facts.expected_owner_matches {
            Ok(TransferResult::OwnerConflict)
        } else if facts.new_owner_at_limit {
            Ok(TransferResult::LimitReached)
        } else {
            Err(crate::DbError::InvalidData(
                "ownership policy rejected compatible locked facts".to_owned(),
            ))
        };
    }
    if action == OwnershipTransferAction::Noop {
        tx.rollback().await?;
        return if facts.new_owner_is_current_owner && facts.expected_owner_matches {
            Ok(TransferResult::AlreadyOwner)
        } else {
            Err(crate::DbError::InvalidData(
                "ownership policy selected noop for incompatible locked facts".to_owned(),
            ))
        };
    }
    if action != OwnershipTransferAction::Transfer
        || !facts.owner_present
        || !facts.expected_owner_matches
        || facts.new_owner_is_current_owner
        || facts.new_owner_at_limit
    {
        tx.rollback().await?;
        return Err(crate::DbError::InvalidData(
            "ownership policy selected transfer for incompatible locked facts".to_owned(),
        ));
    }

    // 5. Apply the exact transfer effect selected by policy.
    sqlx::query(
        "INSERT INTO relay_members (community_id, pubkey, role, added_by) \
         VALUES ($1, $2, 'owner', NULL) \
         ON CONFLICT (community_id, pubkey) DO UPDATE SET role = 'owner', updated_at = now()",
    ...
    // Demote all other owners to member (not admin).
    sqlx::query(
...
mod tests {
    ...
    async fn test_transfer_decision(
        facts: OwnershipTransferFacts,
    ) -> Result<OwnershipTransferAction> {
        Ok(
            if !facts.owner_present || !facts.expected_owner_matches || facts.new_owner_at_limit {
                OwnershipTransferAction::Reject
            } else if facts.new_owner_is_current_owner {
                OwnershipTransferAction::Noop
            } else {
                OwnershipTransferAction::Transfer
            },
        )
    }
    ...
    async fn transfer_ownership_already_owner_is_noop() {
        ...
        let result = transfer_ownership(&pool, community, &owner, &owner, test_transfer_decision)
            .await
    ...
    async fn transfer_ownership_returns_limit_reached_for_maxed_transferee() {
        ...
        // Fill the transferee's configured ownership allowance.
