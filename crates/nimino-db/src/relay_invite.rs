...
//!
//! Invites are backed by durable rows in `relay_invites`. The table stores
...
use nimino_core::invite::{encode_v2_code, hash_v2_code, V2_SECRET_LEN};
...
/// Locked durable invite classification supplied to the Nim policy adapter.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ClaimState {
    /// Matching live invite with capacity.
    Valid,
    /// Matching invite whose expiry passed.
    Expired,
    /// Matching live invite with no remaining capacity.
    Exhausted,
    /// No tenant-scoped invite matched.
    Invalid,
}

/// Facts captured while holding the invite row lock.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct ClaimFacts {
    /// Durable invite classification.
    pub state: ClaimState,
    /// Whether the claimant already belongs to the relay roster.
    pub already_member: bool,
}

/// Exact transaction effect selected by the Nim policy adapter.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ClaimAction {
    /// Reject without mutating the transaction.
    Reject,
    /// Preserve an existing membership and do not consume a use.
    Noop,
    /// Insert membership and consume one use.
    Join,
}
...
///
/// The caller must pass values admitted by the Nim membership policy. Database
/// constraints remain the persistence backstop for `max_uses`.
...
where
    F: FnOnce(ClaimFacts) -> Fut,
    Fut: Future<Output = Result<ClaimAction>>,
{
    let mut tx = pool.begin().await?;
    ...
    let Some(invite) = row else {
        let action = decide(ClaimFacts {
            state: ClaimState::Invalid,
        ...
        return match action {
            ClaimAction::Reject => Ok(ClaimOutcome::Invalid),
            _ => Err(crate::DbError::InvalidData(
                "invite policy admitted a missing invite".to_owned(),
            )),
        };
    };
    ...
    let state = if expires_at <= Utc::now() {
        ClaimState::Expired
    ...
    if action == ClaimAction::Reject {
        tx.rollback().await?;
        let (name, outcome) = match state {
            ClaimState::Expired => ("expired", ClaimOutcome::Expired),
            ClaimState::Exhausted => ("exhausted", ClaimOutcome::Exhausted),
            ClaimState::Invalid => ("invalid", ClaimOutcome::Invalid),
            ClaimState::Valid => ("policy_rejected", ClaimOutcome::Invalid),
        };
        log_claim_outcome(community, Some(invite_id), name, max_uses, Some(use_count));
        return Ok(outcome);
    }

    if action == ClaimAction::Noop {
        if !already_member || !matches!(state, ClaimState::Valid | ClaimState::Exhausted) {
            tx.rollback().await?;
            return Err(crate::DbError::InvalidData(
                "invite policy selected noop for incompatible locked facts".to_owned(),
            ));
        }
    ...
    if action != ClaimAction::Join || already_member || state != ClaimState::Valid {
        tx.rollback().await?;
        return Err(crate::DbError::InvalidData(
            "invite policy selected join for incompatible locked facts".to_owned(),
        ));
    }

    // The conflict branch covers a claimant admitted
    ...
    // Insert join-policy acceptance evidence. This is required for both a
    ...
    // Increment use_count (for every new member, even unlimited).
...
mod tests {
    ...
    async fn test_claim_decision(facts: ClaimFacts) -> Result<ClaimAction> {
        Ok(match (facts.state, facts.already_member) {
            (ClaimState::Valid | ClaimState::Exhausted, true) => ClaimAction::Noop,
            (ClaimState::Valid, false) => ClaimAction::Join,
            _ => ClaimAction::Reject,
        })
    }
    ...
    async fn bounded_claim_exhausts_and_existing_member_retry_does_not_consume() {
        ...
        assert_eq!(
            claim_relay_invite(&pool, community, &hash, &first, None, test_claim_decision)
                .await
                .expect("first claim"),
            ClaimOutcome::Joined {
                use_count: 1,
        ...
        assert_eq!(
            claim_relay_invite(&pool, community, &hash, &second, None, test_claim_decision)
                .await
    ...
    async fn retention_sweep_skips_quiescing_tenant_while_active_bystanders_progress() {
        ...
        sqlx::query(
            "SELECT set_config('nimino.deletion_executor_community', $1, true), \
                    set_config('nimino.deletion_fence_generation', '0', true)",
        )
    ...
    async fn unlimited_invites_count_each_new_member() {
        ...
        for (expected_count, pubkey) in [(1, test_pubkey()), (2, test_pubkey())] {
            assert_eq!(
                claim_relay_invite(&pool, community, &hash, &pubkey, None, test_claim_decision,)
                    .await
