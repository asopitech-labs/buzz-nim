...
// Re-export the canonical enum definitions from nimino-core.
// These live in core (zero I/O deps) so the SDK can share them
// without pulling in sqlx/tokio.
pub use nimino_core::channel::{ChannelType, ChannelVisibility, MemberRole};
...
/// `nimino_channel_ttl:`.
const CHANNEL_MEMBERSHIP_LOCK_NAMESPACE: &str = "nimino_channel_membership:";
...
impl LockedMemberSnapshot {
    ...
    pub async fn replace_member_event(
        ...
        let kind = nimino_core::kind::event_kind_i32(event);
        if kind != 39002 {
            return Err(DbError::InvalidData(
        ...
        if let Some((existing_ts, existing_id)) = existing {
            ...
            {
                return Ok((
                    nimino_core::StoredEvent::with_received_at(
                        event.clone(),
                        Utc::now(),
                        Some(channel_id),
                        false,
...
/// DM channel identity and active-participant fact for one reader.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct DmAccessFact {
    /// DM channel identifier.
    pub channel_id: Uuid,
    /// Whether the reader has an active membership row.
    pub is_member: bool,
}

/// Returns raw DM access facts for policy evaluation in the Nim core.
pub async fn list_dm_access_facts(
    pool: &PgPool,
    community_id: CommunityId,
    pubkey: &[u8],
) -> Result<Vec<DmAccessFact>> {
    let rows = sqlx::query(
        r#"
        SELECT c.id AS channel_id, (cm.channel_id IS NOT NULL) AS is_member
        FROM channels c
...
pub async fn update_channel(
    ...
    if let Some(name) = updates.name.as_mut() {
        *name = nimino_core::channel::canonical_channel_name(name).to_owned();
        if name.is_empty() {
            return Err(DbError::InvalidData("channel name is required".into()));
        }
    }
...
mod tests {
    ...
    const TEST_DB_URL: &str = "postgres://nimino:nimino_dev@localhost:5432/nimino"; // sadscan:disable np.postgres.1 -- local test-only credentials
    ...
    async fn accessible_channel_ids_are_not_truncated_at_one_thousand() {
        let database_url =
            std::env::var("NIMINO_TEST_DATABASE_URL").unwrap_or_else(|_| TEST_DB_URL.to_string());
