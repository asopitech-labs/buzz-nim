...
//! Ephemeral events (kinds 20000–29999) are never stored — process-local delivery only.
...
pub async fn insert_event(
    ...
    // Cast chain: nostr Kind (u16) → i32 (Postgres INT column). Safe: all Nimino kinds fit in i32.
...
mod tests {
    ...
    async fn event_insert_ttl_trigger_handles_permanent_ephemeral_duplicate_and_activation_race() {
        ...
        sqlx::query("SELECT pg_advisory_xact_lock(hashtextextended($1, 0))")
            .bind(format!("nimino_channel_ttl:{community_uuid}:{racing}"))
