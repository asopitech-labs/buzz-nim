...
mod tests {
    ...
    fn needs_action_query_includes_approval_and_reminder_kinds() {
        use nimino_core::kind::{KIND_STREAM_REMINDER, KIND_WORKFLOW_APPROVAL_REQUESTED};
    ...
    fn activity_query_excludes_workflow_execution_kinds() {
        ...
        use nimino_core::kind::{KIND_WORKFLOW_APPROVAL_DENIED, KIND_WORKFLOW_TRIGGERED};
    ...
    fn mentions_query_is_tenant_scoped_and_joins_mentions_by_composite_key() {
        ...
        assert!(
            ...
            "mentions feed must include Nimino Git roots and comments: {sql}"
        );
    }
    ...
    async fn insert_mentions_indexes_rosters_past_bind_parameter_cap() {
        ...
        sqlx::query(
            "INSERT INTO channel_members (community_id, channel_id, pubkey, role) \
             SELECT $1, $2, decode(lpad(to_hex(member), 64, '0'), 'hex'), 'member'::member_role \
             FROM generate_series(1, $3) AS member",
        )
        .bind(community.as_uuid())
        .bind(channel)
