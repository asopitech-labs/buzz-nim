...
//!    is armed per session via the `nimino.created_at_floor` GUC, which the
...
pub async fn verify_floor_guard_behavior(pool: &PgPool) -> crate::Result<()> {
    ...
    let armed: String = sqlx::query_scalar("SHOW nimino.created_at_floor")
        .fetch_one(&mut *tx)
        .await
        .map_err(|e| {
            DbError::InvalidData(format!(
                "nimino.created_at_floor GUC not set on this pool: {e}"
            ))
        })?;
    if armed != CREATED_AT_FLOOR_SECS.to_string() {
        return Err(DbError::InvalidData(format!(
            "nimino.created_at_floor is '{armed}', expected '{CREATED_AT_FLOOR_SECS}': \
             pool is not armed"
        )));
    }
