...
/// An audit row as read back for `nimino moderation audit`.
#[derive(Debug, Clone)]
...
/// Uninterpreted restriction row used as input to the Nim policy boundary.
#[derive(Debug, Clone, Default, PartialEq, Eq)]
pub struct RestrictionFacts {
    /// Whether a scoped row exists.
    pub exists: bool,
    /// Stored ban flag without expiry evaluation.
    pub ban_set: bool,
    /// Stored ban expiry.
    pub ban_expires_at: Option<DateTime<Utc>>,
    /// Stored timeout expiry.
    pub muted_until: Option<DateTime<Utc>>,
}

/// Fetch raw restriction facts without evaluating expiry in SQL.
pub async fn restriction_facts(
    pool: &PgPool,
    community: CommunityId,
    pubkey: &[u8],
) -> Result<RestrictionFacts> {
    let row = sqlx::query(
        "SELECT banned, ban_expires_at, muted_until FROM community_bans \
    ...
    match row {
        Some(row) => Ok(RestrictionFacts {
            exists: true,
            ban_set: row.try_get("banned")?,
            ban_expires_at: row.try_get("ban_expires_at")?,
            muted_until: row.try_get("muted_until")?,
        }),
        None => Ok(RestrictionFacts::default()),
    }
}
...
/// List audit rows, newest first (`nimino moderation audit`).
...
mod tests {
    ...
    async fn expired_ban_does_not_hide_active_timeout() {
        ...
        let facts = restriction_facts(&pool, community, &pubkey)
            .await
            .expect("restriction facts");
        assert!(facts.exists, "policy facts must preserve row existence");
        assert!(facts.ban_set, "policy facts must preserve the raw ban flag");
        assert!(
            facts.ban_expires_at.is_some(),
            "policy facts must let Nim evaluate expiry"
        );
        assert!(
            facts.muted_until.is_some(),
            "policy facts must preserve the raw timeout"
        );
