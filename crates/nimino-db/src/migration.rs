//! Embedded SQLx migrations for Nimino.
//!
...
/// Run all pending Nimino database migrations.
///
...
mod tests {
    ...
    fn embedded_migrator_contains_consolidated_initial_schema() {
        ...
        // The independent fork does not retain the event-based mesh status
        // protocol. Keep the version slot so later migrations remain ordered,
        // but install no compatibility trigger or product artifact.
        assert_eq!(migrations[18].version, 19);
        assert!(!migrations[18].sql.as_str().contains("mesh-member-status"));
        assert!(!migrations[18].sql.as_str().contains("CREATE TRIGGER"));

        // Join policy acceptances retain their original migration version.
        ...
        assert!(ttl_shared.contains("'nimino_channel_ttl:' || NEW.community_id::text"));
        ...
        assert!(roster_fence.contains("'nimino_channel_membership:'"));
            ...
            if *path == this_file {
                ...
                assert_eq!(
                    ...
                    "{} embeds or runs a SQLx migrator outside the schema/destruction \
                     ...
                     nimino_db migration::run_migrations",
                    path.display()
                );
            }
        }
    ...
    async fn cancelled_migration_cannot_expose_shared_lock_while_ddl_backend_lives() {
        ...
        let probe_db = format!("nimino_lock_cancel_{}", uuid::Uuid::new_v4().simple());
    ...
    async fn mobile_push_upgrade_drops_state_without_rewriting_history() {
        ...
        sqlx::query("SELECT set_config('nimino.deletion_executor_community', $1, true)")
            .bind(community_id.to_string())
            .execute(&mut *fence_tx)
            .await
            .expect("set deletion executor community");
        sqlx::query("SELECT set_config('nimino.deletion_fence_generation', '1', true)")
            .execute(&mut *fence_tx)
            .await
