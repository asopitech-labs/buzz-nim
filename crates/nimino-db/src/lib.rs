#![deny(unsafe_code)]
#![warn(missing_docs)]
//! nimino-db — Postgres event store for Nimino.
//!
//! ## Design invariants
//! - AUTH events (kind 22242) are never stored — they carry bearer tokens.
//! - Ephemeral events (20000–29999) are never stored — process-local delivery only.
...
pub mod partition;
/// Nimino product-feedback sidecar persistence.
...
/// Raw operator-plane lifecycle facts for one community host.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct CommunityLifecycleFacts {
    /// Stable community id.
    pub id: CommunityId,
    /// Normalized stored host.
    pub host: String,
    /// First archive timestamp, when archived.
    pub archived_at: Option<DateTime<Utc>>,
    /// Durable deletion lifecycle state.
...
impl Db {
    ...
    /// `nimino.created_at_floor` GUC — this is what makes the replica fence
        ...
        let options = PgPoolOptions::new()
            ...
            .after_connect(|conn, _meta| {
                Box::pin(async move {
                    ...
                    sqlx::query("SELECT set_config('nimino.created_at_floor', $1, false)")
    ...
    > {
        ...
        .await
        {
            ...
            // Runbook: correlate with `nimino_db_read_pool_active` / `_max`
    ...
    /// the writer's: `nimino_db_read_pool_active / nimino_db_read_pool_max` is
    ...
    /// Return communities that may currently serve and replicate product data.
    #[datastore_span(name = "active_community_ids", system = "postgresql")]
    pub async fn active_community_ids(&self) -> Result<Vec<CommunityId>> {
        let ids = sqlx::query_scalar::<_, Uuid>(
            "SELECT id FROM communities \
             WHERE archived_at IS NULL AND deleted_at IS NULL AND deletion_state = 'active' \
             ORDER BY id",
        )
        .fetch_all(&self.pool)
        .await?;
        Ok(ids.into_iter().map(CommunityId::from_uuid).collect())
    }
    ...
    /// request/connection error. nimino-db only reads the durable host map.
    ...
    /// Return uninterpreted lifecycle columns for Nim policy evaluation.
    #[datastore_span(name = "community_lifecycle_facts", system = "postgresql")]
    pub async fn community_lifecycle_facts(
        &self,
        normalized_host: &str,
    ) -> Result<Option<CommunityLifecycleFacts>> {
        let row = sqlx::query(
            "SELECT id, host, archived_at, deletion_state::text AS deletion_state, deleted_at \
             FROM communities WHERE lower(host) = lower($1)",
        )
        .bind(normalized_host)
        .fetch_optional(&self.pool)
        .await?;
        row.map(|row| {
            Ok(CommunityLifecycleFacts {
                id: CommunityId::from_uuid(row.try_get("id")?),
                host: row.try_get("host")?,
    ...
    /// fully-formed [`nimino_core::tenant::TenantContext`] — host included — to
    ...
    ///
    /// Used by the runtime conformance read-seam emitters in `nimino-relay`:
    ...
    pub async fn insert_event_with_serving_write_guard(
        ...
        if kind_u32 == nimino_core::kind::KIND_AUTH {
            return Err(DbError::AuthEventRejected);
        }
        if nimino_core::kind::is_ephemeral(kind_u32) {
    ...
    /// Return raw DM identity and membership facts for Nim policy evaluation.
    #[datastore_span(name = "list_dm_access_facts", system = "postgresql")]
    pub async fn list_dm_access_facts(
        &self,
        community_id: CommunityId,
        pubkey: &[u8],
    ) -> Result<Vec<channel::DmAccessFact>> {
        channel::list_dm_access_facts(&self.pool, community_id, pubkey).await
    }
    ...
    /// Lock, decide, and CAS-persist one workflow run transition.
    #[datastore_span(name = "transition_workflow_run", system = "postgresql")]
    pub async fn transition_workflow_run<F, Fut>(
        &self,
        community_id: CommunityId,
        id: Uuid,
        transition_id: &str,
        ...
        decide: F,
    ) -> Result<workflow::WorkflowTransitionOutcome>
    where
        F: FnOnce(workflow::WorkflowTransitionFacts) -> Fut,
        Fut: Future<Output = Result<workflow::WorkflowTransitionAction>>,
    {
        workflow::transition_workflow_run(
            &self.pool,
            community_id,
            id,
            transition_id,
    ...
    /// Atomically create an approval and persist its kind:46010 event.
    #[datastore_span(name = "create_approval_with_event", system = "postgresql")]
    pub async fn create_approval_with_event(
        &self,
        params: workflow::CreateApprovalParams<'_>,
        event: &nostr::Event,
        channel_id: Option<Uuid>,
    ) -> Result<(StoredEvent, bool)> {
        workflow::create_approval_with_event(&self.pool, params, event, channel_id).await
    }
    ...
    pub async fn transfer_ownership<F, Fut>(
        &self,
        community: CommunityId,
    ...
    where
        F: FnOnce(relay_members::OwnershipTransferFacts) -> Fut,
        Fut: std::future::Future<Output = Result<relay_members::OwnershipTransferAction>>,
    {
    ...
    pub async fn claim_relay_invite<F, Fut>(
        &self,
        community: CommunityId,
        token_hash: &[u8; 32],
        claimer_pubkey: &str,
    ...
    where
        F: FnOnce(relay_invite::ClaimFacts) -> Fut,
        Fut: std::future::Future<Output = Result<relay_invite::ClaimAction>>,
    {
    ...
    /// Fetch uninterpreted restriction facts for the Nim policy boundary.
    #[datastore_span(name = "moderation_restriction_facts", system = "postgresql")]
    pub async fn moderation_restriction_facts(
        &self,
        community: CommunityId,
        pubkey: &[u8],
    ) -> Result<moderation::RestrictionFacts> {
        moderation::restriction_facts(&self.pool, community, pubkey).await
    }
    ...
    pub async fn replace_addressable_event(
        ...
        let kind_i32 = nimino_core::kind::event_kind_i32(event);
    ...
    pub async fn nip43_membership_snapshot_needs_reconciliation(
        ...
        let snapshot = self
            .query_events(&crate::event::EventQuery {
                kinds: Some(vec![nimino_core::kind::KIND_NIP43_MEMBERSHIP_LIST as i32]),
    ...
    pub async fn publish_nip43_membership_locked(
        ...
        let kind_i32 = nimino_core::kind::KIND_NIP43_MEMBERSHIP_LIST as i32;
    ...
    pub async fn replace_parameterized_event(
        ...
        let read_state_t_tag_count = event
            ...
            .count();
        let is_nip_rs = kind_i32 == nimino_core::kind::KIND_READ_STATE as i32
            && d_tag_count == 1
            && has_exact_d_tag
        ...
        let hard_delete_superseded = is_nip_rs;
        ...
        if existing.is_some() {
            if is_nip_rs {
                ...
                sqlx::query("SELECT set_config('nimino.nip_rs_hard_delete', 'on', true)")
                    .execute(&mut *tx)
                    .await?;
            }
...
mod tests {
    ...
    async fn nip_rs_replacement_hard_deletes_payload_and_watermark_rejects_replay() {
        ...
        let old = EventBuilder::new(
            Kind::Custom(nimino_core::kind::KIND_READ_STATE as u16),
            "old",
        )
        .tags(tags.clone())
        .custom_created_at(Timestamp::from(base))
    ...
    async fn coordinate_delete_spares_head_newer_than_the_deletion() {
        ...
        let community = CommunityId::from_uuid(make_community(&db.pool).await);
        let keys = Keys::generate();
        let kind = nimino_core::kind::KIND_PROJECT as i32;
        ...
        let version = |content: &str, offset: u64| {
            EventBuilder::new(
                Kind::Custom(nimino_core::kind::KIND_PROJECT as u16),
                content,
            )
    ...
    async fn nip_rs_hard_delete_fence_fails_closed_and_scopes_opt_in_to_transaction() {
        ...
        for commit in [true, false] {
            ...
            let value: String =
                sqlx::query_scalar("SELECT set_config('nimino.nip_rs_hard_delete', 'on', true)")
                    .fetch_one(&mut *tx)
                    .await
            ...
            let leaked: Option<String> = sqlx::query_scalar(
                "SELECT NULLIF(current_setting('nimino.nip_rs_hard_delete', true), '')",
            )
            .fetch_one(&mut *conn)
            .await
    ...
    async fn database_guard_covers_legacy_writer_and_nip09_deletion() {
        ...
        let base = Timestamp::now().as_secs();
        let a = EventBuilder::new(Kind::Custom(nimino_core::kind::KIND_READ_STATE as u16), "A")
            .tags(tags.clone())
            .custom_created_at(Timestamp::from(base))
            .sign_with_keys(&keys)
            .expect("sign A");
        let x = EventBuilder::new(Kind::Custom(nimino_core::kind::KIND_READ_STATE as u16), "X")
            .tags(tags.clone())
            .custom_created_at(Timestamp::from(base + 1))
            .sign_with_keys(&keys)
            .expect("sign X");
        let b = EventBuilder::new(Kind::Custom(nimino_core::kind::KIND_READ_STATE as u16), "B")
            .tags(tags.clone())
            .custom_created_at(Timestamp::from(base + 2))
            .sign_with_keys(&keys)
            .expect("sign B");
        let c = EventBuilder::new(Kind::Custom(nimino_core::kind::KIND_READ_STATE as u16), "C")
            .tags(tags)
        ...
        async fn legacy_insert(
            ...
            .bind(nimino_core::kind::KIND_READ_STATE as i32)
    ...
    async fn test_usage_metrics_lock_has_single_owner_and_releases_on_drop() {
        ...
        // races any live nimino-relay on the same database (see #3619).
        ...
        // Same key as production (`nimino-relay` USAGE_METRICS_LOCK_KEY) — safe here
    ...
    async fn create_community_with_owner_enforces_per_owner_limit() {
        ...
        let host = format!("limit-test-{limit}-{}.example", Uuid::new_v4().simple());
        assert_eq!(
            db.create_community_with_owner(&host, &owner)
                .await
    ...
    /// ceiling, not the writer's — `nimino_db_read_pool_active / _max` is the
    ...
    ///
    /// The fixture reproduces that state deliberately: a size-1 lazy reader
    /// whose test-only connection hook exceeds the acquire budget, with
    /// `reader_aurora_identity` asserted cold. It routes
    ...
    ///
    /// Timing allows the immediately measured writer-query baseline plus an
    /// upper bound below two reader budgets: it must fail for two stacked
    /// budgets (~300ms) while tolerating scheduler and database latency on one
    /// (~150ms). Asserting a lower bound too would pin the budget's own value,
    /// which `reader_acquire_timeout_is_the_documented_budget` already covers.
    ...
    async fn routed_fallback_spends_one_acquire_budget_when_aurora_cache_is_cold() {
        ...
        let writer = Db::new(&DbConfig {
            database_url: scratch_url.clone(),
            max_connections: 4,
            ..DbConfig::default()
        })
        .await
        .expect("connect armed writer");
        let delayed_reader = PgPoolOptions::new()
            .max_connections(1)
            .min_connections(0)
            .acquire_timeout(Db::READER_ACQUIRE_TIMEOUT)
            .after_connect(|_, _| {
                Box::pin(async {
            ...
            .connect_lazy(&scratch_url)
            .expect("create delayed lazy reader");
        let mut db = Db::from_pools(writer.pool.clone(), delayed_reader);
        db.fence().force_open_for_tests(chrono::Utc::now());
        db.set_replica_read_max_age_for_tests(Some(Duration::from_secs(5)));

        assert_eq!(
            db.read_max_connections, 1,
            "reader max must report 1 for this fixture"
        );
        ...
        let writer_count = db
            .count_events(&query)
            .await
            .expect("writer baseline answers the read");
        let writer_elapsed = writer_start.elapsed();
        assert_eq!(writer_count, 0, "writer baseline uses an empty database");
        ...
        assert!(
            elapsed < writer_elapsed + Duration::from_millis(250),
            "routed fallback must add ONE {}ms acquire budget, not two; writer baseline {}ms, routed {}ms",
        ...
        assert_eq!(
            ...
            Some(&1),
            "unavailable reader must fall back as writer/reader_acquire_timeout; got {reasons:?}"
        );
    ...
    async fn lazy_reader_pool_still_spawns_fence_probe() {
        ...
        // `nimino.created_at_floor` GUC — `spawn_fence_probe` verifies the
    ...
    async fn created_at_floor_guard_aborts_old_channel_rows_at_commit() {
        ...
        let insert_raw = |ev: nostr::Event, channel_id: Option<Uuid>| {
            ...
            async move {
                ...
                sqlx::query("SELECT set_config('nimino.created_at_floor', $1, true)")
        ...
        // Old row WITHOUT a channel (profile shapes) →
    ...
    fn writer_pool_safety_hook_is_single_and_composed() {
        ...
        assert!(connect_pool.contains("nimino.created_at_floor"));
    ...
    async fn armed_pool_rejects_old_channel_inserts_through_public_api() {
        ...
        let effective: String = sqlx::query_scalar("SHOW nimino.created_at_floor")
            .fetch_one(&db.pool)
            .await
