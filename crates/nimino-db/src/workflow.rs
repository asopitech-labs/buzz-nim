...
/// the raw token (same pattern as API tokens in nimino-auth).
...
pub struct WorkflowRunRecord {
    ...
    /// Monotonic revision used by the Nim-owned transition CAS.
    pub revision: i64,
...
pub async fn get_workflow_run(
    ...
    let row = sqlx::query(
        ...
        SELECT community_id, id, workflow_id, status::text AS status, trigger_event_id, current_step,
               revision, execution_trace, trigger_context, started_at, completed_at, error_message, error_code, created_at
        FROM workflow_runs
        WHERE community_id = $1 AND id = $2
        "#,
    )
...
/// Locked facts supplied to the Nim-owned workflow transition policy.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct WorkflowTransitionFacts {
    /// Current durable status.
    pub status: RunStatus,
    /// Current durable step.
    pub current_step: i32,
    /// Current durable CAS revision.
    pub revision: i64,
    /// Whether this transition identifier has already committed.
    pub transition_already_applied: bool,
}

/// State selected by the Nim-owned workflow transition policy.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct WorkflowTransitionState {
    /// Next durable status.
    pub status: RunStatus,
    /// Next durable step.
    pub current_step: i32,
    /// Next durable CAS revision.
    pub revision: i64,
}

/// Persistence action selected by the Nim-owned workflow transition policy.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum WorkflowTransitionAction {
    /// Reject without changing the row.
    Reject,
    /// Persist the supplied next state through CAS.
    Persist(WorkflowTransitionState),
}

/// Result of a workflow transition attempt.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum WorkflowTransitionOutcome {
    /// The transition committed.
    Applied,
    /// The transition identifier had already committed.
    Duplicate,
    /// The policy rejected the transition.
    Rejected,
}

/// Lock one run, ask the supplied policy to decide from durable facts, then
/// persist the selected state through a revision CAS.
#[allow(clippy::too_many_arguments)]
pub async fn transition_workflow_run<F, Fut>(
    ...
    decide: F,
) -> Result<WorkflowTransitionOutcome>
where
    F: FnOnce(WorkflowTransitionFacts) -> Fut,
    Fut: Future<Output = Result<WorkflowTransitionAction>>,
{
    if transition_id.is_empty() || transition_id.len() > 128 {
        return Err(DbError::InvalidData(
            "workflow transition id must contain 1..=128 bytes".to_owned(),
        ));
    }
    ...
    let row = sqlx::query(
        r#"
        SELECT status::text AS status, current_step, revision, transition_ids
        FROM workflow_runs
        WHERE community_id = $1 AND id = $2
        FOR UPDATE
        "#,
    ...
    let transition_ids: Vec<String> = serde_json::from_value(row.try_get("transition_ids")?)?;
    let facts = WorkflowTransitionFacts {
        status: row.try_get::<String, _>("status")?.parse::<RunStatus>()?,
        current_step: row.try_get("current_step")?,
        revision: row.try_get("revision")?,
        transition_already_applied: transition_ids.iter().any(|seen| seen == transition_id),
    };
    let duplicate = facts.transition_already_applied;
    let WorkflowTransitionAction::Persist(next) = decide(facts.clone()).await? else {
        tx.rollback().await?;
        return Ok(if duplicate {
            WorkflowTransitionOutcome::Duplicate
        } else {
            WorkflowTransitionOutcome::Rejected
        });
    };
    if duplicate
        || next.current_step < 0
        || next.revision
            != facts.revision.checked_add(1).ok_or_else(|| {
                DbError::InvalidData("workflow transition revision overflow".to_owned())
            })?
    {
        return Err(DbError::InvalidData(
            "workflow policy returned an invalid CAS state".to_owned(),
        ));
    }
    ...
    let affected = sqlx::query(
        ...
        SET status        = $1::run_status,
            current_step  = $2,
            revision      = $3,
            transition_ids = transition_ids || jsonb_build_array($4::text),
            execution_trace = $5,
            error_code    = $6,
            error_message = $7,
            started_at    = CASE WHEN $8 = 'running' AND started_at IS NULL
                                 THEN NOW() ELSE started_at END,
            completed_at  = CASE WHEN $9 IN ('completed','failed','cancelled')
                                 THEN NOW() ELSE completed_at END
        WHERE community_id = $10 AND id = $11 AND revision = $12
        "#,
    )
    .bind(&status_str)
    ...
    if affected != 1 {
        return Err(DbError::InvalidData(
            "workflow transition CAS lost after row lock".to_owned(),
        ));
    }
    tx.commit().await?;
    Ok(WorkflowTransitionOutcome::Applied)
}
...
/// Atomically persist an approval request and its discoverability event.
///
/// Replaying the same deterministic token/event is idempotent. A token that is
/// already bound to different facts is rejected rather than silently reused.
pub async fn create_approval_with_event(
    pool: &PgPool,
    params: CreateApprovalParams<'_>,
    event: &nostr::Event,
    channel_id: Option<Uuid>,
) -> Result<(nimino_core::StoredEvent, bool)> {
    ...
    if !inserted {
        let matches: bool = sqlx::query_scalar(
            ...
            SELECT EXISTS (
                ...
                WHERE community_id = $1 AND token = $2
                  AND workflow_id = $3 AND run_id = $4 AND step_id = $5
                  AND step_index = $6 AND approver_spec = $7 AND expires_at = $8
            )
            "#,
        )
        .bind(params.community_id.as_uuid())
        ...
        if !matches {
            return Err(DbError::InvalidData(
                "approval token is already bound to different request facts".to_owned(),
            ));
        }
    }
    ...
    .await?;
    if was_inserted {
        crate::insert_mentions_in_transaction(&mut tx, params.community_id, event, channel_id)
            .await?;
    }
    tx.commit().await?;
...
/// Update an approval on a caller-owned transaction.
///
/// Command handlers use this variant so the signed grant/deny event and the
/// approval state change share one commit boundary.
pub async fn update_approval_by_stored_hash_in_transaction(
...
mod tests {
    ...
    #[tokio::test]
    #[ignore = "requires Postgres"]
    async fn approval_and_notification_event_share_one_idempotent_commit() {
        let pool = setup_pool().await;
        let community = make_community(&pool).await;
        let workflow_id = Uuid::new_v4();
        let channel_id = Uuid::new_v4();
        insert_workflow_with_ids(&pool, community, workflow_id, channel_id, "approval-atomic")
            .await;
        let run_id = create_workflow_run(&pool, community, workflow_id, None, None)
            .await
            .expect("create run");
        ...
        let token_hash = hex::encode(hash_approval_token(&token));
        let tags = vec![nostr::Tag::parse(["d", token_hash.as_str()]).expect("d tag")];
        let valid_event = nostr::EventBuilder::new(nostr::Kind::Custom(46010), "approve")
            .tags(tags.clone())
            .sign_with_keys(&keys)
            .expect("sign event");
        let rejected_event = nostr::EventBuilder::new(nostr::Kind::Custom(20001), "ephemeral")
            .tags(tags)
            .sign_with_keys(&keys)
            .expect("sign ephemeral event");
        ...
        assert!(
            create_approval_with_event(&pool, params(), &rejected_event, Some(channel_id))
                .await
                .is_err()
        );
        assert!(matches!(
            get_approval(&pool, community, &token).await,
            Err(DbError::NotFound(_))
        ));

        let (_, first_insert) =
            create_approval_with_event(&pool, params(), &valid_event, Some(channel_id))
                .await
                .expect("atomic approval insert");
        let (_, replay_insert) =
            create_approval_with_event(&pool, params(), &valid_event, Some(channel_id))
                .await
                .expect("idempotent replay");
        assert!(first_insert);
        assert!(!replay_insert);
        assert_eq!(
        ...
        let event_count: i64 =
            sqlx::query_scalar("SELECT COUNT(*) FROM events WHERE community_id = $1 AND id = $2")
                .bind(community.as_uuid())
                .bind(valid_event.id.as_bytes().as_slice())
                .fetch_one(&pool)
                .await
                .expect("count event");
